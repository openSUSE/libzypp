...
struct DownloadProgressReceive
  : public callback::ReceiveReport<media::DownloadProgressReport>
{
  DownloadProgressReceive() { connect(); }
#if 0
        enum Action {
...
struct DownloadResolvableReceive
  : public callback::ReceiveReport<repo::DownloadResolvableReport>
{
  DownloadResolvableReceive() { connect(); }
#if 0
      enum Action {
        ABORT,  // abort and return error
        RETRY,	// retry
  ...
  virtual void finishDeltaDownload() { USR << __PRETTY_FUNCTION__ << endl; }
  ...
  virtual void finishDeltaApply() { USR << __PRETTY_FUNCTION__ << endl; }
  ...
  virtual Action problem( Resolvable::constPtr resolvable_ptr, Error error,
...
ManagedFile repoProvidePackage( const PoolItem &pi )
{
  ...
  repo::DeltaCandidates
    deltas; //( repo::makeDeltaCandidates( _pool.knownRepositoriesBegin(), _pool.knownRepositoriesEnd() ) );
...
int main( int argc, char *argv[] )
{
  ...
  if ( 0 )
  {
    Url url( "http://download.opensuse.org/debug/distribution/11.4/repo/oss/"
             "content.asc" );
    url.setPathName( "/" );
    MediaSetAccess access( url );
