...
inline std::string getXmlNodeVal(
  const std::string &line_r, const std::string &node_r )
{
...
public:
  TestSetup(
    const Arch &sysarch_r = Arch_empty, const Options &options_r = Options() )
  {
    _ctor( Pathname(), sysarch_r, options_r );
  }

  TestSetup( const Pathname &rootdir_r, const Arch &sysarch_r = Arch_empty,
    const Options &options_r = Options() )
  {
...
public:
  ...
  Target &target()
  {
    if ( !getZYpp()->getTarget() )
      getZYpp()->initializeTarget( _rootdir );
    return *getZYpp()->getTarget();
  }
  RepoManager repomanager()
  {
    return RepoManager( RepoManagerOptions::makeTestSetup( _rootdir ) );
  }
...
public:
  ...
  void loadRepo( const Url &url_r, const std::string &alias_r = std::string() )
  {
    ...
    nrepo.setAlias(
      alias_r.empty()
        ? url_r.getHost() + ":" + Pathname::basename( url_r.getPathName() )
        : alias_r );
  ...
  /** Directly load repo from metadata(dir) or solvfile(file) to pool.
    ...
    */
  void loadRepo(
    const Pathname &path_r, const std::string &alias_r = std::string() )
  {
  ...
  /** Directly load repo from some location (url or absolute(!)path).
    ...
    */
  void loadRepo(
    const std::string &loc_r, const std::string &alias_r = std::string() )
  {
    if ( *loc_r.c_str() == '/' )
    {
  ...
  void loadTestcaseRepos( const Pathname &path_r )
  {
    ...
    for ( iostr::EachLine in ( infile ); in; in.next() )
    {
      if ( str::hasPrefix( *in, "\t<channel" ) )
      {
        ...
        repod.priority =
          str::strtonum<unsigned>( getXmlNodeVal( *in, "priority" ) );
    ...
    //
    filesystem::Glob files(
      path_r / "*{.xml,.xml.gz}", filesystem::Glob::kBrace );
...
public:
  /** Detect and load the system located at \a sysRoot.
    ...
    */
  static void LoadSystemAt(
    const Pathname &sysRoot, const Arch &_testSetupArch_r = Arch_x86_64 )
  {
    ...
    {
      USR << str::form( "*** Load Testcase from '%s'", sysRoot.c_str() )
          << endl;
      TestSetup test;
    ...
    {
      USR << str::form( "*** Load TestSetup from '%s'", sysRoot.c_str() )
          << endl;

      TestSetup test( sysRoot, _testSetupArch_r );
      test.loadRepos();

      Pathname solvCachePath(
        RepoManagerOptions::makeTestSetup( test.root() ).repoSolvCachePath );
      Pathname fakeTargetSolv(
        solvCachePath / sat::Pool::systemRepoAlias() / "solv" );
      if ( PathInfo( fakeTargetSolv ).isFile() )
      {
        USR << str::form(
                 "*** Fake TestSetup Target from '%s'", fakeTargetSolv.c_str() )
            << endl;
        test.target();
    ...
    else
    {
      ...
      if ( 1 )
      {
        USR << "*** load target '" << Repository::systemRepoAlias() << "'\t"
            << endl;
      ...
      if ( 1 )
      {
        ...
        for_( it, repos.begin(), repos.end() )
        {
          ...
          USR << str::form( "*** load repo '%s'\t", nrepo.name().c_str() )
              << flush;
          try
          {
            repoManager.loadFromCache( nrepo );
          ...
          catch ( const Exception &exp )
          {
            ...
            USR << str::form( "*** omit broken repo '%s' (do 'zypper refresh')",
                     nrepo.name().c_str() )
                << endl;
            continue;
          }
...
private:
  void _ctor(
    const Pathname &rootdir_r, const Arch &sysarch_r, const Options &options_r )
  {
    _options = options_r;
