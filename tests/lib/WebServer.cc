...
{
  virtual const char *name() const noexcept( true ) { return "falke_name"; }
  virtual std::string message( int ev ) const { return "falke_message"; }
};
...
public:
  ...
  virtual string log() const { return string(); }
...
public:
  ...
  virtual void worker_thread()
  {
    ...
    string webrick_code =
      str::form( "require \"webrick\"; s = WEBrick::HTTPServer.new(:Port => "
                 "%d, :DocumentRoot    => \"%s\"); trap(\"INT\"){ s.shutdown "
                 "}; trap(\"SIGKILL\") { s.shutdown }; s.start;",
        _port, _docroot.c_str() );

    const char *argv[] = {"/usr/bin/ruby", "-e", webrick_code.c_str(), NULL};
  ...
  virtual string log() const { return _log; }
...
void WebServer::start() { _pimpl->start(); }

std::string WebServer::log() const { return _pimpl->log(); }

int WebServer::port() const { return _pimpl->port(); }
...
void WebServer::stop() { _pimpl->stop(); }
