...
inline void testGetCreds( CredentialManager &cm_r, const std::string &url_r,
  const std::string &user_r = "", const std::string &pass_r = "" )
{
  Url url( url_r );
...
BOOST_AUTO_TEST_CASE( read_cred_for_url )
{
  ...
  testGetCreds(
    cm, "ftp://weprovidesoft.fr/download/opensuse/110", "agda", "ichard" );
  testGetCreds( cm, "ftp://magda@weprovidesoft.fr/download/opensuse/110",
    "magda", "richard" );
  testGetCreds(
    cm, "ftp://agda@weprovidesoft.fr/download/opensuse/110", "agda", "ichard" );
  testGetCreds(
    cm, "ftp://unknown@weprovidesoft.fr/download/opensuse/110" ); // NULL
...
BOOST_AUTO_TEST_CASE( save_creds )
{
  ...
  CredentialFileReader(
    opts.globalCredFilePath, bind( &CredCollector::collect, &collector, _1 ) );
...
BOOST_AUTO_TEST_CASE( service_base_url )
{
  ...
  testGetCreds(
    cm, "http://joooha.com/service/path/repo/repofoo", "benson", "absolute" );
  testGetCreds( cm, "http://benson@joooha.com/service/path/repo/repofoo",
    "benson", "absolute" );
  testGetCreds(
    cm, "http://nobody@joooha.com/service/path/repo/repofoo" ); // NULL
}
