...
BOOST_AUTO_TEST_CASE( strange_test )
{
  ...
  iso_url = "iso:/";
  iso_url.setQueryParam(
    "iso", "SUSE-10.1-Beta5/SUSE-Linux-10.1-beta5-i386-CD1.iso" );
  ...
  try
  {
    ...
    if ( eject_src )
    {
      try
      {
        ...
        mm.release(
          src ); //! \todo add the device argument once mm.getDevices() is ready
      }
