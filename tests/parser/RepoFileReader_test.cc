...
static string fedora_repo =
  ...
  "        "
  "http://download.fedora.redhat.com/pub/fedora/linux/releases/$releasever/"
  "Everything/$basearch/os2/\n"
  "mirrorlist=http://mirrors.fedoraproject.org/"
  "mirrorlist?repo=fedora-$releasever&arch=$basearch\n"
...
{
  {
    ...
    parser::RepoFileReader parser(
      input, bind( &RepoCollector::collect, &collector, _1 ) );
  ...
  // fedora
  {
    ...
    BOOST_CHECK_EQUAL(
      Url( "file:///etc/pki/rpm-gpg/RPM-GPG-KEY-fedora" ), repo.gpgKeyUrl() );
