...
BOOST_AUTO_TEST_CASE( ini_read )
{
  ...
  for ( IniDict::section_const_iterator it = dict.sectionsBegin();
        it != dict.sectionsEnd(); ++it )
  {
    MIL << ( *it ) << endl;

    for ( IniDict::entry_const_iterator it2 = dict.entriesBegin( *it );
          it2 != dict.entriesEnd( *it ); ++it2 )
    {
...
BOOST_AUTO_TEST_CASE( ini_read2 )
{
  ...
  BOOST_CHECK( find( dict.sectionsBegin(), dict.sectionsEnd(), "base" ) !=
               dict.sectionsEnd() );
