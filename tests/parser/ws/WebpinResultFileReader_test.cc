...
BOOST_AUTO_TEST_CASE( result_read )
{
  ...
  WebpinResultFileReader reader(
    file, bind( &Collector::callback, &collect, _1 ) );
  ...
  BOOST_CHECK_EQUAL(
    first.repositoryUrl(), Url( "http://download.opensuse.org/repositories/"
                                "home:/burnickl_andreas/openSUSE_10.3" ) );
