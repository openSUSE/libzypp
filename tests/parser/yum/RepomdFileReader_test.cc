...
BOOST_AUTO_TEST_CASE( repomd_read )
{
  ...
  {
    Pathname file = *it;
    if ( ( file.basename().substr( 0, 6 ) == "repomd" ) &&
         ( file.extension() == ".xml" ) )
    {
      cout << *it << endl;

      Collector collect;
      RepomdFileReader( file, RepomdFileReader::ProcessResource( bind(
                                &Collector::callback, &collect, _1, _2 ) ) );
      ...
      while ( ifs && !ifs.eof() && count < collect.items.size() )
      {
        ...
        getline( ifs, dtype );
        BOOST_CHECK_EQUAL(
          collect.items[ count ].first, ResourceType( dtype ) );
