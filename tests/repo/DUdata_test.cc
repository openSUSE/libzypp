...
PoolItem piFind( const std::string &name_r, const std::string &ver_r,
  bool installed_r = false )
{
  PoolQuery q;
  q.addDependency( sat::SolvAttr::name, name_r, Rel::EQ, Edition( ver_r ) );
  q.setStatusFilterFlags(
    installed_r ? PoolQuery::INSTALLED_ONLY : PoolQuery::UNINSTALLED_ONLY );
  if ( q.size() != 1 )
...
{
  return ByteSet(
    mps_r.begin()->commitDiff(), ( ++mps_r.begin() )->commitDiff() );
}

inline ByteSet mkByteSet( int grow_r, int norm_r )
{
  return ByteSet(
    ByteCount( grow_r, ByteCount::K ), ByteCount( norm_r, ByteCount::K ) );
}
...
BOOST_AUTO_TEST_CASE( dudata )
{
  ...
  DiskUsageCounter duc( {DiskUsageCounter::MountPoint( "/grow",
                           DiskUsageCounter::MountPoint::Hint_growonly ),
  ...
  BOOST_CHECK_EQUAL(
    getSize( duc, pool ), mkByteSet( 15, 15 ) ); // (multi)install (old stays)
  ins.status().setTransact( true, ResStatus::USER );
  BOOST_CHECK_EQUAL(
    getSize( duc, pool ), mkByteSet( 15, 10 ) ); // update (old goes)
  ins.status().setTransact( false, ResStatus::USER );
  ...
  BOOST_CHECK_EQUAL(
    getSize( duc, pool ),
    mkByteSet( 5,
      0 ) ); // (multi)install (n could be 5 too, but satsolver does not know about multinstall)
  ins.status().setTransact( true, ResStatus::USER );
  BOOST_CHECK_EQUAL(
    getSize( duc, pool ), mkByteSet( 5, 0 ) ); // update (old goes)
  ins.status().setTransact( false, ResStatus::USER );
