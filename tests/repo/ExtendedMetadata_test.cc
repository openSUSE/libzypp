...
BOOST_AUTO_TEST_CASE( extended_metadata )
{
  ...
  BOOST_CHECK_EQUAL(
    repo.suggestedExpirationTimestamp(), Date( 1227279057 + 3600 ) );
  ...
  for_( it, repo.updatesProductBegin(), repo.updatesProductEnd() )
  {
    cpeids.push_back( it.cpeId() );
    labels.push_back( it.label() );
    count++;
  ...
  for_( it, repo.solvablesBegin(), repo.solvablesEnd() )
  {
    ...
    {
      BOOST_FAIL( str::form( "Repo has package not contemplated in test: %s",
                    s.ident().c_str() )
                    .c_str() );
    }
  }
