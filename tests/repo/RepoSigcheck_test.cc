...
struct KeyRingReceiver : public callback::ReceiveReport<KeyRingReport>
{
  ...
  virtual KeyTrust askUserToAcceptKey(
    const PublicKey &key, const KeyContext &keycontext = KeyContext() )
  {
    TAG;
    cblistCheck( __FUNCTION__ );
  ...
  virtual void infoVerify( const std::string &file_r,
  ...
  virtual bool askUserToAcceptUnsignedFile(
    const std::string &file, const KeyContext &keycontext = KeyContext() )
  {
    TAG;
    cblistCheck( __FUNCTION__ );
  ...
  virtual bool askUserToAcceptUnknownKey( const std::string &file,
    const std::string &id, const KeyContext &keycontext = KeyContext() )
  {
    TAG;
    cblistCheck( __FUNCTION__ );
...
inline std::string chr( const bool &v ) { return v ? "1" : "0"; }
...
BOOST_AUTO_TEST_CASE( init )
{
  ...
  RepoInfo repo;
  std::initializer_list<TriBool> tribools(
    {TriBool( indeterminate ), TriBool( true ), TriBool( false )} );
        ...
        for ( TriBool r_GpgCheck : tribools )
        {
          ...
          for ( TriBool r_RepoGpgCheck : tribools )
          {
            ...
            for ( TriBool r_PkgGpgCheck : tribools )
            {
              ...
              bool cfgGpgCheck =
                indeterminate( r_GpgCheck ) ? g_GpgCheck : bool( r_GpgCheck );
              TriBool cfgRepoGpgCheck =
                indeterminate( r_GpgCheck ) && indeterminate( r_RepoGpgCheck )
                  ? g_RepoGpgCheck
                  : r_RepoGpgCheck;
              TriBool cfgPkgGpgCheck =
                indeterminate( r_GpgCheck ) && indeterminate( r_PkgGpgCheck )
                  ? g_PkgGpgCheck
                  : r_PkgGpgCheck;
#if ( TC_VERBOSE )
              COUT << chr( cfgGpgCheck ) << "\t" << chr( cfgRepoGpgCheck )
                   << "\t" << chr( cfgPkgGpgCheck ) << "\t("
                   << chr( r_GpgCheck ) << "," << chr( g_GpgCheck ) << ")"
                   << "\t(" << chr( r_RepoGpgCheck ) << ","
                   << chr( g_RepoGpgCheck ) << ")"
                   << "\t(" << chr( r_PkgGpgCheck ) << ","
                   << chr( g_PkgGpgCheck ) << ")" << flush;
#endif
              ...
              if ( cfgGpgCheck )
              {
                ...
                BOOST_CHECK_EQUAL(
                  mandatoryCheck,
                  !bool(
                    !cfgRepoGpgCheck ) ); // TriBool: !false = true or indeterminate
              }
              else
              {
              ...
              for ( TriBool r_validSignature :
                tribools ) // indeterminate <==> unsigned repo
              {
                ...
                if ( r_validSignature && !willCheckRepo )
                  ...
                  BOOST_CHECK( sameTriboolState(
                    repo.validRepoSignature(), r_validSignature ) );

                bool willCheckPkg = repo.pkgGpgCheck();
                bool mandatoryCheck = repo.pkgGpgCheckIsMandatory();
#if ( TC_VERBOSE )
                COUT << "\t" << chr( r_validSignature )
                     << ( willCheckPkg ? ( mandatoryCheck ? "!" : "+" ) : "-" )
                     << flush;
#endif
                ...
                else if ( cfgGpgCheck )
                {
                  ...
                  else // TriBool: !true = false or indeterminate/unsigned
                  {
                    ...
                    BOOST_CHECK_EQUAL(
                      mandatoryCheck,
                      !bool(
                        !cfgPkgGpgCheck ) ); // TriBool: !false = true or indeterminate/unsigned
                  }
                }
                else
                {
...
struct KeyRingGuard
{
  KeyRingGuard( KeyRing::DefaultAccept accept_r = KeyRing::ACCEPT_NOTHING )
  {
    ...
    ZConfig &zcfg( ZConfig::instance() );
    COUT << "ZConf " << chr( zcfg.gpgCheck() ) << chr( zcfg.repoGpgCheck() )
         << chr( zcfg.pkgGpgCheck() ) << endl;
#endif
  }
...
void testLoadRepo(
  bool succeed_r, // whether loadRepos should succeed or fail with RepoException
  const std::string &repo_r, // name of the test repo to load
  KeyRing::DefaultAccept
    accept_r, // Callback response bits to set (mimics user input)
  KeyRingReceiver::CallbackList cblist_r ) // Callback sequence list expected
{
  ...
  if ( succeed_r )
    ...
    BOOST_CHECK_THROW(
      test.loadRepo( DATADIR / repo_r, repo_r ), repo::RepoException );
}
...
BOOST_AUTO_TEST_CASE( wrongsig_repo )
{
  ...
  testLoadRepo( false, repo, KeyRing::TRUST_KEY_TEMPORARILY,
    {"reportbegin", "askUserToAcceptKey", "infoVerify",
      "askUserToAcceptVerificationFailed", "reportend"} );
  testLoadRepo( false, repo, KeyRing::ACCEPT_NOTHING,
    {"reportbegin", "askUserToAcceptKey", "reportend"} );
  ...
  testLoadRepo( false, repo, KeyRing::ACCEPT_NOTHING,
    {"reportbegin", "infoVerify", "askUserToAcceptVerificationFailed",
      "reportend"} );
...
BOOST_AUTO_TEST_CASE( signed_repo )
{
  ...
  testLoadRepo( true, repo,
    KeyRing::KeyRing::
      ACCEPT_NOTHING, // relies on wrongsig_repo having accepted the key! (already in trusted ring)
