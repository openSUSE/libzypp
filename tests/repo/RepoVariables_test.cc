...
void helperGenRepVarExpandResults()
{
  ...
  auto varLookup = [&vartable, &varsoff](
    const std::string &name_r ) -> const std::string * {
    if ( varsoff )
      return nullptr;
  ...
  for ( auto &&value : {"", "$", "$${}", "$_:", "$_A:", "$_A_:", "$_A_B:",
          "${_A_B}", "\\${_A_B}" // no escape on level 0
          ,
          "${_A_B\\}" // no close brace
          ,
          "${C:-a$Bba}", "${C:+a$Bba}", "${C:+a${B}ba}",
          "${C:+a\\$Bba}" // escape on level > 0; no var $Bba
          ,
          "${C:+a$Bba\\}" // escape on level > 0; no close brace C
          ,
          "${C:+a${B}ba}", "${C:+a\\${B}ba}" // escape on level > 0; no var ${B}
          ,
  ...
  {
#define CSTR( STR )                                                            \
  str::form( "%-40s", str::gsub( "\"" + STR + "\"", "\\", "\\\\" ).c_str() )
    cout << "RepVarExpandTest( " << CSTR( el.first ) << ", "
         << CSTR( el.second.first ) << ", " << CSTR( el.second.second ) << " );"
         << endl;
  }
}

void RepVarExpandTest( const std::string &string_r,
  const std::string &allUndef_r, const std::string &allDef_r )
{
  std::map<std::string, std::string> vartable;
  bool varsoff = true;
...
{ //              ( STRING                                  , REPLACED_all_vars_undef                 , REPLACED_all_vars_defined                )
  ...
  RepVarExpandTest( "__${D:+\\$X--{${E:-==\\$X{o\\}==}  }--}__\\${B}${}__",
    "__--}__\\${B}${}__", "__$X--{[E]  --}__\\[B]${}__" );
  RepVarExpandTest( "__${D:+\\$X--{${E:-==\\$X{o\\}==}\\}--}__\\${B}${}__",
    "____\\${B}${}__", "__$X--{[E]}--__\\[B]${}__" );
}

void varInAuthExpect( const Url &url_r, const std::string &expHost_r,
  const std::string &expPort_r, const std::string &expPath_r,
  const std::string &user_r = std::string(),
  const std::string &pass_r = std::string() )
{
...
BOOST_AUTO_TEST_CASE( replace_text )
{
  ...
  BOOST_CHECK_EQUAL(
    replacer1( "${arch}it$archit $arch" ), "i686it$archit i686" );
  BOOST_CHECK_EQUAL(
    replacer1( "X${arch}it$archit $arch-it" ), "Xi686it$archit i686-it" );
  ...
  BOOST_CHECK_EQUAL(
    replacer1( "http://foo/$arch/bar" ), "http://foo/i686/bar" );
  ...
  BOOST_CHECK_EQUAL(
    replacer2( Url( "ftp://user:secret@site.org/$arch/" ) ).asCompleteString(),
    "ftp://user:secret@site.org/i686/" );

  BOOST_CHECK_EQUAL(
    replacer2( Url( "http://user:my$arch@site.org/$basearch/" ) )
      .asCompleteString(),
    "http://user:my$arch@site.org/i386/" );

  BOOST_CHECK_EQUAL(
    replacer2( Url( "http://site.org/update/?arch=$arch" ) ).asCompleteString(),
    "http://site.org/update/?arch=i686" );

  BOOST_CHECK_EQUAL(
    replacer2( Url( "http://site.org/update/$releasever/?arch=$arch" ) )
      .asCompleteString(),
  ...
  varInAuthExpect(
    replacer2( Url( "ftp://$arch/path" ) ), "i686", "", "/path" );
  varInAuthExpect(
    replacer2( Url( "ftp://$arch:1234/path" ) ), "i686", "1234", "/path" );
