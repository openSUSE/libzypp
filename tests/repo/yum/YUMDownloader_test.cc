...
BOOST_AUTO_TEST_CASE( yum_download )
{
  ...
  const char *files[] = {//    "filelists.xml.gz",
                         //    "other.xml.gz",
    "patches.xml", "patch-fetchmsttfonts.sh-2333.xml",
    "patch-flash-player-2359.xml", "patch-glabels-2348.xml",
    "patch-gv-2350.xml", "patch-openssl-2349.xml", "patch-tar-2351.xml",
    "primary.xml.gz", "repomd.xml", "repomd.xml.asc", "repomd.xml.key", NULL};

  int i = 0;
  while ( files[ i ] != NULL )
  {
    BOOST_CHECK_MESSAGE(
      PathInfo( localdir + "/repodata/" + files[ i ] ).isExist(),
      ( string( "/repodata/" ) + files[ i ] ).c_str() );
    i++;
  }
}
