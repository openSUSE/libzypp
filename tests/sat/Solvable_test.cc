...
BOOST_AUTO_TEST_CASE( attributes )
{
        ...
        it != r.solvablesEnd(); ++it )
  {
    ...
    if ( s.ident() == "pattern:apparmor" )
    {
      ...
      BOOST_CHECK_EQUAL(
        p->vendor(), "SUSE LINUX Products GmbH, Nuernberg, Germany" );
...
BOOST_AUTO_TEST_CASE( asStringTest )
{
  ...
  BOOST_CHECK_EQUAL(
    sat::Solvable( 2 ).asString(), "product:openSUSE-11.1.x86_64" );
  BOOST_CHECK_EQUAL(
    sat::Solvable( 3693 ).asString(), "autoyast2-2.16.19-0.1.src" );
...
BOOST_AUTO_TEST_CASE( SplitIdent )
{
  ...
  split =
    sat::Solvable::SplitIdent( ResKind::srcpackage, "package:nokind:foo" );
  BOOST_CHECK_EQUAL( split.ident(), "nokind:foo" );
  BOOST_CHECK_EQUAL( split.kind(), ResKind::package );
  BOOST_CHECK_EQUAL( split.name(), "nokind:foo" );
  ...
  split =
    sat::Solvable::SplitIdent( ResKind::srcpackage, "pattern:nokind:foo" );
  BOOST_CHECK_EQUAL( split.ident(), "pattern:nokind:foo" );
  BOOST_CHECK_EQUAL( split.kind(), ResKind::pattern );
  BOOST_CHECK_EQUAL( split.name(), "nokind:foo" );

  split =
    sat::Solvable::SplitIdent( ResKind::package, "srcpackage:nokind:foo" );
  BOOST_CHECK_EQUAL( split.ident(), "nokind:foo" );
  BOOST_CHECK_EQUAL( split.kind(), ResKind::srcpackage );
  BOOST_CHECK_EQUAL( split.name(), "nokind:foo" );

  split =
    sat::Solvable::SplitIdent( ResKind::pattern, "srcpackage:nokind:foo" );
  BOOST_CHECK_EQUAL( split.ident(), "nokind:foo" );
  BOOST_CHECK_EQUAL( split.kind(), ResKind::srcpackage );
  BOOST_CHECK_EQUAL( split.name(), "nokind:foo" );

  split =
    sat::Solvable::SplitIdent( ResKind::srcpackage, "srcpackage:nokind:foo" );
  BOOST_CHECK_EQUAL( split.ident(), "nokind:foo" );
  BOOST_CHECK_EQUAL( split.kind(), ResKind::srcpackage );
  BOOST_CHECK_EQUAL( split.name(), "nokind:foo" );
}
...
BOOST_AUTO_TEST_CASE( duData )
{
  ...
  sat::Solvable s =
    *sat::WhatProvides( Capability( "glibc-devel.x86_64 == 2.8.90-2.3" ) )
       .begin();
