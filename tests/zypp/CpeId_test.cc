...
BOOST_AUTO_TEST_CASE( cpeid_value_string_wildcarded )
{
  for ( const auto &c : {wildcarded} )
  {
    ...
    BOOST_CHECK(
      c !=
      wildcarded ); // !!! According to the CPE Name Matching Specification Version 2.3
// unquoted wildcard characters yield an undefined result (not ==).
  ...
  BOOST_CHECK(
    wildcarded2 !=
    wildcarded2 ); // unquoted wildcard characters yield an undefined result (not ==).
#else
...
BOOST_AUTO_TEST_CASE( cpeid_value_valid )
{
  ...
  for ( char ch = 0; ch < CHAR_MAX; ++ch )
  {
    ...
    char pchstr[] = {'%', hdig[ (unsigned char)( ch ) / 16 ],
      hdig[ (unsigned char)( ch ) % 16 ], '\0'};

    if ( ch == '\0' )
    {
      BOOST_CHECK( Value( chstr ).isNA() );
      BOOST_CHECK_THROW(
        ( Value( chstr, Value::fsFormat ) ), std::invalid_argument );
    ...
    else if ( ch <= ' ' || '~' < ch )
    {
      ...
      BOOST_CHECK_THROW(
        ( Value( chstr, Value::fsFormat ) ), std::invalid_argument );
      BOOST_CHECK_THROW(
        ( Value( chstr, Value::uriFormat ) ), std::invalid_argument );
    }
  ...
  BOOST_CHECK_EQUAL(
    Value( "\\!\\a\\-\\_\\.\\!", Value::fsFormat ).asFs(), "\\!a-_.\\!" );
}
...
BOOST_AUTO_TEST_CASE( cpeid_type_checks )
{
  for ( const auto &c : {Value::ANY, Value::NA, wildcardfree, wildcarded} )
  {
    ...
    BOOST_CHECK_EQUAL(
      c.isWildcardfree(), c.type() == Value::Type::wildcardfree );
...
BOOST_AUTO_TEST_CASE( cpeid_compare )
{
  ...
  BOOST_CHECK(
    compare( Value::ANY, wildcardfree, SetCompare::properSuperset ) );
#if WFN_STRICT_SPEC
  ...
  BOOST_CHECK(
    compare( wildcarded, wildcardfree, SetCompare::properSuperset ) );
...
BOOST_AUTO_TEST_CASE( cpeid_value_string_wildcard )
{
  ...
  for ( const auto &c : {Value( "*\\*" ), Value( "\\**" ), Value( "?" ),
          Value( "??\\?" ), Value( "\\???" )} )
  {
    BOOST_CHECK( !c.isWildcardfree() );
    BOOST_CHECK( c.isWildcarded() );
...
BOOST_AUTO_TEST_CASE( cpeid_basics )
{
  ...
  for ( const auto &c : {CpeId(), CpeId( nullptr ), CpeId( "" ),
          CpeId( std::string() ), CpeId( "cpe:2.3:" ), CpeId( "cpe:/" )} )
  {
...
void testStrconv(
  const std::string &fs, const std::string &uri, const std::string &wfn )
{
  CpeId fromFS( fs );
  CpeId fromURI( uri );
...
BOOST_AUTO_TEST_CASE( cpeid_strconv )
{
  ...
  testStrconv( "cpe:2.3:a:opensuse:lib\\:zypp:14.16.0:beta:*:*:*:*:*:-",
    ...
    "wfn:[part=\"a\",vendor=\"opensuse\",product=\"lib\\:zypp\",version=\"14\\."
    "16\\.0\",update=\"beta\",other=NA]" );

  testStrconv(
    "cpe:2.3:a:hp:insight_diagnostics:7.4.0.1570:-:*:*:online:win2003:x64:*",
    "cpe:/a:hp:insight_diagnostics:7.4.0.1570:-:~~online~win2003~x64~",
    "wfn:[part=\"a\",vendor=\"hp\",product=\"insight_diagnostics\",version="
    "\"7\\.4\\.0\\.1570\",update=NA,sw_edition=\"online\",target_sw="
    "\"win2003\",target_hw=\"x64\"]" );
  ...
  testStrconv( "cpe:2.3:a:foo\\\\bar:big\\$money_manager_2010:*:*:*:*:special:"
               "ipod_touch:80gb:*",
    "cpe:/a:foo%5cbar:big%24money_manager_2010:::~~special~ipod_touch~80gb~",
    "wfn:[part=\"a\",vendor=\"foo\\\\bar\",product=\"big\\$money_manager_"
    "2010\",sw_edition=\"special\",target_sw=\"ipod_touch\",target_hw="
    "\"80gb\"]" );

  BOOST_CHECK_THROW(
    ( CpeId( "cpe:/x:" ) ), std::invalid_argument ); // illegal part 'x'
  BOOST_CHECK_THROW(
    CpeId(
      "cpe:/a:foo%5cbar:big%24money_2010%07:::~~special~ipod_touch~80gb~" ),
    std::invalid_argument ); // illegal %07
  BOOST_CHECK_EQUAL( CpeId( "cpe:/a:foo~bar:big%7emoney_2010" ).asUri(),
    "cpe:/a:foo%7ebar:big%7emoney_2010" ); // unescaped ~ is ok but not preferred
}
