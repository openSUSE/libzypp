...
{
  KeyRing::setDefaultAccept(
    KeyRing::ACCEPT_UNKNOWNKEY | KeyRing::ACCEPT_UNSIGNED_FILE );
  ...
  repo::DeltaCandidates dc(
    list<Repository>( pool.reposBegin(), pool.reposEnd() ), "libzypp" );
  ...
  for_( it, deltas.begin(), deltas.end() )
  {
    ...
    cout << ( it->edition().match( Edition( "4.21.3-2" ) ) == 0 )
         << endl; // match returns -1,0,1
    cout << ( it->edition().match( "4.21.3-2" ) == 0 )
         << endl; // match returns -1,0,1
  }
}
