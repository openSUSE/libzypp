...
BOOST_AUTO_TEST_CASE( fetcher_enqueuedir_noindex )
{
  ...
  // do the test by trusting the SHA1SUMS file signature key
  {
    ...
    BOOST_CHECK(
      PathInfo( dest.path() + "/complexdir/subdir2/subdir2-file1.txt" )
        .isExist() );
    BOOST_CHECK(
      PathInfo( dest.path() + "/complexdir/subdir1/subdir1-file1.txt" )
        .isExist() );
    BOOST_CHECK(
      PathInfo( dest.path() + "/complexdir/subdir1/subdir1-file2.txt" )
        .isExist() );
  }
}
...
BOOST_AUTO_TEST_CASE( fetcher_enqueuedir_autoindex )
{
  ...
  // do the test by trusting the SHA1SUMS file signature key
  {
    ...
    getZYpp()->keyRing()->importKey(
      PublicKey( DATADIR + "/complexdir/subdir1/SHA1SUMS.key" ), true );
    Fetcher fetcher;
    fetcher.setOptions( Fetcher::AutoAddIndexes );
    fetcher.enqueueDir( OnMediaLocation( "/complexdir" ), true );
    fetcher.start( dest.path(), media );
...
BOOST_AUTO_TEST_CASE( fetcher_enqueue_digested_dir_autoindex )
{
  ...
  // do the test by trusting the SHA1SUMS file signature key but with a broken file
  {
    ...
    // add the key as trusted
    getZYpp()->keyRing()->importKey(
      PublicKey( DATADIR + "/complexdir-broken/subdir1/SHA1SUMS.key" ), true );
    fetcher.setOptions( Fetcher::AutoAddIndexes );
    fetcher.enqueueDigestedDir( OnMediaLocation( "/complexdir-broken" ), true );
    BOOST_CHECK_THROW(
      fetcher.start( dest.path(), media ), FileCheckException );
    fetcher.reset();
  }
}
...
BOOST_AUTO_TEST_CASE( fetcher_enqueuebrokendir_noindex )
{
  ...
  // do the test by trusting the SHA1SUMS file signature key but with a broken file
  {
    ...
    BOOST_CHECK(
      PathInfo( dest.path() + "/complexdir-broken/subdir2" ).isExist() );
    BOOST_CHECK(
      PathInfo( dest.path() + "/complexdir-broken/subdir2/subdir2-file1.txt" )
        .isExist() );
    BOOST_CHECK(
      PathInfo( dest.path() + "/complexdir-broken/subdir1/subdir1-file1.txt" )
        .isExist() );
    BOOST_CHECK(
      PathInfo( dest.path() + "/complexdir-broken/subdir1/subdir1-file2.txt" )
        .isExist() );
  }
}
...
BOOST_AUTO_TEST_CASE( fetcher_enqueue_digested_broken_with_autoindex )
{
  ...
  // do the test by trusting the SHA1SUMS file signature key but with a broken file
  {
    ...
    getZYpp()->keyRing()->importKey(
      PublicKey( DATADIR + "/complexdir-broken/subdir1/SHA1SUMS.key" ), true );
    fetcher.setOptions( Fetcher::AutoAddIndexes );
    fetcher.enqueueDigested(
      OnMediaLocation( "/complexdir-broken/subdir1/subdir1-file1.txt" ) );
    BOOST_CHECK_THROW( fetcher.start( dest.path(), media ), Exception );
    fetcher.reset();
  }
}
...
BOOST_AUTO_TEST_CASE( fetcher_enqueue_digested_with_autoindex )
{
  ...
  // checksum in auto discovered index
  {
    ...
    fetcher.setOptions( Fetcher::AutoAddIndexes );
    fetcher.enqueueDigested(
      OnMediaLocation( "/complexdir/subdir1/subdir1-file1.txt" ) );
    fetcher.start( dest.path(), media );
    fetcher.reset();
  }
}
...
BOOST_AUTO_TEST_CASE( fetcher_simple )
{
...

  {
    ...
    loc.setChecksum(
      CheckSum::sha1( "f1d2d2f924e986ac86fdf7b36c94bcdf32beec16" ) );
    fetcher.enqueueDigested( loc );
    BOOST_CHECK_THROW( fetcher.start( dest.path(), media ), Exception );
    fetcher.reset();
  }
...
BOOST_AUTO_TEST_CASE( content_index )
{
  ...
  // test transfering one file by setting the index
  {
    ...
    getZYpp()->keyRing()->importKey(
      PublicKey( DATADIR + "/contentindex/content.key" ), true );
    fetcher.addIndex( OnMediaLocation( "/contentindex/content", 1 ) );
    fetcher.enqueue( loc );
    fetcher.start( dest.path(), media );
    fetcher.reset();
    BOOST_CHECK(
      PathInfo( dest.path() + "/contentindex/subdir1/subdir1-file1.txt" )
        .isExist() );
  }
}
...
BOOST_AUTO_TEST_CASE( enqueue_broken_content_index )
{
  ...
  {
    OnMediaLocation loc(
      "/contentindex-broken-digest/subdir1/subdir1-file1.txt", 1 );
    // key was already imported as trusted
    fetcher.addIndex(
      OnMediaLocation( "/contentindex-broken-digest/content", 1 ) );
    fetcher.enqueue( loc );
    fetcher.start( dest.path(), media );
    fetcher.reset();
    BOOST_CHECK(
      PathInfo(
        dest.path() + "/contentindex-broken-digest/subdir1/subdir1-file1.txt" )
        .isExist() );

    // now retrieve a file that is modified, so the checksum has to fail
    loc = OnMediaLocation(
      "/contentindex-broken-digest/subdir1/subdir1-file2.txt", 1 );
    fetcher.addIndex(
      OnMediaLocation( "/contentindex-broken-digest/content", 1 ) );
    fetcher.enqueue( loc );
    BOOST_CHECK_THROW( fetcher.start( dest.path(), media ), Exception );
...
BOOST_AUTO_TEST_CASE( enqueue_broken_content_noindex )
{
...

  {
    ...
    BOOST_CHECK(
      PathInfo(
        dest.path() + "/contentindex-broken-digest/subdir1/subdir1-file2.txt" )
        .isExist() );
  }
}
...
{
  WebServer web(
    ( Pathname( TESTS_SRC_DIR ) + "/zypp/data/Fetcher/remote-site" ).c_str(),
    10001 );
  web.start();
