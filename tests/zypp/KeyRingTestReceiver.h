  ...
  : public zypp::callback::ReceiveReport<zypp::KeyRingReport>
{
  ...
  ~KeyRingTestReceiver() { disconnect(); }

  void answerAcceptVerFailed( bool answer ) { _answer_ver_failed = answer; }

  bool askedAcceptVerFailed() const { return _asked_user_to_accept_ver_failed; }
  ...
  bool askedAcceptKey() const { return _asked_user_to_accept_key; }
  ...
  virtual bool askUserToAcceptUnsignedFile(
    const std::string &file, const zypp::KeyContext &keycontext )
  {
    MIL << std::endl;
  ...
  virtual bool askUserToAcceptUnknownKey( const std::string &file,
    const std::string &id, const zypp::KeyContext &keycontext )
  {
    MIL << std::endl;
  ...
  virtual KeyRingReport::KeyTrust askUserToAcceptKey(
    const zypp::PublicKey &key, const zypp::KeyContext &keycontext )
  {
    MIL << std::endl;
 ...
 * Can record which callbacks were called
 */
struct KeyRingTestSignalReceiver
  : zypp::callback::ReceiveReport<zypp::KeyRingSignals>
{
  ...
  ~KeyRingTestSignalReceiver() { disconnect(); }

  virtual void trustedKeyAdded( const zypp::PublicKey &key )
  {
    MIL << "TEST: trusted key added to zypp Keyring. Synchronizing keys with "
           "fake rpm keyring"
        << std::endl;
