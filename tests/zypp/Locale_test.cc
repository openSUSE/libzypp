...
BOOST_AUTO_TEST_CASE( static_deps )
{
  ...
  BOOST_CHECK_EQUAL(
    IdString( Locale::enCode ), IdString( LanguageCode::enCode ) );

  // solver communication: Both must lead to the same ID
  BOOST_CHECK_EQUAL(
    Locale::enCode.id(), IdString( LanguageCode::enCode.code() ).id() );
}
...
BOOST_AUTO_TEST_CASE( no_codes )
{
  ...
  BOOST_CHECK_EQUAL(
    Locale::noCode.language().id(), LanguageCode::noCode.id() );
...
BOOST_AUTO_TEST_CASE( locale )
{
  // IdString::Null (rare)
  {
    for ( const Locale &l : {Locale( nullptr ),
            Locale( LanguageCode( nullptr ), CountryCode( nullptr ) )} )
    {
  ...
  {
    for ( const Locale &l :
      {Locale(), Locale( "" ), Locale( "@UTF-8" ), Locale( ".UTF-8" ),
  ...
  {
    for ( const Locale &l :
      {Locale( "de" ), Locale( "de@UTF-8" ), Locale( LanguageCode( "de" ) )} )
    {
