...
BOOST_AUTO_TEST_CASE( InitialSettings ) { PluginScript::defaultTimeout( 3 ); }
...
BOOST_AUTO_TEST_CASE( PluginFrameCtorAssign )
{
  ...
  h.addHeader( {{"a", "a1"}, {"a", "a2"}, {"b", "b1"}, {"b", "b2"}, {"c", "c1"},
    {"c", "c1"}} );
...
BOOST_AUTO_TEST_CASE( PluginFrameExceptipn )
{
  ...
  BOOST_CHECK_THROW(
    doParse( "c" ), PluginFrameException ); // no NL after command
  BOOST_CHECK_THROW(
...
BOOST_AUTO_TEST_CASE( PluginScriptTest )
{
  ...
  BOOST_CHECK_THROW(
    scr.open( "bla" ), PluginScriptException ); // script does not exist
  ...
  BOOST_CHECK_THROW(
    scr.open( "/bin/ls" ), PluginScriptException ); // already open
  ...
  BOOST_CHECK_EQUAL( scr.getPid(), PluginScript::NotConnected );
  BOOST_CHECK_EQUAL(
    scr.script(), "/bin/cat" ); // not reset by close, may be reused by open()
...
BOOST_AUTO_TEST_CASE( PluginScriptSend )
{
  ...
  scr.open();
  BOOST_CHECK_THROW(
    do { scr.send( f ); } while ( true ), PluginScriptSendTimeout );
