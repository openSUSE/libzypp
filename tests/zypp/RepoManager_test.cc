...
#define REPODATADIR                                                            \
  ( Pathname( TESTS_SRC_DIR ) + "/repo/susetags/data/addon_in_subdir" )
...
BOOST_AUTO_TEST_CASE( pluginservices_test )
{
...

  {
    ...
    BOOST_CHECK_EQUAL(
      ( DATADIR / "/plugin-service-lib-1/services/service" ).asFileUrl(),
      service.url().asString() );
  ...
  opts.pluginsPath = DATADIR + "/plugin-service-lib-2";
  {
    ...
    BOOST_CHECK_EQUAL( "service", service.alias() );
    BOOST_CHECK_EQUAL(
      ( DATADIR / "/plugin-service-lib-2/services/service" ).asFileUrl(),
      service.url().asString() );
    // now refresh the service
    manager.refreshServices();
...
BOOST_AUTO_TEST_CASE( repomanager_test )
{
  TmpDir tmpCachePath;
  RepoManagerOptions opts( RepoManagerOptions::makeTestSetup( tmpCachePath ) );
  opts.servicesTargetDistro =
    "sles-10-i586"; // usually determined by the Target

  filesystem::mkdir( opts.knownReposPath );
  filesystem::mkdir( opts.knownServicesPath );
  BOOST_CHECK_EQUAL(
    filesystem::copy_dir_content( DATADIR + "/repos.d", opts.knownReposPath ),
    0 );
  ...
  BOOST_CHECK(
    PathInfo( opts.knownReposPath + "/proprietary.repo_1" ).isExist() );
  ...
  BOOST_CHECK_THROW(
    manager.addRepository( office_dup ), RepoAlreadyExistsException );
  ...
  std::list<RepoInfo> infos;
  manager.getRepositoriesInService(
    "test", insert_iterator<std::list<RepoInfo>>( infos, infos.begin() ) );
  BOOST_CHECK_EQUAL( infos.size(), 2 ); // 2 from new repoindex
  ...
  repo.setBaseUrl(
    ( Pathname( TESTS_SRC_DIR ) / "/repo/yum/data/10.2-updates-subset" )
      .asDirUrl() );
  ...
  BOOST_CHECK_MESSAGE(
    !manager.isCached( repo ), "Repo should not yet be cached" );
  ...
  RepoManager::RefreshCheckStatus ref_stat =
    manager.checkIfToRefreshMetadata( repo, *repo.baseUrlsBegin() );
  SEC << endl << ref_stat << endl;
  BOOST_CHECK_MESSAGE( ref_stat == RepoManager::REPO_UP_TO_DATE ||
                         ref_stat == RepoManager::REPO_CHECK_DELAYED,
  ...
  BOOST_CHECK_MESSAGE( PathInfo( solvfile ).isExist(),
    "Solv file is created after caching: " + solvfile.asString() );
  BOOST_CHECK_MESSAGE( PathInfo( cookiefile ).isExist(),
    "Cookie file is created after caching: " + cookiefile.asString() );
...
BOOST_AUTO_TEST_CASE( repo_seting_test )
{
  ...
  BOOST_CHECK_MESSAGE(
    !repo.keepPackages(), "keepPackages must default to OFF" );
}
