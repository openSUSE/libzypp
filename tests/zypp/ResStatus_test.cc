...
inline Tp *begin( Tp ( &_array )[ N ] )
{
  return _array;
}

template <class Tp, int N>
inline Tp *end( Tp ( &_array )[ N ] )
{
  return _array + ( sizeof( _array ) / sizeof( Tp ) );
}

ResStatus::TransactByValue transactByValues[] = {ResStatus::USER,
  ResStatus::APPL_HIGH, ResStatus::APPL_LOW, ResStatus::SOLVER};
...
// Result evaluation
typedef void ( *Evaluate )( ResStatus::TransactValue,
  ResStatus::TransactByValue,       /* fromState, fromBy */
...
void testTable( Transition transition, Evaluate evaluate )
{
  ...
  //
  for ( ResStatus::TransactByValue *toBy = begin( transactByValues );
        toBy != end( transactByValues ); ++toBy )
  {
    for ( ResStatus::TransactByValue *fromBy = begin( transactByValues );
          fromBy != end( transactByValues ); ++fromBy )
    {
      INT << "=== " << *fromBy << " ==> " << *toBy << " ===" << endl;
      for ( ResStatus::TransactValue *fromState = begin( transactValues );
            fromState != end( transactValues ); ++fromState )
      {
        ResStatus from;
        ...
        for ( bool *toState = begin( transactTo ); toState != end( transactTo );
              ++toState )
        {
          ...
          if ( !done )
            BOOST_CHECK_EQUAL(
              from, result ); // status stays unchaged on failure!
...
#define CHECK_DONE_ALWAYS                                                      \
  X( done, true );                                                             \
  if ( !done )                                                                 \
  return

// Transition succeeds if same or higher TransactByValue
#define CHECK_DONE_IFCAUSER                                                    \
  X( done, toBy >= fromBy );                                                   \
  if ( !done )                                                                 \
  return

// Transition succeeds if a locker (APPL_HIGH or USER)
#define CHECK_DONE_ALWAYS_IFLOCKER                                             \
  X( done, toBy >= ResStatus::APPL_HIGH );                                     \
  if ( !done )                                                                 \
  return

// Transition succeeds if a locker (APPL_HIGH or USER) and  same or higher TransactByValue
#define CHECK_DONE_IFCAUSER_ISLOCKER                                           \
  X( done, toBy >= max( fromBy, ResStatus::APPL_HIGH ) );                      \
  if ( !done )                                                                 \
  return
...
#define CHECK_CAUSER_RAISED                                                    \
  X( result.getTransactByValue(), max( fromBy, toBy ) )
...
#define CHECK_CAUSER_TO_SOLVER                                                 \
  X( result.getTransactByValue(), ResStatus::SOLVER )
...
#define DOCHECK( FROMSTATE, TOSTATE, C_DONE, C_STATE, C_CAUSER )               \
  if ( ResStatus::FROMSTATE == fromState && TOSTATE == toState )               \
  {                                                                            \
    C_DONE;                                                                    \
    CHECK_STATE( C_STATE );                                                    \
    C_CAUSER;                                                                  \
...
void evaluateSetTransact( ResStatus::TransactValue fromState,
  ResStatus::TransactByValue fromBy, bool toState,
  ResStatus::TransactByValue toBy, bool done, ResStatus result )
{
  ResStatus from;
  initStatus( fromState, fromBy, from );
  MIL << from << " =setTransact(" << toState << "," << toBy << ")=>\t" << done
      << ":" << result << endl;

  DOCHECK( TRANSACT, true, CHECK_DONE_ALWAYS, TRANSACT, CHECK_CAUSER_RAISED );
  DOCHECK( TRANSACT, false, CHECK_DONE_IFCAUSER, KEEP_STATE,
    CHECK_CAUSER_RAISED ); // from transact into softlock
  DOCHECK( KEEP_STATE, true, CHECK_DONE_ALWAYS, TRANSACT, CHECK_CAUSER_SET );
  DOCHECK( KEEP_STATE, false, CHECK_DONE_ALWAYS, KEEP_STATE,
    CHECK_CAUSER_STAYS ); // keep is not raised to softlock
...
void evaluateSetSoftTransact( ResStatus::TransactValue fromState,
  ResStatus::TransactByValue fromBy, bool toState,
  ResStatus::TransactByValue toBy, bool done, ResStatus result )
{
  ResStatus from;
  initStatus( fromState, fromBy, from );
  MIL << from << " =setSoftTransact(" << toState << "," << toBy << ")=>\t"
      << done << ":" << result << endl;

  DOCHECK( TRANSACT, true, CHECK_DONE_ALWAYS, TRANSACT, CHECK_CAUSER_RAISED );
  DOCHECK( TRANSACT, false, CHECK_DONE_IFCAUSER, KEEP_STATE,
    CHECK_CAUSER_RAISED ); // from transact into softlock
  DOCHECK( KEEP_STATE, true, CHECK_DONE_IFCAUSER, TRANSACT,
    CHECK_CAUSER_SET ); // leaving KEEP requires sup. causer
  DOCHECK( KEEP_STATE, false, CHECK_DONE_ALWAYS, KEEP_STATE,
    CHECK_CAUSER_STAYS ); // keep is not raised to softlock
  DOCHECK( LOCKED, true, CHECK_DONE_IFCAUSER, TRANSACT, CHECK_CAUSER_SET );
  DOCHECK( LOCKED, false, CHECK_DONE_ALWAYS, LOCKED, CHECK_CAUSER_STAYS );
...
void evaluateSetLock( ResStatus::TransactValue fromState,
  ResStatus::TransactByValue fromBy, bool toState,
  ResStatus::TransactByValue toBy, bool done, ResStatus result )
{
  ResStatus from;
  initStatus( fromState, fromBy, from );
  MIL << from << " =setLock(" << toState << "," << toBy << ")=>\t" << done
      << ":" << result << endl;

  DOCHECK( TRANSACT, true, CHECK_DONE_IFCAUSER_ISLOCKER, LOCKED,
    CHECK_CAUSER_SET ); // transact is 'not locked'
  DOCHECK( TRANSACT, false, CHECK_DONE_ALWAYS, TRANSACT, CHECK_CAUSER_STAYS );
  DOCHECK(
    KEEP_STATE, true, CHECK_DONE_ALWAYS_IFLOCKER, LOCKED, CHECK_CAUSER_SET );
  DOCHECK(
    KEEP_STATE, false, CHECK_DONE_ALWAYS, KEEP_STATE, CHECK_CAUSER_STAYS );
  DOCHECK( LOCKED, true, CHECK_DONE_ALWAYS, LOCKED, CHECK_CAUSER_RAISED );
  DOCHECK(
    LOCKED, false, CHECK_DONE_IFCAUSER, KEEP_STATE, CHECK_CAUSER_TO_SOLVER );
}
...
BOOST_AUTO_TEST_CASE( savestate )
{
  ...
  BOOST_CHECK_EQUAL(
    WhilePoolItemSameStateIsPrivate( ostatus, nstatus ), true );
  nstatus.setLock( true, ResStatus::USER );
  BOOST_CHECK_EQUAL(
    WhilePoolItemSameStateIsPrivate( ostatus, nstatus ), false );
  ostatus = nstatus;
