...
void testUrlAuthority( const Url &url_r, const std::string &host_r,
  const std::string &port_r = std::string(),
...
BOOST_AUTO_TEST_CASE( test_ipv6_url )
{
  ...
  str =
    "http://user:pass@[2001:DB8:0:F102::1]:8080/64/sles11/RC1/CD1?device=eth0";
  url = Url( str );
...
BOOST_AUTO_TEST_CASE( test_url1 )
{
  ...
  BOOST_CHECK_EQUAL( str,
    url.asString(
      zypp::url::ViewOptions() - zypp::url::ViewOption::EMPTY_AUTHORITY ) );
  ...
  BOOST_CHECK_EQUAL(
    zypp::Url( str ).asString(), "ftp://user@localhost/%2f/srv/ftp" );
  ...
  BOOST_CHECK_EQUAL(
    zypp::Url( str ).asString(), "http://just-localhost.example.net:8080/" );
...
BOOST_AUTO_TEST_CASE( test_url2 )
{
  ...
  BOOST_CHECK_EQUAL(
    url.asString(), "http://user@localhost/path/to?arg=val#frag" );
...
BOOST_AUTO_TEST_CASE( test_url3 )
{
  ...
  BOOST_CHECK_EQUAL(
    url.asString(), "http://localhost/path/to?hoho=ha%20ha#frag" );

  // will be encoded as "foo%3Dbar%26key=foo%26bar%3Dvalue"
  key = "foo=bar&key";
  val = "foo&bar=value";
  url.setQueryParam( key, val );
  BOOST_CHECK_EQUAL( url.asString(), "http://localhost/path/"
                                     "to?foo%3Dbar%26key=foo%26bar%3Dvalue&"
                                     "hoho=ha%20ha#frag" );
  ...
  url.setQueryParam( key, val );
  BOOST_CHECK_EQUAL( url.asString(), "http://localhost/path/"
                                     "to?foo%25bar=is%25de%25ad&foo%3Dbar%"
                                     "26key=foo%26bar%3Dvalue&hoho=ha%20ha#"
                                     "frag" );

  // get encoded query parameters and compare with results:
  zypp::url::ParamVec params( url.getQueryStringVec() );
  const char *const result[] = {"foo%25bar=is%25de%25ad",
    "foo%3Dbar%26key=foo%26bar%3Dvalue", "hoho=ha%20ha"};
...
BOOST_AUTO_TEST_CASE( test_url4 )
{
  try
  {
    ...
    for ( m = pmap.begin(); m != pmap.end(); ++m )
    {
      ...
      else if ( "filter" == m->first )
      {
        BOOST_CHECK_EQUAL( m->second, "(cn=*)" );
      }
      else if ( "scope" == m->first )
      {
...
BOOST_AUTO_TEST_CASE( test_url5 )
{
  ...
  BOOST_CHECK_EQUAL(
    Url( zypp::url::encode( str, URL_SAFE_CHARS ) ).asString(), str );
}
