...
BOOST_AUTO_TEST_CASE( gsubTest )
{
  ...
  BOOST_CHECK_EQUAL(
    gsub( "test olds string", olds, news ), "test new string string" );
...
BOOST_AUTO_TEST_CASE( bnc_909772 )
{
  ...
  s += "'"
       "\\\\\" \\ \\\\"
       "'\\ single"; // single quote: all literal, no ' inside

  s += " ";
  s += "\""
       "\\'\\\" \\ \\\\"
       "\"\\ double"; // double quote: all literal except \\ \"
...
BOOST_AUTO_TEST_CASE( conversions_to_bool )
{
  ...
  bool ret;
  ret = true;
  BOOST_CHECK_EQUAL( str::strToBoolNodefault( "TRUE", ret ), true );
  ret = true;
  BOOST_CHECK_EQUAL( str::strToBoolNodefault( "FALSE", ret ), false );
  ret = true;
  BOOST_CHECK_EQUAL( str::strToBoolNodefault( "", ret ), true );
  ret = true;
  BOOST_CHECK_EQUAL( str::strToBoolNodefault( "foo", ret ), true );
