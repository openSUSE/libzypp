...
BOOST_AUTO_TEST_CASE( SysconfigWrite )
{
  ...
  BOOST_REQUIRE_THROW( zypp::base::sysconfig::writeStringVal( "/tmp/wrzlprmpf",
                         "PROXY_ENABLED", "yes", "# fifi\n fofo\n" ),
    zypp::Exception );
  BOOST_CHECK( zypp::base::sysconfig::writeStringVal(
    tmpf.path(), "PROXY_ENABLED", "yes", "# fifi\n fofo\n" ) );
  BOOST_CHECK(
    !zypp::base::sysconfig::writeStringVal( tmpf.path(), "NEW1", "12" ) );
  BOOST_CHECK( zypp::base::sysconfig::writeStringVal(
    tmpf.path(), "NEW2", "13", "# fifi\n# fofo" ) );
  BOOST_CHECK( zypp::base::sysconfig::writeStringVal(
    tmpf.path(), "NEW3", "13\"str\"", "fifi\nffofo" ) );

  std::ostringstream s;
  ExternalProgram( "diff -u " + file.asString() + " " + tmpf.path().asString() +
                   " | tail -n +3" ) >>
    s;
