...
static void migrate_sources( const Options &opt )
{
  if ( getenv( "YAST_IS_RUNNING" ) &&
       ( string( getenv( "YAST_IS_RUNNING" ) ) == "instsys" ) )
  {
    MIL << "YaST is running in instsys. Not migrating old sources. YaST will "
           "do it."
        << endl;
    return;
  }
...
void usage( int argc, char **argv )
{
  ...
  cout << "Usage:" << endl;
  cout << argv[ 0 ] << " [--root root-path] [--fake] [--sp sources-path]"
       << endl;
}
