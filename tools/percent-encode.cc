...
int main( int argc, const char *argv[] )
{
  ...
  if ( argc )
  {
    ...
    else if ( *argv == std::string( "-h" ) || *argv == std::string( "--help" ) )
    {
      ...
      std::cout
        << "Read lines from stdin and write them percent encoded to stdout."
        << std::endl;
      std::cout << "" << std::endl;
      std::cout << "Option:" << std::endl;
      std::cout << " -d, --decode  Decode lines read from stdin instead of "
                   "encoding them."
                << std::endl;
  ...
  std::string ( *coder )( const std::string &str_r ) =
    encode ? doEncode : doDecode;
