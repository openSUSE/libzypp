...
int usage( const std::string &msg_r = std::string(), int exit_r = 100 )
{
  ...
  cerr << "Usage: " << appname
       << " [--root ROOTDIR] [OPTIONS] NAME... [[OPTIONS] NAME...]..." << endl;
  cerr << "  Load all enabled repositories (no refresh) and search for" << endl;
  cerr << "  occurrences of NAME (regex or -x) in package names or dependencies"
       << endl;
  cerr
  ...
  cerr << "  -p/-P    turn on/off looking for provides    (default off)"
       << endl;
  cerr << "  -r/-R    turn on/off looking for requires    (default off)"
       << endl;
  cerr << "  -c/-C    turn on/off looking for conflicts   (default off)"
       << endl;
  cerr << "  -o/-O    turn on/off looking for obsoletes   (default off)"
       << endl;
  cerr << "  -m/-M    turn on/off looking for recommends  (default off)"
       << endl;
  cerr << "  -s/-S    turn on/off looking for supplements (default off)"
       << endl;
  ...
  const std::string &s4 = std::string(), const std::string &s5 = std::string() )
{
  message << "  ";
#define TABEL( N )                                                             \
  static unsigned w##N = 0;                                                    \
  if ( !s##N.empty() )                                                         \
    w##N = std::max( w##N, unsigned( s##N.size() ) );                          \
  message << str::form( " %-*s ", w##N, s##N.c_str() )
#define TABER( N )                                                             \
  static unsigned w##N = 0;                                                    \
  if ( !s##N.empty() )                                                         \
    w##N = std::max( w##N, unsigned( s##N.size() ) );                          \
...
{
  // std::less semantic
  bool operator()( const PoolQuery::const_iterator &lhs,
    const PoolQuery::const_iterator &rhs ) const
  {
    {
      bool l = lhs->isSystem();
      bool r = rhs->isSystem();
...
void dDump( const std::string &spec_r )
{
  ...
  for ( const auto &el : q )
  {
    ...
    {
      message << endl
              << "REPLACES: " << make<Product>( el )->replacedProducts();
    }
...
int main( int argc, char *argv[] )
{
  ...
  for ( ; argc; --argc, ++argv )
  {
    if ( ( *argv )[ 0 ] == '-' )
    {
      ...
      {
        case 'a':
          names = true, requires = provides = true;
          break;
        case 'A':
          names = true, requires = provides = false;
          break;
        case 'D':
          if ( argc > 1 )
          {
    ...
    message << *argv << " [" << ( ignorecase ? 'i' : '_' )
            << ( names ? 'n' : '_' ) << ( requires ? 'r' : '_' )
            << ( provides ? 'p' : '_' ) << ( conflicts ? 'c' : '_' )
            << ( obsoletes ? 'o' : '_' ) << ( recommends ? 'm' : '_' )
            << ( supplements ? 's' : '_' ) << ( enhacements ? 'e' : '_' )
            << "] {" << endl;

    std::set<PoolQuery::const_iterator, PQSort> qsorted;
    for_( it, q.begin(), q.end() ) qsorted.insert( it );
    ...
    for ( auto &&it : qsorted )
    {
      ...
      tableOut( str::numstring( it->id() ), it->asString(),
        str::form( "(%d)%s", it->repository().info().priority(),
                  it->repository().name().c_str() ),
        str::numstring( PoolItem( *it )->buildtime() ) );
      tableOut( "", "", it->vendor().asString() );
      if ( !it.matchesEmpty() )
      {
        for_( match, it.matchesBegin(), it.matchesEnd() )
        {
          tableOut( "", "", match->inSolvAttr().asString().substr( 9, 3 ) +
                              ": " + match->asString() );
        }
      }
    }
