...
inline std::string::size_type backskipWs(
...
inline std::string::size_type backskipNWs(
  const std::string &str_r, std::string::size_type pos_r )
{
  for ( ; pos_r != std::string::npos; --pos_r )
  {
    ...
    if ( ( ch = backskipNWs( str_r, ch ) ) != std::string::npos )
    {
      ...
      if ( ( ch = backskipWs( str_r, ch ) ) != std::string::npos )
      {
        ...
        if ( op_r.parseFrom( str_r.substr( ch + 1, oe - ch ) ) )
        {
          ...
          if ( ch !=
               std::string::npos ) // 'op' is not at str_r begin, so skip WS
...
/** Build \ref Capability from data. No parsing required.
    */
sat::detail::IdType relFromStr( sat::detail::CPool *pool_r, const Arch &arch_r,
  ...
  const ResKind &kind_r )
{
  ...
  if ( split.kind() == ResKind::srcpackage )
  {
    ...
    // libsolv uses.
    nid = ::pool_rel2id(
      pool_r, nid, IdString( ARCH_SRC ).id(), REL_ARCH, /*create*/ true );
  }
  ...
  const ResKind &kind_r )
{
  ...
  static const std::string srcKindPrefix(
    ResKind::srcpackage.asString() + ':' );
  ...
  {
    return relFromStr( pool_r, Arch_empty,
      name_r.substr( srcKindPrefix.size() ), op_r, ed_r, ResKind::srcpackage );
  }
...
Capability::Capability(
  const char *str_r, const ResKind &prefix_r, CtorFlag flag_r )
  : _id( relFromStr( myPool().getPool(), Arch_empty, str_r, prefix_r, flag_r ) )
{
}

Capability::Capability(
  const std::string &str_r, const ResKind &prefix_r, CtorFlag flag_r )
  : _id( relFromStr(
      myPool().getPool(), Arch_empty, str_r.c_str(), prefix_r, flag_r ) )
{
}

Capability::Capability( const Arch &arch_r, const char *str_r,
  const ResKind &prefix_r, CtorFlag flag_r )
  : _id( relFromStr( myPool().getPool(), arch_r, str_r, prefix_r, flag_r ) )
{
}

Capability::Capability( const Arch &arch_r, const std::string &str_r,
  const ResKind &prefix_r, CtorFlag flag_r )
  : _id( relFromStr(
      myPool().getPool(), arch_r, str_r.c_str(), prefix_r, flag_r ) )
{
}

Capability::Capability(
  const char *str_r, CtorFlag flag_r, const ResKind &prefix_r )
  : _id( relFromStr( myPool().getPool(), Arch_empty, str_r, prefix_r, flag_r ) )
{
}

Capability::Capability(
  const std::string &str_r, CtorFlag flag_r, const ResKind &prefix_r )
  : _id( relFromStr( myPool().getPool(), Arch_empty, str_r, prefix_r, flag_r ) )
{
}

Capability::Capability( const Arch &arch_r, const char *str_r, CtorFlag flag_r,
...
Capability::Capability( const Arch &arch_r, const std::string &str_r,
  CtorFlag flag_r, const ResKind &prefix_r )
  : _id( relFromStr( myPool().getPool(), arch_r, str_r, prefix_r, flag_r ) )
{
}
...
Capability::Capability( const std::string &name_r, const std::string &op_r,
  const std::string &ed_r, const ResKind &prefix_r )
  : _id( relFromStr(
      myPool().getPool(), name_r, Rel( op_r ), Edition( ed_r ), prefix_r ) )
{
}
Capability::Capability( const std::string &name_r, Rel op_r,
  const std::string &ed_r, const ResKind &prefix_r )
  : _id( relFromStr(
      myPool().getPool(), name_r, op_r, Edition( ed_r ), prefix_r ) )
{
}
Capability::Capability( const std::string &name_r, Rel op_r,
  const Edition &ed_r, const ResKind &prefix_r )
...
Capability::Capability( const std::string &arch_r, const std::string &name_r,
  const std::string &op_r, const std::string &ed_r, const ResKind &prefix_r )
  : _id( relFromStr( myPool().getPool(), Arch( arch_r ), name_r, Rel( op_r ),
      Edition( ed_r ), prefix_r ) )
{
}
Capability::Capability( const std::string &arch_r, const std::string &name_r,
  Rel op_r, const std::string &ed_r, const ResKind &prefix_r )
  : _id( relFromStr( myPool().getPool(), Arch( arch_r ), name_r, op_r,
      Edition( ed_r ), prefix_r ) )
{
}
Capability::Capability( const std::string &arch_r, const std::string &name_r,
  Rel op_r, const Edition &ed_r, const ResKind &prefix_r )
  : _id( relFromStr(
      myPool().getPool(), Arch( arch_r ), name_r, op_r, ed_r, prefix_r ) )
{
}
Capability::Capability( const Arch &arch_r, const std::string &name_r,
  const std::string &op_r, const std::string &ed_r, const ResKind &prefix_r )
  : _id( relFromStr( myPool().getPool(), arch_r, name_r, Rel( op_r ),
      Edition( ed_r ), prefix_r ) )
{
}
Capability::Capability( const Arch &arch_r, const std::string &name_r, Rel op_r,
  const std::string &ed_r, const ResKind &prefix_r )
  : _id( relFromStr(
      myPool().getPool(), arch_r, name_r, op_r, Edition( ed_r ), prefix_r ) )
{
}
Capability::Capability( const Arch &arch_r, const std::string &name_r, Rel op_r,
  const Edition &ed_r, const ResKind &prefix_r )
  : _id(
      relFromStr( myPool().getPool(), arch_r, name_r, op_r, ed_r, prefix_r ) )
{
}
...
Capability::Capability( ResolverNamespace namespace_r, IdString value_r )
  : _id(::pool_rel2id( myPool().getPool(), asIdString( namespace_r ).id(),
      ( value_r.empty() ? STRID_NULL : value_r.id() ), REL_NAMESPACE,
      /*create*/ true ) )
{
}
...
CapMatch Capability::_doMatch(
  sat::detail::IdType lhs, sat::detail::IdType rhs )
{
  if ( lhs == rhs )
    return CapMatch::yes;
  ...
  if ( l.arch() != r.arch() && !( l.arch().empty() || r.arch().empty() ) )
    return CapMatch::no;
...
bool Capability::isInterestingFileSpec( const char *name_r )
{
  ...
  static const str::regex filenameRegex( "/(s?bin|lib(64)?|etc)/|^/usr/(games/"
                                         "|share/(dict/words|magic\\.mime)$)|^/"
                                         "opt/gnome/games/",
    str::regex::nosubs );
...
Capability Capability::guessPackageSpec(
  const std::string &str_r, bool &rewrote_r )
{
       ...
       sat::WhatProvides( cap ).empty() )
  {
    ...
    if ( pool.byIdentBegin( detail.name() ) !=
         pool.byIdentEnd( detail.name() ) )
    {
      rewrote_r = true;
      return guesscap;
    }
...
void CapDetail::_init()
{
  ...
  {
    _lhs = IdString(
             ( ResKind::srcpackage.asString() + ":" + IdString( _lhs ).c_str() )
               .c_str() )
             .id();
    _archIfSimple = 0;
  }
    ...
    case CapDetail::EXPRESSION:
      switch ( obj.capRel() )
      {
        ...
        case CapDetail::CAP_ARCH:
          return str << obj.lhs().detail() << " " << obj.capRel() << " "
                     << obj.rhs().detail();
          break;
...
std::ostream &operator<<( std::ostream &str, CapDetail::Kind obj )
{
  switch ( obj )
  {
    ...
    case CapDetail::NAMED:
      ...
      break;
    case CapDetail::VERSIONED:
      return str << "VersionedCap";
      break;
    case CapDetail::EXPRESSION:
      return str << "CapExpression";
      break;
  }
  return str << "UnknownCap";
}
