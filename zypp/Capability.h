...
public:
  ...
  Capability()
    : _id( sat::detail::emptyId )
  {
  }

  /** Ctor from id. */
  explicit Capability( sat::detail::IdType id_r )
    : _id( id_r )
  {
  }
  ...
  /** \overload */
  explicit Capability( const std::string &str_r,
    const ResKind &prefix_r = ResKind(), CtorFlag flag_r = UNPARSED );
  /** \overload Explicitly specify the \c arch. */
  Capability( const Arch &arch_r, const char *str_r,
    const ResKind &prefix_r = ResKind(), CtorFlag flag_r = UNPARSED );
  /** \overload Explicitly specify the \c arch. */
  Capability( const Arch &arch_r, const std::string &str_r,
    const ResKind &prefix_r = ResKind(), CtorFlag flag_r = UNPARSED );

  /** \overload Convenience for parsed (name only, no <tt>"[op edition]</tt>) packages: <tt>Capability( "glibc", PARSED ); */
  Capability(
    const char *str_r, CtorFlag flag_r, const ResKind &prefix_r = ResKind() );
  /** \overload */
  Capability( const std::string &str_r, CtorFlag flag_r,
    const ResKind &prefix_r = ResKind() );
  /** \overload Explicitly specify the \c arch. */
  Capability( const Arch &arch_r, const char *str_r, CtorFlag flag_r,
    const ResKind &prefix_r = ResKind() );
  /** \overload */
  Capability( const Arch &arch_r, const std::string &str_r, CtorFlag flag_r,
    const ResKind &prefix_r = ResKind() );
  //@}
  ...
  Capability( const std::string &name_r, const std::string &op_r,
    const std::string &ed_r, const ResKind &prefix_r = ResKind() );
  /** \overload */
  Capability( const std::string &name_r, Rel op_r, const std::string &ed_r,
    const ResKind &prefix_r = ResKind() );
  /** \overload */
  Capability( const std::string &name_r, Rel op_r, const Edition &ed_r,
    const ResKind &prefix_r = ResKind() );
  //@}
  ...
  Capability( const std::string &arch_r, const std::string &name_r,
    const std::string &op_r, const std::string &ed_r,
    const ResKind &prefix_r = ResKind() );
  /** \overload */
  Capability( const std::string &arch_r, const std::string &name_r, Rel op_r,
    const std::string &ed_r, const ResKind &prefix_r = ResKind() );
  /** \overload */
  Capability( const std::string &arch_r, const std::string &name_r, Rel op_r,
    const Edition &ed_r, const ResKind &prefix_r = ResKind() );
  /** \overload */
  Capability( const Arch &arch_r, const std::string &name_r,
    const std::string &op_r, const std::string &ed_r,
    const ResKind &prefix_r = ResKind() );
  /** \overload */
  Capability( const Arch &arch_r, const std::string &name_r, Rel op_r,
    const std::string &ed_r, const ResKind &prefix_r = ResKind() );
  /** \overload */
  Capability( const Arch &arch_r, const std::string &name_r, Rel op_r,
    const Edition &ed_r, const ResKind &prefix_r = ResKind() );
  ...
  //@{
  Capability(
    ResolverNamespace namespace_r, IdString value_r = IdString::Null );
  Capability( ResolverNamespace namespace_r, const char *value_r )
    : Capability( namespace_r, IdString( value_r ) )
  {
  }
...
public:
  ...
  {
    return _doMatch( lhs.id(), Capability( rhs ).id() );
  }
  ...
  {
    return _doMatch( Capability( lhs ).id(), rhs.id() );
  }
  ...
  {
    return _doMatch( Capability( lhs ).id(), Capability( rhs ).id() );
  }
  ...
  {
    return _doMatch( id(), Capability( rhs ).id() );
  }
...
public:
  ...
  {
    return isInterestingFileSpec( name_r.c_str() );
  }
  ...
  /** \overload Taking an additional bool indicating whether \c str_r made
       ...
       */
  static Capability guessPackageSpec(
    const std::string &str_r, bool &rewrote_r );

public:
  /** Expert backdoor. */
  sat::detail::IdType id() const { return _id; }
private:
...
public:
  ...
  IdString arch() const
  {
    return _archIfSimple ? IdString( _archIfSimple ) : IdString();
  }
  ...
  {
    return isExpression() ? Capability( _lhs ) : Capability::Null;
  }
  CapRel capRel() const { return isExpression() ? CapRel( _flag ) : REL_NONE; }
  Capability rhs() const
  {
    return isExpression() ? Capability( _rhs ) : Capability::Null;
  }
  //@}
