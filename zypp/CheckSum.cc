...
const std::string &CheckSum::md5Type() { return Digest::md5(); }

const std::string &CheckSum::shaType()
{
  static std::string _type( "sha" );
  return _type;
}

const std::string &CheckSum::sha1Type() { return Digest::sha1(); }

const std::string &CheckSum::sha224Type() { return Digest::sha224(); }

const std::string &CheckSum::sha256Type() { return Digest::sha256(); }

const std::string &CheckSum::sha384Type() { return Digest::sha384(); }

const std::string &CheckSum::sha512Type() { return Digest::sha512(); }
  ...
  , _checksum( checksum )
{
  switch ( checksum.size() )
  {
    ...
    default:
      if ( _type.empty() )
      {
        WAR << "Can't determine type of " << checksum.size()
            << " byte checksum '" << _checksum << "'" << endl;
        return;
      }
      // else: dubious
      break;
  ...
  std::string msg =
    str::form( _( "Dubious type '%s' for %u byte checksum '%s'" ),
      _type.c_str(), checksum.size(), _checksum.c_str() );
  if ( _type == md5Type() || _type == shaType() || _type == sha1Type() ||
       _type == sha224Type() || _type == sha256Type() ||
       _type == sha384Type() || _type == sha512Type() )
  {
...
std::string CheckSum::type() const { return _type; }

std::string CheckSum::checksum() const { return _checksum; }
