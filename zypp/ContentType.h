...
public:
  ...
  /** Set type
     * \throws std::invalid_argument if string is malformed
     */
  void type( std::string type_r ) { _type = std::move( type_r ); }
  ...
  /**  Set subtype
     * \throws std::invalid_argument if string is malformed
     */
  void subtype( std::string subtype_r ) { _subtype = std::move( subtype_r ); }

public:
  /** Whether type and subtype are empty */
  bool empty() const { return emptyType() && emptySubtype(); }
  /** Whether type is empty */
  bool emptyType() const { return _type.empty(); }
  /** Whether subtype is empty */
  bool emptySubtype() const { return _subtype.empty(); }
...
inline bool operator<( const ContentType &lhs, const ContentType &rhs )
{
  int cmp = lhs.type().compare( rhs.type() );
  return cmp < 0 || ( cmp == 0 && lhs.subtype() < rhs.subtype() );
}
