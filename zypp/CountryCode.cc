...
private:
  ...
  Link getIndex( IdString index_r )
  {
    ...
    return ( it != _indexMap.end() ? it->second
  ...
  Link newIndex( IdString index_r, const std::string &code_r )
  {
    ...
    if ( code_r.size() != 2 )
      WAR << "Malformed CountryCode '" << code_r << "' (expect 2-letter)"
          << endl;

    std::string ucode( str::toUpper( code_r ) );
    if ( ucode != code_r )
    {
      WAR << "Malformed CountryCode '" << code_r << "' (not upper case)"
          << endl;
    ...
    MIL << "Remember CountryCode '" << code_r << "': '" << nval.second << "'"
        << endl;
...
CodeMaps::CodeMaps()
{
  ...
  const Init init[] = {{"AD", N_( "Andorra" )},                   // :AND:020:
    ...
    {"GG", N_( "Guernsey" )}, {"GH", N_( "Ghana" )},              // :GHA:288:
    ...
    {"IM", N_( "Isle of Man" )}, {"IN", N_( "India" )},           // :IND:356:
    {"IO", N_( "British Indian Ocean Territory" )},               // :IOT:086:
    {"IQ", N_( "Iraq" )},                                         // :IRQ:368:
    {"IR", N_( "Iran" )},                                         // :IRN:364:
    {"IS", N_( "Iceland" )},                                      // :ISL:352:
    {"IT", N_( "Italy" )},                                        // :ITA:380:
    {"JE", N_( "Jersey" )}, {"JM", N_( "Jamaica" )},              // :JAM:388:
    ...
    {"ME", N_( "Montenegro" )}, {"MF", N_( "Saint Martin" )},
    ...
    {"RS", N_( "Serbia" )}, {"RU", N_( "Russian Federation" )}, // :RUS:643:
