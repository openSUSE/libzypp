...
#define WFN_ATTRIBUTES                                                         \
  {                                                                            \
    Attribute::part, Attribute::vendor, Attribute::product,                    \
      Attribute::version, Attribute::update, Attribute::edition,               \
      Attribute::language, Attribute::sw_edition, Attribute::target_sw,        \
      Attribute::target_hw, Attribute::other,                                  \
  }
...
inline bool chIsValidRange( char ch ) { return ( '!' <= ch && ch <= '~' ); }
...
/** Digit */
inline bool chIsNum( char ch ) { return ( '0' <= ch && ch <= '9' ); }
  ...
  std::string asUri() const
  {
    ...
    for ( auto ai : WFN_ATTRIBUTES )
    {
      ...
      if ( ai == Attribute::edition )
      {
        ...
        {
          // packing is needed
          val = str::Str() << '~' << val //Attribute::edition
          ...
          if ( chIsAlpha( wfn[ 0 ] ) && chIsAlpha( wfn[ 1 ] ) )
          {
            ...
            if ( wfn[ len ] == '-' )
            {
              if ( chIsAlpha( wfn[ len + 1 ] ) && chIsAlpha( wfn[ len + 2 ] ) )
                len += 3;
              else if ( chIsNum( wfn[ len + 1 ] ) &&
                        chIsNum( wfn[ len + 2 ] ) && chIsNum( wfn[ len + 3 ] ) )
                len += 4;
            }
          }
          if ( wfn.size() != len )
            throw std::invalid_argument( str::Str()
                                         ...
                                         << "' illegal value; expected RFC5646 "
                                            "conform: language ['-' region]" );
        }
        break;
...
{
  Wfn ret;
  if ( cpe_r[ 0 ] == 'c' && cpe_r[ 1 ] == 'p' && cpe_r[ 2 ] == 'e' &&
       cpe_r[ 3 ] == ':' )
  {
    if ( cpe_r[ 4 ] == '/' )
    {
    ...
    else if ( cpe_r[ 4 ] == '2' && cpe_r[ 5 ] == '.' && cpe_r[ 6 ] == '3' &&
              cpe_r[ 7 ] == ':' )
    {
      ret = unbindFs( cpe_r );
    }
...
CpeId::Impl::Wfn CpeId::Impl::unbindUri( const std::string &cpe_r )
{
  ...
  field.reserve( Attribute::
      numAttributes ); // reserve 7 + 4 for packed extened attrs in edition
  if ( str::splitFields( cpe_r.c_str() + 5 /* skip magic 'cpe:/' */,
         std::back_inserter( field ), ":" ) > numUriAttr )
    throw std::invalid_argument( str::Str() << "CpeId:Uri: too many fields ("
                                            << field.size() << "); expected "
                                            << numUriAttr );
  ...
  for ( auto ai : WFN_ATTRIBUTES )
  {
    ...
    {
      // unpacking is needed
      static constexpr unsigned numPacks =
        6u; // dummy_before_~ + edition + 4 extended attributes
      std::vector<std::string> pack;
      pack.reserve( numPacks );
      if ( str::splitFields( field[ ai ], std::back_inserter( pack ), "~" ) >
           numPacks )
        throw std::invalid_argument(
          str::Str() << "CpeId:Uri:edition: too many packs (" << pack.size()
                     << "); expected " << numPacks );
...
CpeId::Impl::Wfn CpeId::Impl::unbindFs( const std::string &cpe_r )
{
  ...
  if ( str::splitFields( cpe_r.c_str() + 8 /* skip magic 'cpe:2.3:' */,
         std::back_inserter( field ), ":" ) > Attribute::numAttributes )
    throw std::invalid_argument(
      str::Str()
      << "CpeId:Fs: too many fields (" << field.size()
      << "); expected 11" /*<< Attribute::numAttributes but g++ currently can't resoolve this as constexpr*/ );
  if (
    !field.empty() &&
    field.back()
      .empty() ) // A trailing ':' leads to an empty (illegal) field, but we fillup missing fields with ANY|"*"
    field.back() = "*";
  field.resize( Attribute::numAttributes, "*" ); // fillup with ANY|"*"
...
CpeId::operator bool() const { return bool( *_pimpl ); }

std::string CpeId::asFs() const { return _pimpl->asFs(); }

std::string CpeId::asUri() const { return _pimpl->asUri(); }

std::string CpeId::asWfn() const { return _pimpl->asWfn(); }
...
#define OUTS( N )                                                              \
  {                                                                            \
    ...
    OUTS( part ), OUTS( vendor ), OUTS( product ), OUTS( version ),
    OUTS( update ), OUTS( edition ), OUTS( language ), OUTS( sw_edition ),
    OUTS( target_sw ), OUTS( target_hw ), OUTS( other ),
#undef OUTS
  };
  return _table[ val_r ];
}
      ...
      switch ( *chp )
      {
        case '\\': // quoted
          ...
          if ( !chIsValidRange( *chp ) )
          {
            if ( *chp )
              throw std::invalid_argument(
                str::Str() << "CpeId:Wfn: illegal quoted character '\\"
                           << reinterpret_cast<void *>( *chp ) << "'" );
            else
              throw std::invalid_argument(
            ...
            throw std::invalid_argument(
              str::Str() << "CpeId:Wfn: unnecessarily quoted character '\\"
                         << *chp << "'" );
          else if ( starting && *chp == '-' && chp + 1 == value_r.end() )
            throw std::invalid_argument(
              str::Str() << "CpeId:Wfn: '\\-' is illegal value" );
          break;
        ...
        default: // everything else unquoted
          if ( !chIsWfnUnescaped( *chp ) )
          {
            if ( chIsValidRange( *chp ) )
              throw std::invalid_argument(
                str::Str() << "CpeId:Wfn: missing quote before '" << *chp
                           << "'" );
            else
              throw std::invalid_argument(
                str::Str() << "CpeId:Wfn: illegal character '"
                           << reinterpret_cast<void *>( *chp ) << "'" );
          }
          break;
      }
        ...
        switch ( *chp )
        {
          case '\\': // may stay quoted
            ...
            if ( chIsWfnUnescaped( *chp ) )
              ...
              throw std::invalid_argument(
                str::Str() << "CpeId:Fs: illegal quoted character '\\" << *chp
                           << "'" );
            else
              throw std::invalid_argument(
          ...
          default:
            if ( chIsWfnUnescaped( *chp ) )
              ...
              throw std::invalid_argument(
                str::Str() << "CpeId:Fs: illegal character '"
                           << reinterpret_cast<void *>( *chp ) << "'" );
            break;
        }
        if ( starting )
    ...
    else
    {
      ...
      for_( chp, encoded_r.begin(), encoded_r.end() )
      {
        ...
        if ( ch == '%' ) // legal '%xx' sequence first
        {
          ...
          if ( d1 != -1 )
          {
            ...
            if ( d2 != -1 )
            {
              ...
              if ( !chIsValidRange( ch ) )
                throw std::invalid_argument(
                  str::Str() << "CpeId:Uri: illegal % encoded character '"
          ...
          throw std::invalid_argument(
            str::Str() << "CpeId:Uri: illegal character '"
                       << reinterpret_cast<void *>( ch ) << "'" );
          ...
          switch ( *chp )
          {
            case '\\':
              ...
              switch ( *chp )
              {
                ...
                default:
                  result << '%' << hdig[ (unsigned char)( *chp ) / 16 ]
                         << hdig[ (unsigned char)( *chp ) % 16 ];
                  break;
              }
              break;
...
inline bool isWildchar( char ch_r ) { return ( ch_r == '*' || ch_r == '?' ); }
...
/** Whether there is an even number of consecutive backslashes before and including \a rbegin_r
     ...
     */
inline bool evenNumberOfBackslashes(
  std::string::const_reverse_iterator rbegin_r,
  std::string::const_reverse_iterator rend_r )
{
...
inline unsigned trueCharsIn( const std::string &str_r,
  std::string::size_type begin_r, std::string::size_type end_r )
{
  unsigned chars = 0;
...
inline bool matchWildcardfreeString(
...
inline bool matchWildcardedString( std::string src, std::string trg )
{
  ...
  switch ( *src.begin() ) // wellformed implies not empty
  {
    ...
    case '?':
      ...
      if ( src.size() == prefx )
        return (
          trg.size() <= prefx ); // "??..?": superset if at most #prefx chars
      // else
      src.erase( 0, prefx );
      break;
    default:
  ...
  for ( std::string::size_type match = trg.find( src, 0 );
        match != std::string::npos; match = trg.find( src, match + 1 ) )
  {
    ...
    if ( suffx != std::string::npos &&
         trueCharsIn( trg, frontSize, trg.size() ) > suffx )
...
bool CpeId::Value::containsWildcard() const
{
  ...
  return ( isWildchar( *value.begin() ) ||
           ( isWildchar( *value.rbegin() ) &&
             evenNumberOfBackslashes( ++value.rbegin(), value.rend() ) ) );
}
...
{
  static const SetCompare kNeedsCloserLook(
  ...
  if ( ret == kNeedsCloserLook )
  {
    if ( srcType == Type::wildcardfree ) // trgType == Type::wildcardfree
    {
      ...
      ret = matchWildcardfreeString( *_value, *trg._value )
              ? SetCompare::equal
              : SetCompare::disjoint;
    }
    else if ( srcType == Type::wildcarded ) // trgType == Type::wildcardfree
    {
      ...
      ret = matchWildcardedString( *_value, *trg._value )
              ? SetCompare::properSuperset
              : SetCompare::disjoint;
    }
  }
  return ret;
...
#else
SetCompare CpeId::Value::setRelationMixinCompare(
  const CpeId::Value &trg ) const
{
  ///////////////////////////////////////////////////////////////////
  // ANY,		ANY		=> equal
  ...
  {
    if ( trg.isNA() )
      ret = SetCompare::equal; // else: SetCompare::disjoint;
  }
  else if ( !trg.isNA() ) // else: SetCompare::disjoint;
  {
    ...
    if ( isWildcarded() )
    {
      if ( trg.isWildcarded() )
      {
        ...
        ret = matchWildcardfreeString( *_value, *trg._value )
                ? SetCompare::equal
                : SetCompare::uncomparable;
      }
      else
      {
        // Needs wildcard compare (src,trg)
        if ( matchWildcardedString( *_value, *trg._value ) )
          ret = SetCompare::properSuperset; // else: SetCompare::disjoint;
      }
    }
    else
    {
      if ( trg.isWildcarded() )
      {
        ...
        if ( matchWildcardedString( *trg._value, *_value ) )
          ret = SetCompare::properSubset; // else: SetCompare::disjoint;
      }
      else
      {
        // simple string compare
        if ( matchWildcardfreeString( *_value, *trg._value ) )
          ret = SetCompare::equal; // else: SetCompare::disjoint;
      }
    }
  }
