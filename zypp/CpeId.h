...
public:
  ...
  struct EAttributeDef
  {
    ...
    static constexpr unsigned numAttributes =
      other + 1; ///< number of attributes
    static const std::string &asString(
      Enum val_r ); ///< string representantion
  };
...
public:
  ...
  std::string asString() const { return asFs(); }
...
private:
  friend SetCompare base::SetRelationMixin<CpeId>::compare(
    const CpeId & ) const;
...
inline std::ostream &operator<<(
  std::ostream &str, const CpeId::Attribute &obj )
{
...
public:
  ...
  bool isANY() const { return !_value; }

  /** Whether value is NA. ) */
  bool isNA() const { return _value && _value->empty(); }

  /** Whether it's a logical value (ANY|NA). */
  bool isLogical() const { return !_value || _value->empty(); }
  /** \overload for Type */
  ...
  bool isString() const { return _value && !_value->empty(); }
  /** \overload for Type */
  ...
  bool isWildcardfree() const { return isString() && !containsWildcard(); }

  /** An attribute value string with wildcards (<tt>[*?]</tt> at begin and/or end) */
  bool isWildcarded() const { return isString() && containsWildcard(); }

public:
  /** Default string representation [\ref asWfn]. */
  std::string asString() const { return asWfn(); }
...
private:
  friend SetCompare base::SetRelationMixin<Value>::compare(
    const Value & ) const;
  /** CPE name matching hook for \ref SetRelationMixin */
