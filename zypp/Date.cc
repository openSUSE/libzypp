...
struct LocaleGuard
{
  LocaleGuard()
  {
    ...
    if ( _mylocale.find( "UTF-8" ) == std::string::npos &&
         ...
         _mylocale != "POSIX" && _mylocale != "C" && _mylocale != "" )
    {
...
inline std::string doForm( const std::string &format_r, Date::TimeBase base_r,
  const Date::ValueType &date_r )
{
  if ( !date_r )
    return "0";
  ...
  LocaleGuard guard;
  static char buf[ 512 ];
  if ( !strftime( buf, 512, format_r.c_str(),
         ( base_r == Date::TB_UTC ? gmtime : localtime )( &date_r ) ) )
    *buf = '\0';
  else
  {
    // strip a trailing '00' in a timeZoneFormat
    unsigned l = ::strlen( buf );
    if ( l >= 5 && ( buf[ l - 1 ] == '0' ) && ( buf[ l - 2 ] == '0' ) &&
         ( buf[ l - 5 ] == '+' || buf[ l - 5 ] == '-' ) )
      buf[ l - 2 ] = '\0';
  }
  return buf;
...
std::string Date::form(
  const std::string &format_r, Date::TimeBase base_r ) const
{
  return doForm( format_r, base_r, _date );
}

std::string Date::print( DateFormat dateFormat_r, TimeFormat timeFormat_r,
  TimeZoneFormat timeZoneFormat_r, TimeBase base_r ) const
{
  str::Str str;
...
std::string Date::printISO( DateFormat dateFormat_r, TimeFormat timeFormat_r,
  TimeZoneFormat timeZoneFormat_r, TimeBase base_r ) const
{
  str::Str str;
  if ( dateFormat_r != DateFormat::none )
...
std::ostream &dumpAsXmlOn(
  std::ostream &str, const Date &obj, const std::string &name_r )
{
  return xmlout::node(
    str, name_r, {
                   {"time_t", Date::ValueType( obj )}, {"text", obj.printISO()},
                 } );
}
