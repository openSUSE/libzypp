...
inline Dep::for_use_in_switch parse( const std::string &strval_r )
{
  const std::map<std::string, Dep::for_use_in_switch> _table = {
    {"provides", Dep::PROVIDES_e}, {"prerequires", Dep::PREREQUIRES_e},
    {"requires", Dep::REQUIRES_e}, {"conflicts", Dep::CONFLICTS_e},
    {"obsoletes", Dep::OBSOLETES_e}, {"recommends", Dep::RECOMMENDS_e},
    {"suggests", Dep::SUGGESTS_e}, {"enhances", Dep::ENHANCES_e},
  ...
  {
    ZYPP_THROW(
      Exception( "Dep parse: illegal string value '" + strval_r + "'" ) );
  }
  return it->second;
}
}
...
const std::string &Dep::asString() const
{
  static const std::map<for_use_in_switch, std::string> _table = {
    {PROVIDES_e, "provides"}, {PREREQUIRES_e, "prerequires"},
    {REQUIRES_e, "requires"}, {CONFLICTS_e, "conflicts"},
    {OBSOLETES_e, "obsoletes"}, {RECOMMENDS_e, "recommends"},
    {SUGGESTS_e, "suggests"}, {ENHANCES_e, "enhances"},
