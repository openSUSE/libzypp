...
DiskUsageCounter::MountPointSet calcDiskUsage(
  DiskUsageCounter::MountPointSet result, const Bitmap &installedmap_r )
{
  if ( result.empty() )
  {
  ...
  // now calc...
  ::pool_calc_duchanges( satpool.get(), const_cast<Bitmap &>( installedmap_r ),
    &duchanges[ 0 ], duchanges.size() );
  ...
  // and process the result
  {
    ...
    for_( it, result.begin(), result.end() )
    {
      ...
      static const ByteCount blockAdjust(
        2, ByteCount::K ); // (files * blocksize) / 2 / 1K; result value in K!

      it->pkg_size = it->used_size             // current usage
                     + duchanges[ idx ].kbytes // package data size
                     + ( duchanges[ idx ].files *
                         ( it->fstype == "btrfs" ? 4096 : it->block_size ) /
                         blockAdjust ); // half block per file
      ++idx;
    }
  }
...
DiskUsageCounter::MountPointSet DiskUsageCounter::disk_usage(
  const ResPool &pool_r ) const
{
...
DiskUsageCounter::MountPointSet DiskUsageCounter::detectMountPoints(
  ...
  else
  {
    ...
    while ( procmounts )
    {
      ...
      if ( !( procmounts.fail() || procmounts.bad() ) )
      {
        ...
        if ( words[ 2 ] == "vfat" || words[ 2 ] == "fat" ||
             words[ 2 ] == "ntfs" || words[ 2 ] == "ntfs-3g" )
        {
          MIL << words[ 1 ] << " contains ignored fs (" << words[ 2 ] << ')'
              << std::endl;
          continue;
        }
        ...
        const char *mpunwanted[] = {
          "/mnt", "/media", "/mounts", "/floppy", "/cdrom", "/suse", "/tmp",
          "/var/tmp", "/var/adm/mount", "/var/adm/YaST",
          /*last*/ 0 /*entry*/
        };
        ...
        if ( words[ 2 ] == "btrfs" )
        {
          ...
          if ( geteuid() != 0 )
          {
            DBG << "Assume snapshots on " << words[ 1 ]
          ...
          else
          {
            ...
            ExternalProgram prog(
              {"btrfs", "subvolume", "list", "-s", words[ 1 ]} );
            std::string line( prog.receiveLine() );
            if ( !line.empty() )
            {
              DBG << "Found a snapshot on " << words[ 1 ] << ": "
                  << line; // has trailing std::endl
              hints |= MountPoint::Hint_growonly;
            }
            prog.kill();
          }
        ...
        {
          WAR << "Unable to statvfs(" << words[ 1 ] << "); errno " << errno
              << std::endl;
          ret.insert( DiskUsageCounter::MountPoint(
            mp, words[ 2 ], 0LL, 0LL, 0LL, 0LL, hints ) );
        }
        else
        {
          //
          ...
          if ( btrfshack )
          {
            ...
            if ( bmp.fstype.empty() ) // 1st occurance
            {
              bmp = DiskUsageCounter::MountPoint( mp, words[ 2 ], sb.f_bsize,
                ...
                ( (long long)( sb.f_blocks - sb.f_bfree ) ) * sb.f_bsize / 1024,
                0LL, hints );
            }
            else if ( bmp.dir > mp )
              bmp.dir = mp;
  ...
  std::ostream &str, const DiskUsageCounter::MountPoint &obj )
{
  str << "dir:[" << obj.dir << "] [ bs: " << obj.blockSize()
      << " ts: " << obj.totalSize() << " us: " << obj.usedSize()
      << " (+-: " << obj.commitDiff() << ")" << ( obj.readonly ? "r" : "" )
      << ( obj.growonly ? "g" : "" ) << " " << obj.fstype << "]";
  return str;
}
