...
public:
  ...
  explicit Edition( IdString::IdType id_r )
    : _str( id_r )
  {
  }
  explicit Edition( const IdString &idstr_r )
    : _str( idstr_r )
  {
  }
  explicit Edition( const std::string &str_r )
    : _str( str_r )
  {
  }
  explicit Edition( const char *cstr_r )
    : _str( cstr_r )
  {
  }
  ...
  /** \overload */
  Edition(
    const char *version_r, const char *release_r, epoch_t epoch_r = noepoch );
...
public:
  ...
  {
    return match( lhs.idStr(), rhs.idStr() );
  }
  static int match( const Edition &lhs, const IdString &rhs )
  {
    return match( lhs.idStr(), rhs );
  }
  ...
  {
    return match( lhs, rhs.idStr() );
  }
  static int match( const IdString &lhs, const IdString &rhs )
  {
    return lhs.compareEQ( rhs ) ? 0 : _doMatch( lhs.c_str(), rhs.c_str() );
  }
