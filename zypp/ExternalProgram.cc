...
ExternalProgram::ExternalProgram( std::string commandline,
  Stderr_Disposition stderr_disp, bool use_pty, int stderr_fd,
  bool default_locale, const Pathname &root )
  : use_pty( use_pty )
  , pid( -1 )
{
  const char *argv[ 4 ];
  ...
  start_program(
    argv, Environment(), stderr_disp, stderr_fd, default_locale, root.c_str() );
}
  ...
  , pid( -1 )
{
  ...
  start_program( argvp, Environment(), stderr_disp, stderr_fd, default_locale,
    root.c_str() );
}

ExternalProgram::ExternalProgram( const Arguments &argv,
  const Environment &environment, Stderr_Disposition stderr_disp, bool use_pty,
  int stderr_fd, bool default_locale, const Pathname &root )
  : use_pty( use_pty )
  , pid( -1 )
{
  const char *argvp[ argv.size() + 1 ];
  ...
  start_program(
    argvp, environment, stderr_disp, stderr_fd, default_locale, root.c_str() );
}
...
ExternalProgram::ExternalProgram(
  const char *binpath, const char *const *argv_1, bool use_pty )
  : use_pty( use_pty )
  , pid( -1 )
{
  int i = 0;
...
ExternalProgram::ExternalProgram( const char *binpath,
  const char *const *argv_1, const Environment &environment, bool use_pty )
  : use_pty( use_pty )
  , pid( -1 )
{
  int i = 0;
...
void ExternalProgram::start_program( const char *const *argv,
  const Environment &environment, Stderr_Disposition stderr_disp, int stderr_fd,
  bool default_locale, const char *root )
{
  pid = -1;
  _exitStatus = 0;
  ...
  if ( ( pid = fork() ) == 0 )
  {
    ...
    for ( Environment::const_iterator it = environment.begin();
    ...
    if ( root )
    {
      ...
      {
        _execError = str::form(
          _( "Can't chroot to '%s' (%s)." ), root, strerror( errno ) );
    ...
    {
      _execError =
        root ? str::form( _( "Can't chdir to '%s' inside chroot '%s' (%s)." ),
                 chdirTo, root, strerror( errno ) )
             : str::form(
                 _( "Can't chdir to '%s' (%s)." ), chdirTo, strerror( errno ) );
      std::cerr << _execError << endl; // After fork log on stderr too
      _exit( 128 ); // No sense in returning! I am forked away!!
    }
    ...
    _execError =
      str::form( _( "Can't exec '%s' (%s)." ), argv[ 0 ], strerror( errno ) );
    std::cerr << _execError << endl; // After fork log on stderr too
...
int ExternalProgram::checkStatus( int status )
{
  ...
  else if ( WIFSIGNALED( status ) )
  {
    ...
    WAR << ")" << endl;
    _execError = str::form( _( "Command was killed by signal %d (%s)." ),
...
bool ExternalProgram::running()
{
  ...
  {
    case -1:
      ERR << "waitpid( " << pid << ") returned error '" << strerror( errno )
          << "'" << endl;
      return false;
      break;
    case 0:
...
bool ExternalProgramWithStderr::stderrGetUpTo(
  std::string &retval_r, const char delim_r, bool returnDelim_r )
{
  if ( !_stderr )
    return false;
