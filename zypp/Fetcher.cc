...
class Fetcher::Impl
{
  friend std::ostream &operator<<(
    std::ostream &str, const Fetcher::Impl &obj );
...
public:
  ...
  void enqueueDir( const OnMediaLocation &resource, bool recursive,
...
private:
  /**
       ...
       */
  void downloadAndReadIndexList(
    MediaSetAccess &media, const Pathname &dest_dir );

  /**
       * download the indexes and reads them
       */
  void downloadIndex( MediaSetAccess &media, const OnMediaLocation &resource,
  ...
  void getDirectoryContent( MediaSetAccess &media,
    const OnMediaLocation &resource, filesystem::DirContent &content );
  ...
  /**
       ...
       */
  bool provideFromCache(
    const OnMediaLocation &resource, const Pathname &dest_dir );
  /**
       * Validates the job against is checkers, by using the file instance
       * on dest_dir
       * \throws Exception
       */
  void validate( const OnMediaLocation &resource, const Pathname &dest_dir,
  ...
  void autoaddIndexes( const filesystem::DirContent &content,
    MediaSetAccess &media, const OnMediaLocation &resource,
    const Pathname &dest_dir );
  /**
       * Provide the resource to \ref dest_dir
       */
  void provideToDest( MediaSetAccess &media, const OnMediaLocation &resource,
    const Pathname &dest_dir, const Pathname &deltafile );
...
void Fetcher::Impl::enqueueDigested( const OnMediaLocation &resource,
...
Fetcher::Options Fetcher::Impl::options() const { return _options; }

void Fetcher::Impl::enqueueDir(
  const OnMediaLocation &resource, bool recursive, const FileChecker &checker )
{
  FetcherJob_Ptr job;
...
void Fetcher::Impl::enqueueDigestedDir(
  const OnMediaLocation &resource, bool recursive, const FileChecker &checker )
{
  FetcherJob_Ptr job;
  job.reset( new FetcherJob( resource ) );
...
void Fetcher::Impl::enqueue(
  const OnMediaLocation &resource, const FileChecker &checker )
{
  FetcherJob_Ptr job;
  job.reset( new FetcherJob( resource ) );
  if ( checker )
...
bool Fetcher::Impl::provideFromCache(
  const OnMediaLocation &resource, const Pathname &dest_dir )
{
  ...
  for_( it_cache, _caches.begin(), _caches.end() )
  {
    ...
    {
      DBG << "File '" << cached_file << "' exist, testing checksum "
          << resource.checksum() << endl;
      // check the checksum
      if ( is_checksum( cached_file, resource.checksum() ) &&
           ( !resource.checksum().empty() ) )
      {
        // cached
        MIL << "file " << resource.filename()
        ...
        if ( dest_full_path != cached_file )
        {
          if ( assert_dir( dest_full_path.dirname() ) != 0 )
            ZYPP_THROW( Exception(
              "Can't create " + dest_full_path.dirname().asString() ) );

          if ( filesystem::hardlinkCopy( cached_file, dest_full_path ) != 0 )
          {
            ERR << "Can't hardlink/copy " << cached_file + " to " + dest_dir
                << endl;
            continue;
          }
        }
...
void Fetcher::Impl::validate( const OnMediaLocation &resource,
  const Pathname &dest_dir, const list<FileChecker> &checkers )
{
  ...
  try
  {
    MIL << "Checking job [" << localfile << "] (" << checkers.size()
        << " checkers )" << endl;
  ...
  {
    ZYPP_THROW( Exception(
      "Unknown error while validating " + resource.filename().asString() ) );
  }
}
  ...
  const Pathname &dest_dir )
{
  auto fnc_addIfInContent( [&]( const std::string &index_r ) -> bool {
    if ( find( content.begin(), content.end(),
           filesystem::DirEntry( index_r, filesystem::FT_FILE ) ) ==
         content.end() )
      return false;
...
void Fetcher::Impl::getDirectoryContent( MediaSetAccess &media,
  const OnMediaLocation &resource, filesystem::DirContent &content )
{
  if ( _dircontent.find( resource.filename().asString() ) != _dircontent.end() )
  {
    filesystem::DirContent filled(
      _dircontent[ resource.filename().asString() ] );
  ...
  else
  {
    ...
    media.dirInfo(
      tofill, resource.filename(), false /* dots */, resource.medianr() );
  ...
  FetcherJob::Flags flags )
{
        ...
        it != content.end(); ++it )
  {
    ...
    if ( str::hasPrefix( it->name, "CHECKSUMS" ) ||
         str::hasPrefix( it->name, "SHA1SUMS" ) )
      continue;
    ...
    switch ( it->type )
    {
      ...
      case filesystem::FT_FILE:
      {
        ...
        if ( flags & FetcherJob::AlwaysVerifyChecksum )
          ...
          enqueue( OnMediaLocation( filename, resource.medianr() )
                     .setChecksum( chksm ) );
        break;
      }
      case filesystem::
...
void Fetcher::Impl::provideToDest( MediaSetAccess &media,
  const OnMediaLocation &resource, const Pathname &dest_dir,
  const Pathname &deltafile )
{
  ...
  if ( !got_from_cache )
  {
    ...
    try
    {
      Pathname tmp_file = media.provideFile( resource,
        resource.optional() ? MediaSetAccess::PROVIDE_NON_INTERACTIVE
                            : MediaSetAccess::PROVIDE_DEFAULT,
        deltafile );
      ...
      if ( assert_dir( dest_full_path.dirname() ) != 0 )
        ZYPP_THROW(
          Exception( "Can't create " + dest_full_path.dirname().asString() ) );
      if ( filesystem::hardlinkCopy( tmp_file, dest_full_path ) != 0 )
      {
        ...
        ZYPP_THROW( Exception( "Can't hardlink/copy " + tmp_file.asString() +
    ...
    catch ( Exception &excpt_r )
    {
      ...
      {
        ZYPP_CAUGHT( excpt_r );
        WAR << "optional resource " << resource << " could not be transferred"
            << endl;
        return;
      }
      else
...
struct ContentReaderHelper : public parser::susetags::ContentFileReader
{
  ContentReaderHelper()
  {
    setRepoIndexConsumer(
      bind( &ContentReaderHelper::consumeIndex, this, _1 ) );
  }
...
void Fetcher::Impl::readContentFileIndex(
  const Pathname &index, const Pathname &basedir )
{
  ContentReaderHelper reader;
  reader.parse( index );
  MIL << index << " contains " << reader._repoindex->mediaFileChecksums.size()
      << " checksums." << endl;
  for_( it, reader._repoindex->mediaFileChecksums.begin(),
...
void Fetcher::Impl::readChecksumsIndex(
  const Pathname &index, const Pathname &basedir )
{
  ...
  if ( !in.fail() )
  {
    ...
    while ( getline( in, buffer ) )
    {
      ...
      CheckSum checksum(
        str::stripFirstWord( buffer, /*ltrim before strip*/ true ) );
      if ( checksum.empty() )
        continue; // empty line | unknown cheksum format

      if ( buffer.empty() )
      {
        WAR << "Missing filename in CHECKSUMS file: " << index.asString()
            << " (" << checksum << ")" << endl;
        continue;
      }
...
void Fetcher::Impl::downloadIndex( MediaSetAccess &media,
  const OnMediaLocation &resource, const Pathname &dest_dir )
{
  ...
  if ( PathInfo( dest_dir + keyloc.filename() ).isExist() )
    getZYpp()->keyRing()->importKey(
      PublicKey( dest_dir + keyloc.filename() ), false );
  else
    WAR << "No public key specified by user for index '" << keyloc.filename()
        << "'" << endl;
...
void Fetcher::Impl::downloadAndReadIndexList(
  MediaSetAccess &media, const Pathname &dest_dir )
{
  ...
  const ProgressData::ReceiverFnc &progress_receiver )
{
        ...
        it_res != _resources.end(); ++it_res )
  {
         ...
         ( _options & AutoAddContentFileIndexes ) )
    {
      ...
      filesystem::DirContent content;
      getDirectoryContent(
        media, ( *it_res )->location.filename().dirname(), content );
      // this method test for the option flags so indexes are added
      // only if the options are enabled
      ...
      autoaddIndexes(
        content, media, ( *it_res )->location.filename().dirname(), dest_dir );
      ...
      MIL << "Autodiscovering signed indexes on '"
          << "/"
          << "' for '" << ( *it_res )->location.filename() << "'" << endl;

      autoaddIndexes( content, media, Pathname( "/" ), dest_dir );
    }

    provideToDest(
      media, ( *it_res )->location, dest_dir, ( *it_res )->deltafile );
...
{
  for ( list<FetcherJob_Ptr>::const_iterator it_res = obj._resources.begin();
        it_res != obj._resources.end(); ++it_res )
  {
    str << *it_res;
  }
  return str;
...
Fetcher::Options Fetcher::options() const { return _pimpl->options(); }

void Fetcher::enqueueDigested( const OnMediaLocation &resource,
  const FileChecker &checker, const Pathname &deltafile )
{
  _pimpl->enqueueDigested( resource, checker, deltafile );
}

void Fetcher::enqueueDir(
  const OnMediaLocation &resource, bool recursive, const FileChecker &checker )
{
  _pimpl->enqueueDir( resource, recursive, checker );
}

void Fetcher::enqueueDigestedDir(
  const OnMediaLocation &resource, bool recursive, const FileChecker &checker )
{
...
void Fetcher::reset() { _pimpl->reset(); }
