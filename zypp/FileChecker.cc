...
void ChecksumFileChecker::operator()( const Pathname &file ) const
{
  ...
  else
  {
    CheckSum real_checksum(
      _checksum.type(), filesystem::checksum( file, _checksum.type() ) );
    if ( ( real_checksum != _checksum ) )
    {
      ...
      {
        WAR << "User accepted " << file << " with WRONG CHECKSUM. (remembered)"
            << std::endl;
        return;
      }
      else if ( report->askUserToAcceptWrongDigest(
                  file, _checksum.checksum(), real_checksum.checksum() ) )
      {
...
void CompositeFileChecker::operator()( const Pathname &file ) const
{
  ...
  for ( list<FileChecker>::const_iterator it = _checkers.begin();
        it != _checkers.end(); ++it )
  {
    if ( *it )
    {
...
void SignatureFileChecker::addPublicKey(
  const Pathname &publickey, const KeyContext &keycontext )
{
  addPublicKey( PublicKey( publickey ), keycontext );
}

void SignatureFileChecker::addPublicKey(
  const PublicKey &publickey, const KeyContext &keycontext )
{
...
void SignatureFileChecker::operator()( const Pathname &file ) const
{
  ...
  {
    ZYPP_THROW(
      ExceptionType( "Signature " + _signature.asString() + " not found." ) );
  }

  MIL << "checking " << file << " file validity using digital signature.."
      << endl;
  _fileValidated = false;
  _fileAccepted = getZYpp()->keyRing()->verifyFileSignatureWorkflow(
    file, file.basename(), _signature, _fileValidated, _context );

  if ( !_fileAccepted )
    ZYPP_THROW(
      ExceptionType( "Signature verification failed for " + file.basename() ) );
}
