...
public:
  ...
  typedef function<void(
    const SignatureFileChecker &checker, const Pathname &file )>
    OnSigValidated;
...
public:
  ...
  const KeyContext &keyContext() const { return _context; }
  ...
  /** Return whether the last file passed to \ref operator() was accepted.
       ...
       */
  bool fileAccepted() const { return _fileAccepted; }
  ...
  /** Return whether the last file passed to \ref operator() was actually sucessfully verified.
       ...
       */
  bool fileValidated() const { return _fileValidated; }

  /**
       * add a public key to the list of known keys
       */
  void addPublicKey(
    const PublicKey &publickey, const KeyContext &keycontext = KeyContext() );
  /** \overload Convenience taking the public keys pathname. */
  void addPublicKey(
    const Pathname &publickey, const KeyContext &keycontext = KeyContext() );
