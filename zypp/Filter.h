...
public:
  ...
  explicit ByLocaleSupport( const Locale &locale_r )
    : _sel(
        boost::bind( boost::mem_fun_ref( (LS1)&sat::Solvable::supportsLocale ),
          _1, locale_r ) )
  {
  }
  ...
  explicit ByLocaleSupport( const LocaleSet &locales_r )
    : _sel(
        boost::bind( boost::mem_fun_ref( (LS2)&sat::Solvable::supportsLocale ),
          _1, locales_r ) )
  {
  }
...
public:
  ...
  bool operator()( const sat::Solvable &solv_r ) const
  {
    return solv_r.name() == _item.name() &&
           ...
           solv_r.arch() == _item.arch() && solv_r.vendor() == _item.vendor();
  }
