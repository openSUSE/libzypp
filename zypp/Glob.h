...
public:
  ...
  class const_iterator
    : public boost::iterator_adaptor<const_iterator // Derived
        ,
        char ** // Base
        ,
        value_type // Value
  ...
  private:
    ...
    reference dereference() const { return ( base() ? *base() : 0 ); }
  };
  ///////////////////////////////////////////////////////////////////
...
public:
  ...
  enum Bits
  {
    ...
    kTildeCheck =
      GLOB_TILDE_CHECK, //!< Like GLOB_TILDE but return an error if the user name is not available.
  };
...
public:
  ...
  {
    clear();
    setDefaultFlags( flags_r );
  }

public:
  /** The default flags passed to \c ::glob(). */
  Flags defaultFlags() const { return _defaultFlags; }

  /** Set the default flags passed to \c ::glob(). */
  void setDefaultFlags( Flags flags_r = Flags() ) { _defaultFlags = flags_r; }
  ...
  /** Returns the value returned by the last call to \c ::glob().
         ...
         * or \c GLOB_NOMATCH.
         */
  int lastGlobReturn() const { return _lastGlobReturn; }

public:
  /** Whether matches were found. */
  bool empty() const { return !( _result && _result->gl_pathc ); }

  /** The number of matches found so far. */
  size_type size() const { return ( _result ? _result->gl_pathc : 0 ); }
...
public:
    ...
    const char *pattern_r, Flags flags_r, TOutputIterator result_r )
  {
    ...
    if ( glob.lastGlobReturn() == 0 )
      for_( it, glob.begin(), glob.end() )( *result_r )++ =
...
inline std::ostream &operator<<(
  std::ostream &str, const Glob::const_iterator &obj )
{
  return str << *obj;
}
