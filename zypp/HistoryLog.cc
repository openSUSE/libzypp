...
static std::string cmdline()
{
  ...
  if ( _val.empty() )
  {
    pid_t mypid = getpid();
    {
      std::ifstream cmdlineStr(
        Pathname( "/proc/" + zypp::str::numstring( mypid ) + "/cmdline" )
          .c_str() );
      char ch;
      const char *sep = "'";
...
void HistoryLog::install( const PoolItem &pi )
{
  ...
  _log << _sep << p->repoInfo().alias()    // 7 repo alias
       << _sep << p->checksum().checksum() // 8 checksum
       << _sep
       << str::escape( ZConfig::instance().userData(), _sep ) // 9 userdata
       << endl;
}
...
void HistoryLog::remove( const PoolItem &pi )
{
  ...
  _log << _sep
       << str::escape( ZConfig::instance().userData(), _sep ) // 7 userdata
       << endl;
}
...
void HistoryLog::addRepository( const RepoInfo &repo )
{
  _log << timestamp()                                        // 1 timestamp
       ...
       << _sep
       << str::escape( ZConfig::instance().userData(), _sep ) // 5 userdata
       << endl;
}
...
void HistoryLog::removeRepository( const RepoInfo &repo )
{
  _log << timestamp()                                           // 1 timestamp
       ...
       << _sep << str::escape( repo.alias(), _sep )             // 3 alias
       << _sep
       << str::escape( ZConfig::instance().userData(), _sep ) // 4 userdata
       << endl;
}
  ...
  const RepoInfo &oldrepo, const RepoInfo &newrepo )
{
  if ( oldrepo.alias() != newrepo.alias() )
  {
    _log << timestamp() // 1 timestamp
         << _sep
         << HistoryActionID::REPO_CHANGE_ALIAS.asString( true ) // 2 action
  ...
  if ( oldrepo.url() != newrepo.url() )
  {
    _log << timestamp() // 1 timestamp
         << _sep
         << HistoryActionID::REPO_CHANGE_URL.asString( true )     // 2 action
