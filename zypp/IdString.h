...
public:
  ...
  /** Ctor from id. */
  constexpr explicit IdString( IdType id_r )
    : _id( id_r )
  {
  }
...
public:
  ...
  constexpr explicit operator bool() const { return _id; }
...
public:
  /** Fast compare equal. */
  bool compareEQ( const IdString &rhs ) const { return ( _id == rhs.id() ); }
  ...
  /** \overload */
  int compare( const std::string &rhs ) const { return compare( rhs.c_str() ); }
