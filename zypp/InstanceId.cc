...
std::string InstanceId::getIdFor( sat::Solvable slv_r ) const
{
  ...
  if ( slv_r.isKind<
         SrcPackage>() ) // libsolv uses no namespace in SrcPackage ident!
  {
    ret += ResKind::srcpackage.c_str();
    ret += ':';
  }

  ret += str::form( "%s-%s-%s.%s@%s", slv_r.ident().c_str(),
    slv_r.edition().version().c_str(), slv_r.edition().release().c_str(),
    slv_r.arch().c_str(), slv_r.repository().alias().c_str() );
  return ret;
}
...
PoolItem InstanceId::findPoolItem( const std::string str_r ) const
{
  ...
  Repository repo(
    sat::Pool::instance().reposFind( str_r.substr( rdelim + 1 ) ) );
  if ( !repo )
    return PoolItem();
  ...
  for_( it, pool.byIdentBegin( ident.kind(), ident.name() ),
    pool.byIdentEnd( ident.kind(), ident.name() ) )
  {
    sat::Solvable solv( ( *it ).satSolvable() );
    if ( solv.repository() == repo && solv.arch() == arch &&
         solv.edition() == ed )
    {
      return *it;
    }
