...
struct KVMapBase : public std::map<std::string, std::string>
{
  ...
  struct CharSep : public KVMapPolicy
  {
    CharSep()
      : KVMapPolicy( std::string( 1, kv ), std::string( 1, f ) )
    {
    }
  };
  ...
  static std::string join( const map_type &kvmap_r, const KVMapPolicy &opts_r )
  {
    ...
    for ( map_type::const_iterator it = kvmap_r.begin(); it != kvmap_r.end();
          ++it )
    {
      if ( !ret.empty() )
      {
...
struct KVMap : public kvmap::KVMapBase
{
  ...
  std::string asString() const { return join( *this, KVMapOpts() ); }
};
