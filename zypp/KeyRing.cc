...
void KeyRingReport::infoVerify( const std::string &file_r,
  const PublicKeyData &keyData_r, const KeyContext &keycontext )
{
}

bool KeyRingReport::askUserToAcceptUnsignedFile(
  const std::string &file, const KeyContext &keycontext )
{
  return _keyRingDefaultAccept.testFlag( KeyRing::ACCEPT_UNSIGNED_FILE );
}

KeyRingReport::KeyTrust KeyRingReport::askUserToAcceptKey(
  const PublicKey &key, const KeyContext &keycontext )
{
...
bool KeyRingReport::askUserToAcceptUnknownKey(
  const std::string &file, const std::string &id, const KeyContext &keycontext )
{
  return _keyRingDefaultAccept.testFlag( KeyRing::ACCEPT_UNKNOWNKEY );
}

bool KeyRingReport::askUserToAcceptVerificationFailed(
  const std::string &file, const PublicKey &key, const KeyContext &keycontext )
{
...
private:
  struct Cache
  {
    ...
    void assertCache( const Pathname &keyring_r )
    {
      ...
      if ( !_keyringK )
        _keyringK.reset(
          new WatchFile( keyring_r / "pubring.kbx", WatchFile::NO_INIT ) );
      if ( !_keyringP )
        _keyringP.reset(
          new WatchFile( keyring_r / "pubring.gpg", WatchFile::NO_INIT ) );
    }
  ...
  const std::list<PublicKeyData> &getData(
    const Pathname &keyring_r, Cache &cache_r ) const
  {
...
struct KeyRing::Impl
{
  ...
  bool isKeyKnown( const std::string &id )
  {
    return publicKeyExists( id, trustedKeyRing() ) ||
           publicKeyExists( id, generalKeyRing() );
  }
  ...
  std::list<PublicKey> publicKeys() { return publicKeys( generalKeyRing() ); }
  ...
  void dumpPublicKey(
    const std::string &id, bool trusted, std::ostream &stream )
  {
    dumpPublicKey(
  ...
  bool verifyFileSignatureWorkflow( const Pathname &file,
    const std::string &filedesc, const Pathname &signature, bool &sigValid_r,
  ...
  bool verifyFileTrustedSignature(
    const Pathname &file, const Pathname &signature )
  {
...
private:
  bool verifyFile(
    const Pathname &file, const Pathname &signature, const Pathname &keyring );
  ...
  void dumpPublicKey(
    const std::string &id, const Pathname &keyring, std::ostream &stream );
  filesystem::TmpFile dumpPublicKeyToTmp(
  ...
  const Pathname generalKeyRing() const { return _general_tmp_dir.path(); }
  const Pathname trustedKeyRing() const { return _trusted_tmp_dir.path(); }
...
void KeyRing::Impl::importKey( const PublicKey &key, bool trusted )
{
  ...
  MIL << "Imported key " << key << " to "
      << ( trusted ? "trustedKeyRing" : "generalKeyRing" ) << endl;
...
{
  PublicKeyData keyDataToDel(
    publicKeyExists( id, trusted ? trustedKeyRing() : generalKeyRing() ) );
  if ( !keyDataToDel )
  {
    WAR << "Key to delete [" << id << "] is not in "
        << ( trusted ? "trustedKeyRing" : "generalKeyRing" ) << endl;
    return;
  }
  deleteKey( id, trusted ? trustedKeyRing() : generalKeyRing() );
  MIL << "Deleted key [" << id << "] from "
      << ( trusted ? "trustedKeyRing" : "generalKeyRing" ) << endl;
...
PublicKeyData KeyRing::Impl::publicKeyExists(
  const std::string &id, const Pathname &keyring )
{
  PublicKeyData ret;
...
PublicKey KeyRing::Impl::exportKey(
  const PublicKeyData &keyData, const Pathname &keyring )
{
...
void KeyRing::Impl::dumpPublicKey(
  const std::string &id, const Pathname &keyring, std::ostream &stream )
{
...
filesystem::TmpFile KeyRing::Impl::dumpPublicKeyToTmp(
  const std::string &id, const Pathname &keyring )
{
...
bool KeyRing::Impl::verifyFileSignatureWorkflow( const Pathname &file,
  const std::string &filedesc, const Pathname &signature, bool &sigValid_r,
  const KeyContext &context )
{
  sigValid_r =
    false; // set true if signature is actually successfully validated!

  callback::SendReport<KeyRingReport> report;
  MIL << "Going to verify signature for " << filedesc << " ( " << file
      << " ) with " << signature << endl;
  ...
  if ( trustedKeyData )
  {
    ...
    if ( generalKeyData )
    {
      ...
      {
        MIL << "Key was updated. Saving new version into trusted keyring: "
            << generalKeyData << endl;
        importKey( exportKey( generalKeyData, generalKeyRing() ), true );
        trustedKeyData = publicKeyExists(
          id, trustedKeyRing() ); // re-read: invalidated by import?
      }
    }
    ...
    {
      return (
        sigValid_r = true ); // signature is actually successfully validated!
    }
    else
    {
      bool res = report->askUserToAcceptVerificationFailed(
        filedesc, exportKey( trustedKeyData, trustedKeyRing() ), context );
  ...
  else
  {
    ...
    if ( generalKeyData )
    {
      ...
      KeyRingReport::KeyTrust reply =
        report->askUserToAcceptKey( key, context );
           ...
           reply == KeyRingReport::KEY_TRUST_AND_IMPORT )
      {
        ...
        {
          return ( sigValid_r =
                     true ); // signature is actually successfully validated!
        }
        else
        {
          bool res =
    ...
    else
    {
      ...
      MIL << "File [" << file << "] ( " << filedesc
          << " ) signed with unknown key [" << id << "]" << endl;
...
std::string KeyRing::Impl::readSignatureKeyId( const Pathname &signature )
{
  if ( !PathInfo( signature ).isFile() )
    ZYPP_THROW(
      KeyRingException( str::Format( _( "Signature file %s not found" ) ) %
                        signature.asString() ) );
  ...
  if ( fprs.size() )
  {
    ...
    MIL << "Determined key id [" << id << "] for signature " << signature
        << endl;
    return id;
  }
  return std::string();
...
std::list<PublicKey> KeyRing::publicKeys() { return _pimpl->publicKeys(); }
...
bool KeyRing::verifyFileSignatureWorkflow( const Pathname &file,
  const std::string &filedesc, const Pathname &signature, bool &sigValid_r,
  const KeyContext &keycontext )
{
  return _pimpl->verifyFileSignatureWorkflow(
    file, filedesc, signature, sigValid_r, keycontext );
}

bool KeyRing::verifyFileSignatureWorkflow( const Pathname &file,
  const std::string filedesc, const Pathname &signature,
  const KeyContext &keycontext )
{
  bool unused;
  return _pimpl->verifyFileSignatureWorkflow(
    file, filedesc, signature, unused, keycontext );
}

bool KeyRing::verifyFileSignature(
  const Pathname &file, const Pathname &signature )
{
  return _pimpl->verifyFileSignature( file, signature );
}

bool KeyRing::verifyFileTrustedSignature(
  const Pathname &file, const Pathname &signature )
{
  return _pimpl->verifyFileTrustedSignature( file, signature );
}

void KeyRing::dumpPublicKey(
  const std::string &id, bool trusted, std::ostream &stream )
{
