...
private:
  ...
  Link newIndex( IdString index_r, const std::string &code_r )
  {
    ...
    if ( code_r.size() > 3 || code_r.size() < 2 )
      WAR << "Malformed LanguageCode '" << code_r << "' (expect 2 or 3-letter)"
          << endl;

    std::string lcode( str::toLower( code_r ) );
    if ( lcode != code_r )
    {
      WAR << "Malformed LanguageCode '" << code_r << "' (not lower case)"
          << endl;
    ...
    MIL << "Remember LanguageCode '" << code_r << "': '" << nval.second << "'"
        << endl;
...
CodeMaps::CodeMaps()
{
  ...
  const LangInit langInit[] = {// language code: aar aa
    {"aar", "aa", N_( "Afar" )},
