...
struct CodeMaps
{
  ...
  {
    return (
      code_r ? withoutTrash( boost::string_ref( code_r ) ) : IdString::Null );
  }
  ...
  LanguageCode language( IdString index_r ) { return getIndex( index_r )._l; }

  CountryCode country( IdString index_r ) { return getIndex( index_r )._c; }
  ...
  Locale fallback( IdString index_r )
  {
    ...
    Locale ret;
    if ( index_r ==
         special ) // "pt_BR"->"en" - by now the only fallback exception
      ret = Locale::enCode;
    else
    {
      const LC &lc( getIndex( index_r ) );
...
private:
  ...
  CodeMaps()
    : _codeMap( {{IdString::Null, LC( LanguageCode( IdString::Null ),
                                    CountryCode( IdString::Null ) )},
        {IdString::Empty, LC( LanguageCode( IdString::Empty ),
                            CountryCode( IdString::Empty ) )}} )
  {
  }
...
const LanguageCode LanguageCode::enCode(
  "en" ); // from in LanguageCode.cc as Locale::enCode depends on it
...
std::string Locale::name() const { return CodeMaps::instance().name( _str ); }
