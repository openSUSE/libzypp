...
public:
  ...
  const LockSet &locks() const { return _locks; }
...
Locks::const_iterator Locks::end() const { return _pimpl->APIlocks().end(); }
...
bool Locks::empty() const { return _pimpl->locks().empty(); }
...
void Locks::readAndApply( const Pathname &file )
{
  ...
  if ( pinfo.isExist() )
  {
    std::insert_iterator<LockSet> ii(
      _pimpl->MANIPlocks(), _pimpl->MANIPlocks().end() );
    LockingOutputIterator<std::insert_iterator<LockSet>> lout( ii );
    readPoolQueriesFromFile(
      file, boost::make_function_output_iterator( lout ) );
  }
  else
...
void Locks::read( const Pathname &file )
{
  ...
  if ( pinfo.isExist() )
    readPoolQueriesFromFile(
      file, std::insert_iterator<LockSet>(
              _pimpl->MANIPlocks(), _pimpl->MANIPlocks().end() ) );
  else
    MIL << "file does not exist(or cannot be stat), no lock added." << endl;
}
...
public:
  LocksCleanPredicate(
    size_t count, callback::SendReport<CleanEmptyLocksReport> &_report )
    : skip_rest( false )
    , searched( 0 )
    , all( count )
    , report( _report )
...
{
  MIL << "merge list old: " << locks().size() << " to add: " << toAdd.size()
      << "to remove: " << toRemove.size() << endl;
...
void Locks::save( const Pathname &file )
{
  ...
  writePoolQueriesToFile(
    file, _pimpl->locks().begin(), _pimpl->locks().end() );
  report->finish( SavingLocksReport::NO_ERROR );
}

void Locks::removeDuplicates() { /* NOP since implementation uses std::set */}
