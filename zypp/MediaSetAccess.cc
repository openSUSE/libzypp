...
MediaSetAccess::MediaSetAccess(
  const Url &url, const Pathname &prefered_attach_point )
  : _url( url )
  , _prefAttachPoint( prefered_attach_point )
{
}

MediaSetAccess::MediaSetAccess( const std::string &label_r, const Url &url,
...
MediaSetAccess::~MediaSetAccess()
{
  ...
  {
  } // don't let exception escape a dtor.
}

void MediaSetAccess::setVerifier(
  unsigned media_nr, media::MediaVerifierRef verifier )
{
...
void MediaSetAccess::releaseFile( const Pathname &file, unsigned media_nr )
{
  ...
  DBG << "Going to release file " << file << " from media number " << media_nr
      << endl;
...
void MediaSetAccess::dirInfo( filesystem::DirContent &retlist,
  const Pathname &dirname, bool dots, unsigned media_nr )
{
  media::MediaManager media_mgr;
  media::MediaAccessId media;
  media = getMediaAccessId( media_nr );
...
Pathname MediaSetAccess::provideFile(
  const Pathname &file, unsigned media_nr, ProvideFileOptions options )
{
...
Pathname MediaSetAccess::provideOptionalFile(
  const Pathname &file, unsigned media_nr )
{
  try
  {
  ...
  const Pathname &deltafile )
{
  ...
  do
  {
    ...
    try
    {
      DBG << "Going to try to provide "
          << ( resource.optional() ? "optional" : "" ) << " file " << file
    ...
    catch ( media::MediaException &excp )
    {
      ...
      do
      {
        ...
        {
          DBG << "Media couldn't provide file " << file << " , releasing."
              << endl;
          try
          {
                  ...
                  typeid( excp ) == typeid( media::MediaNotAttachedException ) )
        {
          reason = media::MediaChangeReport::WRONG;
                  ...
                  typeid( excp ) ==
                    typeid( media::MediaTemporaryProblemException ) )
        {
        ...
        if ( !callback::SendReport<media::MediaChangeReport>::connected() ||
             ( ( options & PROVIDE_NON_INTERACTIVE ) &&
               reason != media::MediaChangeReport::WRONG ) )
        {
        ...
        else
        {
          ...
          user = report->requestMedia( _url, media_nr, _label, reason,
            excp.asUserHistory(), devices, devindex );
        }

        MIL << "ProvideFile exception caught, callback answer: " << user
            << endl;
...
Pathname MediaSetAccess::provideDir( const Pathname &dir, bool recursive,
  unsigned media_nr, ProvideFileOptions options )
{
  OnMediaLocation resource;
...
{
  str << "MediaSetAccess (URL='" << _url << "', attach_point_hint='"
      << _prefAttachPoint << "')";
  return str;
}
