...
Pathname cachedLocation( const OnMediaLocation &loc_r, const RepoInfo &repo_r )
{
  ...
  if ( loc_r.checksum().empty() )
  {
    ...
    if ( CheckSum( CheckSum::md5Type(),
           std::ifstream(
                     ( url.getPathName() / repo_r.path() / loc_r.filename() )
                       .c_str() ) ) !=
  ...
  {
    if ( loc_r.checksum() !=
         CheckSum( loc_r.checksum().type(), std::ifstream( pi.c_str() ) ) )
      return Pathname(); // same name but wrong checksum
  }
...
VendorSupportOption Package::vendorSupport() const
{
  ...
  {
    if ( solv.edition() == edition() && solv.ident() == ident() &&
         identical( solv ) )
    {
      for ( PackageKeyword kw : Keywords( sat::SolvAttr::keywords, solv ) )
      {
        switch ( ret )
        {
          case VendorSupportUnknown:
            if ( kw == support_unsupported )
            {
              ret = VendorSupportUnsupported;
              break;
            }
          case VendorSupportUnsupported:
            if ( kw == support_acc )
            {
              ret = VendorSupportACC;
              break;
            }
          case VendorSupportACC:
            if ( kw == support_l1 )
            {
              ret = VendorSupportLevel1;
              break;
            }
          case VendorSupportLevel1:
            if ( kw == support_l2 )
            {
              ret = VendorSupportLevel2;
              break;
            }
...
Changelog Package::changelog() const
{
  ...
  if ( repository().isSystemRepo() )
  {
    ...
    return header
             ? header->tag_changelog()
             : Changelog(); // might be deleted behind our back (bnc #530595)
  }
...
std::list<std::string> Package::authors() const
{
  ...
  str::split( lookupStrAttribute( sat::SolvAttr::authors ),
...
OnMediaLocation Package::location() const { return lookupLocation(); }
...
{
  return str::form( "%s-%s.%s", sourcePkgName().c_str(),
    sourcePkgEdition().c_str(), sourcePkgType().c_str() );
}
