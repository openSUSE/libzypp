...
std::ostream &operator<<( std::ostream &str, const StatMode &obj )
{
  ...
  str << t << " " << std::setfill( '0' ) << std::setw( 4 ) << std::oct
      << obj.perm();
  return str;
}
...
std::ostream &operator<<( std::ostream &str, const PathInfo &obj )
{
  ...
  {
    str << obj.asStatMode() << " " << std::dec << obj.owner() << "/"
        << obj.group();
...
int assert_dir( const Pathname &path, unsigned mode )
{
  ...
  string::size_type lastpos =
    ( path.relative() ? 2 : 1 ); // skip leasding './' or '/'
  ...
  while ( ( pos = spath.find( '/', lastpos ) ) != string::npos )
  {
    ...
    if ( ret == -1 )
    {
      ...
      {
        ret = errno;
        WAR << " FAILED: mkdir " << dir << ' ' << str::octstring( mode )
            << " errno " << ret << endl;
      }
    }
    else
...
int clean_dir( const Pathname &path )
{
  ...
  return logResult(
    recursive_rmdir_1( path, false /* don't remove path itself */ ) );
}
...
int copy_dir( const Pathname &srcpath, const Pathname &destpath )
{
  ...
  const char *const argv[] = {"/bin/cp", "-dR", "--",
    srcpath.asString().c_str(), destpath.asString().c_str(), NULL};
  ExternalProgram prog( argv, ExternalProgram::Stderr_To_Stdout );
  for ( string output( prog.receiveLine() ); output.length();
...
int copy_dir_content( const Pathname &srcpath, const Pathname &destpath )
{
  ...
  const char *const argv[] = {
    "/bin/cp", "-dR", "--", src.c_str(), destpath.asString().c_str(), NULL};
  ExternalProgram prog( argv, ExternalProgram::Stderr_To_Stdout );
  for ( string output( prog.receiveLine() ); output.length();
        output = prog.receiveLine() )
  {
...
int dirForEach( const Pathname &dir_r,
  function<bool( const Pathname &, const char *const )> fnc_r )
{
  if ( !fnc_r )
    return 0;

  AutoDispose<DIR *> dir(::opendir( dir_r.c_str() ), []( DIR *dir_r ) {
    if ( dir_r )
      ::closedir( dir_r );
  } );
...
int dirForEach( const Pathname &dir_r, const StrMatcher &matcher_r,
  function<bool( const Pathname &, const char *const )> fnc_r )
{
  if ( !fnc_r )
    return 0;
  ...
  return dirForEach(
    dir_r, [&]( const Pathname &dir_r, const char *const name_r ) -> bool {
...
int readdir(
  std::list<std::string> &retlist_r, const Pathname &path_r, bool dots_r )
{
  retlist_r.clear();
  return dirForEach(
    path_r, [&]( const Pathname &dir_r, const char *const name_r ) -> bool {
      if ( dots_r || name_r[ 0 ] != '.' )
...
int readdir(
  std::list<Pathname> &retlist_r, const Pathname &path_r, bool dots_r )
{
  retlist_r.clear();
  return dirForEach(
    path_r, [&]( const Pathname &dir_r, const char *const name_r ) -> bool {
...
int readdir( DirContent &retlist_r, const Pathname &path_r, bool dots_r,
  PathInfo::Mode statmode_r )
{
  retlist_r.clear();
  return dirForEach(
    path_r, [&]( const Pathname &dir_r, const char *const name_r ) -> bool {
      if ( dots_r || name_r[ 0 ] != '.' )
        retlist_r.push_back( DirEntry(
          name_r, PathInfo( dir_r / name_r, statmode_r ).fileType() ) );
      return true;
    } );
}
...
int safe_rename( const Pathname &oldpath, const Pathname &newpath )
{
  ...
  {
    const char *const argv[] = {"/usr/bin/mv", oldpath.asString().c_str(),
      newpath.asString().c_str(), NULL};
    ExternalProgram prog( argv, ExternalProgram::Stderr_To_Stdout );
    for ( string output( prog.receiveLine() ); output.length();
          output = prog.receiveLine() )
    {
...
int copy( const Pathname &file, const Pathname &dest )
{
  ...
  const char *const argv[] = {"/bin/cp", "--remove-destination", "--",
    file.asString().c_str(), dest.asString().c_str(), NULL};
  ExternalProgram prog( argv, ExternalProgram::Stderr_To_Stdout );
  for ( string output( prog.receiveLine() ); output.length();
        output = prog.receiveLine() )
  {
...
int symlink( const Pathname &oldpath, const Pathname &newpath )
{
  ...
  if (::symlink( oldpath.asString().c_str(), newpath.asString().c_str() ) ==
      -1 )
  {
    return logResult( errno );
  }
...
Pathname expandlink( const Pathname &path_r )
{
  ...
  if ( count == 0 )
  {
    ERR << "Expand level limit reached. Probably a cyclic symbolic link."
        << endl;
    return Pathname();
  }
  // symlink
...
int copy_file2dir( const Pathname &file, const Pathname &dest )
{
  ...
  const char *const argv[] = {
    "/bin/cp", "--", file.asString().c_str(), dest.asString().c_str(), NULL};
  ExternalProgram prog( argv, ExternalProgram::Stderr_To_Stdout );
  for ( string output( prog.receiveLine() ); output.length();
        output = prog.receiveLine() )
  {
...
//	METHOD TYPE : std::string
//
std::string sha1sum( const Pathname &file ) { return checksum( file, "SHA1" ); }
...
{
  return (
    filesystem::checksum( file, checksum.type() ) == checksum.checksum() );
}
