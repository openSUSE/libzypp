...
public:
  ...
  //@{
  PathInfo();
  explicit PathInfo( const Pathname &path, Mode initial = STAT );
  explicit PathInfo( const std::string &path, Mode initial = STAT );
  explicit PathInfo( const char *path, Mode initial = STAT );
  //@}
...
public:
  ...
  mode_t perm() const
  {
    return isExist() ? ( statbuf_C.st_mode & ( S_IRWXU | S_IRWXG | S_IRWXO |
...
int readdir(
  std::list<std::string> &retlist, const Pathname &path, bool dots = true );
...
int readdir(
  std::list<Pathname> &retlist, const Pathname &path, bool dots = true );
...
struct DirEntry
{
  ...
  FileType type;
  DirEntry(
    const std::string &name_r = std::string(), FileType type_r = FT_NOT_AVAIL )
    : name( name_r )
    , type( type_r )
  {
  }
...
/**
     ...
     * @return 0 on success, errno on failure.
     **/
int readdir( DirContent &retlist, const Pathname &path, bool dots = true,
...
/**
     ...
     **/
inline mode_t applyUmaskTo( mode_t mode_r ) { return mode_r & ~getUmask(); }
//@}
