...
void Pathname::_assign( const string &name_r )
{
  ...
  auto goParent_f = [&]() {
    ...
    else
    {
      ...
      if ( pos == _name.size() - 3 && _name[ pos + 1 ] == '.' &&
           _name[ pos + 2 ] == '.' )
        _name += "/..";
      else
        _name.erase( pos );
...
Url Pathname::asUrl() const { return asUrl( "dir" ); }

Url Pathname::asDirUrl() const { return asUrl( "dir" ); }

Url Pathname::asFileUrl() const { return asUrl( "file" ); }
...
std::string Pathname::showRootIf(
  const Pathname &root_r, const Pathname &path_r )
{
