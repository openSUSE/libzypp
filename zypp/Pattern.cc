...
public:
  ...
  const_iterator begin() const { return make_map_key_begin( _patternMap ); }

  const_iterator end() const { return make_map_key_end( _patternMap ); }
...
private:
  ...
  void expandIfExtends(
    const Pattern::constPtr &pat_r, const PoolItem &extending_r )
  {
    ...
    {
      if ( providedBy(
             pat_r, Capability( it->c_str() /*, *ResKind::pattern*/ ) ) )
      {
...
sat::Solvable Pattern::autoPackage() const
{
  ...
  {
    Capability pkgCap(
      arch(), autocap.detail().ed().asString(), Rel::EQ, edition() );
...
inline void dependsSetDoCollect(
  sat::Solvable depKeeper_r, Dep dep_r, Pattern::Contents &set_r )
{
  CapabilitySet caps;
...
void Pattern::contentsSet( ContentsSet &collect_r, bool recursively_r ) const
{
  ...
  std::set<sat::Solvable> allDone; // patterns-packages already expanded
  {
    ...
    while ( !reqTodo.empty() )
    {
      ...
      // evaluate result....
      for ( sat::Solvable solv :
        result.req ) // remember unprocessed required patterns-packages...
      {
        if ( collect_r.req.insert( solv ) && recursively_r &&
             isPatternsPackage( solv ) )
          reqTodo.insert( solv );
      }
      for ( sat::Solvable solv :
        result.rec ) // remember unprocessed recommended patterns-packages...
      {
        if ( collect_r.rec.insert( solv ) && recursively_r &&
             isPatternsPackage( solv ) )
          recTodo.insert( solv );
      }
      for ( sat::Solvable solv :
        result.sug ) // NOTE: We don't expand suggested patterns!
      {
