...
public:
  ...
  bool empty() const { return _scripts.empty(); }

  size_t size() const { return _scripts.size(); }
  ...
  const std::list<PluginScript> scripts() const { return _scripts; }
...
private:
  ...
  PluginFrame doSend( PluginScript &script_r, const PluginFrame &frame_r )
  {
    ...
    if ( !( ret.isAckCommand() || ret.isEnomethodCommand() ||
            ( script_r.script() == "/bin/cat" &&
              frame_r.command() != "ERROR" ) ) )
    {
...
bool PluginExecutor::empty() const { return _pimpl->empty(); }

size_t PluginExecutor::size() const { return _pimpl->size(); }

void PluginExecutor::load( const Pathname &path_r ) { _pimpl->load( path_r ); }
