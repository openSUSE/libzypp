...
public:
  ...
  Impl( const std::string &command_r ) { setCommand( command_r ); }
...
public:
  bool empty() const { return _command.empty() && _body.empty(); }
  ...
  std::string &bodyRef() { return _body; }

  void setBody( const std::string &body_r ) { _body = body_r; }
...
public:
  ...
  HeaderList &headerList() { return _header; }

  const HeaderList &headerList() const { return _header; }
  ...
  const std::string &getHeader(
    const std::string &key_r, const std::string &default_r ) const
  {
    constKeyRange r( _header.equal_range( key_r ) );
    if ( r.first == r.second )
      return default_r;
  ...
  HeaderList::value_type mkHeaderPair(
    const std::string &key_r, const std::string &value_r )
  {
    if ( key_r.find_first_of( ":\n" ) != std::string::npos )
      ZYPP_THROW( PluginFrameException( "Illegal char in header key", key_r ) );
    if ( value_r.find_first_of( "\n" ) != std::string::npos )
      ZYPP_THROW(
        PluginFrameException( "Illegal char in header value", value_r ) );
  ...
  void clearHeader( const std::string &key_r ) { _header.erase( key_r ); }
...
inline std::ostream &operator<<(
  std::ostream &str, const PluginFrame::Impl &obj )
{
  return str << "PluginFrame[" << obj.command() << "]("
             << obj.headerList().size() << "){" << obj.body().size() << "}";
}
...
PluginFrame::Impl::Impl( std::istream &stream_r )
{
  ...
  do
  {
    ...
    _header.insert(
      HeaderList::value_type( data.substr( 0, sep ), data.substr( sep + 1 ) ) );
  } while ( true );
...
std::ostream &PluginFrame::Impl::writeTo( std::ostream &stream_r ) const
{
  ...
  // header
  for_( it, _header.begin(), _header.end() ) stream_r << it->first << ':'
                                                      << it->second << endl;
  // body
  stream_r << endl << _body << '\0';
...
PluginFrame::PluginFrame(
  const std::string &command_r, const std::string &body_r )
  : _pimpl( new Impl( command_r, body_r ) )
{
}

PluginFrame::PluginFrame(
  const std::string &command_r, HeaderInitializerList contents_r )
  : _pimpl( new Impl( command_r, contents_r ) )
{
}

PluginFrame::PluginFrame( const std::string &command_r,
  const std::string &body_r, HeaderInitializerList contents_r )
...
bool PluginFrame::empty() const { return _pimpl->empty(); }

const std::string &PluginFrame::command() const { return _pimpl->command(); }
...
const std::string &PluginFrame::body() const { return _pimpl->body(); }

std::string &PluginFrame::bodyRef() { return _pimpl->bodyRef(); }
...
const std::string &PluginFrame::getHeader(
  const std::string &key_r, const std::string &default_r ) const
{
  return _pimpl->getHeader( key_r, default_r );
}

const std::string &PluginFrame::getHeaderNT(
  const std::string &key_r, const std::string &default_r ) const
{
  return _pimpl->getHeaderNT( key_r, default_r );
}

void PluginFrame::setHeader(
  const std::string &key_r, const std::string &value_r )
{
  _pimpl->setHeader( key_r, value_r );
}

void PluginFrame::addHeader(
  const std::string &key_r, const std::string &value_r )
{
