...
public:
  ...
  bool isAckCommand() const { return command() == ackCommand(); }
  ...
  bool isEnomethodCommand() const { return command() == enomethodCommand(); }
...
public:
  ...
  bool headerEmpty() const { return headerList().empty(); }

  /** Return size of the header list. */
  unsigned headerSize() const { return headerList().size(); }

  /** Return iterator pointing to the 1st header (or \ref headerEnd) */
  HeaderListIterator headerBegin() const { return headerList().begin(); }

  /** Return iterator pointing behind the last header. */
  HeaderListIterator headerEnd() const { return headerList().end(); }

  /** Clear the list of headers. */
  void headerClear() { headerList().clear(); }

  /** Whether the header list contains at least one entry for \c key_r. */
  bool hasKey( const std::string &key_r ) const { return !keyEmpty( key_r ); }
  ...
  const std::string &getHeaderNT( const std::string &key_r,
  ...
  /** Set header for \c key_r removing all other occurrences of \c key_r.
       ...
       */
  void setHeader(
    const std::string &key_r, const std::string &value_r = std::string() );
...
public:
  ...
  static std::ostream &writeTo(
    std::ostream &stream_r, const PluginFrame &frame_r )
  {
