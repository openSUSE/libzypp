...
{
  PluginDebugBuffer( const std::string &buffer_r )
    : _buffer( buffer_r )
  {
  }
  ~PluginDebugBuffer()
...
{
  PluginDumpStderr( ExternalProgramWithStderr &prog_r )
    : _prog( prog_r )
  {
  }
  ~PluginDumpStderr()
...
{
public:
  Impl( const Pathname &script_r = Pathname(),
...
public:
  const Pathname &script() const { return _script; }

  const Arguments &args() const { return _args; }

  pid_t getPid() const { return _cmd ? _cmd->getpid() : NotConnected; }

  bool isOpen() const { return _cmd != nullptr; }

  int lastReturn() const { return _lastReturn; }

  const std::string &lastExecError() const { return _lastExecError; }

public:
  void open( const Pathname &script_r = Pathname(),
    const Arguments &args_r = Arguments() );
...
inline std::ostream &operator<<(
  std::ostream &str, const PluginScript::Impl &obj )
{
...
namespace
{
const long PLUGIN_TIMEOUT =
  str::strtonum<long>( getenv( "ZYPP_PLUGIN_TIMEOUT" ) );
const long PLUGIN_SEND_TIMEOUT =
  str::strtonum<long>( getenv( "ZYPP_PLUGIN_SEND_TIMEOUT" ) );
const long PLUGIN_RECEIVE_TIMEOUT =
  str::strtonum<long>( getenv( "ZYPP_PLUGIN_RECEIVE_TIMEOUT" ) );
}

long PluginScript::Impl::_defaultSendTimeout =
  ( PLUGIN_SEND_TIMEOUT > 0 ? PLUGIN_SEND_TIMEOUT
                            : ( PLUGIN_TIMEOUT > 0 ? PLUGIN_TIMEOUT : 30 ) );
long PluginScript::Impl::_defaultReceiveTimeout =
  ( PLUGIN_RECEIVE_TIMEOUT > 0 ? PLUGIN_RECEIVE_TIMEOUT
                               : ( PLUGIN_TIMEOUT > 0 ? PLUGIN_TIMEOUT : 30 ) );

///////////////////////////////////////////////////////////////////

void PluginScript::Impl::open(
  const Pathname &script_r, const Arguments &args_r )
{
  dumpRangeLine( DBG << "Open " << script_r, args_r.begin(), args_r.end() )
    << endl;

  if ( _cmd )
    ZYPP_THROW(
      PluginScriptException( "Already connected", str::Str() << *this ) );

  {
    PathInfo pi( script_r );
    if ( !( pi.isFile() && pi.isX() ) )
      ZYPP_THROW(
        PluginScriptException( "Script is not executable", str::Str() << pi ) );
  }
...
void PluginScript::Impl::send( const PluginFrame &frame_r ) const
{
  if ( !_cmd )
    ZYPP_THROW(
      PluginScriptNotConnected( "Not connected", str::Str() << *this ) );
  ...
  //DBG << " ->[" << fd << " " << (::feof(filep)?'e':'_') << (::ferror(filep)?'F':'_') << "]" << endl;
  {
    ...
    do
    {
          ...
          if ( errno != EINTR )
          {
            ...
            if ( errno == EPIPE )
              ZYPP_THROW( PluginScriptDiedUnexpectedly(
                "Send: script died unexpectedly", str::Str() << Errno() ) );
            else
              ZYPP_THROW( PluginScriptException(
                "Send: send error", str::Str() << Errno() ) );
          }
        }
      }
      else if ( retval == 0 )
      {
        ...
        ZYPP_THROW(
          PluginScriptSendTimeout( "Not ready to write within timeout." ) );
      }
      else // ( retval == -1 )
      {
        if ( errno != EINTR )
        {
          ...
          ZYPP_THROW( PluginScriptException(
            "Error waiting on file descriptor", str::Str() << Errno() ) );
        }
      }
    } while ( true );
  }
  ...
  std::string data;
  {
    ...
    do
    {
      ...
      else if (::feof( filep ) )
      {
        ...
        ZYPP_THROW( PluginScriptDiedUnexpectedly(
          "Receive: script died unexpectedly", str::Str() << Errno() ) );
      }
      else if ( errno != EINTR )
      {
        if ( errno == EWOULDBLOCK )
        {
          ...
          else if ( retval == 0 )
          {
            ...
            ZYPP_THROW( PluginScriptReceiveTimeout(
        ...
        else
        {
          ERR << "read(): " << Errno() << endl;
          ZYPP_THROW( PluginScriptException(
            "Receive: receive error", str::Str() << Errno() ) );
        }
      }
    } while ( true );
  }
...
long PluginScript::defaultSendTimeout() { return Impl::_defaultSendTimeout; }
...
long PluginScript::sendTimeout() const { return _pimpl->_sendTimeout; }

long PluginScript::receiveTimeout() const { return _pimpl->_receiveTimeout; }
...
const Pathname &PluginScript::script() const { return _pimpl->script(); }
...
bool PluginScript::isOpen() const { return _pimpl->isOpen(); }

pid_t PluginScript::getPid() const { return _pimpl->getPid(); }

int PluginScript::lastReturn() const { return _pimpl->lastReturn(); }
...
void PluginScript::open() { _pimpl->open( _pimpl->script(), _pimpl->args() ); }
...
int PluginScript::close() { return _pimpl->close(); }
...
PluginFrame PluginScript::receive() const { return _pimpl->receive(); }
