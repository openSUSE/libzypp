...
public:
  ...
  ResObject::constPtr resolvable() const { return _resolvable; }
...
public:
  bool isUndetermined() const { return status().isUndetermined(); }

  bool isRelevant() const { return !status().isNonRelevant(); }

  bool isSatisfied() const { return status().isSatisfied(); }

  bool isBroken() const { return status().isBroken(); }
  ...
  bool isUnwanted() const { return isBroken() && status().isLocked(); }
...
private:
  ...
  //@{
public:
  void saveState() const { _savedStatus = status(); }
  void restoreState() const { status() = _savedStatus; }
...
{
  return PoolItem(
    new Impl( makeResObject( solvable_r ), solvable_r.isSystem() ) );
}

PoolItem::~PoolItem() {}

ResPool PoolItem::pool() const { return ResPool::instance(); }
...
void PoolItem::setBuddy( const sat::Solvable &solv_r )
{
