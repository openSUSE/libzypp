...
bool isDependencyAttribute( sat::SolvAttr attr_r )
{
  static sat::SolvAttr deps[] = {
    SolvAttr::provides, SolvAttr::requires, SolvAttr::recommends,
    SolvAttr::obsoletes, SolvAttr::conflicts, SolvAttr::suggests,
    SolvAttr::supplements, SolvAttr::enhances,
  };
  for_(
    it, arrayBegin( deps ), arrayEnd( deps ) ) if ( *it == attr_r ) return true;
  return false;
}
...
struct SolvableRangePredicate
{
  ...
  bool operator()( sat::LookupAttr::iterator iter_r )
  {
    if ( !_arch.empty() && iter_r.inSolvable().arch() != _arch )
      return false;
    return overlaps(
      Edition::MatchRange( Rel::EQ, iter_r.inSolvable().edition() ), _range );
  }
...
struct AttrMatchData
{
  ...
  std::string serialize() const
  {
    ...
    str::appendEscaped(
      ret, strMatcher.flags().mode() == Match::OTHER ? "C" : "X" );
  ...
  static AttrMatchData deserialize( const std::string &str_r )
  {
    ...
    if ( words.empty() || words[ 0 ] != "AttrMatchData" )
      ZYPP_THROW(
        Exception( str::Str() << "Expecting AttrMatchData: " << str_r ) );
    if ( words.size() != 5 )
      ZYPP_THROW(
        Exception( str::Str() << "Wrong number of words: " << str_r ) );
    ...
    if ( !words.empty() )
    {
      if ( words[ 0 ] == "EditionRange" )
      {
        switch ( words.size() )
        {
          case 3:
            ret.predicate =
              EditionRangePredicate( Rel( words[ 1 ] ), Edition( words[ 2 ] ) );
            break;
          case 4:
            ret.predicate = EditionRangePredicate(
      ...
      else if ( words[ 0 ] == "SolvableRange" )
      {
        ...
        {
          case 3:
            ret.predicate = SolvableRangePredicate(
  ...
  ResKind kindPredicate = ResKind::
    nokind; // holds the 'kind' part if SolvAttr:name looks for an explicit 'kind:name'
};
...
{
  return ( lhs.attr == rhs.attr && lhs.strMatcher == rhs.strMatcher &&
  ...
  bool operator==( const PoolQuery::Impl &rhs ) const
  {
    if ( _flags == rhs._flags
         ...
         || ( ( ( _flags.isModeString() && rhs._flags.isModeGlob() ) ||
              ...
              _strings.empty() && _attrs.size() == 1 &&
              _attrs.begin()->first == sat::SolvAttr::name ) )
    {
      return ( _strings == rhs._strings && _attrs == rhs._attrs &&
               _uncompiledPredicated == rhs._uncompiledPredicated &&
               _match_word == rhs._match_word &&
               _require_all == rhs._require_all &&
               _status_flags == rhs._status_flags && _edition == rhs._edition &&
               _op == rhs._op && _repos == rhs._repos && _kinds == rhs._kinds );
    }
    return false;
  }
...
struct EmptyFilter
{
  bool operator()( const string &str ) { return !str.empty(); }
};
...
void PoolQuery::Impl::compile() const
{
  ...
  else if ( _attrs.size() == 1 )
  {
    StrContainer joined;
    invokeOnEach(
      _strings.begin(), _strings.end(), EmptyFilter(), MyInserter( joined ) );
    invokeOnEach( _attrs.begin()->second.begin(), _attrs.begin()->second.end(),
      EmptyFilter(), MyInserter( joined ) );
    rcstrings = createRegex( joined, cflags );
    if ( joined.size() > 1 ) // switch to regex for multiple strings
      cflags.setModeRegex();
    _attrMatchList.push_back(
      AttrMatchData( _attrs.begin()->first, StrMatcher( rcstrings, cflags ) ) );
  }
  ...
  else
  {
    ...
    for ( AttrRawStrMap::const_iterator ai = _attrs.begin(); ai != _attrs.end();
          ++ai )
      if ( !ai->second.empty() )
  ...
  attremptycheckend:
    ...
    {
      StrContainer result;
      set_difference( set1.begin(), set1.end(), ai->second.begin(),
        ai->second.end(), inserter( result, result.begin() ) /*, ltstr()*/ );
      if ( !result.empty() )
      {
    ...
    if ( attrvals_empty || attrvals_thesame )
    {
      ...
      else
      {
        invokeOnEach( _strings.begin(), _strings.end(), EmptyFilter(),
          MyInserter( joined ) );
        invokeOnEach( _attrs.begin()->second.begin(),
          _attrs.begin()->second.end(), EmptyFilter(), MyInserter( joined ) );
        rcstrings = createRegex( joined, cflags );
      }
      if ( joined.size() > 1 ) // switch to regex for multiple strings
        cflags.setModeRegex();
    ...
    else
    {
      for_( ai, _attrs.begin(), _attrs.end() )
      {
        ...
        if ( joined.size() > 1 ) // switch to regex for multiple strings
          cflags.setModeRegex();
        _attrMatchList.push_back(
          AttrMatchData( ai->first, StrMatcher( s, cflags ) ) );
      }
    }
  }
  ...
  if ( !_uncompiledPredicated.empty() )
  {
    StrContainer global;
    invokeOnEach(
      _strings.begin(), _strings.end(), EmptyFilter(), MyInserter( global ) );
...
static string wildcards2regex( const string &str )
{
  ...
  for ( pos = 0; ( pos = regexed.find( "*", pos ) ) != std::string::npos;
        pos += 2 )
...
string PoolQuery::Impl::createRegex(
  const StrContainer &container, const Match &flags ) const
{
...
string PoolQuery::Impl::asString() const
{
  ...
  {
    for ( Kinds::const_iterator it = _kinds.begin(); it != _kinds.end(); ++it )
      o << *it << " ";
  }
  o << endl;
  ...
  {
    for ( StrContainer::const_iterator it = _repos.begin(); it != _repos.end();
          ++it )
      o << *it << " ";
  }
  o << endl;
  ...
  o << "status: " << ( _status_flags ? ( _status_flags == INSTALLED_ONLY
                                           ? "INSTALLED_ONLY"
                                           : "UNINSTALLED_ONLY" )
                                     : "ALL" )
    << endl;
...
void PoolQuery::addAttribute(
  const sat::SolvAttr &attr, const std::string &value )
{
  _pimpl->_attrs[ attr ].insert( value );
}

void PoolQuery::addDependency( const sat::SolvAttr &attr,
  const std::string &name, const Rel &op, const Edition &edition )
{
  ...
  const Arch &arch )
{
  ...
  switch ( op.inSwitch() )
  {
    case Rel::ANY_e: // no additional constraint on edition.
      if ( arch.empty() &&
           !explicitKind ) // no additional constraint on arch/kind
      {
  ...
  else
  {
    ...
    attrMatchData.strMatcher =
      StrMatcher( strchr( name.c_str(), ':' ) + 1, Match::OTHER );
...
Match PoolQuery::flags() const { return _pimpl->_flags; }
void PoolQuery::setFlags( const Match &flags ) { _pimpl->_flags = flags; }

void PoolQuery::setInstalledOnly() { _pimpl->_status_flags = INSTALLED_ONLY; }
...
const PoolQuery::StrContainer &PoolQuery::strings() const
{
  return _pimpl->_strings;
}

const PoolQuery::AttrRawStrMap &PoolQuery::attributes() const
{
  return _pimpl->_attrs;
}

const PoolQuery::StrContainer &PoolQuery::attribute(
  const sat::SolvAttr &attr ) const
{
...
const Edition PoolQuery::edition() const { return _pimpl->_edition; }
const Rel PoolQuery::editionRel() const { return _pimpl->_op; }

const PoolQuery::Kinds &PoolQuery::kinds() const { return _pimpl->_kinds; }

const PoolQuery::StrContainer &PoolQuery::repos() const
{
...
{
  return _pimpl->_flags.isModeSubstring();
}
...
bool PoolQuery::matchWord() const { return _pimpl->_match_word; }

bool PoolQuery::requireAll() const { return _pimpl->_require_all; }
...
bool PoolQuery::recover( istream &str, char delim )
{
  ...
  do
  {
    ...
    if ( s.empty() ||
         pos == s.npos ) // some garbage on line... act like blank line
    {
    ...
    string attrName(
      str::trim( string( s, 0, pos ) ) ); // trimmed name of atribute
    string attrValue(
      str::trim( string( s, pos + 1, s.npos ) ) ); //trimmed value
    ...
    else if ( attribute == PoolQueryAttr::complexAttr )
    {
      try
      {
        _pimpl->_uncompiledPredicated.insert(
          AttrMatchData::deserialize( attrValue ) );
      }
...
void PoolQuery::serialize( ostream &str, char delim ) const
{
  ...
  static const zypp::PoolQuery
    q; //not save default options, so create default query example
  ...
  {
    str << PoolQueryAttr::kindAttr.asString() << ": " << it->idStr() << delim;
  }

  if ( editionRel() != Rel::ANY && edition() != Edition::noedition )
    str << PoolQueryAttr::editionAttr.asString() << ": " << editionRel() << " "
        << edition() << delim;
  ...
  if ( matchMode() != q.matchMode() )
  {
    switch ( matchMode() )
    {
      ...
      case Match::SUBSTRING:
        str << PoolQueryAttr::stringTypeAttr.asString() << ": substring"
            << delim;
        break;
      case Match::GLOB:
  ...
  for_( it, _pimpl->_uncompiledPredicated.begin(),
...
string PoolQuery::asString() const { return _pimpl->asString(); }
...
public:
  ...
  /** Provide all matching attributes within this solvable.
         ...
         */
  void matchDetail(
    const base_iterator &base_r, std::vector<base_iterator> &return_r ) const
  {
    if ( base_r == end() )
      return;
    ...
    if ( _attrMatchList.size() == 1 )
    {
      ...
      base
        .stayInThisSolvable(); // avoid discarding matches we found far away from here.
      return_r.push_back( base );

      const AttrMatchData::Predicate &predicate(
        _attrMatchList.front().predicate );
      for ( ++base; base.inSolvable() == inSolvable;
            ++base ) // safe even if base == end()
      {
...
private:
  ...
  bool isAMatch( base_iterator &base_r ) const
  {
    ...
    // Status restriction:
    if ( _status_flags && ( ( _status_flags == PoolQuery::INSTALLED_ONLY ) !=
                            inRepo.isSystemRepo() ) )
    {
      base_r.nextSkipRepo();
      return false;
    ...
    if ( _op != Rel::ANY &&
         !compareByRel(
           _op, inSolvable.edition(), _edition, Edition::Match() ) )
    {
    ...
    bool globalKindOk =
      ( _kinds.empty() || inSolvable.isKind( _kinds.begin(), _kinds.end() ) );
    ...
    if ( _attrMatchList.size() == 1 )
    {
      ...
      if ( matchData.kindPredicate )
      {
        ...
        {
          base_r
            .nextSkipSolvable(); // this matchData will never match in this solvable
          return false;
        }
      }
...
{
  return shared_ptr<detail::PoolQueryMatcher>(
    new detail::PoolQueryMatcher( _pimpl.getPtr() ) );
}
