...
{ /////////////////////////////////////////////////////////////////
  ...
  /** Backported code uses StrMatcher which replaced the sat::AttrMatcher in future.*/
  typedef sat::AttrMatcher StrMatcher;
...
attremptycheckend:
    ...
    if ( ! _uncompiledPredicated.empty() )
    {
	  ...
	  nattr.attrMatcher = joinedStrMatcher( joined, _flags );
          _attrMatchList.push_back( nattr );
        }
        else
        {
          // copy matcher
  ...
  namespace
  {
     ...
     */
    std::string rxEscape( const std::string & str_r, const Match & flags_r )
    {
      if ( str_r.empty() || flags_r.isModeRegex() )
	return str_r;

      if ( flags_r.isModeGlob() )
	return str::rxEscapeGlob( str_r );

      return str::rxEscapeStr( str_r );
    }
  } // namespace
  ///////////////////////////////////////////////////////////////////
  ...
  StrMatcher PoolQuery::Impl::joinedStrMatcher( const StrContainer & container_r, const Match & flags_r ) const
  {
    ...
    // (..|..|..)
    char sep = '(';
    for_( it, container_r.begin(), container_r.end() )
    {
      ret << sep << rxEscape( *it, flags_r );
      if ( sep == '(' )
	sep = '|';
    }
    ret << ')';
