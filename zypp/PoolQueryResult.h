...
/** Helper class to collect (not only) \ref PoolQuery results.
   ...
   * \endcode
   */
class PoolQueryResult : public sat::SolvIterMixin<PoolQueryResult,
                          std::unordered_set<sat::Solvable>::const_iterator>
{
public:
...
public:
  ...
  explicit PoolQueryResult( sat::Solvable result_r ) { operator+=( result_r ); }
  ...
  {
    for_( it, begin_r, end_r ) { operator+=( *it ); }
  }
...
public:
  ...
  /** */
  const_iterator begin() const { return _result.begin(); }
  /** */
  const_iterator end() const { return _result.end(); }
...
public:
  ...
  PoolQueryResult &operator+=( const PoolQuery &query_r )
  {
    try
    {
      for_( it, query_r.begin(), query_r.end() ) _result.insert( *it );
    }
  ...
  PoolQueryResult &operator-=( const PoolQueryResult &query_r )
  {
    if ( &query_r == this ) // catch self removal!
      ...
      for_( it, query_r.begin(), query_r.end() ) _result.erase( *it );
    return *this;
  }
  /** \overload */
  PoolQueryResult &operator-=( const PoolQuery &query_r )
