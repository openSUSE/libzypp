...
{
void fillList(
  std::list<Url> &ret_r, sat::Solvable solv_r, sat::SolvAttr attr_r )
{
...
void fillList(
  std::list<std::string> &ret_r, sat::Solvable solv_r, sat::SolvAttr attr_r )
{
  sat::LookupAttr query( attr_r, solv_r );
  for_( it, query.begin(), query.end() ) { ret_r.push_back( it.asString() ); }
}
}
...
sat::Solvable Product::referencePackage() const
{
  ...
  Capability identCap(
    str::form( "product(%s) = %s", name().c_str(), edition().c_str() ) );
...
std::string Product::flavor() const
{
  ...
  Capability identCap(
    str::form( "product_flavor(%s) = %s", name().c_str(), edition().c_str() ) );
...
{
  return ( lookupNumAttribute( sat::SolvAttr::productEndOfLife, -1 ) !=
...
std::vector<Repository::ContentIdentifier> Product::updateContentIdentifier()
  const
{
  std::vector<Repository::ContentIdentifier> ret;
  sat::LookupAttr q(
    sat::SolvAttr::productUpdatesRepoid, sat::SolvAttr::productUpdates, *this );
  if ( !q.empty() )
  {
    ret.reserve( 2 );
    for_( it, q.begin(), q.end() ) ret.push_back( it.asString() );
  }
  return ret;
}

bool Product::hasUpdateContentIdentifier(
...
{
  return isSystem() &&
         lookupStrAttribute( sat::SolvAttr::productType ) == "base";
}
...
Product::UrlList Product::releaseNotesUrls() const
{
  return urls( "releasenotes" );
}
