...
bool ProgressData::report()
{
  ...
  if ( hasRange() )
  {
    ...
    if ( newVal - _d->_last_val > 10 || now - _d->_last_send > 1 ||
...
std::ostream &operator<<( std::ostream &str, const ProgressData &obj )
{
  if ( obj.hasRange() )
  {
    return str << str::form( "{%u|%s}[%lld,%lld](%lld)%lld%%)", obj.numericId(),
             obj.name().c_str(), obj.min(), obj.max(), obj.val(),
             obj.reportValue() );
  }
  return str << str::form( "{%u|%s}[-,-](%lld)", obj.numericId(),
           obj.name().c_str(), obj.val() );
}
...
CombinedProgressData::CombinedProgressData(
  ProgressData &pd, ProgressData::value_type weight )
  : _weight( weight )
  , _last_value( 0 )
  , _pd( pd )
{
...
bool CombinedProgressData::operator()( const ProgressData &progress )
{
  ...
  float increment = ( (float)( progress.val() - _last_value ) ) /
