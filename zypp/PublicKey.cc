...
inline std::string expiresDetailVerbose( const Date &expires_r )
{
  ...
  if ( ttl <= 90 )
  {
    ...
    { // translators: an annotation to a gpg keys expiry date
      ret += str::form(
        PL_( "(expires in %d day)", "(expires in %d days)", ttl ), ttl );
    }
  }
  return ret;
}
...
PublicSubkeyData::operator bool() const { return !_pimpl->_id.empty(); }

std::string PublicSubkeyData::id() const { return _pimpl->_id; }

Date PublicSubkeyData::created() const { return _pimpl->_created; }

Date PublicSubkeyData::expires() const { return _pimpl->_expires; }

bool PublicSubkeyData::expired() const { return isExpired( _pimpl->_expires ); }
...
{
  return str::Str() << id() << " " << created().printDate() << " ["
...
shared_ptr<PublicKeyData::Impl> PublicKeyData::Impl::fromGpgmeKey(
  gpgme_key_t rawData )
{
...
PublicKeyData::operator bool() const { return !_pimpl->_fingerprint.empty(); }

std::string PublicKeyData::id() const { return _pimpl->_id; }

std::string PublicKeyData::name() const { return _pimpl->_name; }

std::string PublicKeyData::fingerprint() const { return _pimpl->_fingerprint; }

Date PublicKeyData::created() const { return _pimpl->_created; }

Date PublicKeyData::expires() const { return _pimpl->_expires; }

bool PublicKeyData::expired() const { return isExpired( _pimpl->_expires ); }
...
{
  return _pimpl->_id.empty() ? _pimpl->_id
                             : str::toLower( _pimpl->_id.substr( 8, 8 ) );
}

std::string PublicKeyData::gpgPubkeyRelease() const
{
  return _pimpl->_created ? str::hexstring( _pimpl->_created ).substr( 2 )
                          : std::string();
}
...
std::string PublicKeyData::asString() const
{
  ...
  for ( auto &&sub : _pimpl->_subkeys )
    str << ", " << sub.id();
  return str << "] [" << _pimpl->_name.c_str() << "] ["
             << expiresDetail( _pimpl->_expires ) << "]";
}

bool PublicKeyData::hasSubkeys() const { return !_pimpl->_subkeys.empty(); }

Iterable<PublicKeyData::SubkeyIterator> PublicKeyData::subkeys() const
{
  return makeIterable(
    &( *_pimpl->_subkeys.begin() ), &( *_pimpl->_subkeys.end() ) );
}
...
std::ostream &dumpOn( std::ostream &str, const PublicKeyData &obj )
{
  ...
  str << "   id " << obj.id() << endl;
  str << "  cre " << Date::ValueType( obj.created() ) << ' ' << obj.created()
      << endl;
  str << "  exp " << Date::ValueType( obj.expires() ) << ' '
      << obj.expiresAsString() << endl;
  str << "  ttl " << obj.daysToLive() << endl;
  for ( auto &&sub : obj._pimpl->_subkeys )
    str << "  sub " << sub << endl;
  str << "  rpm " << obj.gpgPubkeyVersion() << "-" << obj.gpgPubkeyRelease()
      << endl;
  return str;
}
...
{
  return (
    lhs.fingerprint() == rhs.fingerprint() && lhs.created() == rhs.created() );
}
...
struct PublicKey::Impl
{
    ...
    : _dontUseThisPtrDirectly( new filesystem::TmpFile )
  {
    ...
    if ( filesystem::hardlinkCopy( keyFile_r, path() ) != 0 )
      ZYPP_THROW(
        Exception( "Can't copy public key data from " + keyFile_r.asString() +
                   " to " + path().asString() ) );
  ...
  Impl(
    const filesystem::TmpFile &sharedFile_r, const PublicKeyData &keyData_r )
    : _dontUseThisPtrDirectly( new filesystem::TmpFile( sharedFile_r ) )
    , _keyData( keyData_r )
  {
    if ( !keyData_r )
...
public:
  const PublicKeyData &keyData() const { return _keyData; }

  Pathname path() const
  {
    return ( /*the one and only intended use*/ _dontUseThisPtrDirectly
               ? _dontUseThisPtrDirectly->path()
               : Pathname() );
  }

  const std::list<PublicKeyData> &hiddenKeys() const { return _hiddenKeys; }

protected:
  std::string
  _initHomeDir() ///< readFromFile helper to prepare the 'gpg --homedir'
  {
    Pathname ret( zypp::myTmpDir() / "PublicKey" );
    filesystem::assert_dir( ret );
    return ret.asString();
  }
  ...
  {
    PathInfo info( path() );
    MIL << "Reading pubkey from " << info.path() << " of size " << info.size()
        << " and sha1 " << filesystem::checksum( info.path(), "sha1" ) << endl;
    ...
    {
      ZYPP_THROW( Exception( std::string(
        "Can't read public key data: Setting the keyring path failed!" ) ) );
    }
    ...
    {
      case 0:
        ZYPP_THROW( BadKeyException(
          "File " + path().asString() + " doesn't contain public key data",
          path() ) );
        break;
...
private:
  shared_ptr<filesystem::TmpFile>
    _dontUseThisPtrDirectly; // shared_ptr ok because TmpFile itself is a refernce type (no COW)
...
PublicKey::PublicKey(
  const filesystem::TmpFile &sharedfile, const PublicKeyData &keyData_r )
...
const PublicKeyData &PublicKey::keyData() const { return _pimpl->keyData(); }

Pathname PublicKey::path() const { return _pimpl->path(); }
...
std::string PublicKey::id() const { return keyData().id(); }

std::string PublicKey::name() const { return keyData().name(); }

std::string PublicKey::fingerprint() const { return keyData().fingerprint(); }

Date PublicKey::created() const { return keyData().created(); }

Date PublicKey::expires() const { return keyData().expires(); }

bool PublicKey::expired() const { return keyData().expired(); }

int PublicKey::daysToLive() const { return keyData().daysToLive(); }
...
std::string PublicKey::asString() const { return keyData().asString(); }
