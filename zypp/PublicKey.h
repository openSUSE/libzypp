...
public:
  ...
  Pathname keyFile() const { return _keyfile; }

  /** Ctor taking message.
       * Use \ref ZYPP_THROW to throw exceptions.
       */
  BadKeyException(
    const std::string &msg_r, const Pathname &keyfile = Pathname() )
    : Exception( msg_r )
    , _keyfile( keyfile )
  {
  }
...
inline std::ostream &operator<<(
  std::ostream &str, const PublicSubkeyData &obj )
{
...
public:
  ...
  bool isValid() const { return !( id().empty() || fingerprint().empty() ); }
  ...
  bool providesKey(
    const std::string &id_r ) const ///!< \see \ref PublicKeyData
  {
