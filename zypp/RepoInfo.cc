...
public:
  ...
  {
    if ( type == repo::RepoType::NONE && t != repo::RepoType::NONE )
    {
      // lazy init!
...
public:
  ...
  Pathname licenseTgz( const std::string &name_r ) const
  {
    ...
    if ( !metadataPath().empty() )
    {
      ...
      g.add(
        metadataPath() / path / ( "repodata/*" + licenseStem + ".tar.gz" ) );
      if ( g.empty() )
  ...
  const RepoVariablesReplacedUrlList &baseUrls() const
  {
    ...
    if ( _baseUrls.empty() && !mlurl.asString().empty() )
    {
      ...
      repo::RepoMirrorList rmurls(
        mlurl, metadataPath(), _mirrorListForceMetalink );
  ...
  RepoVariablesReplacedUrlList &baseUrls() { return _baseUrls; }

  bool baseurl2dump() const { return !emptybaseurls && !_baseUrls.empty(); }

  const RepoVariablesReplacedUrlList &gpgKeyUrls() const { return _gpgKeyUrls; }

  RepoVariablesReplacedUrlList &gpgKeyUrls() { return _gpgKeyUrls; }
  ...
  bool hasContent() const
  {
    if ( !_keywords.first && !metadataPath().empty() )
    {
      ...
      Pathname master;
      if ( PathInfo( ( master = metadataPath() / "/repodata/repomd.xml" ) )
             .isFile() )
      {
        //MIL << "GO repomd.." << endl;
...
private:
  ...
  TriBool
    _rawRepoGpgCheck; ///< need to check repo sign.: Y/N/(ZConf(Y/N/gpgCheck))
  TriBool
    _rawPkgGpgCheck; ///< need to check pkg sign.: Y/N/(ZConf(Y/N/gpgCheck))
...
public:
  ...
  {
    return indeterminate( _rawGpgCheck ) ? ZConfig::instance().gpgCheck()
                                         : (bool)_rawGpgCheck;
  }
  TriBool cfgRepoGpgCheck() const
  {
    return indeterminate( _rawGpgCheck ) && indeterminate( _rawRepoGpgCheck )
             ? ZConfig::instance().repoGpgCheck()
             : _rawRepoGpgCheck;
  }
  TriBool cfgPkgGpgCheck() const
  {
    return indeterminate( _rawGpgCheck ) && indeterminate( _rawPkgGpgCheck )
             ? ZConfig::instance().pkgGpgCheck()
             : _rawPkgGpgCheck;
  }
...
public:
  ...
  void packagesPath( Pathname new_r ) { _packagesPath = std::move( new_r ); }
...
unsigned RepoInfo::priority() const { return _pimpl->priority; }

unsigned RepoInfo::defaultPriority() { return Impl::defaultPriority; }

unsigned RepoInfo::noPriority() { return Impl::noPriority; }
...
bool RepoInfo::gpgCheck() const { return _pimpl->cfgGpgCheck(); }
...
bool RepoInfo::repoGpgCheckIsMandatory() const
{
  ...
  if (
    ret &&
    _pimpl
      ->internalUnsignedConfirmed() ) // relax if unsigned repo was confirmed in the past
    ret = false;
  return ret;
}
...
{
  return _pimpl->cfgPkgGpgCheck() ||
         ( gpgCheck() && !bool( validRepoSignature() ) /*enforced*/ );
}

bool RepoInfo::pkgGpgCheckIsMandatory() const
{
  return _pimpl->cfgPkgGpgCheck() ||
         ( gpgCheck() && indeterminate( _pimpl->cfgPkgGpgCheck() ) &&
...
TriBool RepoInfo::validRepoSignature() const
{
  ...
  if ( ret && !repoGpgCheck() )
    ret = false; // invalidate any old signature if repoGpgCheck is off
  return ret;
}
...
inline bool changeGpgCheckTo(
  TriBool ogpg[ 3 ], TriBool g, TriBool r, TriBool p )
{
  bool changed = false;
  if ( changeGpgCheckTo( ogpg[ 0 ], g ) )
    changed = true;
  if ( changeGpgCheckTo( ogpg[ 1 ], r ) )
    changed = true;
  if ( changeGpgCheckTo( ogpg[ 2 ], p ) )
    changed = true;
  return changed;
}
} // namespace
...
bool RepoInfo::setGpgCheck( GpgCheck mode_r )
{
  ...
  switch ( mode_r )
  {
    ...
    case GpgCheck::Default:
      changed =
        changeGpgCheckTo( ogpg, indeterminate, indeterminate, indeterminate );
      break;
    case GpgCheck::Off:
...
void RepoInfo::setMirrorListUrl( const Url &url_r ) // Raw
{
  ...
  _pimpl->_mirrorListForceMetalink = false;
}

void RepoInfo::setMetalinkUrl( const Url &url_r ) // Raw
{
  _pimpl->_mirrorListUrl.raw() = url_r;
  _pimpl->_mirrorListForceMetalink = true;
}
...
void RepoInfo::setPath( const Pathname &path ) { _pimpl->path = path; }

void RepoInfo::setType( const repo::RepoType &t ) { _pimpl->type = t; }
...
void RepoInfo::setKeepPackages( bool keep ) { _pimpl->keeppackages = keep; }

void RepoInfo::setService( const std::string &name ) { _pimpl->service = name; }
...
Pathname RepoInfo::metadataPath() const { return _pimpl->metadataPath(); }

Pathname RepoInfo::packagesPath() const { return _pimpl->packagesPath(); }
...
repo::RepoType RepoInfo::type() const { return _pimpl->type; }
...
bool RepoInfo::gpgKeyUrlsEmpty() const { return _pimpl->gpgKeyUrls().empty(); }
...
{
  return (
    _pimpl->gpgKeyUrls().empty() ? Url() : *_pimpl->gpgKeyUrls().rawBegin() );
}
...
Pathname RepoInfo::path() const { return _pimpl->path; }

std::string RepoInfo::service() const { return _pimpl->service; }
...
{
  return (
    _pimpl->baseUrls().empty() ? Url() : *_pimpl->baseUrls().rawBegin() );
}
...
bool RepoInfo::baseUrlsEmpty() const { return _pimpl->baseUrls().empty(); }

bool RepoInfo::baseUrlSet() const { return _pimpl->baseurl2dump(); }
...
bool RepoInfo::hasContent() const { return _pimpl->hasContent(); }
...
bool RepoInfo::hasLicense() const { return hasLicense( std::string() ); }
...
bool RepoInfo::needToAcceptLicense( const std::string &name_r ) const
{
  ...
  MIL << "License(" << name_r << ") in " << name()
      << " has to be accepted: " << ( accept ? "true" : "false" ) << endl;
  return accept;
}

std::string RepoInfo::getLicense( const Locale &lang_r )
{
  return const_cast<const RepoInfo *>( this )->getLicense(
    std::string(), lang_r );
}
...
std::string RepoInfo::getLicense(
  const std::string &name_r, const Locale &lang_r ) const
{
  ...
  std::string licenseFile( !getLang
                             ? licenseFileFallback
  ...
  cmd.push_back( "-O" );
  cmd.push_back( "-f" );
  cmd.push_back( _pimpl->licenseTgz( name_r )
                   .asString() ); // if it not exists, avlocales was empty.
...
std::ostream &RepoInfo::dumpOn( std::ostream &str ) const
{
  ...
  strif( ( _pimpl->_mirrorListForceMetalink ? "- metalink    : "
                                            : "- mirrorlist  : " ),
...
#define OUTS( T, B )                                                           \
  ( indeterminate( T ) ? ( std::string( "D(" ) + ( B ? "Y" : "N" ) + ")" )     \
                       : ( (bool)T ? "Y" : "N" ) )
  str << "- gpgcheck    : " << OUTS( _pimpl->rawGpgCheck(), gpgCheck() )
      << " repo" << OUTS( _pimpl->rawRepoGpgCheck(), repoGpgCheck() )
      << ( repoGpgCheckIsMandatory() ? "* " : " " ) << "sig"
      << asString( validRepoSignature(), "?", "Y", "N" ) << " pkg"
      << OUTS( _pimpl->rawPkgGpgCheck(), pkgGpgCheck() )
      << ( pkgGpgCheckIsMandatory() ? "* " : " " ) << std::endl;
#undef OUTS
...
std::ostream &RepoInfo::dumpAsIniOn( std::ostream &str ) const
{
  ...
  if ( !( rawMirrorListUrl().asString().empty() ) )
    str << ( _pimpl->_mirrorListForceMetalink ? "metalink=" : "mirrorlist=" )
        << rawMirrorListUrl() << endl;
  ...
  if ( !indeterminate( _pimpl->rawRepoGpgCheck() ) )
    str << "repo_gpgcheck=" << ( _pimpl->rawRepoGpgCheck() ? "1" : "0" )
        << endl;
...
std::ostream &RepoInfo::dumpAsXmlOn(
  std::ostream &str, const std::string &content ) const
{
  ...
  if ( !( tmpstr = mirrorListUrl().asString() ).empty() )
    str << ( _pimpl->_mirrorListForceMetalink ? " metalink=\""
                                              : " mirrorlist=\"" )
        << escape( tmpstr ) << "\"";
  str << ">" << endl;
  ...
  if ( _pimpl->baseurl2dump() )
  {
    for_( it, baseUrlsBegin(),
      baseUrlsEnd() ) // !transform iterator replaces variables
      str
      << "<url>" << escape( ( *it ).asString() ) << "</url>" << endl;
  }
