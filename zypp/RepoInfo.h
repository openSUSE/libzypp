...
public:
  ...
  typedef transform_iterator<repo::RepoVariablesUrlReplacer,
  ...
  /**
       ...
       */
  Url url() const { return ( baseUrlsEmpty() ? Url() : *baseUrlsBegin() ); }
  /**
  ...
  {
    for_( it, begin_r, end_r ) if ( !hasContent( *it ) ) return false;
    return true;
  }
  /** \overload  */
  template <class TContainer>
  ...
  {
    for_( it, begin_r, end_r ) if ( hasContent( *it ) ) return true;
    return false;
  }
  /** \overload  */
  template <class TContainer>
...
public:
  ...
  std::string getLicense(
    const std::string &name_r, const Locale &lang_r = Locale() ) const;
