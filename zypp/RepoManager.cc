...
#define OPT_PROGRESS                                                           \
  const ProgressData::ReceiverFnc & = ProgressData::ReceiverFnc()
...
public:
  ...
  bool collect( const Url &url_r )
  {
    ...
    {
      if ( !_cmPtr )
        _cmPtr.reset( new media::CredentialManager( _root ) );
...
inline bool foundAliasIn(
  const std::string &alias_r, Iterator begin_r, Iterator end_r )
{
  for_( it, begin_r, end_r ) if ( it->alias() == alias_r ) return true;
  return false;
}
/** \overload */
...
template <class Iterator>
inline Iterator findAlias(
  const std::string &alias_r, Iterator begin_r, Iterator end_r )
{
  for_( it, begin_r, end_r ) if ( it->alias() == alias_r ) return it;
  return end_r;
}
/** \overload */
template <class Container>
inline typename Container::iterator findAlias(
...
/** \overload */
template <class Container>
inline typename Container::const_iterator findAlias(
  const std::string &alias_r, const Container &cont_r )
{
  return findAlias( alias_r, cont_r.begin(), cont_r.end() );
}

/** \short Generate a related filename from a repo/service infos alias */
inline std::string filenameFromAlias(
  const std::string &alias_r, const std::string &stem_r )
{
  ...
  MIL << "generating filename for " << stem_r << " [" << alias_r << "] : '"
      << filename << "'" << endl;
  return filename;
}
...
struct RepoCollector : private base::NonCopyable
{
  ...
  bool collect( const RepoInfo &repo )
  {
    ...
    if ( !targetDistro.empty() && !repo.targetDistribution().empty() &&
         repo.targetDistribution() != targetDistro )
    {
      MIL << "Skipping repository meant for '" << repo.targetDistribution()
          << "' distribution (current distro is '" << targetDistro << "')."
          << endl;
...
std::list<RepoInfo> repositories_in_file( const Pathname &file )
{
  ...
  parser::RepoFileReader parser(
    file, bind( &RepoCollector::collect, &collector, _1 ) );
...
std::list<RepoInfo> repositories_in_dir( const Pathname &dir )
{
  ...
  if ( nonroot && !PathInfo( dir ).userMayRX() )
  {
    JobReport::warning(
      str::FormatNAC(
        _( "Cannot read repo directory '%1%': Permission denied" ) ) %
      dir );
  }
  else
  {
    ...
    if ( filesystem::readdir( entries, dir, false ) != 0 )
    {
      ...
      ZYPP_THROW( Exception(
        str::form( _( "Failed to read directory '%s'" ), dir.c_str() ) ) );
    }

    str::regex allowedRepoExt( "^\\.repo(_[0-9]+)?$" );
    for ( std::list<Pathname>::const_iterator it = entries.begin();
          it != entries.end(); ++it )
    {
      if ( str::regex_match( it->extension(), allowedRepoExt ) )
      {
        if ( nonroot && !PathInfo( *it ).userMayR() )
        {
          JobReport::warning(
            str::FormatNAC(
              _( "Cannot read repo file '%1%': Permission denied" ) ) %
            *it );
        }
        else
        {
...
/**
     ...
     */
inline Pathname rawcache_path_for_repoinfo(
  const RepoManagerOptions &opt, const RepoInfo &info )
{
  assert_alias( info );
  return isTmpRepo( info ) ? info.metadataPath()
                           : opt.repoRawCachePath / info.escaped_alias();
}
...
/**
     ...
     */
inline Pathname rawproductdata_path_for_repoinfo(
  const RepoManagerOptions &opt, const RepoInfo &info )
{
...
/**
     ...
     */
inline Pathname packagescache_path_for_repoinfo(
  const RepoManagerOptions &opt, const RepoInfo &info )
{
  assert_alias( info );
  return isTmpRepo( info ) ? info.packagesPath()
                           : opt.repoPackagesCachePath / info.escaped_alias();
}

/**
     * \short Calculates the solv cache path for a repository
     */
inline Pathname solv_path_for_repoinfo(
  const RepoManagerOptions &opt, const RepoInfo &info )
{
  assert_alias( info );
  return isTmpRepo( info ) ? info.metadataPath().dirname() / "%SLV%"
                           : opt.repoSolvCachePath / info.escaped_alias();
}
...
{
  repoCachePath =
    Pathname::assertprefix( root_r, ZConfig::instance().repoCachePath() );
  repoRawCachePath =
    Pathname::assertprefix( root_r, ZConfig::instance().repoMetadataPath() );
  repoSolvCachePath =
    Pathname::assertprefix( root_r, ZConfig::instance().repoSolvfilesPath() );
  repoPackagesCachePath =
    Pathname::assertprefix( root_r, ZConfig::instance().repoPackagesPath() );
  knownReposPath =
    Pathname::assertprefix( root_r, ZConfig::instance().knownReposPath() );
  knownServicesPath =
    Pathname::assertprefix( root_r, ZConfig::instance().knownServicesPath() );
  pluginsPath =
    Pathname::assertprefix( root_r, ZConfig::instance().pluginsPath() );
...
public:
  ...
  ~Impl()
  {
    ...
    if ( ( _reposDirty || env::ZYPP_PLUGIN_APPDATA_FORCE_COLLECT() ) &&
         geteuid() == 0 &&
         ( _options.rootDir.empty() || _options.rootDir == "/" ) )
    {
      try
      {
        std::list<Pathname> entries;
...
public:
  ...
  RefreshCheckStatus checkIfToRefreshMetadata(
    const RepoInfo &info, const Url &url, RawMetadataRefreshPolicy policy );

  void refreshMetadata(
    const RepoInfo &info, RawMetadataRefreshPolicy policy, OPT_PROGRESS );
  ...
  void buildCache(
    const RepoInfo &info, CacheBuildPolicy policy, OPT_PROGRESS );

  repo::RepoType probe(
    const Url &url, const Pathname &path = Pathname() ) const;
  ...
  {
    return PathInfo( solv_path_for_repoinfo( _options, info ) / "solv" )
      .isExist();
  }
  ...
  RepoStatus cacheStatus( const RepoInfo &info ) const
  {
    return RepoStatus::fromCookieFile(
      solv_path_for_repoinfo( _options, info ) / "cookie" );
  }
  ...
  void modifyRepository(
    const std::string &alias, const RepoInfo &newinfo_r, OPT_PROGRESS );

  RepoInfo getRepositoryInfo( const std::string &alias, OPT_PROGRESS );
  RepoInfo getRepositoryInfo(
    const Url &url, const url::ViewOption &urlview, OPT_PROGRESS );
...
public:
  ...
  void refreshService(
    const std::string &alias, const RefreshServiceOptions &options_r );
  void refreshService(
    const ServiceInfo &service, const RefreshServiceOptions &options_r )
  {
    refreshService( service.alias(), options_r );
  }

  void modifyService(
    const std::string &oldAlias, const ServiceInfo &newService );
...
private:
  ...
  Pathname generateNonExistingName(
    const Pathname &dir, const std::string &basefilename ) const;
    ...
    const std::string &alias, OutputIterator out ) const
  {
    ...
    std::copy(
      boost::make_filter_iterator( filter, repos().begin(), repos().end() ),
...
inline std::ostream &operator<<(
  std::ostream &str, const RepoManager::Impl &obj )
{
...
void RepoManager::Impl::saveService( ServiceInfo &service ) const
{
  ...
  Pathname servfile = generateNonExistingName(
    _options.knownServicesPath, generateFilename( service ) );
  ...
  if ( !file )
  {
    ...
    ZYPP_THROW( Exception( str::form(
      _( "Can't open file '%s' for writing." ), servfile.c_str() ) ) );
  }
...
/**
   ...
   */
Pathname RepoManager::Impl::generateNonExistingName(
  const Pathname &dir, const std::string &basefilename ) const
{
...
void RepoManager::Impl::init_knownServices()
{
  ...
  repo::PluginServices(
    _options.pluginsPath / "services", ServiceCollector( _services ) );
}
  ...
  const std::list<std::string> &repoEscAliases_r )
{
  ...
  if ( filesystem::readdir( entries, cachePath_r, false ) == 0 )
  {
    ...
    set_difference( entries.begin(), entries.end(), repoEscAliases_r.begin(),
      repoEscAliases_r.end(), std::inserter( oldfiles, oldfiles.end() ) );
    for ( const std::string &old : oldfiles )
    {
      if ( old ==
           Repository::systemRepoAlias() ) // don't remove the @System solv file
        continue;
...
void RepoManager::Impl::init_knownRepositories()
{
  ...
  if ( PathInfo( _options.knownReposPath ).isExist() )
  {
    ...
    for ( RepoInfo &repoInfo : repositories_in_dir( _options.knownReposPath ) )
    {
      ...
      repoInfo.setMetadataPath(
        rawcache_path_for_repoinfo( _options, repoInfo ) );
      // set the downloaded packages path for the repo
      repoInfo.setPackagesPath(
        packagescache_path_for_repoinfo( _options, repoInfo ) );
      // remember it
      _reposX.insert(
        repoInfo ); // direct access via _reposX in ctor! no reposManip.
    ...
    if ( !orphanedRepos.empty() )
    {
      for ( const auto &repoInfo : orphanedRepos )
      {
        ...
        //   %2% = repository name
        JobReport::warning(
          str::FormatNAC( _( "Unknown service '%1%': Removing orphaned service "
                             "repository '%2%'" ) ) %
          repoInfo.service() % repoInfo.alias() );
        try
        {
...
RepoStatus RepoManager::Impl::metadataStatus( const RepoInfo &info ) const
{
  ...
  switch ( repokind.toEnum() )
  {
    ...
    case RepoType::YAST2_e:
      status = RepoStatus( productdatapath / "content" ) &&
               RepoStatus( mediarootpath / "media.1/media" );
      break;
  ...
  const RepoInfo &info, const Url &url, RawMetadataRefreshPolicy policy )
{
  ...
  try
  {
    ...
    if ( policy != RefreshIfNeededIgnoreDelay )
    {
      ...
      double diff = difftime( (Date::ValueType)Date::now(),
                      (Date::ValueType)oldstatus.timestamp() ) /
                    60;
    ...
    switch ( repokind.toEnum() )
    {
      ...
      case RepoType::RPMPLAINDIR_e:
        newstatus = RepoStatus(
          MediaMounter( url ).getPathName( info.path() ) ); // dir status
        break;
...
void RepoManager::Impl::refreshMetadata( const RepoInfo &info,
  RawMetadataRefreshPolicy policy, const ProgressData::ReceiverFnc &progress )
{
  assert_alias( info );
  assert_urls( info );

  // we will throw this later if no URL checks out fine
  RepoException rexception(
    info,
    PL_( "Valid metadata not found at specified URL",
      "Valid metadata not found at specified URLs", info.baseUrlsSize() ) );
        ...
        it != info.baseUrlsEnd(); ++it )
  {
    try
    {
      ...
      {
        Exception ex(
          str::form( _( "Can't create %s" ), mediarootpath.c_str() ) );
        ZYPP_THROW( ex );
      }

      // create temp dir as sibling of mediarootpath
      filesystem::TmpDir tmpdir(
        filesystem::TmpDir::makeSibling( mediarootpath ) );
      ...
      else if ( repokind.toEnum() == RepoType::RPMPLAINDIR_e )
      {
        ...
        RepoStatus newstatus =
          RepoStatus( media.getPathName( info.path() ) ); // dir status
...
void RepoManager::Impl::cleanMetadata(
  const RepoInfo &info, const ProgressData::ReceiverFnc &progressfnc )
{
...
void RepoManager::Impl::cleanPackages(
  const RepoInfo &info, const ProgressData::ReceiverFnc &progressfnc )
{
  ProgressData progress( 100 );
  progress.sendTo( progressfnc );
  ...
  filesystem::recursive_rmdir(
    packagescache_path_for_repoinfo( _options, info ) );
  progress.toMax();
}

void RepoManager::Impl::buildCache( const RepoInfo &info,
  CacheBuildPolicy policy, const ProgressData::ReceiverFnc &progressrcv )
{
  assert_alias( info );
  Pathname mediarootpath = rawcache_path_for_repoinfo( _options, info );
  Pathname productdatapath = rawproductdata_path_for_repoinfo( _options, info );
  ...
  {
    Exception ex(
      str::form( _( "Can't create %s" ), _options.repoCachePath.c_str() ) );
    ZYPP_THROW( ex );
  }
  ...
  progress.name(
    str::form( _( "Building repository '%s' cache" ), info.label().c_str() ) );
  progress.toMin();
  ...
  {
    Exception ex(
      str::form( _( "Can't create cache at %s - no writing permissions." ),
        base.c_str() ) );
    ZYPP_THROW( ex );
  }
  ...
  switch ( repokind.toEnum() )
  {
    ...
    case RepoType::RPMPLAINDIR_e:
    {
      ...
      ExternalProgram::Arguments cmd;
      cmd.push_back( PathInfo( "/usr/bin/repo2solv" ).isFile()
                       ? "repo2solv"
                       : "repo2solv.sh" );
      ...
      sat::updateSolvFileIndex(
        solvfile ); // content digest for zypper bash completion
    }
    break;
    default:
      ZYPP_THROW(
        RepoUnknownTypeException( info, _( "Unhandled repository type" ) ) );
      break;
  }
...
/** Probe the metadata type of a repository located at \c url.
   ...
   */
repo::RepoType RepoManager::Impl::probe(
  const Url &url, const Pathname &path ) const
{
  MIL << "going to probe the repo type at " << url << " (" << path << ")"
      << endl;

  if ( url.getScheme() == "dir" &&
       !PathInfo( url.getPathName() / path ).isDir() )
  {
  ...
  RepoException enew( str::form(
    _( "Error trying to read from '%s'" ), url.asString().c_str() ) );
  ...
  catch ( const Exception &e )
  {
    ZYPP_CAUGHT( e );
    // TranslatorExplanation '%s' is an URL
    Exception enew( str::form(
      _( "Unknown error reading from '%s'" ), url.asString().c_str() ) );
    enew.remember( e );
    ZYPP_THROW( enew );
  }
...
void RepoManager::Impl::cleanCacheDirGarbage(
  ...
  for_( dir, cachedirs.begin(), cachedirs.end() )
  {
    if ( PathInfo( *dir ).isExist() )
    {
      ...
      for_( subdir, entries.begin(), entries.end() )
      {
        ...
        bool found = false;
        for_( r, repoBegin(),
          repoEnd() ) if ( subdir->basename() == r->escaped_alias() )
        {
          found = true;
          break;
        }
        ...
        if ( !found &&
             ( Date::now() - PathInfo( *subdir ).mtime() > Date::day ) )
...
void RepoManager::Impl::cleanCache(
...
void RepoManager::Impl::loadFromCache(
  const RepoInfo &info, const ProgressData::ReceiverFnc &progressrcv )
{
  assert_alias( info );
  ...
  try
  {
    ...
    const std::string &toolversion(
      sat::LookupRepoAttr( sat::SolvAttr::repositoryToolVersion, repo )
        .begin()
        .asString() );
...
void RepoManager::Impl::addRepository(
  const RepoInfo &info, const ProgressData::ReceiverFnc &progressrcv )
{
  assert_alias( info );
  ...
  progress.sendTo( ProgressReportAdaptor( progressrcv, report ) );
  progress.name(
    str::form( _( "Adding repository '%s'" ), info.label().c_str() ) );
  progress.toMin();
  ...
  if ( !file )
  {
    // TranslatorExplanation '%s' is a filename
    ZYPP_THROW( Exception( str::form(
      _( "Can't open file '%s' for writing." ), repofile.c_str() ) ) );
  }
  ...
  tosave.setPackagesPath( packagescache_path_for_repoinfo( _options, tosave ) );
  {
    ...
    oinfo.setPackagesPath(
      packagescache_path_for_repoinfo( _options, tosave ) );
  }
...
void RepoManager::Impl::addRepositories(
  const Url &url, const ProgressData::ReceiverFnc &progressrcv )
{
  ...
  {
    // TranslatorExplanation '%s' is an URL
    ZYPP_THROW( RepoException( str::form(
      _( "Invalid repo file name at '%s'" ), url.asString().c_str() ) ) );
  }
  ...
  Pathname repofile =
    generateNonExistingName( _options.knownReposPath, filename );
  // now we have a filename that does not exists
  MIL << "Saving " << repos.size() << " repo" << ( repos.size() ? "s" : "" )
      << " in " << repofile << endl;
  ...
  for ( std::list<RepoInfo>::iterator it = repos.begin(); it != repos.end();
        ++it )
  {
...
void RepoManager::Impl::removeRepository(
  const RepoInfo &info, const ProgressData::ReceiverFnc &progressrcv )
{
  ...
  progress.sendTo( ProgressReportAdaptor( progressrcv, report ) );
  progress.name(
    str::form( _( "Removing repository '%s'" ), info.label().c_str() ) );
  ...
  for_( it, repoBegin(), repoEnd() )
  {
    ...
    if ( todelete.filepath().empty() )
    {
      ZYPP_THROW( RepoException(
        todelete, _( "Can't figure out where the repo is stored." ) ) );
    }
    else
    {
      // figure how many repos are there in the file:
      std::list<RepoInfo> filerepos =
        repositories_in_file( todelete.filepath() );
      if ( filerepos.size() ==
             0 // bsc#984494: file may have already been deleted
           || ( filerepos.size() == 1 &&
                filerepos.front().alias() == todelete.alias() ) )
      {
        ...
        if ( !( ret == 0 || ret == ENOENT ) )
        {
          // TranslatorExplanation '%s' is a filename
          ZYPP_THROW(
            RepoException( todelete, str::form( _( "Can't delete '%s'" ),
      ...
      else
      {
        ...
        if ( !file )
        {
          // TranslatorExplanation '%s' is a filename
          ZYPP_THROW(
            Exception( str::form( _( "Can't open file '%s' for writing." ),
              todelete.filepath().c_str() ) ) );
        }
        for ( std::list<RepoInfo>::const_iterator fit = filerepos.begin();
              fit != filerepos.end(); ++fit )
        {
...
void RepoManager::Impl::modifyRepository( const std::string &alias,
  const RepoInfo &newinfo_r, const ProgressData::ReceiverFnc &progressrcv )
{
  RepoInfo toedit = getRepositoryInfo( alias );
  RepoInfo newinfo(
    newinfo_r ); // need writable copy to upadte housekeeping data
  ...
  {
    ZYPP_THROW( RepoException(
      toedit, _( "Can't figure out where the repo is stored." ) ) );
  }
  else
  {
    // figure how many repos are there in the file:
    ...
    if ( toedit.enabled() && !newinfo.enabled() )
    {
      ...
      const Pathname &solvidx =
        solv_path_for_repoinfo( _options, newinfo ) / "solv.idx";
    ...
    newinfo.setPackagesPath(
      packagescache_path_for_repoinfo( _options, newinfo ) );
    {
      // We should fix the API as we must inject those paths
      // into the repoinfo in order to keep it usable.
...
RepoInfo RepoManager::Impl::getRepositoryInfo(
...
RepoInfo RepoManager::Impl::getRepositoryInfo( const Url &url,
  const url::ViewOption &urlview, const ProgressData::ReceiverFnc &progressrcv )
{
  for_( it, repoBegin(), repoEnd() )
  {
...
void RepoManager::Impl::removeService( const std::string &alias )
{
  ...
  if ( location.empty() )
  {
    ZYPP_THROW( ServiceException(
      service, _( "Can't figure out where the service is stored." ) ) );
  }
  ...
  if ( tmpSet.size() == 1 )
  {
    if ( filesystem::unlink( location ) != 0 )
    {
      // TranslatorExplanation '%s' is a filename
      ZYPP_THROW( ServiceException(
        service, str::form( _( "Can't delete '%s'" ), location.c_str() ) ) );
    }
  ...
  else
  {
    ...
    if ( !file )
    {
      // TranslatorExplanation '%s' is a filename
      ZYPP_THROW( Exception( str::form(
        _( "Can't open file '%s' for writing." ), location.c_str() ) ) );
    }
...
void RepoManager::Impl::refreshService(
  const std::string &alias, const RefreshServiceOptions &options_r )
{
  ...
  MIL << "Going to refresh service '" << service.alias()
      << "', url: " << service.url() << ", opts: " << options_r << endl;
          ...
          options_r.testFlag( RefreshService_restoreStatus ) ) )
  {
    ...
    if ( lrf )
    {
      ...
      if ( lrf <= now )
      {
        ...
        {
          MIL << "Skip: '" << service.alias() << "' metadata valid until "
              << lrf << endl;
          return;
        }
      }
      else
        WAR << "Force: '" << service.alias()
  ...
  // parse it
  Date::Duration origTtl =
    service.ttl(); // FIXME Ugly hack: const service.ttl modified when parsing
  ...
  // and in zypper.
  std::pair<DefaultIntegral<bool, false>, repo::ServicePluginInformalException>
    uglyHack;
  try
  {
  ...
  for_( it, collector.repos.begin(), collector.repos.end() )
  {
    ...
    it->setAlias(
      str::form( "%s:%s", service.alias().c_str(), it->alias().c_str() ) );
  ...
  for_( oldRepo, oldRepos.begin(), oldRepos.end() )
  {
    if ( !foundAliasIn( oldRepo->alias(), collector.repos ) )
    {
      if ( oldRepo->enabled() )
      {
        ...
        {
          DBG << "Service removes user enabled repo " << oldRepo->alias()
              << endl;
  ...
  UrlCredentialExtractor urlCredentialExtractor(
    _options.rootDir ); // To collect any credentials stored in repo URLs
  for_( it, collector.repos.begin(), collector.repos.end() )
  {
    ...
    TriBool toBeEnabled(
      indeterminate ); // indeterminate - follow the service request
    DBG << "Service request to " << ( it->enabled() ? "enable" : "disable" )
    ...
    else
    {
      ...
      if ( indeterminate( toBeEnabled ) )
      {
        ...
        if ( oldRepo->enabled() == it->enabled() )
          toBeEnabled =
            it->enabled(); // service requests no change to the system
        else if ( options_r.testFlag( RefreshService_restoreStatus ) )
        {
          toBeEnabled = it->enabled(); // RefreshService_restoreStatus forced
          DBG << "Opt RefreshService_restoreStatus " << it->alias()
              << " forces " << ( toBeEnabled ? "enabled" : "disabled" ) << endl;
        }
        else
        {
          const auto &last = service.repoStates().find( oldRepo->alias() );
          if ( last == service.repoStates().end() ||
               last->second.enabled != it->enabled() )
            toBeEnabled =
              it->enabled(); // service request has changed since last refresh -> follow
          else
          {
            toBeEnabled =
              oldRepo
                ->enabled(); // service request unchaned since last refresh -> keep user modification
            DBG << "User modified service repo " << it->alias() << " may stay "
                << ( toBeEnabled ? "enabled" : "disabled" ) << endl;
          }
        }
      }
      ...
      {
        DBG << "Service repo " << it->alias() << " stays "
            << ( oldRepo->enabled() ? "enabled" : "disabled" ) << endl;
      }
      else if ( toBeEnabled )
      {
      ...
      {
        DBG << "Service repo " << it->alias() << " gets new AUTOREFRESH "
            << it->autorefresh() << endl;
      ...
      {
        DBG << "Service repo " << it->alias() << " gets new PRIORITY "
            << it->priority() << endl;
      ...
      // changed url?
      {
        ...
        urlCredentialExtractor.extract(
          newUrls ); // Extract! to prevent passwds from disturbing the comparison below
        if ( oldRepo->rawBaseUrls() != newUrls )
        {
          DBG << "Service repo " << it->alias() << " gets new URLs " << newUrls
              << endl;
...
#define Z_CHKGPG( I, N )                                                       \
  ...
  {                                                                            \
    DBG << "Service repo " << it->alias() << " gets new " #N "Check "          \
        << ngpg[ I ] << endl;                                                  \
...
void RepoManager::Impl::modifyService(
  const std::string &oldAlias, const ServiceInfo &newService )
{
  ...
  {
    ZYPP_THROW( ServiceException(
      oldService, _( "Can't figure out where the service is stored." ) ) );
  }
...
bool RepoManager::repoEmpty() const { return _pimpl->repoEmpty(); }
...
std::string RepoManager::makeStupidAlias( const Url &url_r )
{
  ...
  ret += Digest::digest( Digest::sha1(),
           str::hexstring( ++serial ) + url_r.asCompleteString() )
           .substr( 0, 8 );
  return ret;
}
...
void RepoManager::cleanMetadata(
  const RepoInfo &info, const ProgressData::ReceiverFnc &progressrcv )
{
  return _pimpl->cleanMetadata( info, progressrcv );
}

void RepoManager::cleanPackages(
  const RepoInfo &info, const ProgressData::ReceiverFnc &progressrcv )
{
...
void RepoManager::buildCache( const RepoInfo &info, CacheBuildPolicy policy,
  const ProgressData::ReceiverFnc &progressrcv )
{
  return _pimpl->buildCache( info, policy, progressrcv );
}

void RepoManager::cleanCache(
  const RepoInfo &info, const ProgressData::ReceiverFnc &progressrcv )
{
...
void RepoManager::loadFromCache(
  const RepoInfo &info, const ProgressData::ReceiverFnc &progressrcv )
{
  return _pimpl->loadFromCache( info, progressrcv );
}

void RepoManager::cleanCacheDirGarbage(
  const ProgressData::ReceiverFnc &progressrcv )
{
...
void RepoManager::addRepository(
  const RepoInfo &info, const ProgressData::ReceiverFnc &progressrcv )
{
  return _pimpl->addRepository( info, progressrcv );
}

void RepoManager::addRepositories(
  const Url &url, const ProgressData::ReceiverFnc &progressrcv )
{
  return _pimpl->addRepositories( url, progressrcv );
}

void RepoManager::removeRepository(
  const RepoInfo &info, const ProgressData::ReceiverFnc &progressrcv )
{
...
void RepoManager::modifyRepository( const std::string &alias,
  const RepoInfo &newinfo, const ProgressData::ReceiverFnc &progressrcv )
{
  return _pimpl->modifyRepository( alias, newinfo, progressrcv );
}

RepoInfo RepoManager::getRepositoryInfo(
  const std::string &alias, const ProgressData::ReceiverFnc &progressrcv )
{
...
bool RepoManager::serviceEmpty() const { return _pimpl->serviceEmpty(); }
...
void RepoManager::refreshService(
  const std::string &alias, const RefreshServiceOptions &options_r )
{
...
void RepoManager::modifyService(
  const std::string &oldAlias, const ServiceInfo &service )
{
