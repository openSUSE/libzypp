...
inline std::ostream &operator<<(
  std::ostream &str, const RepoStatus::Impl &obj )
{
  ...
  : _pimpl( new Impl() )
{
  ...
  if ( info.isExist() )
  {
    ...
    else if ( info.isDir() )
    {
      ...
      _pimpl->_checksum =
        CheckSum::sha1FromString( str::numstring( t ) ).checksum();
    }
...
RepoStatus RepoStatus::fromCookieFile( const Pathname &path_r )
{
  ...
  else
  {
    ...
    ret._pimpl->_timestamp =
      Date( str::strtonum<time_t>( str::stripLastWord( line ) ) );
...
bool RepoStatus::empty() const { return _pimpl->_checksum.empty(); }

Date RepoStatus::timestamp() const { return _pimpl->_timestamp; }
...
RepoStatus operator&&( const RepoStatus &lhs, const RepoStatus &rhs )
{
  ...
  else
  {
    ...
    result._pimpl->_timestamp =
      std::max( lhs._pimpl->_timestamp, rhs._pimpl->_timestamp );
  }
  return result;
}
