...
sat::detail::CRepo *Repository::get() const { return myPool().getRepo( _id ); }
...
std::string Repository::name() const { return info().name(); }

std::string Repository::label() const { return info().label(); }
...
bool Repository::hasContentIdentifier( const ContentIdentifier &id_r ) const
{
  NO_REPOSITORY_RETURN( false );
  sat::LookupRepoAttr q( sat::SolvAttr::repositoryRepoid, *this );
  for_( it, q.begin(), q.end() ) if ( it.asString() == id_r ) return true;
  return false;
}
...
Repository::Keywords Repository::keywords() const
{
  ...
  return Keywords(
    sat::SolvAttr::repositoryKeywords, *this, sat::LookupAttr::REPO_ATTR );
}
...
bool Repository::providesUpdatesFor( const CpeId &cpeid_r ) const
{
  ...
  sat::LookupRepoAttr myIds(
    sat::SolvAttr::repositoryRepoid, *this ); // usually just one, but...
  if ( !myIds.empty() )
  {
...
bool Repository::isUpdateRepo() const
{
  ...
  if ( !myIds.empty() )
  {
    ...
    for_( it, pool.byKindBegin<Product>(), pool.byKindEnd<Product>() )
    {
      for_( myId, myIds.begin(), myIds.end() )
      {
        if ( ( *it )->asKind<Product>()->hasUpdateContentIdentifier(
               myId.asString() ) )
          return true;
      }
    }
...
Repository::SolvableIterator Repository::solvablesBegin() const
{
  NO_REPOSITORY_RETURN( make_filter_iterator( detail::ByRepository( *this ),
    sat::detail::SolvableIterator(), sat::detail::SolvableIterator() ) );
...
void Repository::setInfo( const RepoInfo &info_r )
{
  ...
  if ( info_r.alias() != alias() )
  {
    ZYPP_THROW( Exception(
      str::form( "RepoInfo alias (%s) does not match repository alias (%s)",
...
Repository Repository::nextInPool() const
{
  ...
  for_(
    it, sat::Pool::instance().reposBegin(), sat::Pool::instance().reposEnd() )
  {
    if ( *it == *this )
    {
      if ( ++it != _for_end )
...
{
  return xmlout::node(
    str, "repository", {{"name", obj.name()}, {"alias", obj.alias()}} );
}
...
Repository::ProductInfoIterator::ProductInfoIterator(
  sat::SolvAttr attr_r, Repository repo_r )
{
...
std::string Repository::ProductInfoIterator::label() const
{
  return base_reference()
    .subFind( sat::SolvAttr::repositoryProductLabel )
    .asString();
}

CpeId Repository::ProductInfoIterator::cpeId() const
{
  return CpeId( base_reference()
                  .subFind( sat::SolvAttr::repositoryProductCpeid )
                  .asString(),
    CpeId::noThrow );
}
