...
public:
  ...
  explicit ResKind( sat::detail::IdType id_r )
    : _str( str::toLower( IdString( id_r ).c_str() ) )
  {
  }
  explicit ResKind( const IdString &idstr_r )
    : _str( str::toLower( idstr_r.c_str() ) )
  {
  }
  explicit ResKind( const std::string &str_r )
    : _str( str::toLower( str_r ) )
  {
  }
  explicit ResKind( const char *cstr_r )
    : _str( str::toLower( cstr_r ) )
  {
  }
...
public:
  /** Return libsolv identifier for name.
      ...
      */
  static std::string satIdent(
    const ResKind &refers_r, const std::string &name_r );
  /** \overload */
