...
public:
  ...
  filter_iterator<TFilter, const_iterator> filterBegin(
    const TFilter &filter_r ) const
  {
    return make_filter_begin( filter_r, *this );
  }

  template <class TFilter>
  filter_iterator<TFilter, const_iterator> filterEnd(
    const TFilter &filter_r ) const
  {
  ...
  template <class TFilter>
  Iterable<filter_iterator<TFilter, const_iterator>> filter(
    const TFilter &filter_r ) const
  {
  ...
  /** \name Iterate over all PoolItems by status.
       ...
       */
  //@{
  filter_iterator<filter::ByStatus, const_iterator> byStatusBegin(
    const filter::ByStatus &filter_r ) const
  {
    return make_filter_begin( filter_r, *this );
  }

  filter_iterator<filter::ByStatus, const_iterator> byStatusEnd(
    const filter::ByStatus &filter_r ) const
  {
    return make_filter_end( filter_r, *this );
  }

  Iterable<filter_iterator<filter::ByStatus, const_iterator>> byStatus(
    const filter::ByStatus &filter_r ) const
  {
...
public:
  ...
  byIdent_iterator byIdentBegin( const ByIdent &ident_r ) const
  {
    return make_transform_iterator(
      id2item().equal_range( ident_r.get() ).first,
  ...
  {
    return make_transform_iterator(
      id2item().equal_range( ident_r.get() ).second,
      pool::PoolTraits::Id2ItemValueSelector() );
  }
  ...
  {
    return makeIterable(
      byIdentBegin( kind_r, name_r ), byIdentEnd( kind_r, name_r ) );
  }

  Iterable<byIdent_iterator> byIdent(
  ...
  {
    return makeIterable(
      byIdentBegin<TRes>( name_r ), byIdentEnd<TRes>( name_r ) );
  }
