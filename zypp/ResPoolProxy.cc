...
struct PoolItemSaver
{
  ...
  {
    std::for_each(
      pool_r.begin(), pool_r.end(), std::mem_fun_ref( &PoolItem::saveState ) );
  }
  ...
  bool diffState( ResPool pool_r, const ResKind &kind_r ) const
  {
    // return whether some PoolItem::sameState reported \c false.
    return (
      invokeOnEach( pool_r.byKindBegin( kind_r ), pool_r.byKindEnd( kind_r ),
        std::mem_fun_ref( &PoolItem::sameState ) ) < 0 );
  }
};
...
namespace
{
ui::Selectable::Ptr makeSelectablePtr(
  pool::PoolImpl::Id2ItemT::const_iterator begin_r,
  pool::PoolImpl::Id2ItemT::const_iterator end_r )
{
  pool::PoolTraits::byIdent_iterator begin(
    begin_r, pool::PoolTraits::Id2ItemValueSelector() );
  pool::PoolTraits::byIdent_iterator end(
    end_r, pool::PoolTraits::Id2ItemValueSelector() );
  sat::Solvable solv( begin->satSolvable() );

  return new ui::Selectable( ui::Selectable::Impl_Ptr(
...
struct ResPoolProxy::Impl
{
  ...
  typedef std::unordered_map<sat::detail::IdType, ui::Selectable::Ptr>
    SelectableIndex;
...
public:
  bool empty() const { return _selPool.empty(); }

  size_type size() const { return _selPool.size(); }

  const_iterator begin() const { return make_map_value_begin( _selPool ); }

  const_iterator end() const { return make_map_value_end( _selPool ); }
...
public:
  void saveState() const { PoolItemSaver().saveState( _pool ); }
  ...
  void restoreState() const { PoolItemSaver().restoreState( _pool ); }
  ...
  bool diffState() const { return PoolItemSaver().diffState( _pool ); }
...
inline std::ostream &operator<<(
  std::ostream &str, const ResPoolProxy::Impl &obj )
{
  return str << "ResPoolProxy (" << obj._pool.serial() << ") ["
             << obj._pool.size() << "solv/" << obj.size() << "sel]";
}
...
{
  detail::DumpFilter f;
  return dumpRange( str << obj << " toModify: ", make_filter_begin( f, obj ),
...
bool ResPoolProxy::empty() const { return _pimpl->empty(); }

ResPoolProxy::size_type ResPoolProxy::size() const { return _pimpl->size(); }
...
ResPoolProxy::const_iterator ResPoolProxy::end() const { return _pimpl->end(); }
...
ResPoolProxy::const_iterator ResPoolProxy::byKindBegin(
  const ResKind &kind_r ) const
{
  return _pimpl->byKindBegin( kind_r );
}

ResPoolProxy::const_iterator ResPoolProxy::byKindEnd(
  const ResKind &kind_r ) const
{
...
void ResPoolProxy::saveState() const { _pimpl->saveState(); }
...
void ResPoolProxy::restoreState() const { _pimpl->restoreState(); }
...
bool ResPoolProxy::diffState() const { return _pimpl->diffState(); }
