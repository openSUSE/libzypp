...
public:
  ...
  typedef MapKVIteratorTraits<SelectablePool>::Value_const_iterator
    const_iterator;
...
public:
  ...
  {
    return resolvable_r ? lookup( resolvable_r->satSolvable() )
                        : ui::Selectable::Ptr();
  }
...
private:
  ...
  filter_iterator<TFilter, const_iterator> make_begin(
    TFilter filter_r, const ResKind &kind_r ) const
  {
    return make_filter_iterator(
      filter_r, byKindBegin( kind_r ), byKindEnd( kind_r ) );
  }
  template <class TFilter>
  filter_iterator<TFilter, const_iterator> make_begin(
  ...
  template <class TFilter>
  filter_iterator<TFilter, const_iterator> make_end(
    TFilter filter_r, const ResKind &kind_r ) const
  {
    return make_filter_iterator(
      filter_r, byKindEnd( kind_r ), byKindEnd( kind_r ) );
  }
  template <class TFilter>
  filter_iterator<TFilter, const_iterator> make_end(
    const ResKind &kind_r ) const
...
private:
  struct Impl
  {
    ...
    Impl( const ResPoolProxy &pool_r, const ResKind &kind_r )
      : _pool( pool_r )
      , _kind( new ResKind( kind_r ) )
    {
    }
    void saveState()
    {
      if ( _kind )
        _pool.saveState( *_kind );
      else
        _pool.saveState();
    }
    void restoreState()
...
inline ResPoolProxy::ScopedSaveState ResPoolProxy::scopedSaveState(
  const ResKind &kind_r ) const
{
