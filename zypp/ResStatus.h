...
public:
  ...
  typedef bit::Range<FieldType, TransactDetailField::end, 1>
    LicenceConfirmedField;
  typedef bit::Range<FieldType, LicenceConfirmedField::end, 4> WeakField;
  typedef bit::Range<FieldType, WeakField::end, 1>
    UserLockQueryField; // internal
...
public:
  ...
  enum TransactValue
  {
    ...
    LOCKED =
      bit::RangeValue<TransactField, 1>::value, // locked, must not transact
    TRANSACT =
      bit::RangeValue<TransactField, 2>::value // transact according to state
  };
...
public:
  ...
  /** Debug helper returning the bitfield.
     ...
     * transition rules.
    */
  BitFieldType bitfield() const { return _bitfield; }
...
public:
  ...
  {
    fieldValueAssign<LicenceConfirmedField>(
      toVal_r ? LICENCE_CONFIRMED : LICENCE_UNCONFIRMED );
  }

public:
  bool isRecommended() const { return _bitfield.test( RECOMMENDED ); }

  bool isSuggested() const { return _bitfield.test( SUGGESTED ); }

  bool isOrphaned() const { return _bitfield.test( ORPHANED ); }

  bool isUnneeded() const { return _bitfield.test( UNNEEDED ); }

  bool hasWeak() const { return !fieldValueIs<WeakField>( NO_WEAK ); }

  void resetWeak() { return fieldValueAssign<WeakField>( NO_WEAK ); }
...
public:
  ...
  bool isSatisfied() const { return fieldValueIs<ValidateField>( SATISFIED ); }

  bool isBroken() const { return fieldValueIs<ValidateField>( BROKEN ); }
...
public:
  ...
  bool isInstalled() const { return fieldValueIs<StateField>( INSTALLED ); }

  bool isUninstalled() const { return fieldValueIs<StateField>( UNINSTALLED ); }

public:
  bool staysInstalled() const { return isInstalled() && !transacts(); }

  bool wasInstalled() const { return staysInstalled(); } //for old status

  bool isToBeInstalled() const { return isUninstalled() && transacts(); }

  bool staysUninstalled() const { return isUninstalled() && !transacts(); }

  bool wasUninstalled() const { return staysUninstalled(); } // for old status

  bool isToBeUninstalled() const { return isInstalled() && transacts(); }

  bool isLocked() const { return fieldValueIs<TransactField>( LOCKED ); }

  bool isUserLocked() const { return isLocked() && isByUser(); }
  ...
  bool isKept() const { return fieldValueIs<TransactField>( KEEP_STATE ); }

  bool transacts() const { return fieldValueIs<TransactField>( TRANSACT ); }
  ...
  bool onSystem() const { return ( isInstalled() != transacts() ); }

  /** True if would be off system after commit. */
  bool offSystem() const { return !onSystem(); }

  bool isBySolver() const { return fieldValueIs<TransactByField>( SOLVER ); }

  bool isByApplLow() const { return fieldValueIs<TransactByField>( APPL_LOW ); }
  ...
  bool isByUser() const { return fieldValueIs<TransactByField>( USER ); }
  ...
  {
    return isToBeUninstalled() &&
           fieldValueIs<TransactDetailField>( DUE_TO_OBSOLETE );
  }

  bool isToBeUninstalledDueToUpgrade() const
  {
    return isToBeUninstalled() &&
           fieldValueIs<TransactDetailField>( DUE_TO_UPGRADE );
  }

  bool isToBeInstalledSoft() const
  {
    return isToBeInstalled() &&
           fieldValueIs<TransactDetailField>( SOFT_INSTALL );
  }
  ...
  {
    return isToBeUninstalled() &&
           fieldValueIs<TransactDetailField>( SOFT_REMOVE );
  }
...
private:
  ...
  {
    fieldValueAssign<UserLockQueryField>(
      match_r ? USERLOCK_MATCH : USERLOCK_NOMATCH );
  }
  //@}
...
public:
  ...
  bool setTransact( bool toTansact_r, TransactByValue causer_r )
  {
    if ( toTansact_r == transacts() )
    {
      ...
      fieldValueAssign<TransactDetailField>(
        NO_DETAIL ); // Details has to be set again
      return true;
    }
  ...
  /** Soft toggle between TRANSACT and KEEP_STATE.
     ...
     * \endcode
    */
  bool setSoftTransact(
    bool toTansact_r, TransactByValue causer_r, TransactByValue causerLimit_r )
  {
    if ( fieldValueIs<TransactField>( KEEP_STATE ) &&
  ...
  bool maySetSoftTransact(
    bool val_r, TransactByValue causer, TransactByValue causerLimit_r )
  {
    bit::BitField<FieldType> savBitfield = _bitfield;
  ...
  {
    if ( isInstalled() || !setSoftTransact( true, SOLVER ) )
      return false;
  ...
  bool setStatus( ResStatus newStatus_r )
  {
    ...
    if ( _bitfield.value<StateField>() !=
         newStatus_r._bitfield.value<StateField>() )
      return false;
    // Transaction state change allowed?
    if ( !setTransactValue(
           newStatus_r.getTransactValue(), newStatus_r.getTransactByValue() ) )
      return false;
...
private:
  ...
  ResStatus( StateValue s, ValidateValue v = UNDETERMINED,
    TransactValue t = KEEP_STATE, InstallDetailValue i = EXPLICIT_INSTALL,
