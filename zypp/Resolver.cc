...
//
///////////////////////////////////////////////////////////////////
bool Resolver::verifySystem() { return _pimpl->verifySystem(); }

bool Resolver::resolvePool() { return _pimpl->resolvePool(); }
...
void Resolver::undo() { _pimpl->undo(); }

ResolverProblemList Resolver::problems() { return _pimpl->problems(); }
...
sat::Transaction Resolver::getTransaction() { return _pimpl->getTransaction(); }

bool Resolver::doUpgrade() { return _pimpl->doUpgrade(); }

void Resolver::doUpdate() { _pimpl->doUpdate(); }

void Resolver::setForceResolve( bool yesno_r )
{
  _pimpl->setForceResolve( yesno_r );
}
...
bool Resolver::ignoreAlreadyRecommended() const
{
...
void Resolver::setSolveSrcPackages( bool yesno_r )
{
  _pimpl->setSolveSrcPackages( yesno_r );
}
void Resolver::setDefaultSolveSrcPackages()
{
  _pimpl->setSolveSrcPackages( indeterminate );
}
bool Resolver::solveSrcPackages() const { return _pimpl->solveSrcPackages(); }

void Resolver::setCleandepsOnRemove( bool yesno_r )
{
  _pimpl->setCleandepsOnRemove( yesno_r );
}
...
ZOLV_FLAG_TRIBOOL(
  setAllowNameChange, setDefaultAllowNameChange, allowNameChange )
ZOLV_FLAG_TRIBOOL(
  setAllowArchChange, setDefaultAllowArchChange, allowArchChange )
ZOLV_FLAG_TRIBOOL(
  setAllowVendorChange, setDefaultAllowVendorChange, allowVendorChange )

ZOLV_FLAG_TRIBOOL(
  dupSetAllowDowngrade, dupSetDefaultAllowDowngrade, dupAllowDowngrade )
ZOLV_FLAG_TRIBOOL(
  dupSetAllowNameChange, dupSetDefaultAllowNameChange, dupAllowNameChange )
ZOLV_FLAG_TRIBOOL(
  dupSetAllowArchChange, dupSetDefaultAllowArchChange, dupAllowArchChange )
...
bool Resolver::upgradingRepos() const
{
  return !_pimpl->upgradeRepos().empty();
}
bool Resolver::upgradingRepo( Repository repo_r ) const
{
...
bool Resolver::createSolverTestcase(
  const std::string &dumpPath, bool runSolver )
{
...
solver::detail::ItemCapKindList Resolver::satifiedByInstalled(
  const PoolItem &item )
{
  return _pimpl->satifiedByInstalled( item );
}

solver::detail::ItemCapKindList Resolver::installedSatisfied(
  const PoolItem &item )
{
