...
public:
  ...
  {
    if ( _type == repo::ServiceType::NONE && type_r != repo::ServiceType::NONE )
    {
      // lazy init!
...
void ServiceInfo::setProbedTtl( Date::Duration ttl_r ) const
{
  const_cast<ServiceInfo *>( this )->setTtl( ttl_r );
}
...
{
  return _pimpl->_reposToEnable.empty();
}
ServiceInfo::ReposToEnable::size_type ServiceInfo::reposToEnableSize() const
{
  return _pimpl->_reposToEnable.size();
}
ServiceInfo::ReposToEnable::const_iterator ServiceInfo::reposToEnableBegin()
  const
{
  return _pimpl->_reposToEnable.begin();
}
ServiceInfo::ReposToEnable::const_iterator ServiceInfo::reposToEnableEnd() const
{
  return _pimpl->_reposToEnable.end();
}

bool ServiceInfo::repoToEnableFind( const std::string &alias_r ) const
{
  return (
    _pimpl->_reposToEnable.find( alias_r ) != _pimpl->_reposToEnable.end() );
}
...
void ServiceInfo::clearReposToEnable() { _pimpl->_reposToEnable.clear(); }
...
{
  return _pimpl->_reposToDisable.size();
}
ServiceInfo::ReposToDisable::const_iterator ServiceInfo::reposToDisableBegin()
  const
{
  return _pimpl->_reposToDisable.begin();
}
ServiceInfo::ReposToDisable::const_iterator ServiceInfo::reposToDisableEnd()
  const
{
  return _pimpl->_reposToDisable.end();
}

bool ServiceInfo::repoToDisableFind( const std::string &alias_r ) const
{
  return (
    _pimpl->_reposToDisable.find( alias_r ) != _pimpl->_reposToDisable.end() );
}
...
{
  RepoInfoBase::dumpAsIniOn( str ) << "url = " << rawUrl() << endl
                                   << "type = " << type() << endl;
  ...
  if ( !repoStates().empty() )
  {
    ...
    for ( const auto &el : repoStates() )
    {
      ...
      if ( state.priority != RepoInfo::defaultPriority() )
        str << tag << "_priority=" << state.priority << endl;
    }
  }

  if ( !reposToEnableEmpty() )
    str << "repostoenable = "
        << str::joinEscaped( reposToEnableBegin(), reposToEnableEnd() ) << endl;
  if ( !reposToDisableEmpty() )
    str << "repostodisable = "
        << str::joinEscaped( reposToDisableBegin(), reposToDisableEnd() )
        << endl;
  return str;
}
