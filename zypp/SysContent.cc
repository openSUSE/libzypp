...
/** writeXml helper.
       ...
       * an empty string.
      */
inline std::string attrIf(
  const std::string &tag_r, const std::string &value_r )
{
  std::string ret;
  if ( !value_r.empty() )
  {
...
std::ostream &Writer::Impl::writeXml( std::ostream &str ) const
{
  ...
  str << "  <ident>\n";
  str << "    <name>" << _name << "</name>\n";
  str << "    <version" << attrIf( "epoch", str::numstring( _edition.epoch() ) )
  ...
  {
    str << "    <entry" << attrIf( "kind", ( *it )->kind().asString() )
        << attrIf( "name", ( *it )->name() )
...
const std::string &Writer::name() const { return _pimpl->_name; }
...
const Edition &Writer::edition() const { return _pimpl->_edition; }
...
const std::string &Writer::description() const { return _pimpl->_description; }
...
bool Writer::empty() const { return _pimpl->_onsys.empty(); }

Writer::size_type Writer::size() const { return _pimpl->_onsys.size(); }

Writer::const_iterator Writer::begin() const { return _pimpl->_onsys.begin(); }

Writer::const_iterator Writer::end() const { return _pimpl->_onsys.end(); }
...
const std::string &Reader::Entry::kind() const { return _pimpl->_kind; }

const std::string &Reader::Entry::name() const { return _pimpl->_name; }

const Edition &Reader::Entry::edition() const { return _pimpl->_edition; }

const Arch &Reader::Entry::arch() const { return _pimpl->_arch; }
...
struct SycontentNode : public ParseDef
{
  ...
  {
    ( *this )( "ident", OPTIONAL )( "onsys", OPTIONAL );

    ( *this )[ "ident" ]( "name", OPTIONAL )( "version", OPTIONAL )(
      "description", OPTIONAL )( "created", OPTIONAL );

    ( *this )[ "onsys" ]( "entry", MULTIPLE_OPTIONAL );
  }
};
...
Reader::Impl::Impl( std::istream &input_r )
{
  ...
  rootNode[ "ident" ][ "name" ].setConsumer( new ConsumeString( _name ) );
  ...
  rootNode[ "ident" ][ "created" ].setConsumer( new ConsumeDate( _created ) );

  rootNode[ "onsys" ][ "entry" ].setConsumer( new ConsumeEntries( _content ) );
...
const std::string &Reader::name() const { return _pimpl->_name; }

const Edition &Reader::edition() const { return _pimpl->_edition; }

const std::string &Reader::description() const { return _pimpl->_description; }

const Date &Reader::ctime() const { return _pimpl->_created; }

bool Reader::empty() const { return _pimpl->_content.empty(); }

Reader::size_type Reader::size() const { return _pimpl->_content.size(); }
...
Reader::const_iterator Reader::end() const { return _pimpl->_content.end(); }
