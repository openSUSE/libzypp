...
std::ostream &operator<<(
  std::ostream &str, const Target::DistributionLabel &obj )
{
...
void Target::buildCache() { _pimpl->buildCache(); }

void Target::cleanCache() { _pimpl->clearCache(); }

void Target::load() { _pimpl->load(); }

void Target::reload() { _pimpl->reload(); }

void Target::unload() { _pimpl->unload(); }

target::rpm::RpmDb &Target::rpmDb() { return _pimpl->rpm(); }

Pathname Target::root() const { return _pimpl->root(); }

bool Target::providesFile(
  const std::string &name_str, const std::string &path_str ) const
{
...
Date Target::timestamp() const { return _pimpl->timestamp(); }

Product::constPtr Target::baseProduct() const { return _pimpl->baseProduct(); }
...
void Target::updateAutoInstalled() { return _pimpl->updateAutoInstalled(); }
