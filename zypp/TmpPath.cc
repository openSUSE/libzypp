...
public:
  ...
  bool autoCleanup() const { return ( _flags & Autodelete ); }

  void autoCleanup( bool yesno_r ) { _flags = yesno_r ? CtorDefault : NoOp; }
...
//      METHOD TYPE :
//
TmpPath::operator bool() const { return _impl.get(); }
...
{
  static Pathname p(
    getenv( "ZYPPTMPDIR" ) ? getenv( "ZYPPTMPDIR" ) : "/var/tmp" );
  return p;
}
...
//	METHOD TYPE : Constructor
//
TmpFile::TmpFile( const Pathname &inParentDir_r, const std::string &prefix_r )
{
  // parent dir must exist
  if ( filesystem::assert_dir( inParentDir_r ) != 0 )
  {
    ERR << "Parent directory '" << inParentDir_r << "' can't be created."
        << endl;
    return;
  }
...
//	METHOD TYPE : Constructor
//
TmpDir::TmpDir( const Pathname &inParentDir_r, const std::string &prefix_r )
{
  // parent dir must exist
  if ( filesystem::assert_dir( inParentDir_r ) != 0 )
  {
