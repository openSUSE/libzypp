...
public:
  /** Get type. */
  const ContentType &type() const { return _type; }

  /** Set type. */
  void type( ContentType type_r ) { _type = std::move( type_r ); }
...
public:
  ...
  bool empty() const { return !_dataP || _dataP->empty(); }

  /** Size of \ref data. */
  size_type size() const { return _dataP ? _dataP->size() : 0; }

  /** The \ref data. */
  const DataType &data() const { return dataRef(); }
  ...
  bool reset( const std::string &key_r ) { return set( key_r, AnyType() ); }
  /** \overload const version */
