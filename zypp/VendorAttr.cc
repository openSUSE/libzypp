...
void VendorAttr::_addVendorList( VendorList &vendorList_r ) const
{
  ...
  for_( it, vendorList_r.begin(), vendorList_r.end() )
  {
    *it = str::toLower( *it );
    ...
    {
      if ( nextId != vendorGroupCounter + 1 && nextId != _vendorMap[ *it ] )
      {
      ...
      else
      {
        ...
        WAR
          << "Vendor " << *it
          << " is already used in another vendor group. --> mixing these groups"
          << endl;
      }
    }
  }
...
bool VendorAttr::equivalent(
  const Vendor &lVendor, const Vendor &rVendor ) const
{
  return equivalent( IdString( lVendor ), IdString( rVendor ) );
}

bool VendorAttr::equivalent(
  sat::Solvable lVendor, sat::Solvable rVendor ) const
{
  return equivalent( lVendor.vendor(), rVendor.vendor() );
}

bool VendorAttr::equivalent(
  const PoolItem &lVendor, const PoolItem &rVendor ) const
{
  return equivalent(
    lVendor.satSolvable().vendor(), rVendor.satSolvable().vendor() );
}
