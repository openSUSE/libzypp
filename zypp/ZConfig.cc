...
Arch _autodetectSystemArchitecture()
{
      ...
      for ( iostr::EachLine in ( cpuinfo ); in; in.next() )
      {
        if ( str::hasPrefix( *in, "flags" ) )
        {
               ...
               in->find( "cmov" ) == std::string::npos )
          {
            ...
            WAR << "CPU lacks 'cx8' or 'cmov': architecture downgraded to '"
                << architecture << "'" << endl;
          }
          break;
        }
    ...
    if ( cpuinfo )
    {
      for ( iostr::EachLine in ( cpuinfo ); in; in.next() )
      {
        if ( str::hasPrefix( *in, "type" ) )
        {
          ...
          {
            architecture =
              ( architecture == Arch_sparc64 ? Arch_sparc64v : Arch_sparcv9v );
            WAR << "CPU has 'sun4v': architecture upgraded to '" << architecture
                << "'" << endl;
          }
          else if ( in->find( "sun4u" ) != std::string::npos )
          {
            architecture =
              ( architecture == Arch_sparc64 ? Arch_sparc64 : Arch_sparcv9 );
            WAR << "CPU has 'sun4u': architecture upgraded to '" << architecture
                << "'" << endl;
          }
          else if ( in->find( "sun4m" ) != std::string::npos )
          {
            architecture = Arch_sparcv8;
            WAR << "CPU has 'sun4m': architecture upgraded to '" << architecture
                << "'" << endl;
          }
          break;
        }
    ...
    if ( platform )
    {
      for ( iostr::EachLine in ( platform ); in; in.next() )
      {
        if ( str::hasPrefix( *in, "armv7hl-" ) )
        {
          ...
          WAR
            << "/etc/rpm/platform contains armv7hl-: architecture upgraded to '"
            << architecture << "'" << endl;
          break;
        }
        if ( str::hasPrefix( *in, "armv6hl-" ) )
        {
          ...
          WAR
            << "/etc/rpm/platform contains armv6hl-: architecture upgraded to '"
            << architecture << "'" << endl;
          break;
        }
      }
  ...
  else if ( architecture == Arch_ppc64 )
  {
    ...
    int powerlvl;
    if ( platform && sscanf( platform, "power%d", &powerlvl ) == 1 &&
         powerlvl > 6 )
...
struct Option
{
  ...
  operator const value_type &() const { return _val; }

  /** Set a new value.  */
  void set( const value_type &newval_r ) { _val = newval_r; }

  /** Non-const reference to set a new value. */
  value_type &ref() { return _val; }
...
struct DefaultOption : public Option<Tp>
{
  ...
  void restoreToDefault() { this->set( _default.get() ); }
  ...
  const value_type &getDefault() const { return _default.get(); }

  /** Set a new default value. */
  void setDefault( const value_type &newval_r ) { _default.set( newval_r ); }
...
public:
    ...
    , pluginsPath( "/usr/lib/zypp/plugins" )
  {
              ...
              it != dict.entriesEnd( *sit ); ++it )
        {
          ...
          if ( section == "main" )
          {
            if ( entry == "arch" )
            {
              ...
              if ( carch != cfg_arch )
              {
                WAR << "Overriding system architecture (" << cfg_arch
                    << "): " << carch << endl;
                cfg_arch = carch;
              }
            }
            ...
            else if ( entry == "repo.refresh.locales" )
            {
              ...
              repoRefreshLocales.insert(
                make_transform_iterator( tmp.begin(), transform ),
            ...
            {
              download_media_prefer_download.restoreToDefault(
                str::compareCI( value, "volatile" ) != 0 );
            }
            ...
            else if ( entry == "download.transfer_timeout" )
            {
              ...
              if ( download_transfer_timeout < 0 )
                download_transfer_timeout = 0;
              else if ( download_transfer_timeout > 3600 )
                download_transfer_timeout = 3600;
            }
            ...
            else if ( entry == "multiversion" )
            {
              ...
              str::splitEscaped(
                value, std::inserter( defSpec, defSpec.end() ), ", \t" );
            }
    ...
    if ( getenv( "ZYPP_TESTSUITE_FAKE_ARCH" ) )
    {
      ...
      if ( carch != cfg_arch )
      {
        WAR << "ZYPP_TESTSUITE_FAKE_ARCH: Overriding system architecture ("
            << cfg_arch << "): " << carch << endl;
        cfg_arch = carch;
      }
    }
...
private:
  ...
  struct MultiversionMap
  {
    ...
    MultiversionSpec &getSpec(
      Pathname root_r, const Impl &zConfImpl_r ) // from system at root
    {
      ...
      MultiversionSpec &ret(
        _specMap[ root_r ] ); // creates new entry on the fly
    ...
    MultiversionSpec &
    getDefaultSpec() // Spec from zypp.conf parsing; called before any getSpec
    {
...
Pathname ZConfig::systemRoot() const { return _autodetectSystemRoot(); }
...
Arch ZConfig::systemArchitecture() const { return _pimpl->cfg_arch; }

void ZConfig::setSystemArchitecture( const Arch &arch_r )
{
  if ( arch_r != _pimpl->cfg_arch )
  {
    WAR << "Overriding system architecture (" << _pimpl->cfg_arch
        << "): " << arch_r << endl;
...
Locale ZConfig::textLocale() const { return _pimpl->cfg_textLocale; }

void ZConfig::setTextLocale( const Locale &locale_r )
{
  if ( locale_r != _pimpl->cfg_textLocale )
  {
    WAR << "Overriding text locale (" << _pimpl->cfg_textLocale
        << "): " << locale_r << endl;
...
bool ZConfig::hasUserData() const { return !_pimpl->userData.empty(); }

std::string ZConfig::userData() const { return _pimpl->userData; }
...
bool ZConfig::setUserData( const std::string &str_r )
{
  for_( ch, str_r.begin(), str_r.end() )
  {
    if ( *ch < ' ' && *ch != '\t' )
    {
      ERR << "New user data string rejectded: char " << (int)*ch
...
{
  return ( _pimpl->cfg_cache_path.empty() ? Pathname( "/var/cache/zypp" )
                                          : _pimpl->cfg_cache_path );
}

Pathname ZConfig::repoMetadataPath() const
{
  return ( _pimpl->cfg_metadata_path.empty() ? ( repoCachePath() / "raw" )
                                             : _pimpl->cfg_metadata_path );
}

Pathname ZConfig::repoSolvfilesPath() const
{
  return ( _pimpl->cfg_solvfiles_path.empty() ? ( repoCachePath() / "solv" )
                                              : _pimpl->cfg_solvfiles_path );
}

Pathname ZConfig::repoPackagesPath() const
{
  return ( _pimpl->cfg_packages_path.empty() ? ( repoCachePath() / "packages" )
                                             : _pimpl->cfg_packages_path );
}
...
{
  return ( _pimpl->cfg_config_path.empty() ? Pathname( "/etc/zypp" )
...
Pathname ZConfig::knownReposPath() const
{
  return ( _pimpl->cfg_known_repos_path.empty()
             ? ( configPath() / "repos.d" )
             : _pimpl->cfg_known_repos_path );
}

Pathname ZConfig::knownServicesPath() const
{
  return ( _pimpl->cfg_known_services_path.empty()
             ? ( configPath() / "services.d" )
             : _pimpl->cfg_known_services_path );
}

Pathname ZConfig::varsPath() const
{
  return ( _pimpl->cfg_vars_path.empty() ? ( configPath() / "vars.d" )
                                         : _pimpl->cfg_vars_path );
}

Pathname ZConfig::vendorPath() const
{
  return ( _pimpl->cfg_vendor_path.empty() ? ( configPath() / "vendors.d" )
                                           : _pimpl->cfg_vendor_path );
}

Pathname ZConfig::locksFile() const
{
  return ( _pimpl->locks_file.empty() ? ( configPath() / "locks" )
                                      : _pimpl->locks_file );
}

///////////////////////////////////////////////////////////////////

bool ZConfig::repo_add_probe() const { return _pimpl->repo_add_probe; }
...
{
  return _pimpl->repoRefreshLocales.empty() ? Target::requestedLocales( "" )
                                            : _pimpl->repoRefreshLocales;
}

bool ZConfig::repoLabelIsAlias() const { return _pimpl->repoLabelIsAlias; }
...
Pathname ZConfig::download_mediaMountdir() const
{
  return _pimpl->download_mediaMountdir;
}
void ZConfig::set_download_mediaMountdir( Pathname newval_r )
{
  _pimpl->download_mediaMountdir.set( std::move( newval_r ) );
}
void ZConfig::set_default_download_mediaMountdir()
{
  _pimpl->download_mediaMountdir.restoreToDefault();
}
...
void ZConfig::setRepoGpgCheck( TriBool val_r )
{
  _pimpl->repoGpgCheck.set( val_r );
}
void ZConfig::setPkgGpgCheck( TriBool val_r )
{
  _pimpl->pkgGpgCheck.set( val_r );
}
...
bool ZConfig::solver_dupAllowDowngrade() const
{
  return _pimpl->solver_dupAllowDowngrade;
}
bool ZConfig::solver_dupAllowNameChange() const
{
  return _pimpl->solver_dupAllowNameChange;
}
bool ZConfig::solver_dupAllowArchChange() const
{
  return _pimpl->solver_dupAllowArchChange;
}
bool ZConfig::solver_dupAllowVendorChange() const
{
  return _pimpl->solver_dupAllowVendorChange;
}
...
Pathname ZConfig::solver_checkSystemFile() const
{
  return ( _pimpl->solver_checkSystemFile.empty()
             ? ( configPath() / "systemCheck" )
             : _pimpl->solver_checkSystemFile );
}

Pathname ZConfig::solver_checkSystemFileDir() const
{
  return ( _pimpl->solver_checkSystemFileDir.empty()
             ? ( configPath() / "systemCheck.d" )
             : _pimpl->solver_checkSystemFileDir );
}
...
const std::set<std::string> &ZConfig::multiversionSpec() const
{
  return _pimpl->multiversion();
}
void ZConfig::multiversionSpec( std::set<std::string> new_r )
{
  _pimpl->multiversion().swap( new_r );
  sigMultiversionSpecChanged();
}
void ZConfig::clearMultiversionSpec()
{
  _pimpl->multiversion().clear();
  sigMultiversionSpecChanged();
}
void ZConfig::addMultiversionSpec( const std::string &name_r )
{
  _pimpl->multiversion().insert( name_r );
  sigMultiversionSpecChanged();
}
void ZConfig::removeMultiversionSpec( const std::string &name_r )
{
  _pimpl->multiversion().erase( name_r );
  sigMultiversionSpecChanged();
}

bool ZConfig::apply_locks_file() const { return _pimpl->apply_locks_file; }

Pathname ZConfig::update_dataPath() const
{
  return ( _pimpl->update_data_path.empty() ? Pathname( "/var/adm" )
...
Pathname ZConfig::credentialsGlobalDir() const
{
  return ( _pimpl->credentials_global_dir_path.empty()
             ? Pathname( "/etc/zypp/credentials.d" )
...
Pathname ZConfig::credentialsGlobalFile() const
{
  return ( _pimpl->credentials_global_file_path.empty()
             ? Pathname( "/etc/zypp/credentials.cat" )
             : _pimpl->credentials_global_file_path );
}

///////////////////////////////////////////////////////////////////

std::string ZConfig::distroverpkg() const { return "redhat-release"; }

///////////////////////////////////////////////////////////////////

Pathname ZConfig::pluginsPath() const { return _pimpl->pluginsPath.get(); }
...
std::ostream &ZConfig::about( std::ostream &str ) const
{
  ...
  str << "TextLocale: '" << textLocale() << "' (" << defaultTextLocale() << ")"
      << endl;
  str << "SystemArchitecture: '" << systemArchitecture() << "' ("
      << defaultSystemArchitecture() << ")" << endl;
  return str;
}
