...
const Pathname &ZYppCommitResult::root() const { return _pimpl->_root; }
...
const ZYppCommitResult::TransactionStepList &ZYppCommitResult::
  transactionStepList() const
{
...
std::ostream &operator<<( std::ostream &str, const ZYppCommitResult &obj )
{
  ...
  str << "CommitResult "
      << " (total " << result[ 0 ] << ", done " << result[ 1 ] << ", error "
      << result[ 2 ] << ", skipped " << result[ 3 ] << ", updateMessages "
