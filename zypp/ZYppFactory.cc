...
public:
  ...
  ~ZYppGlobalLock()
  {
    if ( _cleanLock )
      try
      {
        ...
        {
          scoped_lock<file_lock> flock(
            _zyppLockFileLock ); // aquire write lock
  ...
  const Pathname &zyppLockFilePath() const { return _zyppLockFilePath; }
...
static scoped_ptr<ZYppGlobalLock>
  _theGlobalLock; // on/off in sync with _theZYppInstance
...
ZYppFactoryException::ZYppFactoryException(
  const std::string &msg_r, pid_t lockerPid_r, const std::string &lockerName_r )
  : Exception( msg_r )
...
ZYppFactory ZYppFactory::instance() { return ZYppFactory(); }
...
ZYpp::Ptr ZYppFactory::getZYpp() const
{
  ...
  if ( !_instance )
  {
    if ( geteuid() != 0 )
    {
      MIL << "Running as user. Skip creating "
    ...
    {
      bool failed = true;
      const long LOCK_TIMEOUT =
        str::strtonum<long>( getenv( "ZYPP_LOCK_TIMEOUT" ) );
      if ( LOCK_TIMEOUT > 0 )
      {
        MIL << "Waiting whether pid " << globalLock().lockerPid()
            << " ends within $LOCK_TIMEOUT=" << LOCK_TIMEOUT << " sec." << endl;
        unsigned delay = 1;
        Pathname procdir(
          "/proc" / str::numstring( globalLock().lockerPid() ) );
        for ( long i = 0; i < LOCK_TIMEOUT; i += delay )
        {
          ...
          else
          {
            ...
            if ( failed )
            {
              ...
              MIL << "Waiting whether pid " << globalLock().lockerPid()
                  << " ends within " << ( LOCK_TIMEOUT - i ) << " sec." << endl;
              procdir = Pathname(
                "/proc" / str::numstring( globalLock().lockerPid() ) );
            }
            else
      ...
      if ( failed )
      {
        std::string t = str::form(
          _(
            "System management is locked by the application with pid %d (%s).\n"
            "Close this application before trying again." ),
          globalLock().lockerPid(), globalLock().lockerName().c_str() );
        ZYPP_THROW( ZYppFactoryException(
          t, globalLock().lockerPid(), globalLock().lockerName() ) );
      }
    }
    // Here we go...
...
///////////////////////////////////////////////////////////////////
//
bool ZYppFactory::haveZYpp() const { return !_theZYppInstance.expired(); }
