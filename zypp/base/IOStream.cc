...
{
  return forEachLine( str_r, [&]( int num_r, std::string line_r ) -> bool {
    if ( !consume_r )
      return true;
    ...
    if ( flags_r )
    {
      if ( flags_r & PF_TRIM )
        line_r =
          str::trim( line_r, str::Trim( unsigned( flags_r & PF_TRIM ) ) );
      ...
      if ( flags_r & ~PF_TRIM )
      {
        ...
        {
          case '\0':
            if ( flags_r & PF_SKIP_EMPTY )
              return true;
            break;
          case '#':
            if ( flags_r & PF_SKIP_SHARP_COMMENT )
              return true;
            break;
        }
      }
