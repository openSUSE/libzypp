...
/** Copy istream to ostream, prefixing each line with \a indent_r (default <tt>"> "</tt> ).
     * \return reference to the ostream.
     */
inline std::ostream &copyIndent(
  std::istream &from_r, std::ostream &to_r, const std::string &indent_r = "> " )
{
  if ( from_r && to_r )
  {
    char ch;
...
/** Copy istream to ostream, prefixing each line with \a indent_r (default <tt>"> "</tt> ).
     ...
     */
inline void tee(
  std::istream &from_r, std::ostream &to1_r, std::ostream &to2_r )
{
...
public:
  ...
  std::streamoff lineStart() const { return _lineStart; };

  /** Set current line number. */
  void setLineNo( unsigned lineNo_r ) { _lineNo = lineNo_r; }

  /** Access the current line. */
  const std::string &operator*() const { return _line; }
  /** \overload non const access */
  std::string &operator*() { return _line; }
...
/** Simple lineparser: Call functor \a consume_r for each line.
     ...
     */
int forEachLine(
  std::istream &str_r, function<bool( int, std::string )> consume_r );
...
int simpleParseFile( std::istream &str_r, ParseFlags flags_r,
  function<bool( int, std::string )> consume_r );

/** \overload trimming lines, skipping '#'-comments and empty lines. */
inline int simpleParseFile(
  std::istream &str_r, function<bool( int, std::string )> consume_r )
{
  return simpleParseFile(
    str_r, PF_TRIM | PF_SKIP_EMPTY | PF_SKIP_SHARP_COMMENT, consume_r );
}
