...
public:
  ...
  InputStream(
    std::istream &stream_r, const std::string &name_r = std::string() );
  ...
  /** The std::istream.
    ...
    */
  std::istream &stream() const { return *_stream; }

  /** Allow implicit conversion to std::istream.*/
  operator std::istream &() const { return *_stream; }
  ...
  /** Size of the input stream (informal).
    ...
    */
  std::streamoff size() const { return _size; }
  ...
  /** Set the size of the input stream.
     ...
     * just provided.
    */
  void setSize( std::streamoff val_r ) { _size = val_r; }
