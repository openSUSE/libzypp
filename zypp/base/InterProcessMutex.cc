...
ZYppLockedException::ZYppLockedException(
...
InterProcessMutex::Options::Options(
  ConsumerType ptype, const std::string &pname, int ptimeout )
  : name( pname )
  , timeout( ptimeout )
  , type( ptype )
{
  if ( geteuid() == 0 )
    ...
    base = filesystem::TmpPath::defaultLocation() +
           ( string( "zypp-" ) + getenv( "USER" ) );
}
    ...
    if ( !_fd || !_fd->isOpen() )
    {
      ...
      if ( !_fd->isOpen() )
      {
        ZYPP_THROW( Exception(
          str::form( _( "Can't open lock file: %s" ), strerror( errno ) ) ) );
      }
      ...
      {
        ZYPP_THROW( Exception(
          string( "Error getting lock info: " ) + strerror( errno ) ) );
      }
      ...
      if ( lock.l_type != F_UNLCK )
      {
        ...
        LMIL << "pid " << lock.l_pid
             << " is running and has a lock that conflicts with us."
             << std::endl;
      ...
      else if ( ( lock.l_type == F_UNLCK ) && ( _options.type == Reader ) )
      {
        ...
        if ( lock.l_type == F_UNLCK )
        {
          LMIL << "no previous readers, unlinking lock file and retrying."
               << endl;
        ...
        else if ( lock.l_type == F_RDLCK )
        {
          ...
          LMIL << "previous readers on lock file. taking lock as a reader."
               << std::endl;
          memset( &lock, 0, sizeof( struct flock ) );
          lock.l_type = F_RDLCK;
          lock.l_whence = SEEK_SET;
...
InterProcessMutex::~InterProcessMutex()
{
  ...
  {
    Pathname lock_file = lockFilePath();
    LMIL << "dropping " << ( ( _options.type == Reader ) ? "reader" : "writer" )
