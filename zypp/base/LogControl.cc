...
FileLineWriter::FileLineWriter( const Pathname &file_r, mode_t mode_r )
{
  ...
  else
  {
    ...
    std::ofstream *fstr = 0;
    _outs.reset( ( fstr = new std::ofstream(
  ...
  const std::string &message_r )
{
  ...
  return str::form( "%s <%d> %s(%d) [%s] %s(%s):%d %s", now.c_str(), level_r,
    ( gethostname( hostname, 1024 ) ? nohostname : hostname ), getpid(),
    group_r.c_str(), file_r, func_r, line_r, message_r.c_str() );
}
...
{
public:
  bool isExcessive() { return _excessive; }

  void excessive( bool onOff_r ) { _excessive = onOff_r; }
  ...
  void logfile( const Pathname &logfile_r, mode_t mode_r = 0640 )
  {
    if ( logfile_r.empty() )
      ...
      setLineWriter(
        shared_ptr<LogControl::LineWriter>( new log::StderrLineWriter ) );
    else
      setLineWriter( shared_ptr<LogControl::LineWriter>(
        new log::FileLineWriter( logfile_r, mode_r ) ) );
  }
...
public:
  ...
  std::ostream &getStream( const std::string &group_r, LogLevel level_r,
    const char *file_r, const char *func_r, const int line_r )
  {
    ...
    {
      _streamtable[ group_r ][ level_r ].reset(
        new Loglinestream( group_r, level_r ) );
    }
    return _streamtable[ group_r ][ level_r ]->getStream(
      file_r, func_r, line_r );
  }

  /** Format and write out a logline from Loglinebuf. */
  void putStream( const std::string &group_r, LogLevel level_r,
    const char *file_r, const char *func_r, int line_r,
    const std::string &message_r )
  {
    if ( _lineWriter )
      _lineWriter->writeOut( _lineFormater->format(
        group_r, level_r, file_r, func_r, line_r, message_r ) );
  }
...
private:
  ...
  ~LogControlImpl() { _lineWriter.reset(); }
...
std::ostream &getStream( const char *group_r, LogLevel level_r,
  const char *file_r, const char *func_r, const int line_r )
{
  return LogControlImpl::instance().getStream(
...
inline void putStream( const std::string &group_r, LogLevel level_r,
  ...
  const std::string &buffer_r )
{
  LogControlImpl::instance().putStream(
    group_r, level_r, file_r, func_r, line_r, buffer_r );
}

bool isExcessive() { return LogControlImpl::instance().isExcessive(); }
...
{
  LogControlImpl::instance().setLineWriter(
    shared_ptr<LineWriter>( new log::StderrLineWriter ) );
}
