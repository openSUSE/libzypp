...
public:
  /** Singleton access. */
  static LogControl instance() { return LogControl(); }
  ...
  struct LineFormater
  {
    virtual std::string format( const std::string & /*group_r*/,
      logger::LogLevel /*level_r*/, const char * /*file_r*/,
      const char * /*func_r*/, int /*line_r*/,
...
public:
  ...
  struct TmpLineWriter
  {
    ...
    TmpLineWriter( TLineWriter *_allocated_r )
      : _writer( LogControl::instance().getLineWriter() )
    {
      LogControl::instance().setLineWriter(
        shared_ptr<LineWriter>( _allocated_r ) );
    }

    ~TmpLineWriter() { LogControl::instance().setLineWriter( _writer ); }
