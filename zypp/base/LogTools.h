...
std::ostream &dumpRange( std::ostream &str, TIterator begin, TIterator end,
  const std::string &intro = "{", const std::string &pfx = "\n  ",
...
/** \overload for container */
template <class TContainer>
auto rangeLine( const TContainer &cont )
  -> decltype( rangeLine( cont.begin(), cont.end() ) )
{
...
std::ostream &operator<<(
  std::ostream &str, const std::set<Tp, TCmp, TAlloc> &obj )
{
  return dumpRange( str, obj.begin(), obj.end() );
}
...
public:
  ...
  const TPair &pair() const { return *_pair; }
...
public:
  ...
  const TMap &map() const { return *_map; }
...
{
public:
  typedef typename MapKVIteratorTraits<TMap>::Key_const_iterator
...
public:
  ...
  MapKey_const_iterator begin() const { return make_map_key_begin( map() ); }

  MapKey_const_iterator end() const { return make_map_key_end( map() ); }
...
{
public:
  typedef typename MapKVIteratorTraits<TMap>::Value_const_iterator
...
public:
  ...
  MapValue_const_iterator end() const { return make_map_value_end( map() ); }
...
template <class TKey, class Tp>
std::ostream &operator<<(
  std::ostream &str, const std::unordered_map<TKey, Tp> &obj )
{
  return str << dumpMap( obj );
}
...
template <class TKey, class Tp>
std::ostream &operator<<(
  std::ostream &str, const std::multimap<TKey, Tp> &obj )
{
  return str << dumpMap( obj );
}
...
inline std::ostream &operator<<(
  std::ostream &str, const std::basic_ios<char> &obj )
{
  std::string ret( "[" );
  ret += ( obj.good() ? 'g' : '_' );
  ret += ( obj.eof() ? 'e' : '_' );
