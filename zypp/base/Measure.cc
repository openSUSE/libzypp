...
struct Tm
{
  ...
  double asSec( clock_t ticks_r ) const { return double( ticks_r ) / ticks; }
...
private:
  ...
  std::ostream &log() const { return INT; }

  std::ostream &dumpMeasure(
    std::ostream &str_r, const std::string &tag_r = std::string() ) const
  {
    str_r << ( _stop - _start );
...
void Measure::restart() { _pimpl->restart(); }
...
{
  if ( _pimpl )
    _pimpl->elapsed( tag_r );
}
void Measure::elapsed( long tag_r ) const
{
  if ( _pimpl )
    _pimpl->elapsed( asString( tag_r ) );
}

void Measure::stop() { _pimpl.reset(); }
