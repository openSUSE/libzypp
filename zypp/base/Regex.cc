...
regex::regex( const std::string &str, int flags ) { assign( str, flags ); }
...
bool zypp::str::regex_match(
  const char *s, smatch &matches, const regex &regex )
{
  bool r = s && regex.m_valid &&
           !regexec( &regex.m_preg, s, 12, &matches.pmatch[ 0 ], 0 );
  if ( r )
    matches.match_str = s;
  return r;
}
...
smatch::smatch() { memset( &pmatch, -1, sizeof( pmatch ) ); }

std::string smatch::operator[]( unsigned i ) const
{
  if ( i < sizeof( pmatch ) / sizeof( *pmatch ) && pmatch[ i ].rm_so != -1 )
    return match_str.substr(
      pmatch[ i ].rm_so, pmatch[ i ].rm_eo - pmatch[ i ].rm_so );
...
{
  return ( i < sizeof( pmatch ) / sizeof( *pmatch ) && pmatch[ i ].rm_so != -1
             ? pmatch[ i ].rm_so
             : std::string::npos );
}
