...
inline bool regex_match(
  const std::string &s, smatch &matches, const regex &regex )
{
...
public:
  enum RegFlags
  {
    ...
    match_extended =
      REG_EXTENDED, ///< Use POSIX Extended Regular Expression syntax when interpreting regex.
  };
  ...
  regex( const regex &rhs ) { assign( rhs.m_str, rhs.m_flags ); }
...
private:
  friend class smatch;
  friend bool regex_match(
    const char *s, str::smatch &matches, const regex &regex );
