...
public:
  ...
  ~SignalSaver() { ::signal( _signum, _orighandler ); }
private:
  int _signum;
...
public:
  ...
  SigprocmaskSaver() { ::sigprocmask( SIG_SETMASK, NULL, &_origmask ); }
  /** Dtor restoring the original sigprocmask. */
  ~SigprocmaskSaver() { ::sigprocmask( SIG_SETMASK, &_origmask, NULL ); }
public:
  ...
  void suspend() { ::sigsuspend( &_origmask ); }
private:
  ::sigset_t _origmask;
};
