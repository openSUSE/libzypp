...
std::ostream &operator<<( std::ostream &str, const Match &obj )
{
  ...
  switch ( mode )
  {
    case Match::NOTHING:
      ...
      break;
    case Match::OTHER:
      str << mode << "(" << obj.modeval()
          << ")"; // check whether libsolv has introduced new modes!
      break;
    default:
      str << mode;
      break;
...
#undef OUTS
    if ( val )
    {
      if ( sep )
        str << sep;
      str << zypp::str::hexstring(
        val ); // check whether libsolv has introduced new flags.
    }
  }
  return str;
}
...
MatchUnknownModeException::MatchUnknownModeException(
  const Match &mode_r, const std::string &msg_r )
  : MatchException(
      msg_r.empty()
        ? str::form( _( "Unknown match mode '%s'" ), mode_r.asString().c_str() )
        : str::form( _( "Unknown match mode '%s' for pattern '%s'" ),
            mode_r.asString().c_str(), msg_r.c_str() ) )
{
}

MatchInvalidRegexException::MatchInvalidRegexException(
  const std::string &regex_r, int regcomp_r )
  : MatchException(
      regcomp_r
        ? str::form(
            _( "Invalid regular expression '%s': regcomp returned %d" ),
            regex_r.c_str(), regcomp_r )
...
struct StrMatcher::Impl
{
  ...
  void compile() const
  {
    if ( !_matcher )
    {
      ...
      int res =
        ::datamatcher_init( _matcher.get(), _search.c_str(), _flags.get() );
      if ( res )
      {
        _matcher.reset();
  ...
  bool isCompiled() const { return _matcher != nullptr; }
  ...
  const std::string &searchstring() const { return _search; }

  /** Set a new searchstring. */
  void setSearchstring( std::string string_r )
  {
    invalidate();
    _search = std::move( string_r );
  }

  /** The current search flags. */
  const Match &flags() const { return _flags; }
...
private:
  ...
  Impl *clone() const { return new Impl( _search, _flags ); }
};

/** \relates StrMatcher::Impl Stream output */
inline std::ostream &operator<<(
  std::ostream &str, const StrMatcher::Impl &obj )
{
...
StrMatcher::StrMatcher(
  const std::string &search_r, const Match::Mode &flags_r )
  : _pimpl( new Impl( search_r, flags_r ) )
{
}
...
void StrMatcher::compile() const { return _pimpl->compile(); }

bool StrMatcher::isCompiled() const { return _pimpl->isCompiled(); }
...
void StrMatcher::setSearchstring(
  const std::string &string_r, const Match &flags_r )
{
  _pimpl->setSearchstring( string_r );
...
const Match &StrMatcher::flags() const { return _pimpl->flags(); }
...
{
  return (
    lhs.flags() == rhs.flags() && lhs.searchstring() == rhs.searchstring() );
}
