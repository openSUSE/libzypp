...
public:
  ...
  static const Match
    SKIP_KIND; //!< LookupAttr: skip any \c kind: prefix when looking at a \ref Solvable name.
  static const Match
    FILES; //!< LookupAttr: match full path when matching in filelists, otherwise just the basenames.
  static const Match
    CHECKSUMS; //!< LookupAttr: also look for matches in checksums
  //@}
...
public:
  ...
  Match flags() const { return Match( flagval() ); }
...
public:
  ...
  bool isMode( Mode rhs ) const { return modeval() == modeval( rhs ); }
  /** Whether this has mode \ref STRING. */
  bool isModeString() const { return isMode( STRING ); }
  /** Whether this has mode \ref STRINGSTART. */
  bool isModeStringstart() const { return isMode( STRINGSTART ); }
  /** Whether this has mode \ref STRINGEND. */
  bool isModeStringend() const { return isMode( STRINGEND ); }
  /** Whether this has mode \ref SUBSTRING. */
  bool isModeSubstring() const { return isMode( SUBSTRING ); }
  /** Whether this has mode \ref GLOB. */
  bool isModeGlob() const { return isMode( GLOB ); }
  /** Whether this has mode \ref REGEX. */
  bool isModeRegex() const { return isMode( REGEX ); }

  /** Set the mode part to \a rhs . */
  void setMode( Mode rhs ) { _val = modeval( rhs ) | flagval(); }
  /** Set the mode \ref STRING. */
  void setModeString() { setMode( STRING ); }
  /** Set the mode \ref STRINGSTART. */
  void setModeStringstart() { setMode( STRINGSTART ); }
  /** Set the mode \ref STRINGEND. */
  void setModeStringend() { setMode( STRINGEND ); }
  /** Set the mode \ref SUBSTRING. */
  void setModeSubstring() { setMode( SUBSTRING ); }
  /** Set the mode \ref GLOB. */
  void setModeGlob() { setMode( GLOB ); }
  /** Set the mode \ref REGEX. */
  void setModeRegex() { setMode( REGEX ); }
  //@}
...
{
  /** Supplied message. */
  explicit MatchException( const std::string &msg_r )
    : Exception( msg_r )
  {
  }
};
...
{
  /** Supplied message. */
  explicit MatchUnknownModeException( const std::string &msg_r )
    : MatchException( msg_r )
  {
  }

  /** Build message including the \a mode and optional the pattern string. */
  MatchUnknownModeException(
    const Match &mode_r, const std::string &msg_r = std::string() );
};
...
{
  /** Supplied message. */
  explicit MatchInvalidRegexException( const std::string &msg_r )
    : MatchException( msg_r )
  {
  }
...
public:
  ...
  explicit operator bool() const { return !searchstring().empty(); }
...
public:
  ...
  /** \overload */
  bool operator()( const char *string_r ) const { return doMatch( string_r ); }
