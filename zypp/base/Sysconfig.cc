...
bool write( const Pathname &path_r, const std::string &key_r,
  const std::string &val_r, const std::string &newcomment_r )
{
  if ( key_r.empty() )
  {
  ...
  filesystem::TmpFile tmpf( filesystem::TmpFile::makeSibling( path_r ) );
  {
    ...
    iostr::forEachLine(
      InputStream( path_r ), [&]( int num_r, std::string line_r ) -> bool {
        if ( !found && matches( line_r ) )
        {
          o << key_r << '=' << val_r << endl;
          found = true;
          MIL << path_r << ": " << key_r << '=' << val_r << " changed on line "
              << num_r << endl;
        }
        else
          o << line_r << endl;
    ...
    if ( !o )
      ZYPP_THROW(
        Exception( str::Str() << tmpf.path() << ": " << Errno( EIO ) ) );
  }
  ...
  {
    ZYPP_THROW(
      Exception( str::Str() << tmpf.path() << ": " << Errno( res ) ) );
  }
  return found;
}

bool writeStringVal( const Pathname &path_r, const std::string &key_r,
  const std::string &val_r, const std::string &newcomment_r )
{
  return write( path_r, key_r,
    str::Str() << '"' << str::escape( val_r, '"' ) << '"', newcomment_r );
}
