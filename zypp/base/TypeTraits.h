...
private:
  ...
  template <typename C>
  static std::true_type testBeg( typename std::enable_if<
    std::is_same<decltype( static_cast<Signature<C>>( &C::begin ) ),
      Signature<C>>::value,
    void>::type * );
  template <typename C>
  ...
  template <typename C>
  static std::true_type testEnd( typename std::enable_if<
    std::is_same<decltype( static_cast<Signature<C>>( &C::end ) ),
      Signature<C>>::value,
    void>::type * );
  template <typename C>
...
template <typename Tp>
struct has_type_const_iterator : public std::integral_constant<bool,
                                   _detail::has_type_const_iterator<Tp>::value>
{
};

/** Whether \a Tp defines methods <tt>Tp::const_iterator begin/end() const</tt> */
template <typename Tp>
struct has_container_begin_end : public std::integral_constant<bool,
                                   _detail::_has_container_begin_end<Tp>::value>
{
};
...
template <typename Tp>
struct is_container
  : public std::integral_constant<bool, !std::is_same<Tp, std::string>::value &&
                                          has_container_begin_end<Tp>::value>
{
};
