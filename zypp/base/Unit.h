...
public:
  ...
  ValueType factor() const { return _factor; }

  const std::string &symbol() const { return _symbol; }

  unsigned prec() const { return _prec; }

  /** Build string representation of \a val_r. */
  std::string form( ValueType val_r, unsigned field_width_r = 0,
    unsigned unit_width_r = 1 ) const
  {
    return form( val_r, field_width_r, unit_width_r, _prec );
  }

  std::string form( ValueType val_r, unsigned field_width_r,
    unsigned unit_width_r, unsigned prec_r ) const
  {
    return form(
      double( val_r ) / _factor, _symbol, field_width_r, unit_width_r, prec_r );
  }

  static std::string form( double val_r, const std::string &symbol_r,
    unsigned field_width_r, unsigned unit_width_r, unsigned prec_r );
