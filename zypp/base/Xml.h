...
struct Node
{
  ...
  Node( std::ostream &out_r, std::string name_r,
  ...
  Node( std::ostream &out_r, std::string name_r, OptionalContentType,
    const std::initializer_list<Attr> &attrs_r = {} )
    : _out( out_r )
    , _name( std::move( name_r ) )
    , _hasContent( false )
  ...
  Node(
    std::ostream &out_r, std::string name_r, OptionalContentType, Attr attr_r )
  ...
  /** \overload for one */
  Node &addAttr( const Attr &attr_r ) { return addAttr( {attr_r} ); }
...
private:
  ...
  bool isComment() const { return _name.empty(); }
...
/** \relates Node Write a leaf node without PCDATA
     ...
     * \endcode
     */
inline std::ostream &node( std::ostream &out_r, const std::string &name_r,
...
inline std::ostream &node(
  std::ostream &out_r, const std::string &name_r, Node::Attr attr_r )
{
...
//@{
template <class Tp>
inline std::ostream &dumpAsXmlOn(
  std::ostream &str, const Tp &obj, const std::string &name_r )
{
  xmlout::Node guard( str, name_r, xmlout::Node::optionalContent );
  const std::string &content( asString( obj ) );
  if ( !content.empty() )
    *guard << xml::escape( content );
  return str;
}
//@}
//
