...
struct CredentialFileReaderImpl : public parser::IniParser
{
  ...
  CredentialFileReaderImpl(
    const Pathname &input_r, const ProcessCredentials &callback_r )
    : _input( input_r )
  ...
  virtual void consume( const std::string &section_r )
  {
    ...
    {
      ERR << "Ignore invalid URL '" << section_r << "' in file " << _input
          << endl;
