...
bool AuthDataComparator::operator()(
  const AuthData_Ptr &lhs, const AuthData_Ptr &rhs )
{
  static const url::ViewOption vopt =
    url::ViewOption::DEFAULTS - url::ViewOption::WITH_USERNAME -
...
void CredentialManager::Impl::init_userCredentials()
{
  ...
  else if ( PathInfo( _options.userCredFilePath ).isExist() )
  {
    ...
    CredentialFileReader(
      _options.userCredFilePath, bind( &Impl::processCredentials, this, _1 ) );
  }
  else
    DBG << "user cred file does not exist" << endl;

  _credsUser = _credsTmp;
  _credsTmp.clear();
...
static AuthData_Ptr findIn( const CredentialManager::CredentialSet &set,
  const Url &url, url::ViewOption vopt )
{
  const string &username = url.getUsername();
  for ( CredentialManager::CredentialIterator it = set.begin(); it != set.end();
        ++it )
  {
...
AuthData_Ptr CredentialManager::Impl::getCredFromFile( const Pathname &file )
{
  ...
  if ( _credsTmp.empty() )
    WAR << file << " does not contain valid credentials or is not readable."
        << endl;
  else
  {
...
static int save_creds_in_file( const CredentialManager::CredentialSet creds,
  const Pathname &file, const mode_t mode )
{
  int ret = 0;
...
void CredentialManager::saveInFile(
  const AuthData &cred, const Pathname &credFile )
{
  AuthData_Ptr c_ptr;
  c_ptr.reset( new AuthData( cred ) ); // FIX for child classes if needed
...
CredentialManager::CredentialIterator CredentialManager::credsGlobalBegin()
  const
{
