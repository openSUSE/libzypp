...
AttachedMedia MediaAccess::attachedMedia() const
{
  return _handler ? _handler->attachedMedia() : AttachedMedia();
}

bool MediaAccess::isSharedMedia() const
{
  return _handler ? _handler->isSharedMedia() : false;
}

void MediaAccess::resetParentId()
{
  if ( _handler )
    _handler->resetParentId();
}

bool MediaAccess::dependsOnParent() const
{
  return _handler ? _handler->dependsOnParent() : false;
}

bool MediaAccess::dependsOnParent(
  MediaAccessId parentId, bool exactIdMatch ) const
{
  return _handler ? _handler->dependsOnParent( parentId, exactIdMatch ) : false;
}

// open URL
void MediaAccess::open(
  const Url &o_url, const Pathname &preferred_attach_point )
{
  if ( !o_url.isValid() )
  {
  ...
  else if ( scheme == "ftp" || scheme == "tftp" || scheme == "http" ||
            scheme == "https" )
  {
    ...
    else
    {
      ...
      if ( use_multicurl && multicurlenv &&
           ( strcmp( multicurlenv, "0" ) == 0 ) )
      {
        WAR << "multicurl manually disabled." << endl;
        use_multicurl = false;
      }
      else if ( !use_multicurl && multicurlenv &&
                ( strcmp( multicurlenv, "1" ) == 0 ) )
      {
    ...
    for ( it = custom_headers.begin(); it != custom_headers.end(); ++it )
    {
...
std::string MediaAccess::protocol() const
{
  if ( !_handler )
    return "unknown";
...
bool MediaAccess::downloads() const
{
...
void MediaAccess::getDetectedDevices(
...
Pathname MediaAccess::localRoot() const
{
  if ( !_handler )
    return _noPath;
...
void MediaAccess::release( const std::string &ejectDev )
{
  if ( !_handler )
    return;
...
void MediaAccess::provideFile( const Pathname &filename ) const
{
  if ( !_handler )
  {
    ZYPP_THROW(
      MediaNotOpenException( "provideFile(" + filename.asString() + ")" ) );
  }

  _handler->provideFile( filename );
}

void MediaAccess::setDeltafile( const Pathname &filename ) const
{
  if ( !_handler )
  {
    ZYPP_THROW(
      MediaNotOpenException( "setDeltafile(" + filename.asString() + ")" ) );
  }

  _handler->setDeltafile( filename );
}

void MediaAccess::releaseFile( const Pathname &filename ) const
{
  if ( !_handler )
    return;
...
// and a path prefix is preserved to destination
void MediaAccess::provideDir( const Pathname &dirname ) const
{
  if ( !_handler )
  {
    ZYPP_THROW(
      MediaNotOpenException( "provideDir(" + dirname.asString() + ")" ) );
  }

  _handler->provideDir( dirname );
}

void MediaAccess::provideDirTree( const Pathname &dirname ) const
{
  if ( !_handler )
  {
    ZYPP_THROW(
      MediaNotOpenException( "provideDirTree(" + dirname.asString() + ")" ) );
  }

  _handler->provideDirTree( dirname );
}

void MediaAccess::releaseDir( const Pathname &dirname ) const
{
  if ( !_handler )
    return;
...
void MediaAccess::releasePath( const Pathname &pathname ) const
{
  if ( !_handler )
    return;
  ...
  std::list<std::string> &retlist, const Pathname &dirname, bool dots ) const
{
  ...
  {
    ZYPP_THROW(
      MediaNotOpenException( "dirInfo(" + dirname.asString() + ")" ) );
  }
...
bool MediaAccess::doesFileExist( const Pathname &filename ) const
{
  if ( !_handler )
  {
    ZYPP_THROW(
      MediaNotOpenException( "doesFileExist(" + filename.asString() + ")" ) );
  }

  return _handler->doesFileExist( filename );
}

std::ostream &MediaAccess::dumpOn( std::ostream &str ) const
{
  if ( !_handler )
