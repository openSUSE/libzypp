...
size_t MediaBlockList::addBlock( off_t off, size_t size )
{
  haveblocks = true;
...
void MediaBlockList::setFileChecksum(
  std::string ctype, int cl, unsigned char *c )
{
  if ( !cl )
    return;
  fsumtype = ctype;
...
bool MediaBlockList::createFileDigest( Digest &digest ) const
{
  return digest.create( fsumtype );
}

bool MediaBlockList::verifyFileDigest( Digest &digest ) const
{
  if ( !haveFileChecksum() )
    return true;
...
void MediaBlockList::setChecksum(
  size_t blkno, std::string cstype, int csl, unsigned char *cs, size_t cspad )
{
  if ( !csl )
    return;
  if ( !chksumlen )
  ...
  if ( csl != chksumlen || cstype != chksumtype || cspad != chksumpad ||
       blkno != chksums.size() / chksumlen )
    return;
  chksums.resize( chksums.size() + csl );
  memcpy( &chksums[ csl * blkno ], cs, csl );
}

void MediaBlockList::setRsum(
  size_t blkno, int rsl, unsigned int rs, size_t rspad )
{
  if ( !rsl )
    return;
  if ( !rsumlen )
...
bool MediaBlockList::createDigest( Digest &digest ) const
{
  return digest.create( chksumtype );
}

bool MediaBlockList::verifyDigest( size_t blkno, Digest &digest ) const
{
  ...
  if ( dig.empty() || dig.size() < size_t( chksumlen ) )
    return false;
  return memcmp( &dig[ 0 ], &chksums[ chksumlen * blkno ], chksumlen ) ? false
                                                                       : true;
}

unsigned int MediaBlockList::updateRsum(
  unsigned int rs, const char *bytes, size_t len ) const
{
  if ( !len )
    return rs;
  unsigned short s, m;
...
bool MediaBlockList::verifyRsum( size_t blkno, unsigned int rs ) const
{
  if ( !haveRsum( blkno ) )
    return true;
  size_t size = blocks[ blkno ].size;
...
bool MediaBlockList::checkRsum(
  size_t blkno, const unsigned char *buf, size_t bufl ) const
{
...
bool MediaBlockList::checkChecksum(
  size_t blkno, const unsigned char *buf, size_t bufl ) const
{
  if ( blkno >= blocks.size() || bufl < blocks[ blkno ].size )
    return false;
...
bool MediaBlockList::checkChecksumRotated(
  size_t blkno, const unsigned char *buf, size_t bufl, size_t start ) const
{
  if ( blkno >= blocks.size() || bufl < blocks[ blkno ].size )
    return false;
  if ( start == bufl )
...
void MediaBlockList::writeBlock( size_t blkno, FILE *fp,
  const unsigned char *buf, size_t bufl, size_t start,
  vector<bool> &found ) const
{
  if ( blkno >= blocks.size() || bufl < blocks[ blkno ].size )
    return;
...
static size_t fetchnext( FILE *fp, unsigned char *bp, size_t blksize,
  size_t pushback, unsigned char *pushbackp )
{
  size_t l = blksize;
  int c;
...
void MediaBlockList::reuseBlocks( FILE *wfp, string filename )
{
  FILE *fp;
  ...
  if ( rsumlen && !rsums.empty() )
  {
    ...
    {
      if ( blocks[ i ].size != blksize &&
           ( i != nblks - 1 || rsumpad != blksize ) )
        continue;
  ...
  for ( size_t blkno = 0; blkno < blocks.size(); ++blkno )
  {
    if ( !found[ blkno ] )
    {
      ...
      if ( chksumlen && ( blkno + 1 ) * chksumlen <= chksums.size() )
      {
        ...
        memcpy( &nchksums[ ( nblocks.size() - 1 ) * chksumlen ],
          &chksums[ blkno * chksumlen ], chksumlen );
      }
...
std::string MediaBlockList::asString() const
{
  std::string s;
  size_t i, j;
