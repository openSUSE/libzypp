...
public:
   ...
   **/
  inline bool haveBlocks() const { return haveblocks; }
  /**
   ...
   **/
  inline MediaBlock getBlock( size_t blkno ) const { return blocks[ blkno ]; }
  /**
  ...
  inline off_t getFilesize() const { return filesize; }
  inline bool haveFilesize() const { return filesize != off_t( -1 ); }
   ...
   **/
  void setChecksum( size_t blkno, std::string cstype, int csl,
    unsigned char *cs, size_t cspad = 0 );
  bool checkChecksum(
...
private:
  void writeBlock( size_t blkno, FILE *fp, const unsigned char *buf,
    size_t bufl, size_t start, std::vector<bool> &found ) const;
  bool checkChecksumRotated(
