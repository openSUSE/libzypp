...
DeviceList systemDetectDevices( bool supportingDVD_r )
{
  ...
  zypp::AutoDispose<struct udev_enumerate *> enumerate(
  ...
  {
    zypp::AutoDispose<struct udev_device *> device(
    ...
    if ( supportingDVD_r &&
         !::udev_device_get_property_value( device, "ID_CDROM_DVD" ) )
    {
    ...
    if ( devnode.isBlk() )
    {
      MediaSource media( "cdrom", devnode.path().asString(), devnode.devMajor(),
        devnode.devMinor() );
  ...
  try
  {
    ...
    for ( size_t d = 0; d < drv_udis.size(); d++ )
    {
      ...
      if ( drv )
      {
        ...
        MediaSource media( "cdrom", drv.getDeviceFile(), drv.getDeviceMajor(),
          drv.getDeviceMinor() );
        DBG << "Found " << drv_udis[ d ] << ": " << media.asString()
            << std::endl;
...
bool MediaCD::openTray( const std::string &device_r )
{
  ...
  if ( res )
  {
    ...
    {
      WAR << "Eject " << device_r << " failed (" << ::strerror( errno ) << ")"
          << endl;
    }

#if defined( EJECT_TOOL_PATH )
    DBG << "Try to eject " << device_r << " using " << EJECT_TOOL_PATH
        << " utility" << std::endl;
    ...
    for ( std::string out( eject.receiveLine() ); out.length();
          out = eject.receiveLine() )
    {
      DBG << " " << out;
    }
...
bool MediaCD::closeTray( const std::string &device_r )
{
  ...
  if ( res )
  {
    WAR << "Close tray " << device_r << " failed (" << ::strerror( errno )
        << ")" << endl;
    return false;
  }
...
MediaCD::DeviceList MediaCD::detectDevices( bool supportingDVD_r ) const
{
  ...
  if ( detected.empty() )
  {
    WAR << "CD/DVD drive detection with HAL/UDEV failed! Guessing..."
        << std::endl;
    PathInfo dvdinfo( "/dev/dvd" );
    PathInfo cdrinfo( "/dev/cdrom" );
    if ( dvdinfo.isBlk() )
    {
      MediaSource media( "cdrom", dvdinfo.path().asString(), dvdinfo.devMajor(),
        dvdinfo.devMinor() );
      DBG << "Found (GUESS): " << media << std::endl;
      detected.push_back( media );
    }
    if ( cdrinfo.isBlk() &&
         !( cdrinfo.devMajor() == dvdinfo.devMajor() &&
            cdrinfo.devMinor() == dvdinfo.devMinor() ) )
    {
      MediaSource media( "cdrom", cdrinfo.path().asString(), cdrinfo.devMajor(),
        cdrinfo.devMinor() );
      DBG << "Found (GUESS): " << media << std::endl;
      detected.push_back( media );
    }
...
{
  DBG << "next " << next << " last " << _lastdev << " last tried "
      << _lastdev_tried << endl;
  if ( next && _lastdev == -1 )
  ...
  for ( DeviceList::iterator it = _devices.begin();
        !mountsucceeded && it != _devices.end(); ++it, ++count )
  {
    ...
    if ( ret.mediaSource && ret.attachPoint && !ret.attachPoint->empty() )
    {
      DBG << "Using a shared media " << ret.mediaSource->name << " attached on "
          << ret.attachPoint->path << endl;
      removeAttachPoint();
...

    {
      ...
      for ( e = entries.begin(); e != entries.end(); ++e )
      {
             ...
             media->min_nr == dev_info.devMinor() )
        {
          ...
          {
            DBG << "Using a system mounted media " << media->name
                << " attached on " << ap->path << endl;
    ...
    for ( list<string>::iterator fsit = filesystems.begin();
          !mountsucceeded && fsit != filesystems.end(); ++fsit )
    {
      try
      {
        ...
        else
        {
          ...
          ZYPP_THROW(
            MediaMountException( "Unable to verify that the media was mounted",
              it->name, mountpoint ) );
        }
      }
  ...
  if ( !mountsucceeded )
  {
    ...
    if ( !merr.mountOutput().empty() )
    {
      ZYPP_THROW( MediaMountException(
        merr.mountError(), _url.asString(), mountpoint, merr.mountOutput() ) );
    }
    else
    {
      ZYPP_THROW( MediaMountException(
        "Mounting media failed", _url.asString(), mountpoint ) );
    }
  }
...
//
bool MediaCD::isAttached() const { return checkAttached( false ); }
...
void MediaCD::getDetectedDevices(
