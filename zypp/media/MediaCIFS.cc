...
//	DESCRIPTION :
//
MediaCIFS::MediaCIFS( const Url &url_r, const Pathname &attach_point_hint_r )
  : MediaHandler(
      url_r, attach_point_hint_r,
      stripShare(
        url_r.getPathName() ), // urlpath WITHOUT share name at attachpoint
      false )                  // does_download
{
  MIL << "MediaCIFS::MediaCIFS(" << url_r << ", " << attach_point_hint_r << ")"
      << endl;
}
...
void MediaCIFS::attachTo( bool next )
{
  ...
  do // repeat this while the mount returns "Permission denied" error
  {
    ...
    try
    {
      mount.mount( path, mountpoint, "cifs", options.asString(), environment );
  ...
  if ( !mountsucceeded )
  {
    ...
    ZYPP_THROW( MediaMountException(
      "Unable to verify that the media was mounted", path, mountpoint ) );
  }
}
...
//
bool MediaCIFS::isAttached() const { return checkAttached( true ); }
...
bool MediaCIFS::authenticate( AuthData &authdata, bool firstTry ) const
{
  ...
  CredentialManager cm(
    CredManagerOptions( ZConfig::instance().systemRoot() ) );
  ...
  {
    DBG << "callback answer: retry" << endl << "AuthData: " << *smbcred << endl;
