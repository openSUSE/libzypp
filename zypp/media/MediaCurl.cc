...
extern "C" void _do_free_once() { curl_global_cleanup(); }
...
int log_curl(
  CURL *curl, curl_infotype info, char *ptr, size_t len, void *max_lvl )
{
  std::string pfx( " " );
  long lvl = 0;
  switch ( info )
...
static size_t log_redirects_curl(
...
struct ProgressData
{
  ...
  int reportProgress() const
  {
    ...
    if ( report &&
         !( *report )->progress( _dnlPercent, url, _drateTotal, _drateLast ) )
...
inline void escape( string &str_r, const char char_r, const string &escaped_r )
{
  for ( string::size_type pos = str_r.find( char_r ); pos != string::npos;
        pos = str_r.find( char_r, pos ) )
  {
...
void fillSettingsSystemProxy( const Url &url, TransferSettings &s )
{
  ...
  if ( proxy_info.useProxyFor( url ) )
  {
    ...
    try
    {
      ...
      s.setProxy(
        u.asString( url::ViewOption::WITH_SCHEME + url::ViewOption::WITH_HOST +
...
static const char *const anonymousIdHeader()
{
  ...
  static const std::string _value(
    str::trim( str::form( "X-ZYpp-AnonymousId: %s",
...
static const char *const distributionFlavorHeader()
{
  ...
  // is guessed.
  static const std::string _value(
    str::trim( str::form( "X-ZYpp-DistributionFlavor: %s",
...
static const char *const agentString()
{
  ...
  // The target could be not initialized, and then this information
  // is guessed.
  static const std::string _value( str::form( "ZYpp %s (curl %s) %s", VERSION,
...
MediaCurl::MediaCurl( const Url &url_r, const Pathname &attach_point_hint_r )
  ...
  , _customHeaders( 0L )
{
  ...
  MIL << "MediaCurl::MediaCurl(" << url_r << ", " << attach_point_hint_r << ")"
      << endl;
  ...
  if ( !attachPoint().empty() )
  {
    ...
    char *atest = NULL;
    if ( !ainfo.isDir() || !ainfo.userMayRWX() || atemp == NULL ||
         ( atest = ::mkdtemp( atemp ) ) == NULL )
    {
      WAR << "attach point " << ainfo.path() << " is not useable for "
...
TransferSettings &MediaCurl::settings() { return _settings; }
...
void MediaCurl::setupEasy()
{
  ...
  {
    ZYPP_THROW(
      MediaCurlSetOptException( _url, "Error setting error buffer" ) );
  }
  ...
  */
  ...
  if ( _url.getScheme() == "https" )
  {
    ...
    if ( _settings.verifyPeerEnabled() || _settings.verifyHostEnabled() )
    {
      SET_OPTION(
        CURLOPT_CAPATH, _settings.certificateAuthoritiesPath().c_str() );
    }
    ...
    // see bnc#779177
    ret =
      curl_easy_setopt( _curl, CURLOPT_SSL_OPTIONS, CURLSSLOPT_ALLOW_BEAST );
    if ( ret != 0 )
    {
      disconnectFrom();
      ZYPP_THROW( MediaCurlSetOptException( _url, _curlError ) );
...
#endif
    SET_OPTION(
      CURLOPT_SSL_VERIFYPEER, _settings.verifyPeerEnabled() ? 1L : 0L );
    SET_OPTION(
      CURLOPT_SSL_VERIFYHOST, _settings.verifyHostEnabled() ? 2L : 0L );
  ...
  if ( _settings.proxyEnabled() && !_settings.proxy().empty() )
  {
    ...
    if ( proxyuserpwd.empty() )
    {
      ...
      if ( curlconf.proxyuserpwd.empty() )
        DBG << "Proxy: ~/.curlrc does not contain the proxy-user option"
            << endl;
      else
      {
    ...
    {
      DBG << "Proxy: using provided proxy-user '" << _settings.proxyUsername()
          << "'" << endl;
    }
  ...
  if ( _settings.maxDownloadSpeed() != 0 )
    SET_OPTION_OFFT(
      CURLOPT_MAX_RECV_SPEED_LARGE, _settings.maxDownloadSpeed() );
#endif
  ...
  for ( TransferSettings::Headers::const_iterator it =
          vol_settings.headersBegin();
        it != vol_settings.headersEnd(); ++it )
  {
...
bool MediaCurl::checkAttachPoint( const Pathname &apoint ) const
{
...
void MediaCurl::releaseFrom( const std::string &ejectDev ) { disconnect(); }
...
void MediaCurl::getFileCopy(
  const Pathname &filename, const Pathname &target ) const
{
  callback::SendReport<DownloadProgressReport> report;
...
void MediaCurl::evaluateCurlCode(
  const Pathname &filename, CURLcode code, bool timeout_reached ) const
{
  if ( code != 0 )
  {
    Url url;
          ...
          if ( infoRet == CURLE_OK )
          {
            ...
            switch ( httpReturnCode )
            {
              case 401:
              {
                ...
                DBG << msg << " Login failed (URL: " << url.asString() << ")"
                    << std::endl;
                DBG << "MediaUnauthorizedException auth hint: '" << auth_hint
                    << "'" << std::endl;
              ...
              case 403:
              {
                ...
                if ( url.asString().find( "novell.com" ) != string::npos )
                  msg403 =
                    _( "Visit the Novell Customer Center to check whether your "
                       "registration is valid and has not expired." );
...
bool MediaCurl::doGetDoesFileExist( const Pathname &filename ) const
{
  ...
  CURLcode ret = curl_easy_setopt( _curl, CURLOPT_URL, urlBuffer.c_str() );
  if ( ret != 0 )
  {
...
bool MediaCurl::detectDirIndex() const
{
  ...
  char *ptr = NULL;
  CURLcode ret = curl_easy_getinfo( _curl, CURLINFO_EFFECTIVE_URL, &ptr );
  if ( ret == CURLE_OK && ptr != NULL )
  {
    try
    {
      ...
      {
        DBG << "Effective url (" << eurl
            << ") seems to provide the index of a directory" << endl;
        not_a_file = true;
      }
    }
    catch ( ... )
...
void MediaCurl::doGetFileCopy( const Pathname &filename, const Pathname &target,
  callback::SendReport<DownloadProgressReport> &report,
  RequestOptions options ) const
{
  ...
  if ( infoRet == CURLE_OK )
  {
    ...
    if ( httpReturnCode == 304 ||
         ( httpReturnCode == 213 &&
           ( _url.getScheme() == "ftp" ||
             _url.getScheme() == "tftp" ) ) ) // not modified
    {
      DBG << " Not modified.";
      modified = false;
    }
...
void MediaCurl::doGetFileCopyFile( const Pathname &filename,
  const Pathname &dest, FILE *file,
  callback::SendReport<DownloadProgressReport> &report,
  RequestOptions options ) const
{
  DBG << filename.asString() << endl;
  ...
  if ( ftell( file ) == 0 && ret == 0 )
  {
         ...
         httpReturnCode == 200 )
    {
      ...
      if ( curl_easy_getinfo(
             _curl, CURLINFO_CONDITION_UNMET, &conditionUnmet ) == CURLE_OK &&
           conditionUnmet )
      {
  ...
  if ( ret != 0 )
  {
    ERR << "curl error: " << ret << ": " << _curlError << ", temp file size "
        << ftell( file ) << " bytes." << endl;
...
void MediaCurl::getDir( const Pathname &dirname, bool recurse_r ) const
{
        ...
        it != content.end(); ++it )
  {
    ...
    {
      case filesystem::
        FT_NOT_AVAIL: // old directory.yast contains no typeinfo at all
      case filesystem::FT_FILE:
        getFile( filename );
        break;
      case filesystem::
        FT_DIR: // newer directory.yast contain at least directory info
        if ( recurse_r )
        {
        ...
        else
        {
          ...
          if ( res )
          {
            WAR << "Ignore error (" << res << ") on creating local directory '"
                << localPath( filename ) << "'" << endl;
          }
        }
        break;
      default:
...
int MediaCurl::progressCallback(
  void *clientp, double dltotal, double dlnow, double ultotal, double ulnow )
{
  ...
  if ( pdata )
  {
    ...
    long httpReturnCode = 0;
    if ( curl_easy_getinfo(
           pdata->curl, CURLINFO_RESPONSE_CODE, &httpReturnCode ) != CURLE_OK ||
         httpReturnCode == 0 )
...
bool MediaCurl::authenticate(
  const string &availAuthTypes, bool firstTry ) const
{
  ...
  else
  {
    ...
    string prompt_msg =
      str::Format( _( "Authentication required for '%s'" ) ) % _url.asString();
  ...
  if ( credentials )
  {
    ...
    const_cast<MediaCurl *>( this )->_settings.setUsername(
      credentials->username() );
    const_cast<MediaCurl *>( this )->_settings.setPassword(
    ...
    if ( credentials->authType() != CURLAUTH_NONE )
    {
      ...
      const_cast<MediaCurl *>( this )->_settings.setAuthType(
        credentials->authTypeAsString() );
      ret =
        curl_easy_setopt( _curl, CURLOPT_HTTPAUTH, credentials->authType() );
      if ( ret != 0 )
        ZYPP_THROW( MediaCurlSetOptException( _url, _curlError ) );
    }
