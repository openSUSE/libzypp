...
//
MediaDIR::MediaDIR( const Url &url_r, const Pathname & /*attach_point_hint_r*/ )
...
void MediaDIR::attachTo( bool next )
{
  ...
  {
    ZYPP_THROW( MediaBadUrlException(
      url(), "The media URL does not provide any useable directory path" ) );
  }
  else if ( !PathInfo( attachPoint() ).isDir() )
  {
    ZYPP_THROW( MediaBadUrlException( url(), "Specified path '" +
...
//
void MediaDIR::releaseFrom( const std::string &ejectDev ) { return; }
