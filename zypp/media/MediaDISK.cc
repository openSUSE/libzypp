...
//	DESCRIPTION :
//
MediaDISK::MediaDISK( const Url &url_r, const Pathname &attach_point_hint_r )
  : MediaHandler( url_r, attach_point_hint_r,
      url_r.getPathName(), // urlpath below attachpoint
      false )              // does_download
{
  MIL << "MediaDISK::MediaDISK(" << url_r << ", " << attach_point_hint_r << ")"
      << endl;
...
bool MediaDISK::verifyIfDiskVolume( const Pathname &dev_name )
{
  ...
  {
    ERR << "Specified device name " << dev_name << " is not allowed"
        << std::endl;
    return false;
  }
  ...
  if ( !dev_info.isBlk() )
  {
    ERR << "Specified device name " << dev_name << " is not a block device"
        << std::endl;
    return false;
  }
      ...
      for ( it = dlist.begin(); it != dlist.end(); ++it )
      {
             ...
             vol_info.devMinor() == dev_info.devMinor() )
        {
          DBG << "Specified device name " << dev_name
              << " is a volume (disk/by-uuid link " << vol_info.path() << ")"
              << std::endl;
          return true;
        }
      }
  ...
  // (e.g. vbd mapped volumes in a XEN vm)
  {
    ...
    if ( zypp::filesystem::readdir( dlist, dpath ) == 0 )
    {
      ...
      for ( it = dlist.begin(); it != dlist.end(); ++it )
      {
        ...
        {
          DBG << "Specified device name " << dev_name
              << " is a volume (disk/by-label link " << vol_info.path() << ")"
              << std::endl;
          return true;
        }
      }
  ...
  if ( cmd.close() != 0 )
  {
    ERR << cmd.execError() << endl
        ...
        << " is not a usable disk volume" << std::endl;
    return false;
  }
  return true;
}
...
void MediaDISK::attachTo( bool next )
{
  ...
  for ( e = entries.begin(); e != entries.end(); ++e )
  {
         ...
         media->min_nr == dev_info.devMinor() )
    {
      ...
      {
        DBG << "Using a system mounted media " << media->name << " attached on "
            << ap->path << endl;
  ...
  if ( !mountsucceeded )
  {
    ...
    ZYPP_THROW( MediaMountException(
      "Unable to verify that the media was mounted", _device, mountpoint ) );
  }
}
...
//
bool MediaDISK::isAttached() const { return checkAttached( false ); }
