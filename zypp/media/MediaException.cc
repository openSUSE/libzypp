...
{
  str << form(
    _( "Failed to mount %s on %s" ), _source.c_str(), _target.c_str() );
  if ( !_cmdout.empty() )
    str << ": " << _error << " (" << _cmdout << ")";
  else
    str << ": " << _error;
...
{
  return str << form( _( "File '%s' not found on medium '%s'" ),
...
std::ostream &MediaCurlInitException::dumpOn( std::ostream &str ) const
{
  ...
  return str << form( _( "Download (curl) initialization failed for '%s'" ),
           _url.c_str() );
}

std::ostream &MediaSystemException::dumpOn( std::ostream &str ) const
{
  return str << form( _( "System exception '%s' on medium '%s'." ),
           _message.c_str(), _url.c_str() );
}

std::ostream &MediaNotAFileException::dumpOn( std::ostream &str ) const
{
  return str << form( _( "Path '%s' on medium '%s' is not a file." ),
           _path.c_str(), _url.c_str() );
}

std::ostream &MediaNotADirException::dumpOn( std::ostream &str ) const
{
  return str << form( _( "Path '%s' on medium '%s' is not a directory." ),
           _path.c_str(), _url.c_str() );
}
...
std::ostream &MediaBadUrlEmptyFilesystemException::dumpOn(
  std::ostream &str ) const
{
  return str << _( "Empty filesystem in URI" ) << ": " << _url;
}

std::ostream &MediaBadUrlEmptyDestinationException::dumpOn(
  std::ostream &str ) const
{
  return str << _( "Empty destination in URI" ) << ": " << _url;
}

std::ostream &MediaUnsupportedUrlSchemeException::dumpOn(
  std::ostream &str ) const
{
...
{
  // TranslatorExplanation: curl is the name of a library, don't translate
  return str << form( _( "Download (curl) error for '%s':\n"
                         "Error code: %s\n"
                         "Error message: %s\n" ),
...
{
  return str << form(
           _( "Medium '%s' is in use by another instance" ), _name.c_str() );
}
...
{
  str << form(
    _( "Location '%s' is temporarily unaccessible." ), _url.c_str() );
  if ( !_msg.empty() )
    str << endl << _msg;
  return str;
}
...
{
  str << form(
    _( " SSL certificate problem, verify that the CA cert is OK for '%s'." ),
    _url.c_str() );
  if ( !_msg.empty() )
    str << endl << _msg;
  return str;
