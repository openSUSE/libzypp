...
//
///////////////////////////////////////////////////////////////////
MediaISO::MediaISO( const Url &url_r, const Pathname &attach_point_hint_r )
  : MediaHandler( url_r, attach_point_hint_r,
      url_r.getPathName(), // urlpath below attachpoint
      false )              // does_download
{
  MIL << "MediaISO::MediaISO(" << url_r << ", " << attach_point_hint_r << ")"
      << std::endl;
  ...
  try
  {
    ...
    if ( arg.empty() && _isofile.dirname().absolute() )
    {
      src = std::string( "dir:///" );
      src.setPathName(
        url::encode( _isofile.dirname().asString(), URL_SAFE_CHARS ) );
  ...
  else if ( !( src.getScheme() == "hd" || src.getScheme() == "dir" ||
               src.getScheme() == "file" || src.getScheme() == "nfs" ||
               src.getScheme() == "nfs4" || src.getScheme() == "smb" ||
               src.getScheme() == "cifs" ) )
  {
...
// ---------------------------------------------------------------
bool MediaISO::isAttached() const { return checkAttached( false ); }
...
void MediaISO::attachTo( bool next )
{
  ...
  catch ( const MediaException &e1 )
  {
    ...
    MediaMountException e3( "Unable to find iso filename on source media",
  ...
  Mount mount;
  mount.mount( isofile.asString(), mountpoint, _filesystem, mountopts );
...
// ---------------------------------------------------------------
void MediaISO::getDir( const Pathname &dirname, bool recurse_r ) const
{
  MediaHandler::getDir( dirname, recurse_r );
}

// ---------------------------------------------------------------
void MediaISO::getDirInfo(
