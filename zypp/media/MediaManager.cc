...
struct ManagedMedia
{
  ...
  inline void checkAttached( MediaAccessId id )
  {
    if ( !handler->isAttached() )
    {
      DBG << "checkAttached(" << id << ") not attached" << std::endl;
      desired = false;
      ZYPP_THROW( MediaNotAttachedException( handler->url() ) );
    }
  }
...
//////////////////////////////////////////////////////////////////
std::string MediaVerifierBase::info() const
{
  return std::string( typeid( ( *this ) ).name() );
}

//////////////////////////////////////////////////////////////////
std::string NoVerifier::info() const
{
...
public:
  ...
  inline MediaAccessId nextAccessId() { return ++last_accessid; }

  inline bool hasId( MediaAccessId accessId ) const
  {
    return mediaMap.find( accessId ) != mediaMap.end();
  }

  inline ManagedMedia &findMM( MediaAccessId accessId )
  {
  ...
  static inline time_t getMountTableMTime()
  {
    time_t mtime = zypp::PathInfo( "/etc/mtab" ).mtime();
    if ( mtime <= 0 )
    {
      WAR << "Failed to retrieve modification time of '/etc/mtab'" << std::endl;
    }
    return mtime;
  }

  static inline MountEntries getMountEntries() { return Mount::getEntries(); }
};
...
// ---------------------------------------------------------------
MediaAccessId MediaManager::open(
  const Url &url, const Pathname &preferred_attach_point )
{
  ...
  DBG << "Opened new media access using id " << nextId << " to "
      << url.asString() << std::endl;
  return nextId;
}

// ---------------------------------------------------------------
void MediaManager::close( MediaAccessId accessId )
{
  MutexLock glock( g_Mutex );
  ...
  for ( ; m != m_impl->mediaMap.end(); ++m )
  {
    ...
    {
      ZYPP_THROW(
        MediaIsSharedException( m->second.handler->url().asString() ) );
    }
  }

  DBG << "Close to access handler using id " << accessId << " requested"
      << std::endl;
...
// ---------------------------------------------------------------
bool MediaManager::isOpen( MediaAccessId accessId ) const
{
  MutexLock glock( g_Mutex );

  ManagedMediaMap::iterator it( m_impl->mediaMap.find( accessId ) );
  return it != m_impl->mediaMap.end() && it->second.handler->isOpen();
}

// ---------------------------------------------------------------
std::string MediaManager::protocol( MediaAccessId accessId ) const
{
  MutexLock glock( g_Mutex );
...
// ---------------------------------------------------------------
bool MediaManager::downloads( MediaAccessId accessId ) const
{
  MutexLock glock( g_Mutex );
...
// ---------------------------------------------------------------
Url MediaManager::url( MediaAccessId accessId ) const
{
  MutexLock glock( g_Mutex );
...
// ---------------------------------------------------------------
void MediaManager::addVerifier(
  MediaAccessId accessId, const MediaVerifierRef &verifier )
{
  MutexLock glock( g_Mutex );
  ...
  DBG << "MediaVerifier change: id=" << accessId
      << ", verifier=" << verifier->info() << std::endl;
}

// ---------------------------------------------------------------
void MediaManager::delVerifier( MediaAccessId accessId )
{
  MutexLock glock( g_Mutex );
...
// ---------------------------------------------------------------
bool MediaManager::setAttachPrefix( const Pathname &attach_prefix )
{
  MutexLock glock( g_Mutex );
...
void MediaManager::attach( MediaAccessId accessId )
{
  ...
  MIL << "checkDesired(" << accessId << ") of first device failed,"
                                        " going to try others with attach(true)"
      << std::endl;
  ...
  while ( ref.handler->hasMoreDevices() )
  {
    ...
    catch ( const MediaNotDesiredException &ex )
    {
      ...
      if ( !ref.handler->hasMoreDevices() )
      {
        MIL << "No desired media found after trying all detected devices."
            << std::endl;
        ZYPP_RETHROW( ex );
      }

      AttachedMedia media( ref.handler->attachedMedia() );
      DBG << "Skipping " << media.mediaSource->asString()
...
void MediaManager::release(
  MediaAccessId accessId, const std::string &ejectDev )
{
  MutexLock glock( g_Mutex );
...
// ---------------------------------------------------------------
void MediaManager::disconnect( MediaAccessId accessId )
{
  MutexLock glock( g_Mutex );
...
// ---------------------------------------------------------------
bool MediaManager::isAttached( MediaAccessId accessId ) const
{
  MutexLock glock( g_Mutex );
...
// ---------------------------------------------------------------
bool MediaManager::isDesiredMedia( MediaAccessId accessId ) const
{
  MutexLock glock( g_Mutex );
  ...
  DBG << "isDesiredMedia(" << accessId << "): " << ( ref.desired ? "" : "not " )
      << "desired (report by " << ref.verifier->info() << ")" << std::endl;
  return ref.desired;
}

// ---------------------------------------------------------------
bool MediaManager::isDesiredMedia(
  MediaAccessId accessId, const MediaVerifierRef &verifier ) const
{
  ...
  DBG << "isDesiredMedia(" << accessId << "): " << ( desired ? "" : "not " )
      << "desired (report by " << v->info() << ")" << std::endl;
  return desired;
}

// ---------------------------------------------------------------
bool MediaManager::isChangeable( MediaAccessId accessId )
{
  return url( accessId ).getScheme() == "cd" ||
         url( accessId ).getScheme() == "dvd";
}

// ---------------------------------------------------------------
Pathname MediaManager::localRoot( MediaAccessId accessId ) const
{
  MutexLock glock( g_Mutex );
...
// ---------------------------------------------------------------
Pathname MediaManager::localPath(
  MediaAccessId accessId, const Pathname &pathname ) const
{
  MutexLock glock( g_Mutex );
...
// ---------------------------------------------------------------
void MediaManager::provideFile(
  MediaAccessId accessId, const Pathname &filename ) const
{
  MutexLock glock( g_Mutex );
...
// ---------------------------------------------------------------
void MediaManager::setDeltafile(
  MediaAccessId accessId, const Pathname &filename ) const
{
  MutexLock glock( g_Mutex );
...
// ---------------------------------------------------------------
void MediaManager::provideDir(
  MediaAccessId accessId, const Pathname &dirname ) const
{
  MutexLock glock( g_Mutex );
...
// ---------------------------------------------------------------
void MediaManager::provideDirTree(
  MediaAccessId accessId, const Pathname &dirname ) const
{
  MutexLock glock( g_Mutex );
...
// ---------------------------------------------------------------
void MediaManager::dirInfo( MediaAccessId accessId,
...
// ---------------------------------------------------------------
bool MediaManager::doesFileExist(
  MediaAccessId accessId, const Pathname &filename ) const
{
  MutexLock glock( g_Mutex );
  ManagedMedia &ref( m_impl->findMM( accessId ) );
...
// ---------------------------------------------------------------
void MediaManager::getDetectedDevices( MediaAccessId accessId,
...
// ---------------------------------------------------------------
// STATIC
time_t MediaManager::getMountTableMTime()
{
  MutexLock glock( g_Mutex );
...
// ---------------------------------------------------------------
// STATIC
MountEntries MediaManager::getMountEntries()
{
  MutexLock glock( g_Mutex );
...
// ---------------------------------------------------------------
bool MediaManager::isUseableAttachPoint( const Pathname &path, bool mtab ) const
{
  ...
  for ( ; m != m_impl->mediaMap.end(); ++m )
  {
    ...
    if ( ret.mediaSource && ret.attachPoint )
    {
      ...
      else if ( mnt.size() > our.size() && mnt.at( our.size() ) == '/' &&
...
// ---------------------------------------------------------------
AttachedMedia MediaManager::getAttachedMedia( MediaAccessId &accessId ) const
{
  MutexLock glock( g_Mutex );
...
// ---------------------------------------------------------------
AttachedMedia MediaManager::findAttachedMedia(
...
// ---------------------------------------------------------------
void MediaManager::forceReleaseShared( const MediaSourceRef &media )
{
  MutexLock glock( g_Mutex );
