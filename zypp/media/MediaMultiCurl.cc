...
#define CURLVERSION_AT_LEAST( M, N, O )                                        \
...
private:
  ...
  static size_t _writefunction(
    void *ptr, size_t size, size_t nmemb, void *stream );

  size_t headerfunction( char *ptr, size_t size );
  static size_t _headerfunction(
    void *ptr, size_t size, size_t nmemb, void *stream );
...
{
public:
  multifetchrequest( const MediaMultiCurl *context, const Pathname &filename,
    const Url &baseurl, CURLM *multi, FILE *fp,
...
size_t multifetchworker::writefunction( void *ptr, size_t size )
{
  size_t len, cnt;
...
size_t multifetchworker::_writefunction(
  void *ptr, size_t size, size_t nmemb, void *stream )
{
...
size_t multifetchworker::headerfunction( char *p, size_t size )
{
  size_t l = size;
  ...
  {
    WAR << "#" << _workerno << ": setting request filesize to " << filesize
        << endl;
...
size_t multifetchworker::_headerfunction(
  void *ptr, size_t size, size_t nmemb, void *stream )
{
  multifetchworker *me = reinterpret_cast<multifetchworker *>( stream );
...
multifetchworker::multifetchworker(
  int no, multifetchrequest &request, const Url &url )
  : MediaCurl( url, Pathname() )
{
  ...
  if ( _request->_filesize == off_t( -1 ) || !_request->_blklist ||
       !_request->_blklist->haveChecksum( 0 ) )
  {
  ...
  if ( url.getHost() == _request->_context->_url.getHost() )
  {
    ...
    {
      curl_easy_setopt(
        _curl, CURLOPT_USERPWD, _settings.userPassword().c_str() );
...
void multifetchworker::checkdns()
{
  ...
  {
    std::transform(
      schemeproxy.begin(), schemeproxy.end(), schemeproxy.begin(), ::toupper );
    if ( env_isset( schemeproxy ) )
      return;
  }
...
void multifetchworker::adddnsfd( fd_set &rset, int &maxfd )
{
...
void multifetchworker::dnsevent( fd_set &rset )
{
...
bool multifetchworker::checkChecksum()
{
...
bool multifetchworker::recheckChecksum()
{
...
void multifetchworker::stealjob()
{
  ...
  std::list<multifetchworker *>::iterator workeriter =
    _request->_workers.begin();
  double now = 0;
  for ( ; workeriter != _request->_workers.end(); ++workeriter )
  {
    ...
    if ( worker->_state == WORKER_DISCARD || worker->_state == WORKER_DONE ||
         worker->_state == WORKER_SLEEP || !worker->_blksize )
    ...
    if ( !worker->_avgspeed && worker->_blkreceived )
    {
      ...
      if ( now > worker->_blkstarttime )
        worker->_avgspeed =
          worker->_blkreceived / ( now - worker->_blkstarttime );
    }
    ...
    {
      if ( ( worker->_blksize - worker->_blkreceived ) * best->_avgspeed <
           ( best->_blksize - best->_blkreceived ) * worker->_avgspeed )
        best = worker;
    }
    else
    {
  ...
  if ( _state != WORKER_SLEEP )
  {
    ...
    XXX << "me #" << _workerno << ": " << _avgspeed << ", size "
        << best->_blksize << endl;
    XXX << "best #" << best->_workerno << ": " << best->_avgspeed << ", size "
        << ( best->_blksize - best->_blkreceived ) << endl;
    if ( _avgspeed && best->_avgspeed &&
         best->_blksize - best->_blkreceived > 0 &&
         ( best->_blksize - best->_blkreceived ) * _avgspeed <
           best->_blksize * best->_avgspeed )
    {
      if ( !now )
        now = currentTime();
      ...
      if ( sl > 1 )
        sl = 1;
      XXX << "#" << _workerno << ": going to sleep for " << sl * 1000 << " ms"
          << endl;
      _sleepuntil = now + sl;
      _state = WORKER_SLEEP;
...
void multifetchworker::disableCompetition()
{
  std::list<multifetchworker *>::iterator workeriter =
    _request->_workers.begin();
  for ( ; workeriter != _request->_workers.end(); ++workeriter )
...
void multifetchworker::run()
{
  ...
  if ( _noendrange )
    ...
    sprintf( rangebuf, "%llu-%llu", (unsigned long long)_blkstart,
      (unsigned long long)_blkstart + _blksize - 1 );
  XXX << "#" << _workerno << ": BLK " << _blkno << ":" << rangebuf << " "
      << _url << endl;
  if ( curl_easy_setopt( _curl, CURLOPT_RANGE,
         !_noendrange || _blkstart != 0 ? rangebuf : (char *)0 ) != CURLE_OK )
  {
    _request->_activeworkers--;
    _state = WORKER_BROKEN;
...
multifetchrequest::multifetchrequest( const MediaMultiCurl *context,
  const Pathname &filename, const Url &baseurl, CURLM *multi, FILE *fp,
  callback::SendReport<DownloadProgressReport> *report, MediaBlockList *blklist,
  off_t filesize )
  : _context( context )
  , _filename( filename )
  , _baseurl( baseurl )
...
{
  for ( std::list<multifetchworker *>::iterator workeriter = _workers.begin();
        workeriter != _workers.end(); ++workeriter )
  {
    multifetchworker *worker = *workeriter;
    *workeriter = NULL;
    delete worker;
...
void multifetchrequest::run( std::vector<Url> &urllist )
{
  int workerno = 0;
  ...
  for ( ;; )
  {
         ...
         _workers.size() < MAXURLS )
    {
      ...
      multifetchworker *worker =
        new multifetchworker( workerno++, *this, *urliter );
    ...
    if ( !_activeworkers )
    {
            ...
            workeriter != _workers.end(); ++workeriter )
      {
        if ( ( *workeriter )->_state != WORKER_BROKEN )
          continue;
        ZYPP_THROW( MediaCurlException(
          _baseurl, "Server error", ( *workeriter )->_curlError ) );
      }
      break;
    }
    ...
    if ( r == -1 && errno != EINTR )
      ZYPP_THROW(
        MediaCurlException( _baseurl, "select() failed", "unknown error" ) );
    ...
    for ( ;; )
    {
      ...
      if ( mcode != CURLM_OK )
        ZYPP_THROW( MediaCurlException(
          _baseurl, "curl_multi_perform", "unknown error" ) );
      break;
    }
    ...
    if ( now > _lastperiodstart + .5 )
    {
      if ( !_periodavg )
        _periodavg =
          ( _fetchedsize - _lastperiodfetched ) / ( now - _lastperiodstart );
      else
        _periodavg = ( _periodavg +
                       ( _fetchedsize - _lastperiodfetched ) /
                         ( now - _lastperiodstart ) ) /
                     2;
    ...
    while ( ( msg = curl_multi_info_read( _multi, &nqueue ) ) != 0 )
    {
      ...
      if ( curl_easy_getinfo( easy, CURLINFO_PRIVATE, &worker ) != CURLE_OK )
        ZYPP_THROW( MediaCurlException(
          _baseurl, "curl_easy_getinfo", "unknown error" ) );
      ...
      XXX << "#" << worker->_workerno << ": BLK " << worker->_blkno
          << " done code " << cc << " speed " << worker->_avgspeed << endl;
      ...
      if ( cc == CURLE_HTTP_RETURNED_ERROR )
      {
        ...
        if ( statuscode == 416 && !_blklist ) /* Range error */
        {
          if ( _filesize == off_t( -1 ) )
          {
            ...
            {
              XXX << "#" << worker->_workerno << ": retrying with no end range"
                  << endl;
      ...
      if ( cc == 0 )
      {
        ...
        {
          WAR << "#" << worker->_workerno << ": checksum error, disable worker"
              << endl;
        ...
        if ( maxavg && !_stealing )
        {
          ...
          if ( ratio > .01 )
          {
            XXX << "#" << worker->_workerno << ": too slow (" << ratio << ", "
                << worker->_avgspeed << ", #" << maxworkerno << ": " << maxavg
                << "), going to sleep for " << ratio * 1000 << " ms" << endl;
      ...
      else
      {
        ...
        _activeworkers--;
        if ( !_activeworkers &&
             !( urliter != urllist.end() && _workers.size() < MAXURLS ) )
        {
    ...
    if ( _report )
    {
      int percent = _totalsize
                      ? ( 100 * ( _fetchedgoodsize + _fetchedsize ) ) /
                          ( _totalsize + _fetchedsize )
                      : 0;
      double avg = 0;
      if ( now > _starttime )
        avg = _fetchedsize / ( now - _starttime );
      if ( !( *( _report ) )
              ->progress( percent, _baseurl, avg,
                _lastperiodstart == _starttime ? avg : _periodavg ) )
  ...
  {
    multifetchworker *worker = *workeriter;
    WAR << "#" << worker->_workerno << ": state: " << worker->_state
        << " received: " << worker->_received << " url: " << worker->_url
        << endl;
  }
}
...
MediaMultiCurl::MediaMultiCurl(
  const Url &url_r, const Pathname &attach_point_hint_r )
  : MediaCurl( url_r, attach_point_hint_r )
{
  MIL << "MediaMultiCurl::MediaMultiCurl(" << url_r << ", "
...
void MediaMultiCurl::setupEasy()
{
  ...
  for ( ; sl; sl = sl->next )
    _customHeadersMetalink =
      curl_slist_append( _customHeadersMetalink, sl->data );
  _customHeadersMetalink = curl_slist_append( _customHeadersMetalink,
...
static bool looks_like_metalink_fd( int fd )
{
  char buf[ 256 ], *p;
  int l;
  while ( ( l = pread( fd, buf, sizeof( buf ) - 1, (off_t)0 ) ) == -1 &&
          errno == EINTR )
    ;
  if ( l == -1 )
    return 0;
...
int MediaMultiCurl::progressCallback(
  void *clientp, double dltotal, double dlnow, double ultotal, double ulnow )
{
  ...
  if ( !ismetalink )
  {
    ...
    if ( curl_easy_getinfo( _curl, CURLINFO_PRIVATE, &fp ) != CURLE_OK || !fp )
      return MediaCurl::aliveCallback(
  ...
  if ( ismetalink )
  {
    ...
    curl_easy_setopt(
      _curl, CURLOPT_PROGRESSFUNCTION, &MediaCurl::aliveCallback );
    return MediaCurl::aliveCallback( clientp, dltotal, dlnow, ultotal, ulnow );
  }
  curl_easy_setopt(
    _curl, CURLOPT_PROGRESSFUNCTION, &MediaCurl::progressCallback );
  return MediaCurl::progressCallback( clientp, dltotal, dlnow, ultotal, ulnow );
}

void MediaMultiCurl::doGetFileCopy( const Pathname &filename,
  const Pathname &target, callback::SendReport<DownloadProgressReport> &report,
  RequestOptions options ) const
{
  ...
  if ( assert_dir( dest.dirname() ) )
  {
    ...
    ZYPP_THROW( MediaSystemException(
      url, "System error on " + dest.dirname().asString() ) );
  }
  ...
  if ( !buf )
  {
    ...
    Url url( getFileUrl( filename ) );
    ZYPP_THROW(
      MediaSystemException( url, "out of memory for temp file name" ) );
  }
  ...
  if ( PathInfo( target ).isExist() && !( options & OPTION_NO_IFMODSINCE ) )
  {
    ...
    curl_easy_setopt(
      _curl, CURLOPT_TIMEVALUE, (long)PathInfo( target ).mtime() );
  }
  else
  {
  ...
  long httpReturnCode = 0;
  CURLcode infoRet =
    curl_easy_getinfo( _curl, CURLINFO_RESPONSE_CODE, &httpReturnCode );
  if ( infoRet == CURLE_OK )
  {
    DBG << "HTTP response: " + str::numstring( httpReturnCode ) << endl;
    if ( httpReturnCode == 304 ||
         ( httpReturnCode == 213 &&
           _url.getScheme() == "ftp" ) ) // not modified
    {
  ...
  if ( ismetalink )
  {
    ...
    file = NULL;
    Pathname failedFile =
      ZConfig::instance().repoCachePath() / "MultiCurl.failed";
    try
    {
      MetaLinkParser mlp;
    ...
    catch ( Exception &ex )
    {
      ...
      if ( !file )
        ZYPP_THROW( MediaWriteException( destNew ) );
      MediaCurl::doGetFileCopyFile(
        filename, dest, file, report, options | OPTION_NO_REPORT_START );
    }
  }
...
void MediaMultiCurl::multifetch( const Pathname &filename, FILE *fp,
  ...
  off_t filesize ) const
{
  ...
  multifetchrequest req(
    this, filename, baseurl, _multi, fp, report, blklist, filesize );
  ...
  for ( std::vector<Url>::iterator urliter = urllist->begin();
        urliter != urllist->end(); ++urliter )
  {
    try
    {
...
void MediaMultiCurl::checkFileDigest(
  Url &url, FILE *fp, MediaBlockList *blklist ) const
{
  ...
  if ( !blklist->verifyFileDigest( dig ) )
    ZYPP_THROW(
      MediaCurlException( url, "file verification failed", "checksum error" ) );
}
