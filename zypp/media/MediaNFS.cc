...
//	DESCRIPTION :
//
MediaNFS::MediaNFS( const Url &url_r, const Pathname &attach_point_hint_r )
  : MediaHandler( url_r, attach_point_hint_r,
      "/",    // urlpath at attachpoint
      false ) // does_download
{
  MIL << "MediaNFS::MediaNFS(" << url_r << ", " << attach_point_hint_r << ")"
      << endl;
}
...
void MediaNFS::attachTo( bool next )
{
  ...
  bool contains_lock = false, contains_soft = false, contains_timeo = false,
       contains_hard = false;

  for ( it = optionList.begin(); it != optionList.end(); ++it )
  {
    if ( *it == "lock" || *it == "nolock" )
      contains_lock = true;
    else if ( *it == "soft" )
      contains_soft = true;
    else if ( *it == "hard" )
      contains_hard = true;
    else if ( it->find( "timeo" ) != string::npos )
      contains_timeo = true;
  }
...
//
bool MediaNFS::isAttached() const { return checkAttached( true ); }
