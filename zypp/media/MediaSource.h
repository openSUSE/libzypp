...
public:
  ...
  virtual bool equals( const MediaSource &src ) const
  {
    if ( type == src.type )
    {
      if ( maj_nr == 0 )
        ...
        return maj_nr == src.maj_nr && min_nr == src.min_nr;
    }
    return false;
  }
  ...
  virtual std::string asString() const
  {
    ...
    {
      tmp1 =
        "[" + str::numstring( maj_nr ) + "," + str::numstring( min_nr ) + "]";
    }
...
struct AttachedMedia
{
  ...
  AttachedMedia(
    const MediaSourceRef &_mediaSource, const AttachPointRef &_attachPoint )
