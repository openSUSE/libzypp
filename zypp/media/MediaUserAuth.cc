...
bool AuthData::valid() const { return username().size() && password().size(); }

std::ostream &AuthData::dumpOn( std::ostream &str ) const
{
  if ( _url.isValid() )
    str << "["
        << _url.asString( url::ViewOptions() - url::ViewOptions::WITH_USERNAME -
...
long CurlAuthData::auth_type_str2long( std::string &auth_type_str )
{
  ...
  for ( it = list.begin(); it != list.end(); ++it )
  {
    ...
    else if ( ( curl_info && ( curl_info->features & CURL_VERSION_NTLM ) ) &&
              ( *it == "ntlm" ) )
    {
      auth_type |= CURLAUTH_NTLM;
    }
    else if ( ( curl_info && ( curl_info->features & CURL_VERSION_SPNEGO ) ) &&
              ( *it == "spnego" || *it == "negotiate" ) )
    {
      ...
      auth_type |= CURLAUTH_GSSNEGOTIATE;
                ...
                ( curl_info->features & CURL_VERSION_GSSNEGOTIATE ) ) &&
    ...
    {
      ZYPP_THROW( MediaException(
        str::Format( _( "Unsupported HTTP authentication method '%s'" ) ) %
        *it ) );
    }
  }
