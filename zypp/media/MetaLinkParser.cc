...
static struct stateswitch stateswitches[] = {
  ...
  {STATE_FILES, "file", STATE_FILE, 0}, {STATE_FILE, "size", STATE_SIZE, 1},
  ...
  {STATE_M4PIECES, "hash", STATE_M4PHASH, 1}, {NUMSTATES}};
  ...
  void *userData, const xmlChar *name, const xmlChar **atts )
{
  ...
  if ( !pd->swtab[ pd->state ] )
    return;
  for ( sw = pd->swtab[ pd->state ]; sw->from == pd->state;
        sw++ ) /* find name in statetable */
  ...
  *pd->content = 0;
  switch ( pd->state )
  {
    ...
    case STATE_M4PHASH:
    {
      ...
      if ( pd->state == STATE_PHASH &&
           ( !piece || atoi( piece ) != pd->npiece ) )
      {
        pd->state = pd->sbtab[ pd->state ];
        pd->statedepth--;
      }
...
static void XMLCALL endElement( void *userData, const xmlChar *name )
{
  ...
  switch ( pd->state )
  {
    ...
    case STATE_M4HASH:
      ...
      if ( strlen( pd->content ) != size_t( pd->chksuml ) * 2 ||
           !hexstr2bytes( &pd->chksum[ 0 ], pd->content, pd->chksuml ) )
      {
        pd->chksum.clear();
        pd->chksuml = 0;
      }
    ...
    case STATE_M4PHASH:
      ...
      if ( !hexstr2bytes(
             &pd->piece[ pd->piecel * pd->npiece ], pd->content, pd->piecel ) )
      {
...
MetaLinkParser::~MetaLinkParser() { delete pd; }

void MetaLinkParser::parse( const Pathname &filename )
{
  parse( InputStream( filename ) );
}

void MetaLinkParser::parse( const InputStream &is )
{
  char buf[ 4096 ];
  if ( !is.stream() )
...
void MetaLinkParser::parseBytes( const char *buf, size_t len )
{
  if ( !len )
    return;
...
std::vector<Url> MetaLinkParser::getUrls()
{
  std::vector<Url> urls;
  int i;
...
MediaBlockList MetaLinkParser::getBlockList()
{
  size_t i;
    ...
    for ( i = 0; i < nb; i++ )
    {
      ...
      if ( int( i ) < pd->nsha1 )
      {
        ...
        if ( int( i ) < pd->nzsync )
        {
          ...
          bl.setRsum( blkno, 4,
            p[ 0 ] | p[ 1 ] << 8 | p[ 2 ] << 16 | p[ 3 ] << 24, pd->blksize );
        }
      }
      off += pd->blksize;
    }
