...
void ZsyncParser::parse( string filename )
{
  char buf[ 4096 ];
  ...
  while ( is.good() )
  {
    ...
    if ( !strncmp( buf, "Length: ", 8 ) )
      ...
      blksize = atoi( buf + 11 );
    else if ( !strncmp( buf, "URL: http://", 12 ) ||
              !strncmp( buf, "URL: https://", 13 ) ||
              !strncmp( buf, "URL: ftp://", 11 ) ||
              !strncmp( buf, "URL: tftp://", 12 ) )
  ...
  if ( filesize )
  {
    ...
    for ( i = 0; i < nblks; i++ )
    {
      ...
      bl.setRsum( blkno, rsl,
        rp[ 0 ] << 24 | rp[ 1 ] << 16 | rp[ 2 ] << 8 | rp[ 3 ], blksize );
      is.read( (char *)rp, csl );
...
std::vector<Url> ZsyncParser::getUrls()
{
  std::vector<Url> ret;
  size_t i;
...
MediaBlockList ZsyncParser::getBlockList() { return bl; }
