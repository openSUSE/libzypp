...
void ProgressTracker::updateStats(double dltotal, double dlnow)
{
  ...
  if ( _timeLast < now )
  {
    auto duration = std::chrono::duration_cast<std::chrono::seconds>(now - *_timeLast).count();
    if ( duration >= 1 ) {
      _drateLast = (_dnlNow - _dnlLast) / std::max( std::chrono::duration_cast<std::chrono::seconds>(now - *_timeLast).count() , int64_t(1) );
      // start new period
      _timeLast  = now;
      _dnlLast   = _dnlNow;
    }
