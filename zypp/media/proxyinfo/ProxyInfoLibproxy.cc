...
static pxProxyFactory *getProxyFactory()
{
  ...
  if ( sysconfigProxy.hasChanged() )
  {
    ...
    TmpUnsetEnv envguard[] __attribute__( ( __unused__ ) ) = {
...
std::string ProxyInfoLibproxy::proxy( const Url &url_r ) const
{
  ...
  const url::ViewOption vopt =
    url::ViewOption::WITH_SCHEME + url::ViewOption::WITH_HOST +
    url::ViewOption::WITH_PORT + url::ViewOption::WITH_PATH_NAME;

  char **proxies = px_proxy_factory_get_proxies(
    _factory, (char *)url_r.asString( vopt ).c_str() );
  if ( !proxies )
    return "";
