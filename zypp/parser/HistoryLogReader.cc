...
{
  Impl( const Pathname &historyFile_r, const Options &options_r,
  ...
  void readFrom(
    const Date &date_r, const ProgressData::ReceiverFnc &progress_r );
  void readFromTo( const Date &fromDate_r, const Date &toDate_r,
...
bool HistoryLogReader::Impl::parseLine(
  const std::string &line_r, unsigned lineNr_r )
{
  // parse into fields
  ...
  if ( fields.size() >= 2 )
    str::trim(
      fields[ 1 ] ); // for whatever reason writer is padding the action field
  ...
  catch ( const Exception &excpt )
  {
    ...
    {
      ERR << "Invalid history log entry on line #" << lineNr_r << " '" << line_r
          << "'" << endl;
      ParseException newexcpt(
        str::Str() << "Error in history log on line #" << lineNr_r );
  ...
  {
    WAR << "Stop parsing requested by consumer callback on line #" << lineNr_r
        << endl;
    return false;
  }
  return true;
...
void HistoryLogReader::Impl::readAll(
  const ProgressData::ReceiverFnc &progress_r )
{
...
void HistoryLogReader::Impl::readFrom(
  const Date &date_r, const ProgressData::ReceiverFnc &progress_r )
{
  InputStream is( _filename );
  iostr::EachLine line( is );
...
void HistoryLogReader::Impl::readFromTo( const Date &fromDate_r,
  const Date &toDate_r, const ProgressData::ReceiverFnc &progress_r )
{
  InputStream is( _filename );
  iostr::EachLine line( is );
...
HistoryLogReader::HistoryLogReader( const Pathname &historyFile_r,
  const Options &options_r, const ProcessData &callback_r )
...
void HistoryLogReader::readFrom(
  const Date &date_r, const ProgressData::ReceiverFnc &progress_r )
{
  _pimpl->readFrom( date_r, progress_r );
}

void HistoryLogReader::readFromTo( const Date &fromDate_r, const Date &toDate_r,
  const ProgressData::ReceiverFnc &progress_r )
{
