...
//
IniDict::IniDict(
  const InputStream &is, const ProgressData::ReceiverFnc &progress )
{
  read( is, progress );
}
...
IniDict::entry_const_iterator IniDict::entriesEnd(
  const std::string &section ) const
{
  SectionSet::const_iterator secit = _dict.find( section );
  if ( secit == _dict.end() )
