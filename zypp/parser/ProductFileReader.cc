...
std::string ProductFileData::Upgrade::summary() const
{
  return _pimpl->_summary;
}
std::string ProductFileData::Upgrade::repository() const
{
  return _pimpl->_repository;
}
std::string ProductFileData::Upgrade::product() const
{
  return _pimpl->_product;
}
...
std::string ProductFileData::productline() const
{
  return _pimpl->_productline;
}
std::string ProductFileData::registerTarget() const
{
  return _pimpl->_registerTarget;
}
std::string ProductFileData::registerRelease() const
{
  return _pimpl->_registerRelease;
}
std::string ProductFileData::registerFlavor() const
{
  return _pimpl->_registerFlavor;
}

std::string ProductFileData::updaterepokey() const
{
  return _pimpl->_updaterepokey;
}

const ProductFileData::Upgrades &ProductFileData::upgrades() const
{
  return _pimpl->_upgrades;
}

std::ostream &operator<<( std::ostream &str, const ProductFileData &obj )
{
  str << str::form( "|product|%s|%s|%s|%s|", obj.name().c_str(),
    obj.edition().c_str(), obj.arch().c_str(), obj.vendor().c_str() );
  if ( !obj.upgrades().empty() )
  {
    for_( it, obj.upgrades().begin(), obj.upgrades().end() ) str << endl
                                                                 << "    "
                                                                 << *it;
  }
  return str;
...
std::ostream &operator<<(
  std::ostream &str, const ProductFileData::Upgrade &obj )
{
  str << str::form( "|upgrade|%s|%s|%s|%s|%s|", obj.name().c_str(),
    obj.repository().c_str(), obj.product().c_str(), obj.status().c_str(),
...
struct ProductNode : public xml::ParseDef
{
  ProductNode( ProductFileData::Impl &pdata_r )
    ...
    , _pdata( pdata_r )
  {
    ( *this )( "vendor", OPTIONAL, xml::parseDefAssign( _pdata._vendor ) )(
      ...
      "arch", MANDTAORY, xml::parseDefAssign( _pdata._arch ) )( "shortsummary",
      OPTIONAL, xml::parseDefAssign( _pdata._shortName ) )( "summary",
      MULTIPLE_OPTIONAL, xml::parseDefAssign( _ttext )( "lang", _tlocale ) >>
                           bind( &ProductNode::doneLocalizedDefault, this, _1,
                             boost::ref( _pdata._summary ) ) )(
      "productline", OPTIONAL, xml::parseDefAssign( _pdata._productline ) )(
      "register", OPTIONAL )( "updaterepokey", OPTIONAL,
      xml::parseDefAssign( _pdata._updaterepokey ) )( "upgrades", OPTIONAL );
    ...
    ( *this )[ "upgrades" ]( "upgrade", MULTIPLE_OPTIONAL,
      xml::parseDefAssign() >> bind( &ProductNode::doneUpgrade, this, _1 ) );
    ...
    setConsumer(
      xml::parseDefAssign() >> bind( &ProductNode::done, this, _1 ) );
  ...
  {
    ProductFileData::Upgrade cdata(
      new ProductFileData::Upgrade::Impl( _upgrade ) );
...
bool ProductFileReader::scanDir(
  const Consumer &consumer_r, const Pathname &dir_r )
{
