...
{
public:
  RepoFileParser( const InputStream &is_r ) { read( is_r ); }

  using IniDict::consume; // don't hide overloads we don't redefine here

  virtual void consume( const std::string &section_r, const std::string &key_r,
  ...
  virtual void garbageLine(
    const std::string &section_r, const std::string &line_r )
  {
    switch ( _inMultiline )
    {
...
RepoFileReader::RepoFileReader( const Pathname &repo_file,
  const ProcessRepo &callback, const ProgressData::ReceiverFnc &progress )
  : _callback( callback )
{
