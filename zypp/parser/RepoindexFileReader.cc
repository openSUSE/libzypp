  ...
  std::string replace( const std::string &val_r ) const
  {
    ...
    for ( ; vbeg != std::string::npos; vbeg = val_r.find( "%{", vbeg ) )
    {
      ...
      if ( iter != _vars.end() )
      {
        ...
        WAR << "Undefined variable %{" << val_r.substr( nbeg, nend - nbeg )
            << "} in '" << val_r << "'" << endl;
      vbeg = nend + 1;
    }
    if ( cbeg < val_r.size() )
  ...
  : _callback( callback )
{
  ...
  reader.foreachNode(
    bind( &RepoindexFileReader::Impl::consumeNode, this, _1 ) );
}
    ...
    if ( reader_r->name() == "repo" )
    {
      ...
      if ( getAttrValue( "alias", reader_r, attrValue ) )
      {
        ...
        throw ParseException(
          str::form( _( "Required attribute '%s' is missing." ), "alias" ) );
      ...
      // SLES HACK: or path, but beware of the hardcoded '/repo' prefix!
      {
        ...
        {
          if ( pathstr.empty() )
            throw ParseException( str::form(
              _( "One or both of '%s' or '%s' attributes is required." ), "url",
              "path" ) );
          else
