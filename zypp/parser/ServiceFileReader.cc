...
public:
  static void parseServices(
    const Pathname &file, const ServiceFileReader::ProcessService &callback );
};

void ServiceFileReader::Impl::parseServices(
  const Pathname &file, const ServiceFileReader::ProcessService &callback /*,
                                  const ProgressData::ReceiverFnc &progress*/ )
{
  ...
  for ( parser::IniDict::section_const_iterator its = dict.sectionsBegin();
        its != dict.sectionsEnd(); ++its )
  {
    MIL << ( *its ) << endl;

    ServiceInfo service( *its );
    std::map<std::string, std::pair<std::string, ServiceInfo::RepoState>>
      repoStates; // <repo_NUM,< alias,RepoState >>

    for ( IniDict::entry_const_iterator it = dict.entriesBegin( *its );
          it != dict.entriesEnd( *its ); ++it )
    {
      // MIL << (*it).first << endl;
      if ( it->first == "name" )
      ...
      else if ( str::startsWith( it->first, "repo_" ) )
      {
        ...
        if ( str::regex_match( it->first.c_str() + 5 /*repo_*/, what, rxexpr ) )
        {
          ...
          if ( what.size() > 3 )
          {
            // attribute
            if ( what[ 3 ] == "enabled" )
              repoStates[ tag ].second.enabled =
                str::strToBool( it->second, repoStates[ tag ].second.enabled );
            else if ( what[ 3 ] == "autorefresh" )
              repoStates[ tag ].second.autorefresh = str::strToBool(
                it->second, repoStates[ tag ].second.autorefresh );
    ...
    if ( !repoStates.empty() )
    {
      ...
      for ( const auto &el : repoStates )
      {
        if ( el.second.first.empty() )
          ERR << "Missing alias for repo_" << el.first << "; ignore entry"
              << endl;
        else
...
ServiceFileReader::ServiceFileReader(
  const Pathname &repo_file, const ProcessService &callback /*,
                                    const ProgressData::ReceiverFnc &progress */ )
{
