...
public:
  ...
  bool hasRepoIndex() const { return _repoindex != nullptr; }
...
public:
  bool setFileCheckSum(
    std::map<std::string, CheckSum> &map_r, const std::string &value ) const
  {
    bool error = false;
...
//
std::string ContentFileReader::errPrefix(
  unsigned lineNo_r, const std::string &msg_r, const std::string &line_r ) const
{
  return str::form( "%s:%u:%s | %s", _pimpl->_inputname.c_str(), lineNo_r,
    line_r.c_str(), msg_r.c_str() );
}
...
//	METHOD TYPE : void
//
void ContentFileReader::parse(
  const InputStream &input_r, const ProgressData::ReceiverFnc &fnc_r )
{
  ...
  for ( ; line; line.next() )
  {
    ...
    else if ( key == "KEY" )
    {
      ...
      {
        ZYPP_THROW( ParseException( errPrefix( line.lineNo(),
          "Expected [KEY algorithm checksum filename]", *line ) ) );
      }
    }
    else if ( key == "META" )
    {
      if ( _pimpl->setFileCheckSum(
             _pimpl->repoindex().metaFileChecksums, value ) )
      {
        ZYPP_THROW( ParseException( errPrefix(
          line.lineNo(), "Expected [algorithm checksum filename]", *line ) ) );
      }
    }
    else if ( key == "HASH" )
    {
