...
public:
  ...
  ~Impl()
  {
    for ( SubNodes::iterator it = _subnodes.begin(); it != _subnodes.end();
          ++it )
    {
  ...
  {
    return Traits::ModeBits( _mode ).isEqual<Traits::TypeBits>(
      Traits::BIT_OPTIONAL );
  }
  ...
  {
    return Traits::ModeBits( _mode ).isEqual<Traits::TypeBits>(
      Traits::BIT_MANDTAORY );
  }

  bool singleDef() const
  {
    return Traits::ModeBits( _mode ).isEqual<Traits::VisitBits>(
      Traits::BIT_ONCE );
  }
...
private:
  ...
  std::string exstr( const std::string &what_r, const Impl &impl_r,
...
void ParseDef::Impl::addNode( const ImplPtr &subnode_r )
{
  ...
  if ( !res.second )
  {
    ZYPP_THROW( ParseDefBuildException(
      exstr( "Multiple definiton of subnode " + subnode_r->_name, *this ) ) );
  }
  if ( res.first->second->_parent )
  {
    ZYPP_THROW( ParseDefBuildException(
      exstr( "Can not reparent subnode " + subnode_r->_name, *this ) ) );
  }
...
void ParseDef::Impl::take( Reader &reader_r )
{
  if ( reader_r->nodeType() != XML_READER_TYPE_ELEMENT )
  {
    if ( reader_r->depth() == 0 )
    {
      ...
      do
      {
        ...
        {
          ZYPP_THROW(
            ParseDefValidateException( exstr( "Unexpected EOF ", *this ) ) );
        }
    ...
    else
    {
      ZYPP_THROW( ParseDefValidateException(
        exstr( "Expected ELEMENT", *this, reader_r ) ) );
    }
  }
  if ( reader_r->name() != _name )
  {
    ZYPP_THROW( ParseDefValidateException(
      exstr( "Wrong ELEMENT name", *this, reader_r ) ) );
  }
  if ( _visited >= 1 && !multiDef() )
  {
    ZYPP_THROW( ParseDefValidateException(
      exstr( "Multiple definitions", *this, reader_r ) ) );
  }
      ...
      switch ( reader_r->nodeType() )
      {
        case XML_READER_TYPE_ELEMENT:
          ...
          // corresponding end node, or an exception was thrown.
          {
            ...
            {
              if ( ParseDef::_debug )
                WAR << "Skip unknown node " << *reader_r << " in " << *this
                    << endl;
              skipNode( reader_r );
            }
          }
        ...
        case XML_READER_TYPE_END_ELEMENT:
          ...
          if ( reader_r->depth() == _parseDepth && reader_r->name() == _name )
          {
            done = true;
          }
          else
          {
            ZYPP_THROW( ParseDefValidateException(
              exstr( "unexpected END_ELEMENT name", *this, reader_r ) ) );
          }
          break;
  ...
  for ( SubNodes::iterator it = _subnodes.begin(); it != _subnodes.end(); ++it )
  {
    ...
    {
      ZYPP_THROW( ParseDefValidateException(
        exstr( "Mandatory ELEMENT missing", *( it->second ), reader_r ) ) );
    }
...
{
  if ( !reader_r.seekToEndNode(
         reader_r->depth(), reader_r->name().asString() ) )
  {
    ZYPP_THROW( ParseDefValidateException(
      exstr( str::form( "EOF while looking for [%d] <\\%s>", reader_r->depth(),
               reader_r->name().c_str() ),
        *this ) ) );
  }
  return true;
...
{
  return str << "ParseDef(" << obj._name << ", " << obj._mode << ", visits "
             << obj._visited << ")";
}
...
const std::string &ParseDef::name() const { return _pimpl->_name; }

ParseDef::Mode ParseDef::mode() const { return _pimpl->_mode; }

bool ParseDef::isOptional() const { return _pimpl->isOptional(); }

bool ParseDef::isMandatory() const { return _pimpl->isMandatory(); }

bool ParseDef::singleDef() const { return _pimpl->singleDef(); }

bool ParseDef::multiDef() const { return _pimpl->multiDef(); }

unsigned ParseDef::visited() const { return _pimpl->_visited; }

ParseDef &ParseDef::addNode( ParseDef &subnode_r )
{
  _pimpl->addNode( subnode_r._pimpl.getPtr() );
  return *this;
}
...
void ParseDef::cancelConsumer() { _pimpl->_callback.cancelRedirect(); }
...
void ParseDef::take( Reader &reader_r ) { _pimpl->take( reader_r ); }
