...
int ioclose( void * /*context_r*/ ) { return 0; }
...
void structuredErrorFunc( void *userData, xmlErrorPtr error )
{
  if ( error )
  {
    ...
    std::string err(
      str::form( "%s[%d] %s", Pathname::basename( error->file ).c_str(),
        error->line, str::stripSuffix( error->message, "\n" ).c_str() ) );
...
{
  ParseException()
    : Exception( "Parse error: " + ( structuredErrors.empty()
                                       ? std::string( "unknown error" )
                                       : structuredErrors.back() ) )
  {
...
//
Reader::Reader( const InputStream &stream_r, const Validate &validate_r )
  : _stream( stream_r )
...
bool Reader::seekToNode( int depth_r, const std::string &name_r )
{
  do
  {
    if ( _node.depth() == depth_r && _node.name() == name_r &&
