...
inline bool hardLockQueriesRemove(
  pool::PoolTraits::HardLockQueries &activeLocks_r, IdString ident_r )
{
...
inline bool hardLockQueriesAdd(
  pool::PoolTraits::HardLockQueries &activeLocks_r, IdString ident_r )
{
...
public:
  /** convenience. */
  const sat::Pool satpool() const { return sat::Pool::instance(); }

  /** Housekeeping data serial number. */
  const SerialNumber &serial() const { return satpool().serial(); }
...
public:
  /**  */
  bool empty() const { return satpool().solvablesEmpty(); }

  /**  */
  size_type size() const { return satpool().solvablesSize(); }
...
public:
  ...
  const HardLockQueries &hardLockQueries() const { return _hardLockQueries; }
  ...
  void reapplyHardLocks() const
  {
    ...
    for_( it, begin(), end() )
    {
      resstatus::UserLockQueryManip::reapplyLock(
        it->status(), locked.contains( *it ) );
    }
  }
  ...
  void setHardLockQueries( const HardLockQueries &newLocks_r )
  {
    ...
    for_( it, begin(), end() )
    {
      resstatus::UserLockQueryManip::setLock(
        it->status(), locked.contains( *it ) );
    }
  }
      ...
      if ( pool.capacity() )
      {
        for ( sat::detail::SolvableIdType i = pool.capacity() - 1; i != 0; --i )
        {
          ...
          else if ( reusedIDs || ( s && !pi ) )
          {
            // new PoolItem to add
            pi =
              PoolItem::makePoolItem( s ); // the only way to create a new one!
