...
struct ByPoolItem
{
  bool operator()( const PoolItem &pi ) const { return bool( pi ); }
};
...
{
  typename TPair::second_type &operator()( TPair &__x ) const
  {
    return __x.second;
  }

  const typename TPair::second_type &operator()( const TPair &__x ) const
  {
    return __x.second;
  }
};
...
public:
  ...
  /** ident index */
  typedef std::unordered_multimap<sat::detail::IdType, PoolItem> Id2ItemT;
