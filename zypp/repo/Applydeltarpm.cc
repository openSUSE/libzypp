...
/******************************************************************
      ...
      */
bool applydeltarpm(
  const char *const argv_r[], const Progress &report_r = Progress() )
{
...
bool check( const std::string &sequenceinfo_r, bool quick_r )
{
  ...
  const char *const argv[] = {"/usr/bin/applydeltarpm",
    ( quick_r ? "-C" : "-c" ), "-s", sequenceinfo_r.c_str(), NULL};
...
bool check( const Pathname &delta_r, bool quick_r )
{
  ...
  const char *const argv[] = {"/usr/bin/applydeltarpm",
    ( quick_r ? "-C" : "-c" ), delta_r.asString().c_str(), NULL};
...
/******************************************************************
    ...
    */
bool provide(
  const Pathname &delta_r, const Pathname &new_r, const Progress &report_r )
{
  // cleanup on error
  ...
  const char *const argv[] = {"/usr/bin/applydeltarpm", "-p",
    "-p", // twice to get percent output one per line
    delta_r.asString().c_str(), new_r.asString().c_str(), NULL};
...
bool provide( const Pathname &old_r, const Pathname &delta_r,
  const Pathname &new_r, const Progress &report_r )
{
  // cleanup on error
  AutoDispose<const Pathname> guard( new_r, filesystem::unlink );
  ...
  const char *const argv[] = {"/usr/bin/applydeltarpm", "-p",
    "-p", // twice to get percent output one per line
    "-r", old_r.asString().c_str(), delta_r.asString().c_str(),
    new_r.asString().c_str(), NULL};
