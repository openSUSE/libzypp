...
inline std::ostream &operator<<(
  std::ostream &str, const DeltaCandidates::Impl &obj )
{
...
DeltaCandidates::DeltaCandidates(
  const std::list<Repository> &repos, const std::string &pkgname )
...
std::list<DeltaRpm> DeltaCandidates::deltaRpms(
  const Package::constPtr &package ) const
{
  ...
  for_( rit, _pimpl->repos.begin(), _pimpl->repos.end() )
  {
    ...
    for_( it, q.begin(), q.end() )
    {
             ...
             _pimpl->pkgname )
      {
        ...
        if ( !package || ( package->name() == delta.name() &&
