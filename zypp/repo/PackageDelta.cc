...
DeltaRpm::DeltaRpm( sat::LookupAttr::iterator deltaInfo_r )
{
  ...
  _location.setLocation( str::form( "%s/%s-%s.%s", locdir.c_str(),
    locname.c_str(), locevr.c_str(), locsuffix.c_str() ) );

  _baseversion.setSequenceinfo(
    str::form( "%s-%s-%s", seqname.c_str(), seqevr.c_str(), seqnum.c_str() ) );
}

std::ostream &operator<<( std::ostream &str, const DeltaRpm &obj )
{
  return str << "DeltaRpm[" << obj.name() << "-" << obj.edition() << "."
             << obj.arch() << "](" << obj.location() << '|'
