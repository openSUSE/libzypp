...
bool PackageProviderPolicy::queryInstalled(
...
protected:
  ...
  RpmDb::CheckPackageResult packageSigCheck(
    const Pathname &path_r, bool isMandatory_r, UserData &userData ) const
  {
    if ( !_target )
    ...
    if ( _target )
    {
      ...
      detail.push_back( RpmDb::CheckPackageDetail::value_type(
        ret, "OOps. Target is not initialized!" ) );
  ...
  /** React on signature verification error user action
       ...
       */
  void resolveSignatureErrorAction(
    repo::DownloadResolvableReport::Action action_r ) const
  {
    switch ( action_r )
    {
  ...
  void defaultReportSignatureError( RpmDb::CheckPackageResult ret,
    const std::string &detail_r = std::string() ) const
  {
    str::Str msg;
    msg << _package->asUserString() << ": "
        << _( "Signature verification failed" ) << " " << ret;
    if ( !detail_r.empty() )
      msg << "\n" << detail_r;
    resolveSignatureErrorAction( report()->problem(
      _package, repo::DownloadResolvableReport::INVALID, msg.str() ) );
  }
...
private:
  ...
  ScopedGuard newReport() const
  {
    ...
    return shared_ptr<void>( static_cast<void *>( 0 ),
      bind( mem_fun_ref( static_cast<void ( shared_ptr<Report>::* )()>(
...
ManagedFile PackageProviderImpl<TPackage>::providePackage() const
{
  ...
  // Check toplevel cache
  {
    ...
    if ( info.packagesPath().dirname() !=
         topCache.repoPackagesCachePath ) // not using toplevel cache
    {
      ...
      if ( !loc.checksum().empty() ) // no cache hit without checksum
      {
        PathInfo pi( topCache.repoPackagesCachePath /
                     info.packagesPath().basename() / info.path() /
                     loc.filename() );
        if ( pi.isExist() &&
             loc.checksum() ==
               CheckSum( loc.checksum().type(), std::ifstream( pi.c_str() ) ) )
        {
          ...
          const Pathname &dest(
            info.packagesPath() / info.path() / loc.filename() );
               ...
               filesystem::hardlinkCopy( pi.path(), dest ) == 0 )
          {
            ...
            MIL << "provided Package from toplevel cache " << _package << " at "
                << ret << endl;
    ...
    do
    {
      ...
      try
      {
        ...
        if ( info.pkgGpgCheck() )
        {
          ...
          ResObject::constPtr roptr(
            _package ); // gcc6 needs it more explcit. Has problem deducing
          userData.set( "ResObject",
            roptr ); // a type for '_package->asKind<ResObject>()'...
          ...
          if ( str::startsWith( VERSION, "16.15." ) )
          {
            ...
            // as it should.
            if ( pkgGpgCheckIsMandatory && !info.gpgCheck() &&
                 !info.repoGpgCheck() )
              pkgGpgCheckIsMandatory = false;
          }
          RpmDb::CheckPackageResult res =
            packageSigCheck( ret, pkgGpgCheckIsMandatory, userData );
#else
          RpmDb::CheckPackageResult res =
            packageSigCheck( ret, info.pkgGpgCheckIsMandatory(), userData );
#endif
          ...
          if ( res != RpmDb::CHK_OK )
          {
            if (
              userData.hasvalue(
                "Action" ) ) // pkgGpgCheck report provided an user error action
            {
              resolveSignatureErrorAction( userData.get(
                "Action", repo::DownloadResolvableReport::ABORT ) );
            }
            else if (
              userData.haskey(
                "Action" ) ) // pkgGpgCheck requests the default problem report (wo. details)
            {
              ...
              switch ( res )
              {
                ...
                default:
                  ...
                  defaultReportSignatureError(
                    res, str::Str() << userData.get<RpmDb::CheckPackageDetail>(
                           "CheckPackageDetail" ) );
                  break;
              }
            }
      ...
      catch ( const FileCheckException &excpt )
      {
        ...
        if ( !_retry )
        {
          ...
          switch ( report()->problem( _package,
            repo::DownloadResolvableReport::INVALID,
            str::form( _( "Package %s seems to be corrupted during transfer. "
                                        ...
                                        package_str.c_str() ) ) )
          {
            case repo::DownloadResolvableReport::RETRY:
              ...
              break;
            case repo::DownloadResolvableReport::IGNORE:
              ZYPP_THROW( SkipRequestException(
      ...
      catch ( const Exception &excpt )
      {
        ...
        if ( !_retry )
        {
          ...
          std::string detail_str( str::form(
            _(
              "Failed to provide Package %s. Do you want to retry retrieval?" ),
            package_str.c_str() ) );
          detail_str += str::form( "\n\n%s", excpt.asUserHistory().c_str() );

          switch ( report()->problem(
            _package, repo::DownloadResolvableReport::IO, detail_str.c_str() ) )
          {
            case repo::DownloadResolvableReport::RETRY:
              _retry = true;
              break;
            case repo::DownloadResolvableReport::IGNORE:
              ZYPP_THROW(
                SkipRequestException( "User requested skip of file", excpt ) );
              break;
            case repo::DownloadResolvableReport::ABORT:
              ZYPP_THROW(
                AbortRequestException( "User requested to abort", excpt ) );
              break;
            default:
              ZYPP_RETHROW( excpt );
              break;
  ...
  report()->finish(
    _package, repo::DownloadResolvableReport::NO_ERROR, std::string() );
...
public:
  RpmPackageProvider( RepoMediaAccess &access_r,
    const Package::constPtr &package_r, const DeltaCandidates &deltas_r,
    const PackageProviderPolicy &policy_r )
...
ManagedFile RpmPackageProvider::tryDelta( const DeltaRpm &delta_r ) const
{
  ...
  report()->startDeltaDownload(
    delta_r.location().filename(), delta_r.location().downloadSize() );
  ManagedFile delta;
  try
  {
    ProvideFilePolicy policy;
    policy.progressCB(
      bind( &RpmPackageProvider::progressDeltaDownload, this, _1 ) );
    delta = _access.provideFile(
      delta_r.repository().info(), delta_r.location(), policy );
  }
  catch ( const Exception &excpt )
  {
...
inline PackageProvider::Impl *make( RepoMediaAccess &access_r,
  const PoolItem &pi_r, const DeltaCandidates &deltas_r,
  const PackageProviderPolicy &policy_r )
{
  if ( pi_r.isKind<Package>() )
    return new RpmPackageProvider(
      access_r, pi_r->asKind<Package>(), deltas_r, policy_r );
  else if ( pi_r.isKind<SrcPackage>() )
    return new PackageProviderImpl<SrcPackage>(
      access_r, pi_r->asKind<SrcPackage>(), policy_r );
  else
    ZYPP_THROW( Exception( str::Str() << "Don't know how to cache non-package "
                                      << pi_r.asUserString() ) );
}

inline PackageProvider::Impl *make( RepoMediaAccess &access_r,
  const PoolItem &pi_r, const PackageProviderPolicy &policy_r )
{
  if ( pi_r.isKind<Package>() )
    return new PackageProviderImpl<Package>(
      access_r, pi_r->asKind<Package>(), policy_r );
  else if ( pi_r.isKind<SrcPackage>() )
    return new PackageProviderImpl<SrcPackage>(
      access_r, pi_r->asKind<SrcPackage>(), policy_r );
  else
...
bool PackageProvider::isCached() const { return _pimpl->isCached(); }
