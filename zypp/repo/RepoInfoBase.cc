...
void RepoInfoBase::setEnabled( bool enabled ) { _pimpl->_enabled = enabled; }
...
{
  return indeterminate( _pimpl->_autorefresh ) ? false
                                               : (bool)_pimpl->_autorefresh;
}

std::string RepoInfoBase::alias() const { return _pimpl->_alias; }
...
std::string RepoInfoBase::rawName() const { return _pimpl->_name.raw(); }
...
Pathname RepoInfoBase::filepath() const { return _pimpl->_filepath; }
...
std::ostream &RepoInfoBase::dumpAsXmlOn(
  std::ostream &str, const std::string &content ) const
{
  return str << "<!-- there's no XML representation of RepoInfoBase -->"
             << endl;
}
