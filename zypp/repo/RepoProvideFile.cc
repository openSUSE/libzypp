  ...
  : public callback::ReceiveReport<media::DownloadProgressReport>
{
  ...
  virtual bool progress(
    int value, const Url &file, double dbps_avg = -1, double dbps_current = -1 )
  {
    bool ret = true;
    if ( _oldRec )
  ...
  virtual Action problem(
    const Url &file, Error error, const std::string &description )
  {
    if ( _oldRec )
...
private:
  void setVerifierForRepo( RepoInfo repo, shared_ptr<MediaSetAccess> media )
  {
    ...
    if ( !repo.metadataPath().empty() )
    {
      if ( PathInfo( mediafile ).isExist() )
      {
        ...
        if ( str )
        {
          ...
          MIL << "Repository '" << repo.alias() << "' has " << media_nr
              << " medias" << endl;

          for ( unsigned i = 1; i <= media_nr; ++i )
          {
            media::MediaVerifierRef verifier(
              new repo::SUSEMediaVerifier( vendor, mediaid, i ) );
      ...
      {
        WAR << "No media verifier for repo '" << repo.alias()
            << "' media/media.1 does not exist in '" << repo.metadataPath()
            << "'" << endl;
      }
    }
    else
    {
      WAR << "'" << repo.alias()
          << "' metadata path is empty. Can't set verifier. Probably this "
             "repository does not come from RepoManager."
          << endl;
    }
  }
...
ManagedFile RepoMediaAccess::provideFile( RepoInfo repo_r,
  const OnMediaLocation &loc_rx, const ProvideFilePolicy &policy_r )
{
  const OnMediaLocation locWithPath(
    OnMediaLocation( loc_rx ).prependPath( repo_r.path() ) );
  ...
  DownloadFileReportHack dumb(
    bind( mem_fun_ref( &ProvideFilePolicy::progress ), ref( policy_r ), _1 ) );

  RepoException repo_excpt(
    repo_r, str::form( _( "Can't provide file '%s' from repository '%s'" ),
              locWithPath.filename().c_str(), repo_r.alias().c_str() ) );
  ...
  {
    WAR << "Destination dir '" << destinationDir
    ...
    /* incremented in the loop */ )
  {
    ...
    try
    {
      MIL << "Providing file of repo '" << repo_r.alias() << "' from " << url
          << endl;
      shared_ptr<MediaSetAccess> access =
        _impl->mediaAccessForUrl( url, repo_r );
