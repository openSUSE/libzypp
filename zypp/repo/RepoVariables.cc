...
namespace zypp
{
  namespace repo
  {
    ...
    RepoVariablesStringReplacerNg::RepoVariablesStringReplacerNg(RepoVarRetrieverFunctor varRetriever)
      : _varRetriever( std::move(varRetriever) )
    {}

    std::string RepoVariablesStringReplacerNg::operator()(const std::string &value_r) const
    {
      if ( _varRetriever )
        return repo::RepoVarExpand()( value_r, _varRetriever );
      return value_r;
    }

    std::string RepoVariablesStringReplacerNg::operator()( std::string &&value_r ) const
    {
      if ( _varRetriever )
        return repo::RepoVarExpand()( std::move(value_r), _varRetriever );
      return value_r;
    }

    RepoVariablesUrlReplacerNg::RepoVariablesUrlReplacerNg(RepoVarRetrieverFunctor varRetriever)
      : _varRetriever( std::move(varRetriever) )
    { }

    zypp::Url RepoVariablesUrlReplacerNg::operator()(const zypp::Url &value) const
    {
      if ( _varRetriever ) {
        zypp::Url::ViewOptions toReplace = value.getViewOptions() - zypp::url::ViewOption::WITH_USERNAME - zypp::url::ViewOption::WITH_PASSWORD;
        ...
        zypp::Url tmpurl { value };
        tmpurl.setViewOptions( toReplace );
        const std::string & replaced( zypp::repo::RepoVarExpand()( zypp::hotfix1050625::asString( tmpurl ), _varRetriever ) );

        zypp::Url newurl;
        if ( !replaced.empty() )
        {
          newurl = replaced;
          newurl.setUsername( value.getUsername( zypp::url::E_ENCODED ), zypp::url::E_ENCODED );
          newurl.setPassword( value.getPassword( zypp::url::E_ENCODED ), zypp::url::E_ENCODED );
