...
{
  bool
    _embedded;       ///< A (formerly) embedded string may have esacped \c $, \c closebrace and \c backslash
  const char *_sbeg; ///< start of string to scan
  const char
    *_vbeg; ///< [$]{variable:-word} / [$]{variable} / if embedded also on [\\]
  ...
  bool nextVar()
  {
    ...
    {
      if ( _vbeg &&
           !_vend ) // loop internal: no findVarEnd at current $; skip it
  ...
  bool hasVar() const { return _vend; }
  ...
  std::string var() const { return std::string( _vbeg, _vend ); }

  /** Return the var name */
  std::string varName() const { return std::string( _nbeg, _nend ); }
  ...
  {
    return ( varIsConditional() ? std::string( _nend + 2, _vend - 1 )
                                : std::string() );
  }

  /** Have unwritten data before var? */
  bool hasVarPrefix() const { return ( _sbeg != _vbeg ); }

  /** Return unwritten data before var */
  std::string varPrefix() const { return std::string( _sbeg, _vbeg ); }

  /** Indicate all data up to _vend were written */
  void wroteVar() { _sbeg = _vend; }
...
private:
  ...
  /** Valid var name char */
  bool isnamech( int ch ) const { return ch == '_' || isalnum( ch ); }
  ...
  bool findVarEnd( const char *vbeg, const char *&nbeg, const char *&nend,
...
bool _expand( std::string &, const std::string &value_r, unsigned level_r,
  RepoVarExpand::VarRetriever &varRetriever_r );

inline std::string expand( const std::string &value_r, unsigned level_r,
  RepoVarExpand::VarRetriever &varRetriever_r )
{
  std::string ret;
...
inline std::string expand( std::string &&value_r, unsigned level_r,
  RepoVarExpand::VarRetriever &varRetriever_r )
{
  std::string ret;
  if ( !_expand( ret, value_r, level_r, varRetriever_r ) )
...
/** Expand variables in \a value_r depending on \a level-r
      ...
      */
inline bool _expand( std::string &result_r, const std::string &value_r,
  unsigned level_r, RepoVarExpand::VarRetriever &varRetriever_r )
{
#if ( ZYPP_DBG_VAREXPAND )
  cout << std::string( 2 * level_r, ' ' ) << "\033[7m>>" << value_r
       << "<<\033[27m" << endl;
  ...
  if ( !value_r.empty() )
  {
    ...
    while ( scan.nextVar() )
    {
      ...
      const std::string *const knownVar =
        ( varRetriever_r ? varRetriever_r( scan.varName() ) : nullptr );
      const std::string &varValue( knownVar ? *knownVar : _emptyValue );

#if ( ZYPP_DBG_VAREXPAND )
      dbg << std::string( dbgsbeg, scan._vbeg ) << "\033[3"
          << ( ( dbgi % 5 ) + 1 ) << "m" << scan.var() << "\033[0m";
      cout << dbg.str() << "|<< " << scan.varName() << " "
           << ( knownVar ? "(" + varValue + ")" : "-" ) << " {"
           << scan.varEmbedded() << "}" << endl;
      dbgsbeg = scan._vend;
      dbgi++;
#endif // ZYPP_DBG_VAREXPAND
      ...
      else if ( varType == '-' ) // ':-' default value
      {
        ...
        if ( varValue.empty() )
          substitutionValue =
            expand( scan.varEmbedded(), level_r + 1, varRetriever_r );
        else
          substitutionValue = varValue;
      }
      ...
      if ( mustSubstitute )
      {
        ...
        scan
          .wroteVar(); // this moves scan._sbeg so we can later see what's already written
      }
    }

#if ( ZYPP_DBG_VAREXPAND )
    dbg << std::string( dbgsbeg )
        << ( scan._sbeg == value_r.c_str() ? "<<\033[36m(moved)\033[0m" : "" );
#endif // ZYPP_DBG_VAREXPAND
  ...
  dbg << "<<";
  cout << dbg.str() << endl;
  cout << std::string( 2 * level_r, ' ' ) << "\033[36m->" << result_r
       << "<-\033[0m" << endl;
#endif // ZYPP_DBG_VAREXPAND
  return expanded;
}
...
std::string RepoVarExpand::operator()(
  const std::string &value_r, VarRetriever varRetriever_r ) const
{
  return expand( value_r, 0, varRetriever_r );
}

std::string RepoVarExpand::operator()(
  std::string &&value_r, VarRetriever varRetriever_r ) const
{
  ...
  const std::string *_lookup( const std::string &name_r )
  {
    if ( empty() ) // at init / after reset
    {
      ...
      filesystem::dirForEach(
        ZConfig::instance().systemRoot() / ZConfig::instance().varsPath(),
      ...
      deriveFromReleasever(
        "releasever", /*dont't overwrite user defined values*/ false );
...
private:
  ...
  void splitReleaseverTo( const std::string &releasever_r, std::string *major_r,
           ...
           strcmp( name_r.c_str() + 10, "_major" ) == 0 ) )
    {
      ...
      if ( !val.empty() )
      {
        ...
        if ( val != operator[]( "$releasever" ) )
        {
          ...
          deriveFromReleasever(
            "$releasever", /*overwrite previous values*/ true );
        }
      ...
      else if ( !count( name_r ) )
      {
        ...
        if ( val != operator[]( "$_releasever" ) )
        {
          ...
          deriveFromReleasever(
            "$_releasever", /*overwrite previous values*/ true );
        }
...
std::string RepoVariablesStringReplacer::operator()(
...
Url RepoVariablesUrlReplacer::operator()( const Url &value ) const
{
  ...
  const std::string &replaced(
    RepoVarExpand()( value.asString( toReplace ), RepoVarsMap::lookup ) );
  Url newurl;
  if ( !replaced.empty() )
  {
    newurl = replaced;
...
void repoVariablesReset() { repo::RepoVarsMap::instance().clear(); }
