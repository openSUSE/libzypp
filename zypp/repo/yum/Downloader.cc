...
{
  RepoStatus ret(
    media.provideFile( repoInfo().path() / "/repodata/repomd.xml" ) );
...
static OnMediaLocation loc_with_path_prefix(
  const OnMediaLocation &loc, const Pathname &prefix )
{
...
static Pathname search_deltafile( const Pathname &dir, const Pathname &file )
{
  ...
  if ( !filesystem::readdir( retlist, dir, false ) )
  {
    for_( it, retlist.begin(), retlist.end() )
    {
      ...
      if ( fn.size() >= basesize &&
           fn.substr( fn.size() - basesize, basesize ) == base )
        deltafile = *it;
    }
  }
  return deltafile;
...
bool Downloader::patches_Callback(
  const OnMediaLocation &loc_r, const string &id_r )
{
  OnMediaLocation loc_with_path(
    loc_with_path_prefix( loc_r, repoInfo().path() ) );
  MIL << id_r << " : " << loc_with_path << endl;
  this->enqueueDigested( loc_with_path, FileChecker(),
    search_deltafile( _delta_dir + "repodata", loc_r.filename() ) );
  return true;
}
...
{
  RepomdFileReaderCallback2(
    const RepomdFileReader::ProcessResource &origCallback_r )
  ...
  bool repomd_Callback2( const OnMediaLocation &loc_r,
    const ResourceType &dtype_r, const std::string &typestr_r )
  {
    ...
    {
      // susedata.LANG
      if ( str::hasPrefix( typestr_r, "susedata." ) &&
           !wantLocale( Locale( typestr_r.c_str() + 9 ) ) )
        return true; // skip it
    }
...
private:
  RepomdFileReader::ProcessResource
    _origCallback;          ///< Original Downloader callback
  LocaleSet _wantedLocales; ///< Locales do download
};
} // namespace
///////////////////////////////////////////////////////////////////

bool Downloader::repomd_Callback(
  const OnMediaLocation &loc_r, const ResourceType &dtype_r )
{
  // NOTE: Filtering of unwanted files is done in RepomdFileReaderCallback2!

  // schedule file for download
  const OnMediaLocation &loc_with_path(
    loc_with_path_prefix( loc_r, repoInfo().path() ) );
  this->enqueueDigested( loc_with_path, FileChecker(),
    search_deltafile( _delta_dir + "repodata", loc_r.filename() ) );
  ...
  const ProgressData::ReceiverFnc &progressrcv )
{
  ...
  RepomdFileReaderCallback2 pimpl(
    bind( &Downloader::repomd_Callback, this, _1, _2 ) );

  // setup parser
  RepomdFileReader( dest_dir / masterIndex,
    RepomdFileReader::ProcessResource2( bind(
      &RepomdFileReaderCallback2::repomd_Callback2, &pimpl, _1, _2, _3 ) ) );
