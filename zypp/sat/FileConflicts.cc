...
std::string FileConflicts::Conflict::asUserString() const
{
    ...
    static const char *text[ 2 ][ 2 ] = {
      ...
      {// TranslatorExplanation %1%(filename) %2%(package1) %3%(package2)
        ...
        N_( "File %1%\n"
            ...
            "  conflicts with file from install of\n"
            "     %3%" )}};
    return str::FormatNAC( text[ lhsSolvable().isSystem() ? 0 : 1 ]
                               [ rhsSolvable().isSystem() ? 0 : 1 ] ) %
           lhsFilename() % lhsSolvable().asUserString() %
  ...
  else
  {
    ...
    return str::FormatNAC( text[ lhsSolvable().isSystem() ? 0 : 1 ]
                               [ rhsSolvable().isSystem() ? 0 : 1 ] ) %
           lhsFilename() % lhsSolvable().asUserString() % rhsFilename() %
           rhsSolvable().asUserString();
  }
}
...
std::ostream &operator<<(
  std::ostream &str, const FileConflicts::Conflict &obj )
{
  if ( obj.lhsFilename() == obj.rhsFilename() )
    return str << str::Format( "%s:\n    %s[%s]\n    %s[%s]" ) %
                    obj.lhsFilename() % obj.lhsSolvable() % obj.lhsFilemd5() %
                    obj.rhsSolvable() % obj.rhsFilemd5();

  return str << str::Format( "%s - %s:\n    %s[%s]\n    %s[%s]" ) %
                  obj.lhsFilename() % obj.rhsFilename() % obj.lhsSolvable() %
                  obj.lhsFilemd5() % obj.rhsSolvable() % obj.rhsFilemd5();
}
...
namespace
{
std::ostream &dumpAsXmlHelper( std::ostream &str, const std::string &tag_r,
  IdString filename_r, IdString md5sum_r, Solvable solv_r )
{
  ...
  std::ostream &str, const FileConflicts::Conflict &obj )
{
  ...
  dumpAsXmlHelper( *guard << "\n", "lhs", obj.lhsFilename(), obj.lhsFilemd5(),
    obj.lhsSolvable() );
  dumpAsXmlHelper( *guard << "\n", "rhs", obj.rhsFilename(), obj.rhsFilemd5(),
    obj.rhsSolvable() );
  return str;
}
