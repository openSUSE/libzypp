...
public:
  ...
  Impl( SolvAttr attr_r, Location loc_r )
    : _attr( attr_r )
    , _parent( attr_r.parent() )
    , _solv( loc_r == REPO_ATTR ? SOLVID_META : noSolvableId )
  {
  }
...
public:
  SolvAttr attr() const { return _attr; }
  ...
  const StrMatcher &strMatcher() const { return _strMatcher; }
...
public:
  bool pool() const { return !( _repo || _solv ); }
  ...
  Repository repo() const { return _repo; }
  ...
  Solvable solvable() const { return _solv; }
  ...
  SolvAttr parent() const { return _parent; }

  void setParent( SolvAttr attr_r ) { _parent = attr_r; }
...
public:
  LookupAttr::iterator begin() const
  {
    ...
    detail::DIWrap dip( whichRepo, _solv.id(), _attr.id(),
      _strMatcher.searchstring(), _strMatcher.flags().get() );
  ...
  LookupAttr::iterator end() const { return iterator(); }
...
LookupAttr::LookupAttr(
  SolvAttr attr_r, SolvAttr parent_r, Repository repo_r, Location loc_r )
  : _pimpl( new Impl( attr_r, repo_r, loc_r ) )
{
  _pimpl->setParent( parent_r );
}
...
SolvAttr LookupAttr::attr() const { return _pimpl->attr(); }

void LookupAttr::setAttr( SolvAttr attr_r ) { _pimpl->setAttr( attr_r ); }
...
bool LookupAttr::pool() const { return _pimpl->pool(); }

void LookupAttr::setPool( Location loc_r ) { _pimpl->setPool( loc_r ); }

Repository LookupAttr::repo() const { return _pimpl->repo(); }
...
Solvable LookupAttr::solvable() const { return _pimpl->solvable(); }
...
SolvAttr LookupAttr::parent() const { return _pimpl->parent(); }

void LookupAttr::setParent( SolvAttr attr_r ) { _pimpl->setParent( attr_r ); }

///////////////////////////////////////////////////////////////////

LookupAttr::iterator LookupAttr::begin() const { return _pimpl->begin(); }

LookupAttr::iterator LookupAttr::end() const { return _pimpl->end(); }

bool LookupAttr::empty() const { return begin() == end(); }
...
DIWrap::DIWrap( RepoIdType repoId_r, SolvableIdType solvId_r, IdType attrId_r,
  ...
  , _mstring( mstring_r )
{
  ::dataiterator_init( _dip, sat::Pool::instance().get(), repoId_r, solvId_r,
    attrId_r, _mstring.empty() ? 0 : _mstring.c_str(), flags_r );
}
...
enum SubType
{
  ST_NONE, // no sub-structure
  ST_FLEX, // flexarray
  ST_SUB
}; // inside sub-structure
...
LookupAttr::iterator LookupAttr::iterator::subEnd() const { return iterator(); }
...
LookupAttr::iterator LookupAttr::iterator::subFind(
  const C_Str &attrname_r ) const
{
  if ( attrname_r.empty() )
    return subBegin();
...
unsigned LookupAttr::iterator::asUnsigned() const { return asInt(); }
...
bool LookupAttr::iterator::asBool() const { return asInt(); }
    ...
    switch ( solvAttrType() )
    {
      ...
      case REPOKEY_TYPE_CONSTANTID:
        if ( _dip->data && _dip->data->localpool )
          return ::stringpool_id2str(
            &_dip->data->spool, _dip->kv.id ); // in local pool
        else
      ...
      case REPOKEY_TYPE_DIRSTRARRAY:
        ...
        return ( _dip->flags & SEARCH_FILES ? _dip->kv.str
                                            : ::repodata_dir2str( _dip->data,
                                                _dip->kv.id, _dip->kv.str ) );
        break;
    }
  }
  return 0;
...
std::string LookupAttr::iterator::asString() const
{
  if ( _dip )
  {
    switch ( solvAttrType() )
    {
      ...
      {
        detail::IdType id =
          ::repodata_globalize_id( _dip->data, _dip->kv.id, 1 );
...
CheckSum LookupAttr::iterator::asCheckSum() const
{
  if ( _dip )
  {
    ...
    {
      case REPOKEY_TYPE_MD5:
        return CheckSum::md5(::repodata_chk2str(
          _dip->data, solvAttrType(), (unsigned char *)_dip->kv.str ) );
        break;

      case REPOKEY_TYPE_SHA1:
        return CheckSum::sha1(::repodata_chk2str(
          _dip->data, solvAttrType(), (unsigned char *)_dip->kv.str ) );
        break;

      case REPOKEY_TYPE_SHA224:
        return CheckSum::sha224(::repodata_chk2str(
          _dip->data, solvAttrType(), (unsigned char *)_dip->kv.str ) );
        break;

      case REPOKEY_TYPE_SHA256:
        return CheckSum::sha256(::repodata_chk2str(
          _dip->data, solvAttrType(), (unsigned char *)_dip->kv.str ) );
        break;

      case REPOKEY_TYPE_SHA384:
        return CheckSum::sha384(::repodata_chk2str(
          _dip->data, solvAttrType(), (unsigned char *)_dip->kv.str ) );
        break;

      case REPOKEY_TYPE_SHA512:
        return CheckSum::sha512(::repodata_chk2str(
          _dip->data, solvAttrType(), (unsigned char *)_dip->kv.str ) );
        break;
    }
  }
...
bool LookupAttr::iterator::dip_equal(
  const detail::CDataiterator &lhs, const detail::CDataiterator &rhs ) const
{
...
template <>
CheckSum LookupAttr::iterator::asType<CheckSum>() const
{
  return asCheckSum();
}
...
std::ostream &operator<<(
  std::ostream &str, const zypp::sat::detail::CDataiterator *obj )
{
