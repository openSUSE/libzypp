...
class LookupAttr::TransformIterator
  : public boost::iterator_adaptor<TransformIterator<TResult, TAttr> // Derived
      ,
...
public:
  ...
  explicit TransformIterator( const LookupAttr::iterator &val_r )
  {
...
public:
  ...
  void nextSkipSolvAttr() { this->base_reference().nextSkipSolvAttr(); }

  /** On the next call to \ref operator++ advance to the next \ref Solvable. */
  void nextSkipSolvable() { this->base_reference().nextSkipSolvable(); }

  /** On the next call to \ref operator++ advance to the next \ref Repository. */
  void nextSkipRepo() { this->base_reference().nextSkipRepo(); }

  /** Immediately advance to the next \ref SolvAttr. */
  void skipSolvAttr() { this->base_reference().skipSolvAttr(); }

  /** Immediately advance to the next \ref Solvable. */
  void skipSolvable() { this->base_reference().skipSolvable(); }

  /** Immediately advance to the next \ref Repository. */
  void skipRepo() { this->base_reference().skipRepo(); }
  //@}

  /** \name Current position info. */
  //@{
  /** The current \ref Repository. */
  Repository inRepo() const { return this->base_reference().inRepo(); }

  /** The current \ref Solvabele. */
  Solvable inSolvable() const { return this->base_reference().inSolvable(); }

  /** The current \ref SolvAttr. */
  SolvAttr inSolvAttr() const { return this->base_reference().inSolvAttr(); }
  //@}
...
std::ostream &operator<<(
  std::ostream &str, const ArrayAttr<TResult, TAttr> &obj );
...
class ArrayAttr
{
  friend std::ostream &operator<<<TResult, TAttr>(
    std::ostream &str, const ArrayAttr<TResult, TAttr> &obj );
...
public:
  ...
  ArrayAttr(
    SolvAttr attr_r, LookupAttr::Location loc_r = LookupAttr::SOLV_ATTR )
    : _q( attr_r, loc_r )
  {
  }
