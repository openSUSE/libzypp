...
Map::~Map() { ::map_free( _pimpl.get() ); }

bool Map::empty() const { return ( _pimpl->size == 0 ); }

Map::size_type Map::size() const { return _pimpl->size << 3; }

void Map::grow( size_type size_r ) { ::map_grow( _pimpl.get(), size_r ); }

void Map::setAll() { assignAll( true ); }

void Map::clearAll() { assignAll( false ); }
...
#define M_RANGE_CKECK( IDX, LOC )                                              \
  if ( ( ( IDX ) >> 3 ) >= size_type( _pimpl->size ) )                         \
  throw std::out_of_range( "zypp::sat::Map::" LOC )
...
bool operator==( const Map &lhs, const Map &rhs )
{
  ...
  return ( l == r ||
           ( l->size == r->size && ::memcmp( l->map, r->map, l->size ) == 0 ) );
}
