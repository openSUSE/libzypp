...
detail::CPool *Pool::get() const { return myPool().getPool(); }

Pool::size_type Pool::capacity() const { return myPool()->nsolvables; }

const SerialNumber &Pool::serial() const { return myPool().serial(); }

const SerialNumber &Pool::serialIDs() const { return myPool().serialIDs(); }

void Pool::prepare() const { return myPool().prepare(); }

Pathname Pool::rootDir() const { return myPool().rootDir(); }
...
bool Pool::reposEmpty() const { return !myPool()->urepos; }

Pool::size_type Pool::reposSize() const { return myPool()->urepos; }

Pool::RepositoryIterator Pool::reposBegin() const
{
  if ( myPool()->urepos )
  { // repos[0] == NULL
    for_( it, myPool()->repos + 1,
      myPool()->repos +
        myPool()->nrepos ) if ( *it ) return RepositoryIterator( it );
  }
  return reposEnd();
}
...
Pool::size_type Pool::solvablesSize() const
{
  ...
  // invalid Solvables.
  size_type ret = 0;
  for_( it, reposBegin(), reposEnd() ) { ret += it->solvablesSize(); }
  return ret;
}
...
Pool::SolvableIterator Pool::solvablesEnd() const { return SolvableIterator(); }
...
Repository Pool::addRepoSolv(
  const Pathname &file_r, const std::string &alias_r )
{
...
void Pool::initRequestedLocales( const LocaleSet &locales_r )
{
  myPool().initRequestedLocales( locales_r );
}
const LocaleSet &Pool::getAddedRequestedLocales() const
{
  return myPool().getAddedRequestedLocales();
}
const LocaleSet &Pool::getRemovedRequestedLocales() const
{
  return myPool().getRemovedRequestedLocales();
}
...
{
  return str << "sat::pool(" << obj.serial() << ")[" << obj.capacity() << "]{"
             << obj.reposSize() << "repos|" << obj.solvablesSize() << "slov}";
}
...
void updateSolvFileIndex( const Pathname &solvfile_r )
{
  ...
  {
    int fd = ::open(
      solvidxfile.c_str(), O_CREAT | O_EXCL | O_WRONLY | O_TRUNC, 0644 );
    if ( fd == -1 )
    {
...
#define idstr( V ) pool_id2str( _pool, _solv->V )
        if ( _solv->arch == ARCH_SRC || _solv->arch == ARCH_NOSRC )
          idx << "srcpackage:" << idstr( name ) << SEP << idstr( evr ) << SEP
              << "noarch" << endl;
        else
          idx << idstr( name ) << SEP << idstr( evr ) << SEP << idstr( arch )
              << endl;
      }
    }
  }
