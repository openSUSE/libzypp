...
template <>
sat::detail::CQueue *rwcowClone<sat::detail::CQueue>(
  const sat::detail::CQueue *rhs )
{
...
Queue::~Queue() { ::queue_free( _pimpl.get() ); }

bool Queue::empty() const { return ( _pimpl->count == 0 ); }

Queue::size_type Queue::size() const { return _pimpl->count; }

Queue::const_iterator Queue::begin() const { return _pimpl->elements; }
...
{
  for_( it, begin(), end() ) if ( *it == val_r ) return it;
  return end();
}
...
#define M_RANGE_CKECK( IDX, LOC )                                              \
  if ( IDX >= size_type( _pimpl->count ) )                                     \
  throw std::out_of_range( "zypp::sat::Queue::" LOC )

const Queue::value_type &Queue::at( size_type idx_r ) const
{
  M_RANGE_CKECK( idx_r, "at" );
...
void Queue::clear() { ::queue_empty( *this ); }
...
void Queue::push( value_type val_r ) { ::queue_push( _pimpl.get(), val_r ); }
...
Queue::value_type Queue::pop() { return ::queue_pop( _pimpl.get() ); }
...
Queue::value_type Queue::pop_front() { return ::queue_shift( _pimpl.get() ); }
...
std::ostream &dumpOn( std::ostream &str, const Queue &obj )
{
  ...
  {
    str << endl;
    for_( it, obj.begin(), obj.end() ) str << "  " << Solvable( *it ) << endl;
  }
  return str << "}";
}
...
bool operator==( const Queue &lhs, const Queue &rhs )
{
  ...
  return ( l == r || ( l->count == r->count &&
                       ::memcmp( l->elements, r->elements, l->count ) == 0 ) );
}
