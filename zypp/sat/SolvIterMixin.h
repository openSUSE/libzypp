...
public:
  ...
  bool contains( const TSolv &solv_r ) const
  {
    ...
    for_( it, self().begin(), self().end() ) if ( *it == solv ) return true;
    return false;
  }
  //@}
...
public:
  ...
  Solvable_iterator solvableBegin() const { return self().begin(); }
  Solvable_iterator solvableEnd() const { return self().end(); }
...
private:
  typedef filter_iterator<solvitermixin_detail::UnifyByIdent, Solvable_iterator>
    UnifiedSolvable_iterator;

public:
  /** \name Iterate ui::Selectable::Ptr */
  //@{
  typedef transform_iterator<ui::asSelectable, UnifiedSolvable_iterator>
    Selectable_iterator;
  Selectable_iterator selectableBegin() const
  {
    return make_transform_iterator(
      unifiedSolvableBegin(), ui::asSelectable() );
  }
...
private:
  ...
  UnifiedSolvable_iterator unifiedSolvableBegin() const
  {
    return make_filter_iterator(
      solvitermixin_detail::UnifyByIdent(), solvableBegin(), solvableEnd() );
  }
  UnifiedSolvable_iterator unifiedSolvableEnd() const
  {
    return make_filter_iterator(
      solvitermixin_detail::UnifyByIdent(), solvableEnd(), solvableEnd() );
  }
