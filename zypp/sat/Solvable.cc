...
detail::CSolvable *Solvable::get() const { return myPool().getSolvable( _id ); }
...
Solvable Solvable::nextInRepo() const
{
  ...
  for ( detail::SolvableIdType next = _id + 1;
        next < unsigned( _solvable->repo->end ); ++next )
  {
...
std::string Solvable::lookupStrAttribute(
  const SolvAttr &attr, const Locale &lang_r ) const
{
  NO_SOLVABLE_RETURN( std::string() );
  const char *s = 0;
  if ( !lang_r )
  ...
  else
  {
    ...
    {
      if ( ( s = ::solvable_lookup_str_lang(
               _solvable, attr.id(), l.c_str(), 0 ) ) )
        return s;
    }
...
unsigned long long Solvable::lookupNumAttribute(
  const SolvAttr &attr, unsigned long long notfound_r ) const
{
...
CheckSum Solvable::lookupCheckSumAttribute( const SolvAttr &attr ) const
{
  ...
  const char *s =
    ::solvable_lookup_checksum( _solvable, attr.id(), &chksumtype );
  if ( !s )
    return CheckSum();
  switch ( chksumtype )
  {
    case REPOKEY_TYPE_MD5:
      return CheckSum::md5( s );
    case REPOKEY_TYPE_SHA1:
      return CheckSum::sha1( s );
    case REPOKEY_TYPE_SHA224:
      return CheckSum::sha224( s );
    case REPOKEY_TYPE_SHA256:
      return CheckSum::sha256( s );
    case REPOKEY_TYPE_SHA384:
      return CheckSum::sha384( s );
    case REPOKEY_TYPE_SHA512:
      return CheckSum::sha512( s );
  }
...
OnMediaLocation Solvable::lookupLocation() const
{
  ...
  ret.setDownloadSize(
    ByteCount( lookupNumAttribute( SolvAttr::downloadsize ) ) );
...
bool Solvable::isKind( const ResKind &kind_r ) const
{
  ...
  return (::strncmp( ident, kind, ksize ) == 0 && ident[ ksize ] == ':' );
}
...
RepoInfo Solvable::repoInfo() const { return repository().info(); }
...
{
  NO_SOLVABLE_RETURN(
    ( _id == detail::systemSolvableId ? "systemSolvable" : "noSolvable" ) );
  return str::form( "%s-%s.%s", IdString( _solvable->name ).c_str(),
    IdString( _solvable->evr ).c_str(), IdString( _solvable->arch ).c_str() );
}
...
std::string Solvable::asUserString() const
{
  NO_SOLVABLE_RETURN(
    ( _id == detail::systemSolvableId ? "systemSolvable" : "noSolvable" ) );
  return str::form( "%s-%s.%s (%s)", IdString( _solvable->name ).c_str(),
    IdString( _solvable->evr ).c_str(), IdString( _solvable->arch ).c_str(),
...
bool Solvable::identical( const Solvable &rhs ) const
{
  ...
  return rhssolvable && ( _solvable == rhssolvable ||
                          ::solvable_identical( _solvable, rhssolvable ) );
}

///////////////////////////////////////////////////////////////////
namespace
{
inline Capabilities _getCapabilities(
  detail::IdType *idarraydata_r, ::Offset offs_r )
{
...
Capabilities Solvable::recommends() const
{
  ...
  return _getCapabilities(
    _solvable->repo->idarraydata, _solvable->recommends );
}
...
Capabilities Solvable::supplements() const
{
  NO_SOLVABLE_RETURN( Capabilities() );
  return _getCapabilities(
    _solvable->repo->idarraydata, _solvable->supplements );
}
Capabilities Solvable::prerequires() const
{
  ...
  return offs ? Capabilities( _solvable->repo->idarraydata + offs,
                  detail::solvablePrereqMarker )
              : Capabilities();
}

CapabilitySet Solvable::providesNamespace(
  const std::string &namespace_r ) const
{
  ...
  for_( it, caps.begin(), caps.end() )
  {
    ...
    if ( str::hasPrefix( caprep.name().c_str(), namespace_r ) &&
         *( caprep.name().c_str() + namespace_r.size() ) == '(' )
      ret.insert( *it );
  }
  return ret;
}

CapabilitySet Solvable::valuesOfNamespace(
  const std::string &namespace_r ) const
{
  NO_SOLVABLE_RETURN( CapabilitySet() );
  CapabilitySet ret;
...
/** Expand \ref Capability and call \c fnc_r for each namespace:language
       ...
       */
int invokeOnEachSupportedLocale(
  Capability cap_r, function<bool( const Locale & )> fnc_r )
{
  ...
  if ( detail.kind() == CapDetail::EXPRESSION )
  {
    switch ( detail.capRel() )
    {
      ...
      case CapDetail::CAP_NAMESPACE:
        ...
        {
          return ( !fnc_r || fnc_r( Locale( IdString( detail.rhs().id() ) ) ) )
                   ? 1
                   : -1; // negative on abort.
        }
        break;
...
/** Expand \ref Capability and call \c fnc_r for each namespace:language
       ...
       * false to indicate abort.
       */
inline int invokeOnEachSupportedLocale(
  Capabilities cap_r, function<bool( Locale )> fnc_r )
{
  int cnt = 0;
...
struct NoMatchIn
{
  NoMatchIn( const LocaleSet &locales_r )
    : _locales( locales_r )
  {
  }
...
bool Solvable::supportsLocale( const Locale &locale_r ) const
{
  ...
  return invokeOnEachSupportedLocale( supplements(),
           bind( std::not_equal_to<Locale>(), locale_r, _1 ) ) < 0;
}
...
bool Solvable::supportsLocale( const LocaleSet &locales_r ) const
{
  ...
  return invokeOnEachSupportedLocale( supplements(), NoMatchIn( locales_r ) ) <
         0;
}
...
std::string Solvable::licenseToConfirm( const Locale &lang_r ) const
{
  ...
  if ( ret.empty() && isKind<Product>() )
  {
    ...
    if ( ri.needToAcceptLicense( riname ) ||
         !ui::Selectable::get( *this )->hasInstalledObj() )
      ret = ri.getLicense(
        riname, lang_r ); // bnc#908976: suppress informal license upon update
  }
  return ret;
}
...
std::ostream &operator<<( std::ostream &str, const Solvable &obj )
{
  ...
  return str << "(" << obj.id() << ")"
             << ( obj.isKind( ResKind::srcpackage ) ? "srcpackage:" : "" )
