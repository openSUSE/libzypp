...
public:
  ...
  {
    for_( it, begin, end ) if ( isKind( *it ) ) return true;
    return false;
  }
  ...
  bool identIsAutoInstalled() const { return identIsAutoInstalled( ident() ); }
...
public:
  ...
  {
    return ( get() == rhs.get() ||
             ( ident() == rhs.ident() && edition() == rhs.edition() &&
               arch() == rhs.arch() ) );
  }
...
public:
  ...
  /** \overload operator[] */
  Capabilities operator[]( Dep which_r ) const { return dep( which_r ); }
...
class SolvableIterator
  : public boost::iterator_adaptor<SolvableIterator // Derived
      ,
      CSolvable * // Base
      ,
...
private:
  ...
  Solvable dereference() const { return _val; }

  void increment() { assignVal( _val.nextInPool() ); }

private:
  void assignVal( const Solvable &val_r )
  {
    _val = val_r;
    base_reference() = _val.get();
  }
...
struct asSolvable
{
  ...
  Solvable operator()( const Solvable &solv_r ) const { return solv_r; }
