...
struct SolvableType
{
  ...
  {
    return Solvable( static_cast<const Derived &>( *this ) );
  }
  ...
  {
    return satSolvable().isKind( kind_r );
  }
  template <class TRes>
  bool isKind() const
  {
    return satSolvable().template isKind<TRes>();
  }
  template <class TIterator>
  bool isKind( TIterator begin, TIterator end ) const
  {
    return satSolvable().isKind( begin, end );
  }
  ...
  {
    return satSolvable().identIsAutoInstalled();
  }
  bool multiversionInstall() const
  {
    return satSolvable().multiversionInstall();
  }
  ...
  {
    return satSolvable().identical( rhs );
  }
  template <class RDerived>
  bool identical( const SolvableType<RDerived> &rhs ) const
  {
    return satSolvable().identical( rhs.satSolvable() );
  }

  bool sameNVRA( const Solvable &rhs ) const
  {
    return satSolvable().sameNVRA( rhs );
  }
  template <class RDerived>
  bool sameNVRA( const SolvableType<RDerived> &rhs ) const
  {
    return satSolvable().sameNVRA( rhs.satSolvable() );
  }
  ...
  {
    return satSolvable()[ which_r ];
  }

  CapabilitySet providesNamespace( const std::string &namespace_r ) const
  {
    return satSolvable().providesNamespace( namespace_r );
  }
  CapabilitySet valuesOfNamespace( const std::string &namespace_r ) const
  {
    return satSolvable().valuesOfNamespace( namespace_r );
  }

  bool supportsLocales() const { return satSolvable().supportsLocales(); }
  bool supportsLocale( const Locale &locale_r ) const
  {
    return satSolvable().supportsLocale( locale_r );
  }
  bool supportsLocale( const LocaleSet &locales_r ) const
  {
    return satSolvable().supportsLocale( locales_r );
  }
  bool supportsRequestedLocales() const
  {
    return satSolvable().supportsRequestedLocales();
  }
  LocaleSet getSupportedLocales() const
  {
    return satSolvable().getSupportedLocales();
  }
  ...
  {
    return satSolvable().summary( lang_r );
  }
  std::string description( const Locale &lang_r = Locale() ) const
  {
    return satSolvable().description( lang_r );
  }
  std::string insnotify( const Locale &lang_r = Locale() ) const
  {
    return satSolvable().insnotify( lang_r );
  }
  std::string delnotify( const Locale &lang_r = Locale() ) const
  {
    return satSolvable().delnotify( lang_r );
  }
  std::string licenseToConfirm( const Locale &lang_r = Locale() ) const
  {
    return satSolvable().licenseToConfirm( lang_r );
  }
  bool needToAcceptLicense() const
  {
    return satSolvable().needToAcceptLicense();
  }

public:
  std::string lookupStrAttribute( const SolvAttr &attr ) const
  {
    return satSolvable().lookupStrAttribute( attr );
  }
  std::string lookupStrAttribute(
    const SolvAttr &attr, const Locale &lang_r ) const
  {
    return satSolvable().lookupStrAttribute( attr, lang_r );
  }
  ...
  unsigned long long lookupNumAttribute(
    const SolvAttr &attr, unsigned long long notfound_r ) const
  {
    return satSolvable().lookupNumAttribute( attr, notfound_r );
  }
  ...
  {
    return satSolvable().lookupLocation();
  }
...
inline std::ostream &operator<<(
  std::ostream &str, const SolvableType<Derived> &obj )
{
...
inline bool operator==(
  const SolvableType<LDerived> &lhs, const SolvableType<RDerived> &rhs )
{
...
/** \overload */
template <class Derived>
inline bool compareByNVR(
  const SolvableType<Derived> &lhs, const Solvable &rhs )
{
...
template <class Derived>
inline bool compareByNVR(
  const Solvable &lhs, const SolvableType<Derived> &rhs )
{
