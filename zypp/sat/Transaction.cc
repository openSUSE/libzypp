...
public:
    ...
    , _trans( nullptr )
  {
    ...
    _trans =
      ::transaction_create_decisionq( myPool().getPool(), decisionq, noobsmap );
    ...
    for_(
      it, _trans->steps.elements, _trans->steps.elements + _trans->steps.count )
    {
  ...
  ~Impl() { ::transaction_free( _trans ); }

public:
  bool valid() const { return _watcher.isClean( myPool().serial() ); }
  ...
  bool empty() const { return ( _trans->steps.count == 0 ); }

  size_t size() const { return _trans->steps.count; }
  ...
  const_iterator end( const RW_pointer<Transaction::Impl> &self_r ) const
  {
    return const_iterator(
      self_r, _trans->steps.elements + _trans->steps.count );
  }
  ...
  const_iterator find( const RW_pointer<Transaction::Impl> &self_r,
    const sat::Solvable &solv_r ) const
  {
    detail::IdType *it( _find( solv_r ) );
    return it ? const_iterator( self_r, it ) : end( self_r );
  }
  iterator find(
    const RW_pointer<Transaction::Impl> &self_r, const sat::Solvable &solv_r )
  {
    detail::IdType *it( _find( solv_r ) );
    return it ? iterator( self_r, it ) : end( self_r );
  }
...
public:
  ...
  StringQueue autoInstalled() const { return _autoInstalled; }

  void autoInstalled( const StringQueue &queue_r ) { _autoInstalled = queue_r; }
...
public:
  StepType stepType( Solvable solv_r ) const
  {
    if ( !solv_r )
    {
      ...
      return isIn( _systemErase, solv_r.id() ) ? TRANSACTION_ERASE
                                               : TRANSACTION_IGNORE;
    }

    switch (
      ::transaction_type( _trans, solv_r.id(), SOLVER_TRANSACTION_RPM_ONLY ) )
    {
      case SOLVER_TRANSACTION_ERASE:
        return TRANSACTION_ERASE;
        break;
      case SOLVER_TRANSACTION_INSTALL:
        return TRANSACTION_INSTALL;
        break;
      case SOLVER_TRANSACTION_MULTIINSTALL:
        return TRANSACTION_MULTIINSTALL;
        break;
    }
...
private:
  detail::IdType *_find( const sat::Solvable &solv_r ) const
  {
    ...
    {
      for_( it, _trans->steps.elements,
        _trans->steps.elements + _trans->steps.count )
      {
...
private:
  ...
  set_type
    _systemErase; // @System packages to be eased (otherse are TRANSACTION_IGNORE)
  pmmap_type _pmMap; // Post mortem data of deleted @System solvables

  StringQueue
    _autoInstalled; // ident strings of all packages that would be auto-installed after the transaction is run.
...
inline std::ostream &operator<<(
  std::ostream &str, const Transaction::Impl &obj )
{
  return str << "Transaction: " << obj.size() << " ("
...
bool Transaction::valid() const { return _pimpl->valid(); }

bool Transaction::order() { return _pimpl->order(); }

bool Transaction::empty() const { return _pimpl->empty(); }

size_t Transaction::size() const { return _pimpl->size(); }
...
Transaction::iterator Transaction::begin() { return _pimpl->begin( _pimpl ); }
...
Transaction::iterator Transaction::end() { return _pimpl->end( _pimpl ); }

Transaction::const_iterator Transaction::find(
  const sat::Solvable &solv_r ) const
{
...
{
  for_( it, obj.begin(), obj.end() ) { str << *it << endl; }
  return str;
}
...
Transaction_const_iterator::Transaction_const_iterator(
  const Transaction_iterator &iter_r )
