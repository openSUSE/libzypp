...
public:
  ...
  {
    TRANSACTION_IGNORE =
      0x00,                     /**< [ ] Nothing (includes implicit deletes due to obsoletes and non-package actions) */
    TRANSACTION_ERASE = 0x10,   /**< [-] Delete item */
    TRANSACTION_INSTALL = 0x20, /**< [+] Install(update) item */
    TRANSACTION_MULTIINSTALL =
      0x30 /**< [M] Install(multiversion) item (\see \ref ZConfig::multiversion) */
  };
...
public:
  ...
  /** Return the corresponding \ref Solvable.
	 ...
	 */
  Solvable satSolvable() const { return _solv; }
...
/** \ref Transaction iterator.
       */
class Transaction_iterator
  : public boost::iterator_adaptor<Transaction_iterator // Derived
      ,
...
public:
  ...
  Transaction_iterator(
    const RW_pointer<Transaction::Impl> &pimpl_r, base_type id_r )
...
private:
  ...
  reference dereference() const { return Transaction::Step( _pimpl, *base() ); }
...
/** \ref Transaction const_iterator.
       */
class Transaction_const_iterator
  : public boost::iterator_adaptor<Transaction_const_iterator // Derived
      ,
      const detail::IdType * // Base
      ,
...
inline Transaction::const_iterator Transaction::find(
  const ResObject::constPtr &resolvable_r ) const
{
  return ( resolvable_r ? find( resolvable_r->satSolvable() ) : end() );
}

inline Transaction::iterator Transaction::find(
  const ResObject::constPtr &resolvable_r )
{
  return ( resolvable_r ? find( resolvable_r->satSolvable() ) : end() );
}

inline Transaction::const_iterator Transaction::find(
  const PoolItem &pi_r ) const
{
...
inline Transaction::action_iterator Transaction::actionBegin(
  StepStages filter_r ) const
{
...
{
  size_t cnt = 0;
  for_( it, actionBegin( filter_r ), actionEnd() )++ cnt;
  return cnt;
}
