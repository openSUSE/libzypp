...
/** Container of \b installed \ref Solvable which would be
     ...
     */
class WhatObsoletes
  : public SolvIterMixin<WhatObsoletes, detail::WhatProvidesIterator>,
...
public:
  ...
  explicit WhatObsoletes( Solvable item_r );

  /** Ctor from \ref PoolItem. */
  explicit WhatObsoletes( const PoolItem &item_r );

  /** Ctor from \ref ResObject::constPtr. */
  explicit WhatObsoletes( const ResObject_constPtr item_r );
  ...
  WhatObsoletes( TIterator begin, TIterator end )
    : _begin( 0 )
  {
    for_( it, begin, end ) ctorAdd( *it );
    ctorDone();
  }

public:
  /** Whether the container is empty. */
  bool empty() const { return !( _begin && *_begin ); }
