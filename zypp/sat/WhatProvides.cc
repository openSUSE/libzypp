...
void collectProviders( Iterator begin_r, Iterator end_r,
  std::unordered_set<detail::IdType> &collect_r )
{
...
WhatProvides::const_iterator WhatProvides::begin() const
{
  ...
  return const_iterator(
    &myPool().getPool()->whatprovidesdata, _pimpl->_offset );
}
...
std::ostream &operator<<( std::ostream &str, const WhatProvidesIterator &obj )
{
  ...
  str << str::form(
    "<%p(%p)>", obj._baseRef, ( obj._baseRef ? *obj._baseRef : 0 ) );
  return str;
}
