...
public:
  ...
  explicit WhatProvides( Capability cap_r );

  /** Ctor collecting all providers of capabilities in \c caps_r. */
  explicit WhatProvides( Capabilities caps_r );

  /** Ctor collecting all providers of capabilities in \c caps_r. */
  explicit WhatProvides( const CapabilitySet &caps_r );
...
/** \ref WhatProvides iterator.
     ...
     */
class WhatProvidesIterator
  : public boost::iterator_adaptor<WhatProvidesIterator // Derived
      ,
      ...
      >
{
  friend std::ostream &operator<<(
    std::ostream &str, const WhatProvidesIterator &obj );
...
public:
  ...
  /** Ctor with pointer to 1st elemment of an array.
         ...
         */
  explicit WhatProvidesIterator(
    const detail::IdType *const base_r, unsigned offset_r = 0 )
    : iterator_adaptor_( base_r )
  ...
  /** Ctor with pointer to pointer to 1st elemment of an array.
         ...
         */
  explicit WhatProvidesIterator(
    const detail::IdType *const *baseRef_r, unsigned offset_r )
    : iterator_adaptor_( 0 )
    , _baseRef( baseRef_r )
    , _offset( offset_r )
  {
...
private:
  ...
  reference dereference() const { return Solvable( getId() ); }
#if 0
