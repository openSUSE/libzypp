...
detail::IdType PoolImpl::nsCallback(
  CPool *, void *data, detail::IdType lhs, detail::IdType rhs )
{
  ...
  switch ( lhs )
  {
    ...
    {
      const TrackedLocaleIds &localeIds(
        reinterpret_cast<PoolImpl *>( data )->trackedLocaleIds() );
      return localeIds.contains( IdString( rhs ) ) ? RET_systemProperty
                                                   : RET_unsupported;
    }
    break;
    ...
    case NAMESPACE_MODALIAS:
    {
      ...
      return target::Modalias::instance().query(
               str::hexdecode( IdString( rhs ).c_str() ) )
               ? RET_systemProperty
               : RET_unsupported;
    }
    break;
...
//	METHOD TYPE : Dtor
//
PoolImpl::~PoolImpl() { ::pool_free( _pool ); }
...
void PoolImpl::prepare() const
{
  ...
  if ( sysconfigFile.hasChanged() )
  {
    ...
    const_cast<PoolImpl *>( this )->depSetDirty(
      "/etc/sysconfig/storage change" );
  }
  if ( _watcher.remember( _serial ) )
  {
    ...
    ::pool_setarch(
      _pool, ZConfig::instance().systemArchitecture().asString().c_str() );
  }
  ...
  {
    // initial seting
    const_cast<PoolImpl *>( this )->setTextLocale(
      ZConfig::instance().textLocale() );
  }
}
...
void PoolImpl::_deleteRepo( CRepo *repo_r )
{
  ...
  {
    _serialIDs
      .setDirty(); // Indicate resusePoolIDs - ResPool must also invalidate it's PoolItems
...
void PoolImpl::_postRepoAdd( CRepo *repo_r )
{
  if ( !isSystemRepo( repo_r ) )
  {
    ...
    {
      Arch::CompatSet sysarchs(
        Arch::compatSet( ZConfig::instance().systemArchitecture() ) );
      for_( it, sysarchs.begin(), sysarchs.end() ) sysids.insert( it->id() );
    ...
    for ( detail::IdType i = repo_r->start; i < repo_r->end; ++i )
    {
      ...
      else if ( blockSize )
      {
        ...
        ::repo_free_solvable_block(
          repo_r, blockBegin, blockSize, /*resusePoolIDs*/ false );
...
detail::SolvableIdType PoolImpl::_addSolvables(
  CRepo *repo_r, unsigned count_r )
{
  setDirty( __FUNCTION__, repo_r->name );
...
void PoolImpl::setTextLocale( const Locale &locale_r )
{
  ...
  dumpRangeLine(
    MIL << "pool_set_languages: ", fallbacklist.begin(), fallbacklist.end() )
    << endl;
...
void PoolImpl::initRequestedLocales( const LocaleSet &locales_r )
{
  if ( _requestedLocalesTracker.setInitial( locales_r ) )
  {
    ...
    MIL << "Init RequestedLocales: " << _requestedLocalesTracker << " ="
        << locales_r << endl;
  }
}
...
void PoolImpl::setRequestedLocales( const LocaleSet &locales_r )
{
  if ( _requestedLocalesTracker.set( locales_r ) )
  {
    ...
    MIL << "New RequestedLocales: " << _requestedLocalesTracker << " ="
        << locales_r << endl;
  }
}
...
const PoolImpl::TrackedLocaleIds &PoolImpl::trackedLocaleIds() const
{
  if ( !_trackedLocaleIdsPtr )
  {
    ...
    const base::SetTracker<LocaleSet> &localesTracker(
    ...
    {
      for ( ; lang && localeIds.current().insert( IdString( lang ) ).second;
            lang = lang.fallback() )
      {
    ...
    {
      for ( ; lang && !localeIds.current().count( IdString( lang ) );
            lang = lang.fallback() )
      {
...
void PoolImpl::multiversionListInit() const
{
  ...
  for ( const std::string &spec : ZConfig::instance().multiversionSpec() )
  {
    ...
    for ( Solvable solv : WhatProvides( Capability(
            provides ? spec.c_str() + prefix.size() : spec.c_str() ) ) )
    {
    ...
    MIL << "Multiversion install " << spec << ": " << ( nsize - size )
        << " matches" << endl;
    size = nsize;
  }
}

void PoolImpl::multiversionSpecChanged() { _multiversionListPtr.reset(); }
...
const std::set<std::string> &PoolImpl::requiredFilesystems() const
{
  if ( !_requiredFilesystemsPtr )
  {
    ...
    str::split(
      base::sysconfig::read( sysconfigStoragePath() )[ "USED_FS_LIST" ],
