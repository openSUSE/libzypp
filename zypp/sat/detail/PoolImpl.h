...
public:
  ...
  CPool *operator->() { return _pool; }

public:
  /** Serial number changing whenever the content changes. */
  const SerialNumber &serial() const { return _serial; }

  /** Serial number changing whenever resusePoolIDs==true was used. ResPool must also invalidate it's PoolItems! */
  const SerialNumber &serialIDs() const { return _serialIDs; }
...
private:
  ...
  static detail::IdType nsCallback(
...
public:
  ...
  CRepo *systemRepo() const { return _pool->installed; }
...
public:
  ...
  bool validSolvable( const CSolvable &slv_r ) const { return slv_r.repo; }
  /** \overload Check also for id_r being in range of _pool->solvables. */
  bool validSolvable( SolvableIdType id_r ) const
  {
    return id_r < unsigned( _pool->nsolvables ) &&
           validSolvable( _pool->solvables[ id_r ] );
  }
  /** \overload Check also for slv_r being in range of _pool->solvables. */
  bool validSolvable( const CSolvable *slv_r ) const
  {
    return _pool->solvables <= slv_r &&
           slv_r <= _pool->solvables + _pool->nsolvables &&
           validSolvable( *slv_r );
  }

public:
  CPool *getPool() const { return _pool; }

  /** \todo a quick check whether the repo was meanwhile deleted. */
  CRepo *getRepo( RepoIdType id_r ) const { return id_r; }
...
public:
  /** Get id of the first valid \ref Solvable.
           ...
           */
  SolvableIdType getFirstId() const { return getNextId( 1 ); }
...
public:
  /** */
  const RepoInfo &repoInfo( RepoIdType id_r ) { return _repoinfos[ id_r ]; }
  /** Also adjust repo priority and subpriority accordingly. */
  void setRepoInfo( RepoIdType id_r, const RepoInfo &info_r );
  /** */
  void eraseRepoInfo( RepoIdType id_r ) { _repoinfos.erase( id_r ); }
...
public:
  ...
  {
    prepare();
    return ::pool_whatprovides( _pool, cap_r.id() );
  }
