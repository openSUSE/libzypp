...
Resolver::~Resolver() { delete _satResolver; }
...
#define ZOLV_FLAG_TRIBOOL( ZSETTER, ZGETTER, ZVARNAME, ZVARDEFAULT )           \
  ...
  {                                                                            \
    _satResolver->ZVARNAME =                                                   \
      indeterminate( state_r ) ? ZVARDEFAULT : bool( state_r );                \
  }                                                                            \
  bool Resolver::ZGETTER() const { return _satResolver->ZVARNAME; }
...
ZOLV_FLAG_TRIBOOL(
  setAllowNameChange, allowNameChange, _allownamechange, true ) // bsc#1071466
ZOLV_FLAG_TRIBOOL(
  setAllowArchChange, allowArchChange, _allowarchchange, false )
ZOLV_FLAG_TRIBOOL( setAllowVendorChange, allowVendorChange, _allowvendorchange,
  ZConfig::instance().solver_allowVendorChange() )

ZOLV_FLAG_TRIBOOL( dupSetAllowDowngrade, dupAllowDowngrade, _dup_allowdowngrade,
...
ResPool Resolver::pool() const { return _pool; }
...
void Resolver::addWeak( const PoolItem &item ) { _addWeak.push_back( item ); }
...
struct UndoTransact : public resfilter::PoolItemFilterFunctor
{
  ...
  {
    item.status().resetTransact(
      resStatus ); // clear any solver/establish transactions
    return true;
  }
};
...
bool Resolver::resolvePool()
{
  ...
  return _satResolver->resolvePool(
    _extra_requires, _extra_conflicts, _addWeak, _upgradeRepos );
}
...
bool Resolver::resolveQueue( solver::detail::SolverQueueItemList &queue )
{
  ...
  for ( SolverQueueItemList::const_iterator iter = _removed_queue_items.begin();
        iter != _removed_queue_items.end(); iter++ )
  {
    for ( SolverQueueItemList::const_iterator iterQueue = queue.begin();
          iterQueue != queue.end(); iterQueue++ )
    {
...
{
  if ( _satResolver && _isInstalledBy.empty() && _installs.empty() )
  {
    ...
    {
      // Requires
      for ( Capabilities::const_iterator capIt =
              ( *instIter )->dep( Dep::REQUIRES ).begin();
            capIt != ( *instIter )->dep( Dep::REQUIRES ).end(); ++capIt )
      {
        ...
        for_( iter, possibleProviders.begin(), possibleProviders.end() )
        {
          ...
          if ( provider.status().staysInstalled() )
          { // Is already satisfied by an item which is installed
            ItemCapKind capKindisInstalledBy(
              provider, *capIt, Dep::REQUIRES, false );
            _satifiedByInstalled.insert(
              make_pair( *instIter, capKindisInstalledBy ) );

            ItemCapKind installedSatisfied(
              *instIter, *capIt, Dep::REQUIRES, false );
            _installedSatisfied.insert(
              make_pair( provider, installedSatisfied ) );
          }
        }
      }
      ...
      if ( !( _satResolver->onlyRequires() ) )
      {
        //Recommends
        for ( Capabilities::const_iterator capIt =
                ( *instIter )->dep( Dep::RECOMMENDS ).begin();
              capIt != ( *instIter )->dep( Dep::RECOMMENDS ).end(); ++capIt )
        {
          sat::WhatProvides possibleProviders( *capIt );
          for_( iter, possibleProviders.begin(), possibleProviders.end() )
          {
            ...
            if ( !found && provider.status().isToBeInstalled() )
            {
              ...
              else
              {
                // no initial installation cause it has been set be e.g. user
                ItemCapKind capKindisInstalledBy(
                  *instIter, *capIt, Dep::RECOMMENDS, false );
                _isInstalledBy.insert(
                  make_pair( provider, capKindisInstalledBy ) );
              }
              ItemCapKind capKindisInstalledBy(
                provider, *capIt, Dep::RECOMMENDS, !alreadySetForInstallation );
              _installs.insert( make_pair( *instIter, capKindisInstalledBy ) );
            }
            ...
            { // Is already satisfied by an item which is installed
              ItemCapKind capKindisInstalledBy(
                provider, *capIt, Dep::RECOMMENDS, false );
              _satifiedByInstalled.insert(
                make_pair( *instIter, capKindisInstalledBy ) );
        ...
        //Supplements
        for ( Capabilities::const_iterator capIt =
                ( *instIter )->dep( Dep::SUPPLEMENTS ).begin();
              capIt != ( *instIter )->dep( Dep::SUPPLEMENTS ).end(); ++capIt )
        {
          sat::WhatProvides possibleProviders( *capIt );
          for_( iter, possibleProviders.begin(), possibleProviders.end() )
          {
            ...
            while (
              pos != _isInstalledBy.end() && pos->first == *instIter && !found )
            {
              alreadySetForInstallation = true;
              ItemCapKind capKind = pos->second;
              if ( capKind.item() == provider )
                found = true;
              pos++;
            }

            if ( !found && instIter->status().isToBeInstalled() )
            {
              ...
              else
              {
                // no initial installation cause it has been set be e.g. user
                ItemCapKind capKindisInstalledBy(
                  provider, *capIt, Dep::SUPPLEMENTS, false );
                _isInstalledBy.insert(
                  make_pair( *instIter, capKindisInstalledBy ) );
              }
              ItemCapKind capKindisInstalledBy( *instIter, *capIt,
            ...
            if ( instIter->status().staysInstalled() )
            { // Is already satisfied by an item which is installed
              ItemCapKind capKindisInstalledBy( *instIter, *capIt,
                Dep::SUPPLEMENTS, !alreadySetForInstallation );
              _satifiedByInstalled.insert(
              ...
              ItemCapKind installedSatisfied(
                provider, *capIt, Dep::SUPPLEMENTS, false );
              _installedSatisfied.insert(
                make_pair( *instIter, installedSatisfied ) );
            }
          }
        }
      }
...
ItemCapKindList Resolver::isInstalledBy( const PoolItem &item )
{
  ...
  for ( ItemCapKindMap::const_iterator iter = _isInstalledBy.find( item );
...
ItemCapKindList Resolver::installs( const PoolItem &item )
{
  ...
  for ( ItemCapKindMap::const_iterator iter = _installs.find( item );
        iter != _installs.end(); )
  {
    ItemCapKind info = iter->second;
    PoolItem iterItem = iter->first;
...
ItemCapKindList Resolver::satifiedByInstalled( const PoolItem &item )
{
  ...
  for ( ItemCapKindMap::const_iterator iter = _satifiedByInstalled.find( item );
...
ItemCapKindList Resolver::installedSatisfied( const PoolItem &item )
{
  ...
  for ( ItemCapKindMap::const_iterator iter = _installedSatisfied.find( item );
