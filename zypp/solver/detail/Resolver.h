...
private:
  ...
  bool
    _solveSrcPackages; // whether to generate solver jobs for selected source packges.
  bool
    _cleandepsOnRemove; // whether removing a package should also remove no longer needed requirements

  bool
    _ignoreAlreadyRecommended; //ignore recommended packages that have already been recommended by the installed packages
  //@}
...
public:
  ...
  {
    if ( repo_r && !repo_r.isSystemRepo() )
      _upgradeRepos.insert( repo_r );
  }
  ...
  {
    _verifying = indeterminate( state_r ) ? false : bool( state_r );
  }

  bool solveSrcPackages() const { return _solveSrcPackages; }
  void setSolveSrcPackages( TriBool state_r )
  {
    _solveSrcPackages = indeterminate( state_r ) ? false : bool( state_r );
  }
