...
std::ostream &SolverQueueItemDelete::dumpOn( std::ostream &os ) const
{
  os << "[" << ( _soft ? "Soft" : "" ) << "Delete: " << _name << "]";
...
#define MAYBE_CLEANDEPS                                                        \
  ( pool().resolver().cleandepsOnRemove() ? SOLVER_CLEANDEPS : 0 )
...
SolverQueueItem_Ptr SolverQueueItemDelete::copy( void ) const
{
  SolverQueueItemDelete_Ptr new_delete =
    new SolverQueueItemDelete( pool(), _name );
...
int SolverQueueItemDelete::cmp( SolverQueueItem_constPtr item ) const
{
  int cmp = this->compare( item );
  if ( cmp != 0 )
    return cmp;
  SolverQueueItemDelete_constPtr del =
    dynamic_pointer_cast<const SolverQueueItemDelete>( item );
  if ( _name != del->_name )
  {
