...
const Pathname &SystemCheck::file() { return _file; }

const Pathname &SystemCheck::dir() { return _dir; }

const CapabilitySet &SystemCheck::requiredSystemCap() const { return _require; }
...
bool SystemCheck::loadFiles() const
{
  ...
  filesystem::dirForEach(
    _dir, [this]( const Pathname &dir_r, const char *const &name_r ) -> bool {
...
std::ostream &operator<<( std::ostream &str, const SystemCheck &obj )
{
  str << _file << endl;
  str << "requires" << endl;
  for ( CapabilitySet::const_iterator it = _require.begin();
        it != _require.end(); ++it )
    str << "  " << *it << endl;

  str << "conflicts" << endl;
  for ( CapabilitySet::const_iterator it = _conflict.begin();
        it != _conflict.end(); ++it )
    str << "  " << *it << endl;
