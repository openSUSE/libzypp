...
inline std::string xml_tag_enclose(
  const std::string &text, const std::string &tag, bool escape = false )
{
  string result;
  result += "<" + tag + ">";
...
std::string helixXML( const Capability &cap )
{
  ...
  else if ( detail.isExpression() )
  {
    if ( detail.capRel() == CapDetail::CAP_AND &&
         detail.lhs().detail().isNamed() && detail.rhs().detail().isNamed() )
    {
      // packageand dependency
      str << "<dep name='packageand(" << IdString( detail.lhs().id() ) << ":"
          << IdString( detail.rhs().id() ) << ")' />" << endl;
    }
    else if ( detail.capRel() == CapDetail::CAP_NAMESPACE &&
              detail.lhs().id() == NAMESPACE_OTHERPROVIDERS )
    {
      str << "<dep name='otherproviders(" << IdString( detail.rhs().id() )
          << ")' />" << endl;
    }
    else
    {
...
inline string helixXML( const PoolItem &obj, Dep deptag_r )
{
  ...
  if ( !caps.empty() )
    out << TAB << xml_tag_enclose( helixXML( caps ), deptag_r.asString() )
        << endl;
  return out.str();
}
...
std::string helixXML( const PoolItem &item )
{
  ...
  str << TAB << xml_tag_enclose( item.vendor().asString(), "vendor", true )
      << endl;
  str << TAB
      << xml_tag_enclose( item.buildtime().asSeconds(), "buildtime", true )
      << endl;
...
public:
  ...
  void addResolvable( const PoolItem item ) { *file << helixXML( item ); }
...
public:
  HelixControl( const std::string &controlPath,
    const RepositoryTable &sourceTable, const Arch &systemArchitecture,
  ...
  void addDependencies(
    const CapabilitySet &capRequire, const CapabilitySet &capConflict );
...
HelixControl::HelixControl( const std::string &controlPath,
  const RepositoryTable &repoTable, const Arch &systemArchitecture,
  const target::Modalias::ModaliasList &modaliasList,
  const std::set<std::string> &multiversionSpec, const std::string &systemPath )
  ...
  , _inSetup( true )
{
  ...
  *file << "<?xml version=\"1.0\"?>" << endl
        ...
        it != repoTable.end(); ++it )
  {
    ...
    *file << TAB << "- generated   : "
          << ( it->first.generatedTimestamp() ).form( "%Y-%m-%d %H:%M:%S" )
          << endl;
    *file << TAB << "- outdated    : "
          << ( it->first.suggestedExpirationTimestamp() )
               .form( "%Y-%m-%d %H:%M:%S" )
          << endl;
    *file << TAB << " -->" << endl;
  ...
  {
    *file << TAB << "<modalias name=\"" << xml_escape( *it ) << "\" />" << endl;
  }

  for_( it, multiversionSpec.begin(), multiversionSpec.end() )
  {
    *file << TAB << "<multiversion name=\"" << *it << "\" />" << endl;
  }
...
void HelixControl::addDependencies(
  const CapabilitySet &capRequire, const CapabilitySet &capConflict )
{
  for ( CapabilitySet::const_iterator iter = capRequire.begin();
        iter != capRequire.end(); iter++ )
  {
    *file << "<addRequire "
          << " name=\"" << iter->asString() << "\""
          << "/>" << endl;
  }
  for ( CapabilitySet::const_iterator iter = capConflict.begin();
        iter != capConflict.end(); iter++ )
  {
    *file << "<addConflict "
          << " name=\"" << iter->asString() << "\""
          << "/>" << endl;
...
bool Testcase::createTestcase(
  Resolver &resolver, bool dumpPool, bool runSolver )
{
  ...
  for ( const PoolItem &pi : pool )
  {
    ...
    else
    {
      ...
      if ( dumpPool )
      {
        ...
        {
          repoTable[ repo ] = new HelixResolvable(
            dumpPath + "/" + str::numstring( (long)repo.id() ) +
            "-package.xml.gz" );
        }
    ...
    if ( pi.status().isToBeInstalled() && !( pi.status().isBySolver() ) )
    {
      items_to_install.push_back( pi );
    }
    if ( pi.status().isKept() && !( pi.status().isBySolver() ) )
    {
      items_keep.push_back( pi );
    }
    if ( pi.status().isToBeUninstalled() && !( pi.status().isBySolver() ) )
    {
      items_to_remove.push_back( pi );
    }
    if ( pi.status().isLocked() && !( pi.status().isBySolver() ) )
    {
  ...
  HelixControl control( dumpPath + "/solver-test.xml", repoTable,
    ZConfig::instance().systemArchitecture(),
    target::Modalias::instance().modaliasList(),
    ZConfig::instance().multiversionSpec(), "solver-system.xml.gz" );
  ...
  control.addDependencies(
    resolver.extraRequires(), resolver.extraConflicts() );
