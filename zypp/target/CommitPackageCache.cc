  ...
  : _impl( new Impl )
{
  ...
  _impl->_repos.insert( _impl->_repos.begin(), pool.knownRepositoriesBegin(),
...
ManagedFile RepoProvidePackage::operator()(
  const PoolItem &pi_r, bool fromCache_r )
{
  ManagedFile ret;
  if ( fromCache_r )
  {
    repo::PackageProvider pkgProvider(
      _impl->_access, pi_r, _impl->_packageProviderPolicy );
  ...
  else if ( pi_r.isKind<Package>() ) // may make use of deltas
  {
    ...
    repo::PackageProvider pkgProvider(
      _impl->_access, pi_r, deltas, _impl->_packageProviderPolicy );
...
CommitPackageCache::CommitPackageCache(
  const Pathname & /*rootDir_r*/, const PackageProvider &packageProvider_r )
...
void CommitPackageCache::setCommitList(
...
bool CommitPackageCache::preloaded() const { return _pimpl->preloaded(); }
