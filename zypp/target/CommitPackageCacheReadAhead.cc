...
//	METHOD TYPE : Ctor
//
CommitPackageCacheReadAhead::CommitPackageCacheReadAhead(
...
//	METHOD TYPE : void
//
void CommitPackageCacheReadAhead::cacheLastInteractive(
...
//	METHOD TYPE : void
//
void CommitPackageCacheReadAhead::doCacheLastInteractive(
  const PoolItem &citem_r )
{
  ...
  for_( it, commitList().begin(), commitList().end() )
  {
    ...
    if ( IMediaKey( pi ) == _lastInteractive && pi.status().isToBeInstalled() &&
  ...
  if ( addToCache )
    MIL << "Cached " << _lastInteractive << ": " << addToCache << " items."
        << endl;
}
...
ManagedFile CommitPackageCacheReadAhead::get( const PoolItem &citem_r )
{
  ...
  if ( onInteractiveMedia( citem_r ) )
  {
    ...
    if ( current != _lastInteractive )
    {
      ...
      DBG << "Interactive change [" << ++_dbgChanges << "] from "
          << _lastInteractive << " to " << current << endl;
