...
struct IMediaKey
{
  ...
  explicit IMediaKey( const PoolItem &obj_r )
    : _repo( obj_r->repository() )
    , _mediaNr( obj_r->mediaNr() )
  {
  }

  explicit IMediaKey( const ResObject::constPtr &obj_r )
    : _repo( obj_r->repository() )
    , _mediaNr( obj_r->mediaNr() )
  {
  }
  ...
  bool operator!=( const IMediaKey &rhs ) const { return !operator==( rhs ); }

  bool operator<( const IMediaKey &rhs ) const
  {
    return ( _repo.id() < rhs._repo.id() ||
             ( _repo.id() == rhs._repo.id() && _mediaNr < rhs._mediaNr ) );
  }
