...
inline std::string toJSON(
  const ZYppCommitResult::TransactionStepList &steps_r )
{
...
inline std::string toJSON( const sat::Transaction::Step &step_r )
{
  ...
  {
    case Transaction::TRANSACTION_IGNORE: /*empty*/
      break;
    case Transaction::TRANSACTION_ERASE:
      ret.add( strType, strTypeDel );
      break;
    case Transaction::TRANSACTION_INSTALL:
      ret.add( strType, strTypeIns );
      break;
    case Transaction::TRANSACTION_MULTIINSTALL:
      ret.add( strType, strTypeMul );
      break;
  }

  switch ( step_r.stepStage() )
  {
    case Transaction::STEP_TODO: /*empty*/
      break;
    case Transaction::STEP_DONE:
      ret.add( strStage, strStageDone );
      break;
    case Transaction::STEP_ERROR:
      ret.add( strStage, strStageFailed );
      break;
  }

  {
    ...
    json::Object s{{strSolvableN, ident.asString()},
      {strSolvableV, ed.version()}, {strSolvableR, ed.release()},
...
namespace
{
inline PluginFrame transactionPluginFrame(
  const std::string &command_r, ZYppCommitResult::TransactionStepList &steps_r )
{
  return PluginFrame(
    command_r, json::Object{{"TransactionStepList", steps_r}}.asJSON() );
}
} // namespace
///////////////////////////////////////////////////////////////////
...
void writeUpgradeTestcase()
{
  ...
  getZYpp()->resolver()->createSolverTestcase(
    next.asString(), false /*no solving*/ );
}
...
/** Execute script and report against report_r.
       ...
       */
std::pair<bool, PatchScriptReport::Action> doExecuteScript(
  const Pathname &root_r, const Pathname &script_r,
  callback::SendReport<PatchScriptReport> &report_r )
{
  ...
  historylog.comment(
    script_r.asString() + _( " executed" ), /*timestamp*/ true );
  ExternalProgram prog( script_r.asString(), ExternalProgram::Stderr_To_Stdout,
    false, -1, true, root_r );
  ...
  std::pair<bool, PatchScriptReport::Action> ret(
    std::make_pair( false, PatchScriptReport::ABORT ) );
  ...
  if ( prog.close() != 0 )
  {
    ...
    sstr << script_r << _( " execution failed" ) << " (" << prog.execError()
         << ")" << endl;
...
/** Execute script and report against report_r.
       ...
       */
bool executeScript( const Pathname &root_r, const Pathname &script_r,
  callback::SendReport<PatchScriptReport> &report_r )
{
  std::pair<bool, PatchScriptReport::Action> action(
    std::make_pair( false, PatchScriptReport::ABORT ) );
  ...
  INT << "Abort on unknown ACTION request " << action.second << " returned"
      << endl;
  return false; // abort.
}
...
/** Look for update scripts named 'name-version-release-*' and
       ...
       */
bool RunUpdateScripts( const Pathname &root_r, const Pathname &scriptsPath_r,
  const std::vector<sat::Solvable> &checkPackages_r, bool aborting_r )
{
  if ( checkPackages_r.empty() )
    return true; // no installed packages to check

  MIL << "Looking for new update scripts in (" << root_r << ")" << scriptsPath_r
      << endl;
  ...
  {
    std::string prefix(
      str::form( "%s-%s", it->name().c_str(), it->edition().c_str() ) );
    for_( sit, scripts.begin(), scripts.end() )
    {
      if ( !str::hasPrefix( *sit, prefix ) )
        continue;

      if ( ( *sit )[ prefix.size() ] != '\0' &&
           ( *sit )[ prefix.size() ] != '-' )
      ...
      else
      {
        ...
        std::string msg( str::form( _( "%s already executed as %s)" ),
          localPath.asString().c_str(), unify[ unifytag ].c_str() ) );
      ...
      else
      {
        ...
        if ( !executeScript(
               root_r, localPath, report ) ) // script path without root prefix!
...
inline std::string notificationCmdSubst(
  const std::string &cmd_r, const UpdateNotificationFile &notification_r )
{
  std::string ret( cmd_r );
#define SUBST_IF( PAT, VAL )                                                   \
  if ( ret.find( PAT ) != std::string::npos )                                  \
  ret = str::gsub( ret, PAT, VAL )
...
void sendNotification(
  const Pathname &root_r, const UpdateNotifications &notifications_r )
{
  ...
  {
    ERR << "Can't send Notification: Missing 'format |' in command spec."
        << endl;
    HistoryLog().comment(
      str::Str() << _( "Error sending update message notification." ),
      /*timestamp*/ true );
    return;
  }

  std::string formatStr(
    str::toLower( str::trim( cmdspec.substr( 0, pos ) ) ) );
  ...
  {
    ERR << "Can't send Notification: Unknown format '" << formatStr
  ...
  if ( format == NONE || format == SINGLE )
  {
    for_( it, notifications_r.begin(), notifications_r.end() )
    {
      ...
      str::splitEscaped( notificationCmdSubst( commandStr, *it ),
        std::back_inserter( command ) );

      ExternalProgram prog(
        command, ExternalProgram::Stderr_To_Stdout, false, -1, true, root_r );
      if ( true ) // Wait for feedback
      {
        ...
        if ( ret != 0 )
        {
          ERR << "Notification command returned with error (" << ret << ")."
              << endl;
          HistoryLog().comment(
            str::Str() << _( "Error sending update message notification." ),
            /*timestamp*/ true );
  ...
  else if ( format == DIGEST || format == BULK )
  {
    ...
    str::splitEscaped(
      notificationCmdSubst( commandStr, *notifications_r.begin() ),
      std::back_inserter( command ) );
  ...
  {
    INT << "Can't send Notification: Missing handler for 'format |' in command "
           "spec."
        << endl;
    HistoryLog().comment(
      str::Str() << _( "Error sending update message notification." ),
...
/** Look for update messages named 'name-version-release-*' and
       ...
       * \see http://en.opensuse.org/Software_Management/Code11/Scripts_and_Messages
       */
void RunUpdateMessages( const Pathname &root_r, const Pathname &messagesPath_r,
  ...
  ZYppCommitResult &result_r )
{
  ...
  MIL << "Looking for new update messages in (" << root_r << ")"
      << messagesPath_r << endl;
  ...
  for_( it, checkPackages_r.begin(), checkPackages_r.end() )
  {
    ...
    for_( sit, messages.begin(), messages.end() )
    {
      ...
      result_r.rUpdateMessages().push_back(
        UpdateNotificationFile( *it, localPath ) );
      historylog.comment(
        str::Str() << _( "New update message" ) << " " << localPath,
        /*timestamp*/ true );
    }
  }
...
void XRunUpdateMessages( const Pathname &root_r, const Pathname &messagesPath_r,
  const std::vector<sat::Solvable> &checkPackages_r,
  ZYppCommitResult &result_r )
{
...
TargetImpl::TargetImpl( const Pathname &root_r, bool doRebuild_r )
  ...
  , _hardLocksFile(
      Pathname::assertprefix( _root, ZConfig::instance().locksFile() ) )
{
...
void updateFileContent( const Pathname &filename,
  boost::function<bool()> condition, boost::function<string()> value )
{
  string val = value();
  ...
  if ( condition() )
  {
    ...
    else
    {
      ...
      ZYPP_THROW( Exception(
        "Can't openfile '" + filename.asString() + "' for writing" ) );
    }
  }
}
...
void TargetImpl::createAnonymousId() const
{
  ...
  {
    updateFileContent(
      idpath, boost::bind( fileMissing, idpath ), generateRandomId );
  }
...
Pathname TargetImpl::defaultSolvfilesPath() const
{
  return Pathname::assertprefix(
    _root, ZConfig::instance().repoSolvfilesPath() /
             sat::Pool::instance().systemRepoAlias() );
}
...
bool TargetImpl::buildCache()
{
  ...
  RepoStatus rpmstatus( RepoStatus( _root / "var/lib/rpm/Name" ) &&
                        RepoStatus( _root / "etc/products.d" ) );
  ...
  if ( solvexisted )
  {
    ...
    if ( cookie.isExist() )
    {
      ...
      MIL << "Read cookie: " << rpmsolvcookie
          << " says: " << ( build_rpm_solv ? "outdated" : "uptodate" ) << endl;
    }
  }
  ...
  if ( build_rpm_solv )
  {
    ...
    Pathname oldSolvFile(
      solvexisted ? rpmsolv : Pathname() ); // to speedup rpmdb2solv
    ...
    if ( !tmpsolv )
    {
      ...
      ex.remember(
        str::form( "Cannot create temporary file under %s.", base.c_str() ) );
    ...
    cmd.push_back( "-p" );
    cmd.push_back(
      Pathname::assertprefix( _root, "/etc/products.d" ).asString() );
    ...
    sat::updateSolvFileIndex(
      rpmsolv ); // content digest for zypper bash completion
...
void TargetImpl::reload() { load( false ); }
...
void TargetImpl::load( bool force )
{
  ...
  MIL << "adding " << rpmsolv << " to pool(" << satpool.systemRepoAlias() << ")"
      << endl;
  ...
  }
  {
    if ( !PathInfo( _autoInstalledFile.file() ).isExist() )
    {
      ...
      Pathname historyFile(
        Pathname::assertprefix( _root, ZConfig::instance().historyLogFile() ) );
      if ( PathInfo( historyFile ).isExist() )
      {
        SolvIdentFile::Data onSystemByUser(
          getUserInstalledFromHistory( historyFile ) );
...
//
///////////////////////////////////////////////////////////////////
ZYppCommitResult TargetImpl::commit(
  ResPool pool_r, const ZYppCommitPolicy &policy_rX )
{
  // ----------------------------------------------------------------- //
  ...
  {
    result.rTransactionStepList().insert(
      steps.end(), result.transaction().begin(), result.transaction().end() );
  }
    ...
    for_( it, steps.begin(), steps.end() )
    {
      ...
      if ( !report->show( patch ) )
      {
        ...
        ZYPP_THROW( TargetAbortedException(
          N_( "Installation has been aborted as directed." ) ) );
      }
    }
  }
  else
  ...
  if ( !policy_r.dryRun() || policy_r.downloadMode() == DownloadOnly )
  {
    ...
    if ( policy_r.downloadMode() != DownloadAsNeeded )
    {
      ...
      for_( it, steps.begin(), steps.end() )
      {
        ...
        if ( pi->isKind<Package>() || pi->isKind<SrcPackage>() )
        {
          ...
          catch ( const SkipRequestException &exp )
          {
            ...
            it->stepStage( sat::Transaction::STEP_ERROR );
            miss = true;
            WAR << "Skipping cache preload package " << pi->asKind<Package>()
                << " in commit" << endl;
            continue;
          }
          catch ( const Exception &exp )
      ...
      packageCache.preloaded(
        true ); // try to avoid duplicate infoInCache CBs in commit
    }
  ...
  MIL << "TargetImpl::commit(<pool>, " << policy_r << ") returns: " << result
      << endl;
  return result;
}
...
{
  NotifyAttemptToModify( ZYppCommitResult &result_r )
    : _result( result_r )
  {
  }
...
void TargetImpl::commit( const ZYppCommitPolicy &policy_r,
  CommitPackageCache &packageCache_r, ZYppCommitResult &result_r )
{
  // steps: this is our todo-list
  ZYppCommitResult::TransactionStepList &steps(
    result_r.rTransactionStepList() );
    ...
    if ( citem->isKind<Package>() )
    {
      ...
      if ( citem.status().isToBeInstalled() )
      {
        ...
        catch ( const Exception &e )
        {
          ...
          // TODO see if packageCache fails to handle errors correctly.
          ZYPP_CAUGHT( e );
          INT << "Unexpected Error: Skipping package " << p << " in commit"
              << endl;
          step->stepStage( sat::Transaction::STEP_ERROR );
          continue;
        }
...
#warning Exception handling
        ...
        rpm::RpmInstFlags flags(
          policy_r.rpmInstFlags() & rpm::RPMINST_JUSTDB );
        ...
        //
        if ( p->multiversionInstall() )
          flags |= rpm::RPMINST_NOUPGRADE;
        if ( policy_r.dryRun() )
          flags |= rpm::RPMINST_TEST;
        if ( policy_r.rpmExcludeDocs() )
          flags |= rpm::RPMINST_EXCLUDEDOCS;
        if ( policy_r.rpmNoSignature() )
          flags |= rpm::RPMINST_NOSIGNATURE;

        attemptToModify();
        try
        {
          progress.tryLevel(
            target::rpm::InstallResolvableReport::RPM_NODEPS_FORCE );
    ...
    else if ( !policy_r.dryRun() ) // other resolvables (non-Package)
    {
      ...
      if ( !citem.buddy() )
      {
        if ( citem->isKind<Product>() )
        {
          ...
          if ( citem.status().isToBeInstalled() )
          {
            ERR << "Can't install orphan product without release-package! "
                << citem << endl;
          }
          else
          {
            ...
            {
              ERR << "Can't remove orphan product without 'referenceFilename'! "
                  << citem << endl;
            }
            else
            {
              PathInfo referenceFile(
                Pathname::assertprefix( _root, Pathname( "/etc/products.d" ) ) /
                referenceFilename );
              if ( !referenceFile.isFile() ||
                   filesystem::unlink( referenceFile.path() ) != 0 )
              {
                ERR << "Delete orphan product failed: " << referenceFile
                    << endl;
              }
            }
          }
        }
        else if ( citem->isKind<SrcPackage>() &&
  ...
  if ( !successfullyInstalledPackages.empty() )
  {
    ...
    RunUpdateMessages( _root, ZConfig::instance().update_messagesPath(),
      successfullyInstalledPackages, result_r );
  }
...
rpm::RpmDb &TargetImpl::rpm() { return _rpm; }

bool TargetImpl::providesFile(
  const std::string &path_str, const std::string &name_str ) const
{
  return _rpm.hasFile( path_str, name_str );
}

Date TargetImpl::timestamp() const { return _rpm.timestamp(); }
...
parser::ProductFileData baseproductdata( const Pathname &root_r )
{
  ...
  PathInfo baseproduct(
    Pathname::assertprefix( root_r, "/etc/products.d/baseproduct" ) );
  ...
  else if ( PathInfo( Pathname::assertprefix( root_r, "/etc/products.d" ) )
              .isDir() )
  {
    ERR << "baseproduct symlink is dangling or missing: " << baseproduct
        << endl;
  }
  return ret;
}
...
LocaleSet TargetImpl::requestedLocales( const Pathname &root_r )
{
  ...
  return RequestedLocalesFile( home( needroot ) / "RequestedLocales" )
    .locales();
}
...
// static version:
Target::DistributionLabel TargetImpl::distributionLabel(
...
std::string TargetImpl::distributionVersion( const Pathname &root_r )
{
  std::string distributionVersion =
    baseproductdata( staticGuessRoot( root_r ) ).edition().version();
  if ( distributionVersion.empty() )
  {
    ...
    if ( ZConfig::instance().systemRoot() == Pathname() )
    {
      try
      {
        ...
        tmprpmdb->initDatabase(
          /*default ctor uses / but no additional keyring exports */ );
      }
      catch ( ... )
      {
        return "";
...
{
  return firstNonEmptyLineIn(
    staticGuessRoot( root_r ) / "/var/lib/zypp/LastDistributionFlavor" );
}
...
std::string guessAnonymousUniqueId( const Pathname &root_r )
{
  // bsc#1024741: Omit creating a new uid for chrooted systems (if it already has one, fine)
  std::string ret(
    firstNonEmptyLineIn( root_r / "/var/lib/zypp/AnonymousUniqueId" ) );
  if ( ret.empty() && root_r != "/" )
  {
...
ManagedFile TargetImpl::provideSrcPackage(
