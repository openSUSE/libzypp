...
struct FileConflictsCB
{
  FileConflictsCB( sat::detail::CPool *pool_r, ProgressData &progress_r )
    ...
    , _state(::rpm_state_create( pool_r, ::pool_get_rootdir( pool_r ) ),
        ::rpm_state_free )
  {
  }
  ...
  void *operator()( sat::detail::CPool *pool_r, sat::detail::IdType id_r )
  {
    ...
    if ( _visited.find( id_r ) == _visited.end() )
    {
      ...
      if ( !ret &&
           sat::Solvable( id_r )
             .isKind<Package>() ) // only packages have filelists
  ...
  const sat::Queue &noFilelist() const { return _noFilelist; }

  static void *invoke(
    sat::detail::CPool *pool_r, sat::detail::IdType id_r, void *cbdata_r )
  {
...
void TargetImpl::commitFindFileConflicts(
  const ZYppCommitPolicy &policy_r, ZYppCommitResult &result_r )
{
  sat::Queue todo;
  sat::FileConflicts conflicts;
  int newpkgs = result_r.transaction().installedResult( todo );
  MIL << "Checking for file conflicts in " << newpkgs << " new packages..."
      << endl;
  if ( !newpkgs )
    return;
  ...
  try
  {
    ...
    auto sendProgress = [&]( const ProgressData &progress_r ) -> bool {
      ...
      {
        progress
          .noSend(); // take care progress DTOR does not trigger a final report (2nd exeption)
        ZYPP_THROW( AbortRequestException() );
      }
      return true;
    };
    ...
    unsigned count = ::pool_findfileconflicts( sat::Pool::instance().get(),
      todo, newpkgs, conflicts,
      FINDFILECONFLICTS_USE_SOLVABLEFILELIST |
        FINDFILECONFLICTS_CHECK_DIRALIASING | FINDFILECONFLICTS_USE_ROOTDIR,
      &FileConflictsCB::invoke, &cb );
    progress.toMax();
    progress.noSend();
