...
NoHalException::NoHalException()
  : Exception(
      _( "Sorry, but this version of libzypp was built without HAL support." ) )
{
}
////////////////////////////////////////////////////////////////
} // namespace hal
...
public:
  ...
  inline bool isSet() const { return dbus_error_is_set( &error ); }

  inline HalException halException(
...
// -----------------------------------------------------------
inline void VERIFY_CONTEXT( const zypp::RW_pointer<HalContext_Impl> &h )
{
  if ( !h )
  {
...
// -----------------------------------------------------------
inline void VERIFY_DRIVE( const zypp::RW_pointer<HalDrive_Impl> &d )
{
  if ( !d )
  {
...
// -----------------------------------------------------------
inline void VERIFY_VOLUME( const zypp::RW_pointer<HalVolume_Impl> &v )
{
  if ( !v )
  {
...
////////////////////////////////////////////////////////////////
std::ostream &HalException::dumpOn( std::ostream &str ) const
{
  ...
  , pcon( false ) // we allways use shared connections at the moment
{
  ...
  if ( !conn )
  {
    ZYPP_THROW( err.halException( _( "Unable to create dbus connection" ) ) );
  }
  ...
  if ( !hctx )
  {
    ...
    ZYPP_THROW(
      HalException( _( "libhal_ctx_new: Can't create libhal context" ) ) );
  }
...
// --------------------------------------------------------------
HalContext &HalContext::operator=( const HalContext &context )
{
  MutexLock lock( g_Mutex );
...
// --------------------------------------------------------------
std::vector<std::string> HalContext::getAllDevices() const
{
  MutexLock lock( g_Mutex );
...
// --------------------------------------------------------------
HalDrive HalContext::getDriveFromUDI( const std::string &udi ) const
{
  MutexLock lock( g_Mutex );
  VERIFY_CONTEXT( h_impl );
...
// --------------------------------------------------------------
HalVolume HalContext::getVolumeFromUDI( const std::string &udi ) const
{
  MutexLock lock( g_Mutex );
  VERIFY_CONTEXT( h_impl );
...
// --------------------------------------------------------------
HalVolume HalContext::getVolumeFromDeviceFile(
  const std::string &device_file ) const
{
  MutexLock lock( g_Mutex );
  VERIFY_CONTEXT( h_impl );

  LibHalVolume *vol =
    libhal_volume_from_device_file( h_impl->hctx, device_file.c_str() );
  if ( vol )
    return HalVolume( new HalVolume_Impl( vol ) );
  else
    return HalVolume();
...
// --------------------------------------------------------------
std::vector<std::string> HalContext::findDevicesByCapability(
  const std::string &capability ) const
{
  ...
  names = libhal_find_device_by_capability(
    h_impl->hctx, capability.c_str(), &count, &err.error );
  if ( !names )
  {
    ZYPP_THROW( err.halException() );
  }
...
// --------------------------------------------------------------
bool HalContext::getDevicePropertyBool(
  const std::string &udi, const std::string &key ) const
{
  MutexLock lock( g_Mutex );
  VERIFY_CONTEXT( h_impl );
  ...
  ret = libhal_device_get_property_bool(
    h_impl->hctx, udi.c_str(), key.c_str(), &err.error );
  if ( err.isSet() )
  {
    ZYPP_THROW( err.halException() );
  }
...
// --------------------------------------------------------------
int32_t HalContext::getDevicePropertyInt32(
  const std::string &udi, const std::string &key ) const
{
  MutexLock lock( g_Mutex );
  VERIFY_CONTEXT( h_impl );
  ...
  ret = libhal_device_get_property_int(
    h_impl->hctx, udi.c_str(), key.c_str(), &err.error );
  if ( err.isSet() )
  {
    ZYPP_THROW( err.halException() );
...
// --------------------------------------------------------------
uint64_t HalContext::getDevicePropertyUInt64(
  const std::string &udi, const std::string &key ) const
{
  MutexLock lock( g_Mutex );
  VERIFY_CONTEXT( h_impl );
  ...
  ret = libhal_device_get_property_uint64(
    h_impl->hctx, udi.c_str(), key.c_str(), &err.error );
  if ( err.isSet() )
  {
    ZYPP_THROW( err.halException() );
...
// --------------------------------------------------------------
double HalContext::getDevicePropertyDouble(
  const std::string &udi, const std::string &key ) const
{
  MutexLock lock( g_Mutex );
  VERIFY_CONTEXT( h_impl );
...
// --------------------------------------------------------------
std::string HalContext::getDevicePropertyString(
  const std::string &udi, const std::string &key ) const
{
  MutexLock lock( g_Mutex );
  VERIFY_CONTEXT( h_impl );
  ...
  ptr = libhal_device_get_property_string(
    h_impl->hctx, udi.c_str(), key.c_str(), &err.error );
  if ( err.isSet() )
  {
    ZYPP_THROW( err.halException() );
...
// --------------------------------------------------------------
void HalContext::setDevicePropertyBool(
  const std::string &udi, const std::string &key, bool value )
{
  MutexLock lock( g_Mutex );
  VERIFY_CONTEXT( h_impl );
  ...
  ret = libhal_device_set_property_bool(
    h_impl->hctx, udi.c_str(), key.c_str(), value ? 1 : 0, &err.error );
  if ( !ret )
  {
    ZYPP_THROW( err.halException() );
  }
...
// --------------------------------------------------------------
void HalContext::setDevicePropertyInt32(
  const std::string &udi, const std::string &key, int32_t value )
{
  ...
  ret = libhal_device_set_property_int(
    h_impl->hctx, udi.c_str(), key.c_str(), value, &err.error );
  if ( !ret )
  {
    ZYPP_THROW( err.halException() );
  }
...
// --------------------------------------------------------------
void HalContext::setDevicePropertyUInt64(
  const std::string &udi, const std::string &key, uint64_t value )
{
  MutexLock lock( g_Mutex );
  VERIFY_CONTEXT( h_impl );
  ...
  ret = libhal_device_set_property_uint64(
    h_impl->hctx, udi.c_str(), key.c_str(), value, &err.error );
  if ( !ret )
  {
    ZYPP_THROW( err.halException() );
...
// --------------------------------------------------------------
void HalContext::setDevicePropertyDouble(
  const std::string &udi, const std::string &key, double value )
{
  MutexLock lock( g_Mutex );
  VERIFY_CONTEXT( h_impl );
  ...
  ret = libhal_device_set_property_double(
    h_impl->hctx, udi.c_str(), key.c_str(), value, &err.error );
  if ( !ret )
  {
    ZYPP_THROW( err.halException() );
...
// --------------------------------------------------------------
void HalContext::setDevicePropertyString(
  const std::string &udi, const std::string &key, const std::string &value )
{
  MutexLock lock( g_Mutex );
  VERIFY_CONTEXT( h_impl );
  ...
  ret = libhal_device_set_property_string(
    h_impl->hctx, udi.c_str(), key.c_str(), value.c_str(), &err.error );
  if ( !ret )
  {
    ZYPP_THROW( err.halException() );
  }
...
// --------------------------------------------------------------
void HalContext::removeDeviceProperty(
  const std::string &udi, const std::string &key )
{
  MutexLock lock( g_Mutex );
  VERIFY_CONTEXT( h_impl );
  ...
  ret = libhal_device_remove_property(
    h_impl->hctx, udi.c_str(), key.c_str(), &err.error );
  if ( !ret )
  {
    ZYPP_THROW( err.halException() );
...
// --------------------------------------------------------------
HalDrive &HalDrive::operator=( const HalDrive &drive )
{
  MutexLock lock( g_Mutex );
...
// --------------------------------------------------------------
std::string HalDrive::getUDI() const
{
  MutexLock lock( g_Mutex );
  VERIFY_DRIVE( d_impl );
...
// --------------------------------------------------------------
std::string HalDrive::getTypeName() const
{
  MutexLock lock( g_Mutex );
  VERIFY_DRIVE( d_impl );
...
// --------------------------------------------------------------
std::string HalDrive::getDeviceFile() const
{
  MutexLock lock( g_Mutex );
  VERIFY_DRIVE( d_impl );
...
// --------------------------------------------------------------
unsigned int HalDrive::getDeviceMajor() const
{
  MutexLock lock( g_Mutex );
  VERIFY_DRIVE( d_impl );
...
// --------------------------------------------------------------
unsigned int HalDrive::getDeviceMinor() const
{
  MutexLock lock( g_Mutex );
  VERIFY_DRIVE( d_impl );
...
// --------------------------------------------------------------
bool HalDrive::usesRemovableMedia() const
{
  MutexLock lock( g_Mutex );
  VERIFY_DRIVE( d_impl );
...
// --------------------------------------------------------------
std::vector<std::string> HalDrive::getCdromCapabilityNames() const
{
  MutexLock lock( g_Mutex );
  VERIFY_DRIVE( d_impl );
...
// --------------------------------------------------------------
std::vector<std::string> HalDrive::findAllVolumes() const
{
  MutexLock lock( g_Mutex );
  VERIFY_DRIVE( d_impl );
  ...
  names =
    libhal_drive_find_all_volumes( d_impl->hal->hctx, d_impl->drv, &count );
...
// --------------------------------------------------------------
HalVolume &HalVolume::operator=( const HalVolume &volume )
{
  MutexLock lock( g_Mutex );
...
// --------------------------------------------------------------
std::string HalVolume::getUDI() const
{
  MutexLock lock( g_Mutex );
...
// --------------------------------------------------------------
std::string HalVolume::getDeviceFile() const
{
  MutexLock lock( g_Mutex );
  VERIFY_VOLUME( v_impl );
...
// --------------------------------------------------------------
unsigned int HalVolume::getDeviceMajor() const
{
  MutexLock lock( g_Mutex );
  VERIFY_VOLUME( v_impl );
...
// --------------------------------------------------------------
unsigned int HalVolume::getDeviceMinor() const
{
  MutexLock lock( g_Mutex );
  VERIFY_VOLUME( v_impl );
...
// --------------------------------------------------------------
bool HalVolume::isPartition() const
{
  MutexLock lock( g_Mutex );
  VERIFY_VOLUME( v_impl );
...
// --------------------------------------------------------------
bool HalVolume::isMounted() const
{
  MutexLock lock( g_Mutex );
  VERIFY_VOLUME( v_impl );
...
// --------------------------------------------------------------
std::string HalVolume::getFSType() const
{
  MutexLock lock( g_Mutex );
  VERIFY_VOLUME( v_impl );
...
// --------------------------------------------------------------
std::string HalVolume::getFSUsage() const
{
  MutexLock lock( g_Mutex );
  VERIFY_VOLUME( v_impl );
...
// --------------------------------------------------------------
std::string HalVolume::getMountPoint() const
{
  VERIFY_VOLUME( v_impl );
...
std::ostream &HalException::dumpOn( std::ostream &str ) const { return str; }
...
// --------------------------------------------------------------
HalContext::HalContext( bool ) { ZYPP_THROW( NoHalException() ); }
HalContext::~HalContext() {}
HalContext &HalContext::operator=( const HalContext & ) { return *this; }
HalContext::operator HalContext::bool_type() const { return 0; }
void HalContext::connect() {}
std::vector<std::string> HalContext::getAllDevices() const
{
  return std::vector<std::string>();
}
HalDrive HalContext::getDriveFromUDI( const std::string & ) const
{
  return HalDrive();
}
HalVolume HalContext::getVolumeFromUDI( const std::string & ) const
{
  return HalVolume();
}
HalVolume HalContext::getVolumeFromDeviceFile( const std::string & ) const
{
  return HalVolume();
}
std::vector<std::string> HalContext::findDevicesByCapability(
...
void HalContext::setDevicePropertyBool(
  const std::string &, const std::string &, bool value )
{
}
void HalContext::removeDeviceProperty(
...
// --------------------------------------------------------------
HalDrive::HalDrive() { ZYPP_THROW( NoHalException() ); }
HalDrive::~HalDrive() {}
HalDrive &HalDrive::operator=( const HalDrive & ) { return *this; }
HalDrive::operator HalDrive::bool_type() const { return 0; }
std::string HalDrive::getUDI() const { return std::string(); }
std::string HalDrive::getTypeName() const { return std::string(); }
std::string HalDrive::getDeviceFile() const { return std::string(); }
unsigned int HalDrive::getDeviceMinor() const { return 0; }
unsigned int HalDrive::getDeviceMajor() const { return 0; }
bool HalDrive::usesRemovableMedia() const { return false; }
std::vector<std::string> HalDrive::getCdromCapabilityNames() const
{
  return std::vector<std::string>();
}
...
// --------------------------------------------------------------
HalVolume::HalVolume() { ZYPP_THROW( NoHalException() ); }
HalVolume::~HalVolume() {}
HalVolume &HalVolume::operator=( const HalVolume & ) { return *this; }
HalVolume::operator HalVolume::bool_type() const { return 0; }
std::string HalVolume::getUDI() const { return std::string(); }
std::string HalVolume::getDeviceFile() const { return std::string(); }
unsigned int HalVolume::getDeviceMinor() const { return 0; }
unsigned int HalVolume::getDeviceMajor() const { return 0; }
bool HalVolume::isDisc() const { return false; }
bool HalVolume::isPartition() const { return false; }
bool HalVolume::isMounted() const { return false; }
std::string HalVolume::getFSType() const { return std::string(); }
std::string HalVolume::getFSUsage() const { return std::string(); }
std::string HalVolume::getMountPoint() const { return std::string(); }
