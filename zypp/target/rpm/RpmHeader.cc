...
int unameToUid( const char *thisUname, uid_t *uid )
{
       ...
       strcmp( thisUname, lastUname ) != 0 )
  {
    if ( lastUnameAlloced < thisUnameLen + 1 )
    {
      ...
      lastUname =
        (char *)realloc( lastUname, lastUnameAlloced ); /* XXX memory leak */
    }
    /*@-boundswrite@*/
...
int gnameToGid( const char *thisGname, gid_t *gid )
{
       ...
       strcmp( thisGname, lastGname ) != 0 )
  {
    if ( lastGnameAlloced < thisGnameLen + 1 )
    {
      ...
      lastGname =
        (char *)realloc( lastGname, lastGnameAlloced ); /* XXX memory leak */
    }
    /*@-boundswrite@*/
    ...
    if ( grent == NULL )
    {
      ...
      if ( grent == NULL )
      {
        ...
        else if ( strcmp( thisGname, "mail" ) == 0 )
        {
          /*@-boundswrite@*/
          *gid = lastGid = 12;
          /*@=boundswrite@*/
...
//
RpmHeader::constPtr RpmHeader::readPackage(
  const Pathname &path_r, VERIFICATION verification_r )
{
  ...
  if ( !nh )
  {
    WAR << "Error reading header from " << path_r << " error(" << res << ")"
        << endl;
    return (RpmHeader *)0;
  }
...
//        METHOD TYPE : bool
//
bool RpmHeader::isSrc() const { return has_tag( RPMTAG_SOURCEPACKAGE ); }

bool RpmHeader::isNosrc() const
{
  return has_tag( RPMTAG_SOURCEPACKAGE ) &&
         ( has_tag( RPMTAG_NOSOURCE ) || has_tag( RPMTAG_NOPATCH ) );
}
...
//        DESCRIPTION :
//
std::string RpmHeader::tag_name() const { return string_val( RPMTAG_NAME ); }
...
//        DESCRIPTION :
//
Arch RpmHeader::tag_arch() const { return Arch( string_val( RPMTAG_ARCH ) ); }
...
//        DESCRIPTION :
//
Date RpmHeader::tag_buildtime() const { return int_val( RPMTAG_BUILDTIME ); }
...
//        DESCRIPTION :
//
CapabilitySet RpmHeader::PkgRelList_val(
  tag tag_r, bool pre, std::set<std::string> *freq_r ) const
{
  CapabilitySet ret;
  ...
  for ( unsigned i = 0; i < count; ++i )
  {
         ...
         ( ( !pre ) && !( f & RPMSENSE_PREREQ ) ) )
    {
      ...
      {
        ZYPP_CAUGHT( excpt_r );
        WAR << "Invalid capability: " << n << " " << op << " " << v << endl;
      }
    }
  }
...
//        DESCRIPTION :
//
ByteCount RpmHeader::tag_size() const { return int_val( RPMTAG_SIZE ); }
...
//        DESCRIPTION :
//
std::string RpmHeader::tag_group() const { return string_val( RPMTAG_GROUP ); }
...
//        DESCRIPTION :
//
std::string RpmHeader::tag_url() const { return string_val( RPMTAG_URL ); }
...
//        DESCRIPTION :
//
std::string RpmHeader::tag_os() const { return string_val( RPMTAG_OS ); }

std::string RpmHeader::tag_prein() const { return string_val( RPMTAG_PREIN ); }
...
std::string RpmHeader::tag_preun() const { return string_val( RPMTAG_PREUN ); }
...
std::list<FileInfo> RpmHeader::tag_fileinfos() const
{
  ...
  if ( string_list( RPMTAG_BASENAMES, basenames ) )
  {
    ...
    for ( unsigned i = 0; i < basenames.size(); ++i )
    {
      ...
      FileInfo info = {dirnames[ dirindexes[ i ] ] + basenames[ i ],
        filesizes[ i ], md5sums[ i ], uid, gid, mode_t( filemodes[ i ] ),
        filemtimes[ i ], bool( fileflags[ i ] & RPMFILE_GHOST ),
        filelinks[ i ]};
