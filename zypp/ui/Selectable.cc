...
IdString Selectable::ident() const { return _pimpl->ident(); }

ResKind Selectable::kind() const { return _pimpl->kind(); }

const std::string &Selectable::name() const { return _pimpl->name(); }

Status Selectable::status() const { return _pimpl->status(); }

bool Selectable::setStatus(
  Status state_r, ResStatus::TransactByValue causer_r )
{
  return _pimpl->setStatus( state_r, causer_r );
}

PoolItem Selectable::installedObj() const { return _pimpl->installedObj(); }

PoolItem Selectable::candidateObj() const { return _pimpl->candidateObj(); }
...
PoolItem Selectable::setCandidate(
...
PoolItem Selectable::setCandidate(
  ResObject::constPtr newCandidate_r, ResStatus::TransactByValue causer_r )
{
  return _pimpl->setCandidate( PoolItem( newCandidate_r ), causer_r );
}

bool Selectable::setOnSystem(
  const PoolItem &newCandidate_r, ResStatus::TransactByValue causer_r )
{
  if ( identicalInstalled( newCandidate_r ) )
    return setFate( UNMODIFIED, causer_r );
  return setCandidate( newCandidate_r, causer_r ) &&
         setFate( TO_INSTALL, causer_r );
}

PoolItem Selectable::theObj() const { return _pimpl->theObj(); }

////////////////////////////////////////////////////////////////////////

bool Selectable::availableEmpty() const { return _pimpl->availableEmpty(); }
...
bool Selectable::installedEmpty() const { return _pimpl->installedEmpty(); }
...
bool Selectable::picklistEmpty() const { return _pimpl->picklistEmpty(); }
...
bool Selectable::isUnmaintained() const { return _pimpl->isUnmaintained(); }
...
bool Selectable::isUndetermined() const { return _pimpl->isUndetermined(); }

bool Selectable::isRelevant() const { return _pimpl->isRelevant(); }

bool Selectable::isSatisfied() const { return _pimpl->isSatisfied(); }

bool Selectable::isBroken() const { return _pimpl->isBroken(); }
...
bool Selectable::isUnwanted() const { return locked() && isBroken(); }
  ...
  switch ( fate_r )
  {
    ...
    case UNMODIFIED:
      switch ( status() )
      {
        ...
        default:
          return setStatus(
            hasInstalledObj() ? S_KeepInstalled : S_NoInst, causer_r );
          break;
      }
      break;
  }
