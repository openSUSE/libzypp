...
public:
  ...
  /** \overload */
  ResStatus &backup( const PoolItem &pi_r ) { return backup( pi_r.status() ); }

  /** Backup status. */
  ResStatus &operator()( ResStatus &status_r ) { return backup( status_r ); }
  /** \overload */
  ...
  bool restore()
  {
    for_( rit, _backup.rbegin(), _backup.rend() ) rit->replay();
...
public:
  ...
  bool transact( const PoolItem &pi_r, Causer causer_r )
  {
    ...
    if ( !status.setLock( false, causer_r ) )
      return false;
    if ( !status.setTransact( true, causer_r ) )
      return false;
    return true;
  }
  ...
  {
    ResStatus &status( backup( pi_r ) );
    if ( !status.setTransact( false, causer_r ) )
      return false;
    if ( !status.setLock( true, causer_r ) )
      return false;
    return true;
  }
  ...
  {
    for_( it, begin_r,
      end_r ) if ( !( this->*action_r )( *it, causer_r ) ) return false;
    return true;
  }
...
{
  StatusHelper(
    const Selectable::Impl &impl, ResStatus::TransactByValue causer_r )
    : _impl( impl )
  ...
  // Queries
  //
  bool hasInstalled() const { return bool( inst ); }

  bool hasCandidate() const { return bool( cand ); }

  bool hasInstalledOnly() const { return inst && !cand; }

  bool hasCandidateOnly() const { return cand && !inst; }

  bool hasBoth() const { return inst && cand; }
    ...
    if ( cand )
    {
      if ( inst )
      {
        for_( it, _impl.installedBegin(), _impl.installedEnd() )
        {
          ...
          if ( !inststatus.setTransact( false, causer ) )
            return restore();
          if ( !inststatus.setLock( false, causer ) )
            return restore();
      ...
      if ( !candstatus.setTransact( true, causer ) )
        return restore();
      return true;
    }
    return false;
  ...
  bool setDelete()
  {
    if ( inst )
    {
      ...
      for_( it, _impl.installedBegin(), _impl.installedEnd() )
      {
        ...
        if ( !inststatus.setLock( false, causer ) )
          return restore();
        if ( !inststatus.setTransact( true, causer ) )
          return restore();
      }
      return true;
    }
...
private:
  ...
  bool resetTransactingCandidates()
  {
    for_( it, _impl.availableBegin(), _impl.availableEnd() )
    {
      ...
      if ( !status.setTransact( false, causer ) )
        return false;
    }
    return true;
  }
  bool unlockCandidates()
  {
    for_( it, _impl.availableBegin(), _impl.availableEnd() )
    {
      ...
      if ( !status.setTransact( false, causer ) )
        return false;
      if ( !status.setLock( false, causer ) )
        return false;
    }
    return true;
  }
...
bool Selectable::Impl::setStatus(
  Status state_r, ResStatus::TransactByValue causer_r )
{
...
PoolItem Selectable::Impl::setCandidate(
  const PoolItem &newCandidate_r, ResStatus::TransactByValue causer_r )
{
...
bool Selectable::Impl::pickInstall(
  const PoolItem &pi_r, ResStatus::TransactByValue causer_r, bool yesno_r )
{
  if ( identicalInstalled( pi_r ) )
    return setPickStatus(
      pi_r, ( yesno_r ? S_Update : S_KeepInstalled ), causer_r );
  return setPickStatus( pi_r, ( yesno_r ? S_Install : S_NoInst ), causer_r );
}

bool Selectable::Impl::pickDelete(
  const PoolItem &pi_r, ResStatus::TransactByValue causer_r, bool yesno_r )
{
  return setPickStatus( pi_r, ( yesno_r ? S_Del : S_KeepInstalled ), causer_r );
}

bool Selectable::Impl::setPickStatus(
  const PoolItem &pi_r, Status state_r, ResStatus::TransactByValue causer_r )
{
  if ( pi_r.ident() != ident() )
  ...
  for_( it, installedBegin(), installedEnd() ) if ( identical( *it, pi_r ) )
    i.push_back( *it );
  for_( it, availableBegin(), availableEnd() ) if ( identical( *it, pi_r ) )
    a.push_back( *it );
  ...
  switch ( state_r )
  {
    case S_Protected:
      ...
      {
        if ( !backup.forEach(
               i.begin(), i.end(), &StatusBackup::lock, causer_r ) )
          return backup.restore();
        if ( !backup.forEach(
               a.begin(), a.end(), &StatusBackup::setTransactFalse, causer_r ) )
          return backup.restore();
        return true;
      }
      break;
    ...
    case S_Taboo:
      ...
      {
        if ( !backup.forEach(
               a.begin(), a.end(), &StatusBackup::lock, causer_r ) )
          return backup.restore();
        return true;
      }
      break;
    ...
    case S_Del:
      ...
      {
        if ( !backup.forEach(
               i.begin(), i.end(), &StatusBackup::transact, causer_r ) )
          return backup.restore();
        if ( !backup.forEach(
               a.begin(), a.end(), &StatusBackup::setTransactFalse, causer_r ) )
          return backup.restore();
    ...
    case S_Install:
      if ( i.empty() && !a.empty() )
      {
        ...
        if ( cand.multiversionInstall() )
        {
          if ( !backup.forEach( availableBegin(), availableEnd(),
                 &StatusBackup::unsetNonMultiTransact, causer_r ) )
            return backup.restore();
          // maybe unlock candidate only?
          if ( !backup.forEach(
                 a.begin(), a.end(), &StatusBackup::unlock, causer_r ) )
            return backup.restore();
          if ( !cand.status().setTransact( true, causer_r ) )
            return backup.restore();
          return true;
        }
        else
        {
          ...
          if ( !backup.forEach( availableBegin(), availableEnd(),
                 &StatusBackup::unsetMultiTransact, causer_r ) )
            return backup.restore();
          if ( !setCandidate( cand, causer_r ) )
            return backup.restore();
          if ( !setStatus( installedEmpty() ? S_Install : S_Update, causer_r ) )
            return backup.restore();
          return true;
        }
      }
    ...
    case S_Update:
      if ( !i.empty() && !a.empty() )
      {
        ...
        {
          if ( !backup.forEach(
                 i.begin(), i.end(), &StatusBackup::unlock, causer_r ) )
            return backup.restore();
          if ( !backup.forEach( i.begin(), i.end(),
                 &StatusBackup::setTransactTrue, ResStatus::SOLVER ) )
            return backup.restore();
          if ( !backup.forEach( availableBegin(), availableEnd(),
                 &StatusBackup::unsetNonMultiTransact, causer_r ) )
            return backup.restore();
...
Status Selectable::Impl::pickStatus( const PoolItem &pi_r ) const
{
  ...
  for_(
    it, i.begin(), i.end() ) if ( it->status().isLocked() ) return S_Protected;

  if ( i.empty() )
  {
    bool allALocked = true;
    for_( it, a.begin(), a.end() ) if ( !it->status().isLocked() )
    {
      allALocked = false;
      break;
    }
  ...
  if ( traits::isPseudoInstalled( kind() ) &&
       ( ta ? ta : *a.begin() )
         .status()
         .isSatisfied() ) // no installed, so we must have candidate
    return S_KeepInstalled;
