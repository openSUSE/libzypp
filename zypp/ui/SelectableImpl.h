...
public:
  ...
  Impl( const ResKind &kind_r, const std::string &name_r, TIterator begin_r,
    TIterator end_r )
...
public:
  ...
  PoolItem updateCandidateObj() const
  {
    ...
    if ( !( ResPool::instance().resolver().allowVendorChange() ||
            VendorAttr::instance().equivalent(
              defaultCand->vendor(), installed->vendor() ) ) )
      return PoolItem();

    // check arch change (arch noarch changes are allowed)
    if ( defaultCand->arch() != installed->arch() &&
         !( defaultCand->arch() == Arch_noarch ||
  ...
  bool availableEmpty() const { return _availableItems.empty(); }

  available_size_type availableSize() const { return _availableItems.size(); }

  available_iterator availableBegin() const { return _availableItems.begin(); }

  available_iterator availableEnd() const { return _availableItems.end(); }
  ...
  bool installedEmpty() const { return _installedItems.empty(); }

  installed_size_type installedSize() const { return _installedItems.size(); }

  installed_iterator installedBegin() const { return _installedItems.begin(); }

  installed_iterator installedEnd() const { return _installedItems.end(); }
  ...
  const PickList &picklist() const
  {
    if ( !_picklistPtr )
    {
      ...
      _picklistPtr->insert(
        _picklistPtr->end(), availableBegin(), availableEnd() );
    }
    return *_picklistPtr;
  }

  bool picklistEmpty() const { return picklist().empty(); }

  picklist_size_type picklistSize() const { return picklist().size(); }

  picklist_iterator picklistBegin() const { return picklist().begin(); }

  picklist_iterator picklistEnd() const { return picklist().end(); }

  ////////////////////////////////////////////////////////////////////////

  bool isUnmaintained() const { return availableEmpty(); }
  ...
  {
    if ( candidateObj() )
      candidateObj().status().setLicenceConfirmed( val_r );
  }
...
private:
  ...
  PoolItem defaultCandidate() const
  {
    if ( !installedEmpty() )
    {
      ...
      bool solver_allowVendorChange(
        ResPool::instance().resolver().allowVendorChange() );
      for ( const PoolItem &ipi : installed() )
      {
        PoolItem
          sameArch; // in case there's no same vendor at least stay with same arch.
        for ( const PoolItem &api : available() )
        {
          // 'same arch' includes allowed changes to/from noarch.
          if ( ipi.arch() == api.arch() || ipi.arch() == Arch_noarch ||
               api.arch() == Arch_noarch )
          {
            if ( !solver_allowVendorChange )
            {
              if ( VendorAttr::instance().equivalent( ipi, api ) )
                ...
                !sameArch ) // remember best same arch in case no same vendor found
                sameArch = api;
            }
...
inline std::ostream &operator<<(
  std::ostream &str, const Selectable::Impl &obj )
{
  return str << '[' << obj.kind() << ']' << obj.name() << ": " << obj.status()
             << " (I " << obj.installedSize() << ")"
             << " (A " << obj.availableSize() << ")" << obj.candidateObj();
}
