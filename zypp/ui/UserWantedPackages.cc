...
{
  return poolProxy().byKindBegin<T>();
}
template <class T>
PoolProxyIterator poolProxyEnd()
{
  return poolProxy().byKindEnd<T>();
}
...
static inline PoolProxyIterator patchesBegin()
{
  return poolProxyBegin<Patch>();
}
...
static void addDirectlySelectedPackages( set<string> &pkgNames )
{
  for ( PoolProxyIterator it = pkgBegin(); it != pkgEnd(); ++it )
  {
    ...
    {
      DBG << "Explicit user transaction on pkg \"" << ( *it )->name() << "\""
          << endl;
...
void addPkgSetPackages( set<string> &pkgNames )
{
        ...
        it != poolProxyEnd<PkgSet_T>(); ++it )
  {
    ...
    typename PkgSet_T::constPtr pkgSet = dynamic_pointer_cast<const PkgSet_T>(
      ( *it )->theObj() ? ( *it )->theObj().resolvable() : 0L );

    if ( pkgSet && ( *it )->toModify() )
    {
      DBG << ( *it )->theObj()->kind().asString() << " will be transacted: \""
          << pkgSet->name() << "\"" << endl;
...
static void addPatchPackages( set<string> &pkgNames )
{
  for ( PoolProxyIterator patch_it = patchesBegin(); patch_it != patchesEnd();
        ++patch_it )
  {
    Patch::constPtr patch = dynamic_pointer_cast<const Patch>(
      ( *patch_it )->theObj() ? ( *patch_it )->theObj().resolvable() : 0 );
