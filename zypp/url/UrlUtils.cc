...
// ---------------------------------------------------------------
std::string encode(
  const std::string &str, const std::string &safe, EEncoding eflag )
{
  ...
  while ( beg < len )
  {
    ...
    if ( pos != std::string::npos )
    {
      ...
      if ( eflag == E_ENCODED && pos + 2 < len && str.at( pos ) == '%' &&
...
// ---------------------------------------------------------------
void split( ParamMap &pmap, const std::string &str, const std::string &psep,
  const std::string &vsep, EEncoding eflag )
{
  ParamVec pvec;
