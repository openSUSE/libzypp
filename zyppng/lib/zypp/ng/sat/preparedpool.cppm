...
export module zyppng:sat_preparedpool;
...
export namespace zyppng::sat {
  ...
  public:
    ...
    // Move-construction is supported (the reference binding is copied);
    // move-assignment is not possible because references cannot be rebound,
    // so it would be implicitly deleted anyway. Spell it out for clarity.
    PreparedPool( PreparedPool && ) = default;
    PreparedPool & operator=( PreparedPool && ) = delete;
