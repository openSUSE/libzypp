...
// Iterator.h, Iterable.h, iterators.h: FilterIterator/makeIterable templates.
#include <zypp-core/Pathname.h>
#include <zypp-core/Date.h>

export module zyppng:sat_repository;

import :sat_poolconstants;
import :sat_poolmember;
import :sat_solvable;
import :cpeid;
...
// Module-purview-only — not exported.
namespace zyppng::sat::detail {
  struct ByRepository;
  template<> ::zyppng::sat::Pool &       poolFromType( ::zyppng::sat::Repository & );
  template<> const ::zyppng::sat::Pool & poolFromType( const ::zyppng::sat::Repository & );
} // namespace zyppng::sat::detail
...
// Module-purview-only — internal iterators and filters, not exported.
namespace zyppng::sat::detail
{
      /**
      ...
      struct ByRepository
      {
        public:
          ...
          ByRepository() : _repository( detail::noRepoId ) {}

          bool operator()( const sat::Solvable & slv_r ) const
          { return slv_r.repository() == _repository.id(); }
...
export namespace zyppng
{
  namespace sat
  {
    ...
    inline zypp::Iterable<Repository::SolvableIterator> Repository::solvables() const
    { return zypp::makeIterable( solvablesBegin(), solvablesEnd() ); }
